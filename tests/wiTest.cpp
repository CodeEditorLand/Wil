...
// Do not include most headers until after the WIL headers to ensure that we're
// not inadvertently adding any unnecessary dependencies to STL, WRL, or
// indirectly retrieved headers
...
TEST_CASE("WindowsInternalTests::CommonHelpers",
		  "[resource]"){{wil::unique_handle spHandle;
REQUIRE(spHandle == nullptr);
REQUIRE(nullptr == spHandle);
REQUIRE_FALSE(spHandle != nullptr);
REQUIRE_FALSE(nullptr != spHandle);

// equivalence check will static_assert because spMutex does not allow pointer
// access
wil::mutex_release_scope_exit spMutex;
// REQUIRE(spMutex == nullptr);
// REQUIRE(nullptr == spMutex);

// equivalence check will static_assert because spFile does not use nullptr_t as
// a invalid value
...
void __stdcall EmptyResultMacrosLoggingCallback(wil::FailureInfo *, PWSTR,
												size_t) WI_NOEXCEPT {}
...
void TestErrorCallbacks() {
	{
		...
		auto monitor = wil::ThreadFailureCallback(
			[&](wil::FailureInfo const &failure) -> bool {
...
void StressErrorCallbacks() {
	auto restore =
		witest::AssignTemporaryValue(&wil::g_fResultOutputDebugString, false);
	...
	for (size_t index = 0; index < threadCount; index++) {
		...
		THROW_IF_WIN32_BOOL_FALSE(::QueueUserWorkItem(
			ErrorCallbackThreadTest, eventArray[index].get(), 0));
#else
...
TEST_CASE("WindowsInternalTests::ResultMacrosStress",
		  "[LocalOnly][result_macros][stress]") {
	auto restore = witest::AssignTemporaryValue(
		&wil::g_pfnResultLoggingCallback, EmptyResultMacrosLoggingCallback);
	StressErrorCallbacks();
}
#endif

#define E_AD HRESULT_FROM_WIN32(ERROR_ACCESS_DENIED)
void SetAD() { ::SetLastError(ERROR_ACCESS_DENIED); }
...
const HRESULT E_hrNtAssertionFailure =
	wil::details::NtStatusToHr(STATUS_ASSERTION_FAILURE);

wil::StoredFailureInfo g_log;

void __stdcall ResultMacrosLoggingCallback(wil::FailureInfo *pFailure, PWSTR,
										   size_t) WI_NOEXCEPT {
	g_log = *pFailure;
}
...
enum class EType {
	...
	Expected = 0x02,
	Msg = 0x04,
	FailFast = 0x08, // overall fail fast (throw exception on successful result
					 // code, for example)
	FailFastMacro = 0x10, // explicit use of fast fail fast (FAIL_FAST_IF...)
	NoContext = 0x20 // file and line info can be wrong (throw does not happen
					 // in context to code)
};
DEFINE_ENUM_FLAG_OPERATORS(EType);

template <typename TLambda>
bool VerifyResult(unsigned int lineNumber, EType type, HRESULT hr,
				  TLambda &&lambda) {
	bool succeeded = true;
#ifdef WIL_ENABLE_EXCEPTIONS
	try {
		...
		{
			didFailFast =
				witest::DoesCodeCrash([&]() { lambdaResult = lambda(); });
		}
		if (WI_IsFlagSet(type, EType::FailFast)) {
			...
			if (WI_IsFlagClear(type, EType::Expected)) {
				if (SUCCEEDED(hr)) {
					...
					REQUIRE(
						(WI_IsFlagSet(type, EType::NoContext) ||
						 (g_log.GetFailureInfo().uLineNumber == lineNumber)));
					REQUIRE(g_log.GetFailureInfo().hr == hr);
					REQUIRE(
						(WI_IsFlagClear(type, EType::Msg) ||
						 (nullptr !=
						  wcsstr(g_log.GetFailureInfo().pszMessage, L"msg"))));
					REQUIRE((WI_IsFlagClear(type, EType::FailFastMacro) ||
							 (didFailFast)));
					REQUIRE((WI_IsFlagSet(type, EType::FailFastMacro) ||
							 (!didFailFast)));
				}
			}
		}
...
#ifdef WIL_ENABLE_EXCEPTIONS
template <typename TLambda> HRESULT TranslateException(TLambda &&lambda) {
	try {
		lambda();
...
#define REQUIRE_RETURNS(hr, lambda)                                            \
	REQUIRE(VerifyResult(__LINE__, EType::None, hr, lambda))
#define REQUIRE_RETURNS_MSG(hr, lambda)                                        \
	REQUIRE(VerifyResult(__LINE__, EType::Msg, hr, lambda))
#define REQUIRE_RETURNS_EXPECTED(hr, lambda)                                   \
	REQUIRE(VerifyResult(__LINE__, EType::Expected, hr, lambda))
...
#define REQUIRE_FAILFAST_MSG(hr, lambda)                                       \
	REQUIRE(                                                                   \
		VerifyResult(__LINE__, EType::FailFastMacro | EType::Msg, hr, [&] {    \
			auto fn = (lambda);                                                \
			fn();                                                              \
			return hr;                                                         \
		}))
...
TEST_CASE("WindowsInternalTests::ResultMacros", "[result_macros]") {
	auto restoreLoggingCallback = witest::AssignTemporaryValue(
		&wil::g_pfnResultLoggingCallback, ResultMacrosLoggingCallback);
#ifdef WIL_ENABLE_EXCEPTIONS
	auto restoreExceptionCallback = witest::AssignTemporaryValue(
		&wil::g_pfnResultFromCaughtException, TestResultCaughtFromException);
#endif

	REQUIRE_RETURNS(S_OK, [] { RETURN_HR(MDEC(hrOKRef())); });
	REQUIRE_RETURNS_MSG(
		S_OK, [] { RETURN_HR_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__); });
	...
	REQUIRE_LOG_MSG(
		E_AD, [] { LOG_WIN32_MSG(ERROR_ACCESS_DENIED, "msg: %d", __LINE__); });
	...
	REQUIRE_THROWS_RESULT(
		S_OK, [] { REQUIRE(S_OK == THROW_IF_FAILED(MDEC(hrOKRef()))); });
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(S_OK ==
				THROW_IF_FAILED_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] { REQUIRE(S_OK == LOG_IF_FAILED(MDEC(hrOKRef()))); });
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(S_OK ==
				LOG_IF_FAILED_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(
		S_OK, [] { REQUIRE(S_OK == FAIL_FAST_IF_FAILED(MDEC(hrOKRef()))); });
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(S_OK ==
				FAIL_FAST_IF_FAILED_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__));
	});
	...
	REQUIRE_THROWS_MSG(
		E_FAIL, [] { THROW_IF_FAILED_MSG(E_FAIL, "msg: %d", __LINE__); });
	...
	REQUIRE_FAILFAST_MSG(
		E_FAIL, [] { FAIL_FAST_IF_FAILED_MSG(E_FAIL, "msg: %d", __LINE__); });
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(fTRUE == THROW_IF_WIN32_BOOL_FALSE_MSG(MDEC(fTRUERef()),
													   "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
	...
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(fTRUE == LOG_IF_WIN32_BOOL_FALSE_MSG(MDEC(fTRUERef()),
													 "msg: %d", __LINE__));
	});
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(fTRUE == FAIL_FAST_IF_WIN32_BOOL_FALSE_MSG(
							 MDEC(fTRUERef()), "msg: %d", __LINE__));
	});
	...
	REQUIRE_LOG_MSG(E_AD, [] {
		SetAD();
		REQUIRE(fFALSE ==
				LOG_IF_WIN32_BOOL_FALSE_MSG(fFALSE, "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(E_AD, [] {
		SetAD();
	...
	REQUIRE_THROWS_RESULT(
		S_OK, [] { REQUIRE(S_OK == THROW_IF_WIN32_ERROR(MDEC(hrOKRef()))); });
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(S_OK ==
				THROW_IF_WIN32_ERROR_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK,
				[] { REQUIRE(S_OK == LOG_IF_WIN32_ERROR(MDEC(hrOKRef()))); });
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(S_OK ==
				LOG_IF_WIN32_ERROR_MSG(MDEC(hrOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
		REQUIRE(S_OK == FAIL_FAST_IF_WIN32_ERROR(MDEC(hrOKRef())));
	});
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(S_OK == FAIL_FAST_IF_WIN32_ERROR_MSG(MDEC(hrOKRef()), "msg: %d",
													 __LINE__));
	});
	...
	REQUIRE_LOG_MSG(E_hrOutOfPaper, [] {
		REQUIRE(ERROR_OUT_OF_PAPER == LOG_IF_WIN32_ERROR_MSG(ERROR_OUT_OF_PAPER,
															 "msg: %d",
															 __LINE__));
	});
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(STATUS_WAIT_0 == THROW_IF_NTSTATUS_FAILED_MSG(
									 MDEC(ntOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
		REQUIRE(STATUS_WAIT_0 == LOG_IF_NTSTATUS_FAILED(MDEC(ntOKRef())));
	});
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(STATUS_WAIT_0 == LOG_IF_NTSTATUS_FAILED_MSG(
									 MDEC(ntOKRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(STATUS_WAIT_0 == FAIL_FAST_IF_NTSTATUS_FAILED_MSG(
									 MDEC(ntOKRef()), "msg: %d", __LINE__));
	});
	...
	REQUIRE_RETURNS_MSG(E_hrNtAssertionFailure, [] {
		RETURN_IF_NTSTATUS_FAILED_MSG(STATUS_ASSERTION_FAILURE, "msg: %d",
									  __LINE__);
		return S_OK;
	});
	...
	REQUIRE_THROWS_MSG(E_hrNtAssertionFailure, [] {
		THROW_IF_NTSTATUS_FAILED_MSG(STATUS_ASSERTION_FAILURE, "msg: %d",
									 __LINE__);
	});
	REQUIRE_LOG(E_hrNtAssertionFailure, [] {
	...
	REQUIRE_LOG_MSG(E_hrNtAssertionFailure, [] {
		REQUIRE(STATUS_ASSERTION_FAILURE ==
				static_cast<DWORD>(LOG_IF_NTSTATUS_FAILED_MSG(
					STATUS_ASSERTION_FAILURE, "msg: %d", __LINE__)));
	});
	REQUIRE_FAILFAST(E_hrNtAssertionFailure, [] {
		FAIL_FAST_IF_NTSTATUS_FAILED(STATUS_ASSERTION_FAILURE);
	});
	REQUIRE_FAILFAST_MSG(E_hrNtAssertionFailure, [] {
		FAIL_FAST_IF_NTSTATUS_FAILED_MSG(STATUS_ASSERTION_FAILURE, "msg: %d",
										 __LINE__);
	});
	...
	REQUIRE_LOG(E_OUTOFMEMORY, [] {
		REQUIRE(STATUS_NO_MEMORY ==
				static_cast<DWORD>(LOG_IF_NTSTATUS_FAILED(STATUS_NO_MEMORY)));
	});
	REQUIRE_LOG_MSG(E_OUTOFMEMORY, [] {
		REQUIRE(STATUS_NO_MEMORY ==
				static_cast<DWORD>(LOG_IF_NTSTATUS_FAILED_MSG(
					STATUS_NO_MEMORY, "msg: %d", __LINE__)));
	});
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(pValid == THROW_IF_NULL_ALLOC_MSG(MDEC(pValidRef()), "msg: %d",
												  __LINE__));
	});
	REQUIRE_LOG(
		S_OK, [] { REQUIRE(pValid == LOG_IF_NULL_ALLOC(MDEC(pValidRef()))); });
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(pValid ==
				LOG_IF_NULL_ALLOC_MSG(MDEC(pValidRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
		REQUIRE(pValid == FAIL_FAST_IF_NULL_ALLOC(MDEC(pValidRef())));
	});
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(pValid == FAIL_FAST_IF_NULL_ALLOC_MSG(MDEC(pValidRef()),
													  "msg: %d", __LINE__));
	});
	...
	REQUIRE_THROWS_MSG(
		E_FAIL, [] { THROW_HR_IF_MSG(E_FAIL, fTrue, "msg: %d", __LINE__); });
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(fFalse == THROW_HR_IF_MSG(MDEC(S_OK), MDEC(fFalseRef()),
										  "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
	...
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(fFalse == LOG_HR_IF_MSG(MDEC(S_OK), MDEC(fFalseRef()),
										"msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(fFalse == FAIL_FAST_HR_IF_MSG(MDEC(S_OK), MDEC(fFalseRef()),
											  "msg: %d", __LINE__));
	});
	REQUIRE_RETURNS(S_OK, [] {
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(fFalse == THROW_HR_IF_MSG(E_FAIL, MDEC(fFalseRef()), "msg: %d",
										  __LINE__));
	});
	REQUIRE_LOG(
		S_OK, [] { REQUIRE(fFalse == LOG_HR_IF(E_FAIL, MDEC(fFalseRef()))); });
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(fFalse ==
				LOG_HR_IF_MSG(E_FAIL, MDEC(fFalseRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
		REQUIRE(fFalse == FAIL_FAST_HR_IF(E_FAIL, MDEC(fFalseRef())));
	});
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(fFalse == FAIL_FAST_HR_IF_MSG(E_FAIL, MDEC(fFalseRef()),
											  "msg: %d", __LINE__));
	});
	...
	REQUIRE_LOG_MSG(E_FAIL, [] {
		REQUIRE(pNull ==
				LOG_HR_IF_NULL_MSG(E_FAIL, pNull, "msg: %d", __LINE__));
	});
	...
	REQUIRE_RETURNS_MSG(S_OK, [] {
		RETURN_HR_IF_NULL_MSG(MDEC(S_OK), MDEC(pValidRef()), "msg: %d",
							  __LINE__);
		return S_OK;
	});
	REQUIRE_RETURNS_EXPECTED(S_OK, [] {
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(pValid == THROW_HR_IF_NULL_MSG(MDEC(S_OK), MDEC(pValidRef()),
											   "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
	...
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(pValid == LOG_HR_IF_NULL_MSG(MDEC(S_OK), MDEC(pValidRef()),
											 "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(pValid == FAIL_FAST_HR_IF_NULL_MSG(MDEC(S_OK),
												   MDEC(pValidRef()), "msg: %d",
												   __LINE__));
	});
	REQUIRE_RETURNS(S_OK, [] {
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(pValid == THROW_HR_IF_NULL_MSG(E_FAIL, MDEC(pValidRef()),
											   "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
	...
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(pValid == LOG_HR_IF_NULL_MSG(E_FAIL, MDEC(pValidRef()),
											 "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(pValid == FAIL_FAST_HR_IF_NULL_MSG(E_FAIL, MDEC(pValidRef()),
												   "msg: %d", __LINE__));
	});
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(fFalse == THROW_LAST_ERROR_IF_MSG(MDEC(fFalseRef()), "msg: %d",
												  __LINE__));
	});
	REQUIRE_LOG(
		S_OK, [] { REQUIRE(fFalse == LOG_LAST_ERROR_IF(MDEC(fFalseRef()))); });
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(fFalse ==
				LOG_LAST_ERROR_IF_MSG(MDEC(fFalseRef()), "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
		REQUIRE(fFalse == FAIL_FAST_LAST_ERROR_IF(MDEC(fFalseRef())));
	});
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(fFalse == FAIL_FAST_LAST_ERROR_IF_MSG(MDEC(fFalseRef()),
													  "msg: %d", __LINE__));
	});
	...
	REQUIRE_LOG_MSG(E_AD, [] {
		SetAD();
		REQUIRE(pNull ==
				LOG_LAST_ERROR_IF_NULL_MSG(pNull, "msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(E_AD, [] {
		SetAD();
		FAIL_FAST_LAST_ERROR_IF_NULL(pNull);
	...
	REQUIRE_THROWS_MSG(S_OK, [] {
		REQUIRE(pNull != THROW_LAST_ERROR_IF_NULL_MSG(MDEC(pValidRef()),
													  "msg: %d", __LINE__));
	});
	REQUIRE_LOG(S_OK, [] {
	...
	REQUIRE_LOG_MSG(S_OK, [] {
		REQUIRE(pNull != LOG_LAST_ERROR_IF_NULL_MSG(MDEC(pValidRef()),
													"msg: %d", __LINE__));
	});
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_FAILFAST_MSG(S_OK, [] {
		REQUIRE(pNull != FAIL_FAST_LAST_ERROR_IF_NULL_MSG(MDEC(pValidRef()),
														  "msg: %d", __LINE__));
	});
	...
	// FAIL_FAST_IMMEDIATE* directly invokes __fastfail, which we can't catch,
	// so disabled for now REQUIRE_FAILFAST_UNSPECIFIED([] {
	// FAIL_FAST_IMMEDIATE(); }); REQUIRE_FAILFAST_UNSPECIFIED([] {
	// FAIL_FAST_IMMEDIATE_IF_FAILED(E_FAIL); });
	REQUIRE_FAILFAST(S_OK, [] {
	...
	REQUIRE_THROWS_RESULT(
		E_AD, [] { THROW_EXCEPTION(MDEC(DerivedAccessDeniedException())); });
	REQUIRE_THROWS_MSG(E_AD, [] {
		THROW_EXCEPTION_MSG(MDEC(DerivedAccessDeniedException()), "msg: %d",
							__LINE__);
	});
	...
	REQUIRE_RETURNS(S_OK, [] {
		return wil::ResultFromException([] { THROW_IF_FAILED(hrOK); });
	});
	REQUIRE_RETURNS(E_FAIL, [] {
		return wil::ResultFromException([] { THROW_IF_FAILED(hrFAIL); });
	});
	...
	REQUIRE_LOG_MSG(E_OUTOFMEMORY, [] {
		std::unique_ptr<int> pInt;
		LOG_HR_IF_NULL_MSG(MDEC(E_OUTOFMEMORY), MDEC(pInt), "msg: %d",
						   __LINE__);
	});
	REQUIRE_FAILFAST(E_FAIL, [] {
		std::unique_ptr<int> pInt;
	...
	REQUIRE_THROWS_MSG(E_OUTOFMEMORY, [] {
		std::unique_ptr<int> pInt;
		THROW_HR_IF_NULL_MSG(MDEC(E_OUTOFMEMORY), MDEC(pInt), "msg: %d",
							 __LINE__);
	});
	REQUIRE_LOG(S_OK, [] {
		std::unique_ptr<int> pInt(new int(5));
	...
	// REQUIRE_FAILFAST_UNSPECIFIED([] { std::unique_ptr<int> pInt;
	// FAIL_FAST_IMMEDIATE_IF_NULL(pNull); });
	REQUIRE_FAILFAST(S_OK, [] {
		std::unique_ptr<int> pInt(new int(5));
...
TEST_CASE("WindowsInternalTests::UniqueHandle", "[resource][unique_any]") {
	{
		...
		wil::unique_handle spValidHandle(
			::CreateEventEx(nullptr, nullptr, CREATE_EVENT_INITIAL_SET, 0));
	...
	}
	{
		...
		wil::unique_hfile
			spNullHandle; // = nullptr;       // method explicitly disabled as
						  // nullptr isn't the invalid value
		REQUIRE(spNullHandle.get() == INVALID_HANDLE_VALUE);

		// assignment from the invalid value
		// spNullHandle = nullptr;                          // method explicitly
		// disabled as nullptr isn't the invalid value
		REQUIRE(spNullHandle.get() == INVALID_HANDLE_VALUE);
		...
		wil::unique_hfile spValidHandle(
			::CreateFile2(tempFileName, GENERIC_READ | GENERIC_WRITE,
						  FILE_SHARE_DELETE, CREATE_ALWAYS, &params));
#else
		wil::unique_hfile spValidHandle(::CreateFileW(
			tempFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_DELETE,
		...
		// nullptr-assignment -- uncomment to check intentional compilation
		// error spNullHandle = nullptr;
		...
		REQUIRE(spValidHandle.get() == INVALID_HANDLE_VALUE);
		REQUIRE(spMoveHandle.get() == handleValue);
		// uncomment to test intentional compilation error due to conflict with
		// INVALID_HANDLE_VALUE spNullHandle.reset(nullptr);
		...
		*(&spMoveHandle) =
			::CreateFile2(tempFileName2, GENERIC_READ | GENERIC_WRITE,
						  FILE_SHARE_DELETE, CREATE_ALWAYS, &params2);
#else
		*(&spMoveHandle) = ::CreateFileW(
			tempFileName2, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_DELETE,
			nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, nullptr);
#endif
...
TEST_CASE("WindowsInternalTests::SharedHandle", "[resource][shared_any]") {
	...
	spFromUnique.reset(wil::unique_handle(
		::CreateEventEx(nullptr, nullptr, CREATE_EVENT_INITIAL_SET, 0)));
	REQUIRE(spFromUnique.get() != nullptr);
	...
	// swap and compare
	wil::shared_handle sp1(
		::CreateEventEx(nullptr, nullptr, CREATE_EVENT_INITIAL_SET, 0));
	wil::shared_handle sp2(
		::CreateEventEx(nullptr, nullptr, CREATE_EVENT_INITIAL_SET, 0));
	auto ptr1 = sp1.get();
	auto ptr2 = sp2.get();
	sp1.swap(sp2);
...
template <typename event_t> void EventTestCommon() {
	// Constructor tests...
	event_t e1;
	REQUIRE_FALSE(e1);
...
template <typename mutex_t> void MutexTestCommon() {
	// Constructor tests...
	mutex_t m1;
	REQUIRE_FALSE(m1);
...
template <typename semaphore_t> void SemaphoreTestCommon() {
	// Constructor tests...
	semaphore_t m1;
	REQUIRE_FALSE(m1);
...
template <typename test_t> void MutexRaiiTests() {
	test_t var1;
	var1.create();
	...
	// try_create
	bool exists = false;
	REQUIRE(var1.try_create(L"wiltestmutex", 0, MUTEX_ALL_ACCESS, nullptr,
							&exists));
	REQUIRE_FALSE(exists);
	test_t var2;
	REQUIRE(var2.try_create(L"wiltestmutex", 0, MUTEX_ALL_ACCESS, nullptr,
							&exists));
	REQUIRE(exists);
	test_t var3;
	REQUIRE_FALSE(
		var3.try_create(L"\\illegal\\chars\\too\\\\many\\\\namespaces", 0,
						MUTEX_ALL_ACCESS, nullptr, &exists));
	REQUIRE(::GetLastError() != ERROR_SUCCESS);

	// try_open
	test_t var4;
	REQUIRE_FALSE(
		var4.try_open(L"\\illegal\\chars\\too\\\\many\\\\namespaces"));
	REQUIRE(::GetLastError() != ERROR_SUCCESS);
	REQUIRE(var4.try_open(L"wiltestmutex"));
}

template <typename test_t> void SemaphoreRaiiTests() {
	test_t var1;
	var1.create(1, 1);
	...
	// try_create
	bool exists = false;
	REQUIRE(var1.try_create(1, 1, L"wiltestsemaphore", MUTEX_ALL_ACCESS,
							nullptr, &exists));
	REQUIRE_FALSE(exists);
	test_t var2;
	REQUIRE(var2.try_create(1, 1, L"wiltestsemaphore", MUTEX_ALL_ACCESS,
							nullptr, &exists));
	REQUIRE(exists);
	test_t var3;
	REQUIRE_FALSE(
		var3.try_create(1, 1, L"\\illegal\\chars\\too\\\\many\\\\namespaces",
						MUTEX_ALL_ACCESS, nullptr, &exists));
	REQUIRE(::GetLastError() != ERROR_SUCCESS);
...
TEST_CASE("WindowsInternalTests::HandleWrappers", "[resource][unique_any]") {
	...
	REQUIRE(
		SUCCEEDED(testEventNoExcept.create(wil::EventOptions::ManualReset)));
	...
	auto unique_cotaskmem_string_failfast1 =
		wil::make_cotaskmem_string_failfast(L"Foo");
	REQUIRE(wcscmp(L"Foo", unique_cotaskmem_string_failfast1.get()) == 0);

	auto unique_cotaskmem_string_nothrow1 =
		wil::make_cotaskmem_string_nothrow(L"Foo");
	REQUIRE(wcscmp(L"Foo", unique_cotaskmem_string_nothrow1.get()) == 0);

	auto unique_cotaskmem_string_nothrow2 =
		wil::make_cotaskmem_string_nothrow(L"");
	...
	auto unique_cotaskmem_string_range2 =
		wil::make_cotaskmem_string(nullptr, 2);
	...
	auto unique_cotaskmem_string_range3 =
		wil::make_cotaskmem_string(nullptr, 0);
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
	{
		...
		auto unique_cotaskmem_string_secure_failfast1 =
			wil::make_cotaskmem_string_secure_failfast(L"Foo");
		REQUIRE(wcscmp(L"Foo",
					   unique_cotaskmem_string_secure_failfast1.get()) == 0);

		auto unique_cotaskmem_string_secure_nothrow1 =
			wil::make_cotaskmem_string_secure_nothrow(L"Foo");
		REQUIRE(wcscmp(L"Foo", unique_cotaskmem_string_secure_nothrow1.get()) ==
				0);

		auto unique_cotaskmem_string_secure_nothrow2 =
			wil::make_cotaskmem_string_secure_nothrow(L"");
		REQUIRE(wcscmp(L"", unique_cotaskmem_string_secure_nothrow2.get()) ==
				0);

#ifdef WIL_ENABLE_EXCEPTIONS
		auto unique_cotaskmem_string_secure_te1 =
			wil::make_cotaskmem_string_secure(L"Foo");
		REQUIRE(wcscmp(L"Foo", unique_cotaskmem_string_secure_te1.get()) == 0);

		auto unique_cotaskmem_string_secure_te2 =
			wil::make_cotaskmem_string_secure(L"");
		REQUIRE(wcscmp(L"", unique_cotaskmem_string_secure_te2.get()) == 0);
#endif
	}

	auto unique_hlocal_string_failfast1 =
		wil::make_hlocal_string_failfast(L"Foo");
	REQUIRE(wcscmp(L"Foo", unique_hlocal_string_failfast1.get()) == 0);

	auto unique_hlocal_string_nothrow1 =
		wil::make_hlocal_string_nothrow(L"Foo");
	...
	auto unique_hlocal_ansistring_failfast1 =
		wil::make_hlocal_ansistring_failfast("Foo");
	REQUIRE(strcmp("Foo", unique_hlocal_ansistring_failfast1.get()) == 0);

	auto unique_hlocal_ansistring_nothrow1 =
		wil::make_hlocal_ansistring_nothrow("Foo");
	REQUIRE(strcmp("Foo", unique_hlocal_ansistring_nothrow1.get()) == 0);

	auto unique_hlocal_ansistring_nothrow2 =
		wil::make_hlocal_ansistring_nothrow("");
	...
	auto unique_hlocal_ansistring_range1 =
		wil::make_hlocal_ansistring("Foo", 2);
	REQUIRE(strcmp("Fo", unique_hlocal_ansistring_range1.get()) == 0);

	auto unique_hlocal_ansistring_range2 =
		wil::make_hlocal_ansistring(nullptr, 2);
	...
	auto unique_hlocal_ansistring_range3 =
		wil::make_hlocal_ansistring(nullptr, 0);
...

	{
		...
		auto unique_hlocal_string_secure_failfast1 =
			wil::make_hlocal_string_secure_failfast(L"Foo");
		REQUIRE(wcscmp(L"Foo", unique_hlocal_string_secure_failfast1.get()) ==
				0);

		auto unique_hlocal_string_secure_nothrow1 =
			wil::make_hlocal_string_secure_nothrow(L"Foo");
		REQUIRE(wcscmp(L"Foo", unique_hlocal_string_secure_nothrow1.get()) ==
				0);

		auto unique_hlocal_string_secure_nothrow2 =
			wil::make_hlocal_string_secure_nothrow(L"");
		REQUIRE(wcscmp(L"", unique_hlocal_string_secure_nothrow2.get()) == 0);

#ifdef WIL_ENABLE_EXCEPTIONS
		auto unique_hlocal_string_secure_te1 =
			wil::make_hlocal_string_secure(L"Foo");
		REQUIRE(wcscmp(L"Foo", unique_hlocal_string_secure_te1.get()) == 0);

		auto unique_hlocal_string_secure_te2 =
			wil::make_hlocal_string_secure(L"");
		REQUIRE(wcscmp(L"", unique_hlocal_string_secure_te2.get()) == 0);
#endif
	}

	auto unique_process_heap_string_failfast1 =
		wil::make_process_heap_string_failfast(L"Foo");
	REQUIRE(wcscmp(L"Foo", unique_process_heap_string_failfast1.get()) == 0);

	auto unique_process_heap_string_nothrow1 =
		wil::make_process_heap_string_nothrow(L"Foo");
	REQUIRE(wcscmp(L"Foo", unique_process_heap_string_nothrow1.get()) == 0);

	auto unique_process_heap_string_nothrow2 =
		wil::make_process_heap_string_nothrow(L"");
	...
	auto unique_process_heap_string_range1 =
		wil::make_process_heap_string(L"Foo", 2);
	REQUIRE(wcscmp(L"Fo", unique_process_heap_string_range1.get()) == 0);

	auto unique_process_heap_string_range2 =
		wil::make_process_heap_string(nullptr, 2);
	...
	auto unique_process_heap_string_range3 =
		wil::make_process_heap_string(nullptr, 0);
	...
	auto unique_variant_bstr_failfast1 =
		wil::make_variant_bstr_failfast(L"Foo");
	REQUIRE(wcscmp(L"Foo", V_UNION(unique_variant_bstr_failfast1.addressof(),
								   bstrVal)) == 0);

	auto unique_variant_bstr_nothrow1 = wil::make_variant_bstr_nothrow(L"Foo");
	REQUIRE(wcscmp(L"Foo", V_UNION(unique_variant_bstr_nothrow1.addressof(),
								   bstrVal)) == 0);

	auto unique_variant_bstr_nothrow2 = wil::make_variant_bstr_nothrow(L"");
	REQUIRE(wcscmp(L"", V_UNION(unique_variant_bstr_nothrow2.addressof(),
								bstrVal)) == 0);
	...
	REQUIRE(wcscmp(L"Foo",
				   V_UNION(unique_variant_bstr_te1.addressof(), bstrVal)) == 0);

	auto unique_variant_bstr_te2 = wil::make_variant_bstr(L"");
	REQUIRE(wcscmp(L"",
				   V_UNION(unique_variant_bstr_te2.addressof(), bstrVal)) == 0);
	...
	{
		auto cleanupMemory = wil::SecureZeroMemory_scope_exit(
			testString2.get(),
			wcslen(testString2.get()) * sizeof(testString2.get()[0]));
	}
...
TEST_CASE("WindowsInternalTests::Locking", "[resource]") {
	{
		...
		}
		{
			...
			auto lockRecursiveExclusive =
				wil::TryAcquireSRWLockExclusive(&rwlock);
...
void TestOutHandle(_Out_ HANDLE *pHandle) { *pHandle = nullptr; }

void TestOutAlloc(_Out_ int **ppInt) { *ppInt = new int(5); }

void TestCoTask(_Outptr_result_buffer_(*charCount) PWSTR *ppsz,
				size_t *charCount) {
	*charCount = 0;
...
void TestVoid(_Out_ void **ppv) { *ppv = nullptr; }

void TestByte(_Out_ BYTE **ppByte) { *ppByte = nullptr; }
...
TEST_CASE("WindowsInternalTests::WistdTests", "[resource][wistd]") {
	...
	// REQUIRE_FAILFAST_UNSPECIFIED([]{ auto spTooBigFF =
	// wil::make_unique_failfast<int[]>(static_cast<size_t>(-1)); });
	...
	count = 0;
	{
		wistd::function<void(int)> fn;
		{
			object_counter c{state};
			REQUIRE(state.instance_count() == 1);
			fn = [&count, c](int param) { count += param; };
			REQUIRE(state.instance_count() == 2);
		}
		REQUIRE(state.instance_count() == 1);
		fn(3);
	...
	{
		// Size Check -- the current implementation allows for 10 pointers to be
		// passed through the lambda
		int a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12;
		(void)a11;
		(void)a12;

		wistd::function<void()> fn = [&a1, &a2, &a3, &a4, &a5, &a6, &a7, &a8,
									  &a9, &a10]() {
			(void)a1;
			(void)a2;
			(void)a3;
		...
		// "The sizeof(wistd::function) has grown too large for the reserved
		// buffer (10 pointers).  Refactor to reduce size of the capture."
		// wistd::function<void()> fn2 = [&a1, &a2, &a3, &a4, &a5, &a6, &a7,
		// &a8, &a9, &a10, &a11]()
		// {
...
template <typename test_t> void EventRaiiTests() {
	test_t var1;
	...
	// try_create
	bool exists = false;
	REQUIRE(var1.try_create(wil::EventOptions::ManualReset, L"wiltestevent",
							nullptr, &exists));
	REQUIRE_FALSE(exists);
	test_t var2;
	REQUIRE(var2.try_create(wil::EventOptions::ManualReset, L"wiltestevent",
							nullptr, &exists));
	REQUIRE(exists);
	test_t var3;
	REQUIRE_FALSE(var3.try_create(
		wil::EventOptions::ManualReset,
		L"\\illegal\\chars\\too\\\\many\\\\namespaces", nullptr, &exists));
	REQUIRE(::GetLastError() != ERROR_SUCCESS);
...
void EventTests() {
	static_assert(
		sizeof(wil::unique_event_nothrow) == sizeof(HANDLE),
		"event_t should be sizeof(HANDLE) to allow for raw array utilization");
	...
	REQUIRE(
		FAILED(event4.create(wil::EventOptions::ManualReset,
							 L"\\illegal\\chars\\too\\\\many\\\\namespaces")));

#ifdef WIL_ENABLE_EXCEPTIONS
	static_assert(
		sizeof(wil::unique_event) == sizeof(HANDLE),
		"event_t should be sizeof(HANDLE) to allow for raw array utilization");
...
template <typename T> void TestUniquePropVariant() {
	{
	...
	// constructor test
	{
		...
		REQUIRE(
			((spPropVariant2.intVal == 12) && (spPropVariant2.vt == VT_INT)));
...
TEST_CASE("WindowsInternalTests::Win32HelperTests", "[win32_helpers]") {
	...
	REQUIRE(systemTime64 ==
			wil::filetime::to_int64(wil::filetime::from_int64(systemTime64)));
	REQUIRE((systemTime64 + wil::filetime_duration::one_hour) ==
			(systemTime64 + (wil::filetime_duration::one_minute * 60)));
	auto systemTimePlusOneHour =
		wil::filetime::add(systemTime, wil::filetime_duration::one_hour);
	auto systemTimePlusOneHour64 =
		wil::filetime::to_int64(systemTimePlusOneHour);
	REQUIRE(systemTimePlusOneHour64 ==
...
TEST_CASE("WindowsInternalTests::InitOnceNonTests") {
	...
	// Call, but fail. Should transport the HRESULT back, but mark us as not the
	// winner
	called = false;
	winner = false;
	REQUIRE(E_FAIL == wil::init_once_nothrow(
						  init,
						  [&] {
							  called = true;
							  return E_FAIL;
	...
	static volatile bool always_true =
		true; // So that the compiler can't determine that we unconditionally
			  // throw below (warning C4702)
	REQUIRE_THROWS_AS(winner = wil::init_once(init,
											  [&] {
												  called = true;
												  THROW_HR_IF(E_FAIL,
															  always_true);
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
TEST_CASE("WindowsInternalTests::TestUniquePointerCases",
		  "[resource][unique_any]") {
	...
	{
		wil::unique_process_heap_ptr<void> heapMemory(
	...
	}
	{
		auto cotaskmemArrayMemory =
			wil::make_unique_cotaskmem_nothrow<BYTE[]>(12);
	...
	}
	{
		...
		auto cotaskmemArrayMemory =
			wil::make_unique_cotaskmem_nothrow<S[]>(size);
		REQUIRE(static_cast<bool>(cotaskmemArrayMemory));
		bool verified = true;
	...
	{
		wil::unique_cotaskmem_secure_ptr<void> cotaskmemMemory(
			CoTaskMemAlloc(100));
		REQUIRE(static_cast<bool>(cotaskmemMemory));
	}
	{
		auto cotaskmemMemory =
			wil::make_unique_cotaskmem_secure_nothrow<DWORD>(42);
		REQUIRE(static_cast<bool>(cotaskmemMemory));
		REQUIRE(*cotaskmemMemory == static_cast<DWORD>(42));
	}
	{
	...
	{
		auto cotaskmemArrayMemory =
			wil::make_unique_cotaskmem_secure_nothrow<BYTE[]>(12);
		REQUIRE(static_cast<bool>(cotaskmemArrayMemory));
	}
	{
		struct S {
		...
		const size_t size = 12;
		auto cotaskmemArrayMemory =
			wil::make_unique_cotaskmem_secure_nothrow<S[]>(size);
		REQUIRE(static_cast<bool>(cotaskmemArrayMemory));
		bool verified = true;
		for (auto &elem : wil::make_range(cotaskmemArrayMemory.get(), size))
			if (elem.s != 42)
	...
	{
		auto localArrayMemory =
			wil::make_unique_hlocal_secure_nothrow<BYTE[]>(12);
		REQUIRE(static_cast<bool>(localArrayMemory));
	}
	{
		struct S {
		...
		const size_t size = 12;
		auto localArrayMemory =
			wil::make_unique_hlocal_secure_nothrow<S[]>(size);
		REQUIRE(static_cast<bool>(localArrayMemory));
		bool verified = true;
		for (auto &elem : wil::make_range(localArrayMemory.get(), size))
			if (elem.s != 42)
...

	{
		...
		// auto cotaskmemMemory = wil::make_unique_cotaskmem_nothrow<S>();
		// auto cotaskmemArrayMemory =
		// wil::make_unique_cotaskmem_nothrow<S[]>(1); auto cotaskmemMemory2 =
		// wil::make_unique_cotaskmem_secure_nothrow<S>(); auto
		// cotaskmemArrayMemory2 =
		// wil::make_unique_cotaskmem_secure_nothrow<S[]>(1);

		// auto localMemory = wil::make_unique_hlocal_nothrow<S>();
		// auto localArrayMemory = wil::make_unique_hlocal_nothrow<S[]>(1);
		// auto localMemory2 = wil::make_unique_hlocal_secure_nothrow<S>();
		// auto localArrayMemory2 =
		// wil::make_unique_hlocal_secure_nothrow<S[]>(1);
	}
}
#endif

void GetDWORDArray(_Out_ size_t *count,
				   _Outptr_result_buffer_(*count) DWORD **numbers) {
	const size_t size = 5;
...
void GetHSTRINGArray(_Out_ ULONG *count,
					 _Outptr_result_buffer_(*count) HSTRING **strings) {
	const size_t size = 5;
	auto ptr = static_cast<HSTRING *>(::CoTaskMemAlloc(sizeof(HSTRING) * size));
	REQUIRE(ptr);
	for (UINT i = 0; i < size; ++i) {
		REQUIRE_SUCCEEDED(WindowsCreateString(
			L"test", static_cast<UINT32>(wcslen(L"test")), &ptr[i]));
	}
	*strings = ptr;
	*count = static_cast<ULONG>(size);
}

void GetPOINTArray(_Out_ UINT32 *count,
				   _Outptr_result_buffer_(*count) POINT **points) {
	const size_t size = 5;
...
#ifdef WIL_ENABLE_EXCEPTIONS
void GetHANDLEArray(_Out_ size_t *count,
					_Outptr_result_buffer_(*count) HANDLE **events) {
	const size_t size = 5;
	HANDLE *ptr =
		reinterpret_cast<HANDLE *>(::CoTaskMemAlloc(sizeof(HANDLE) * size));
...
interface __declspec(uuid("EDCA4ADC-DF46-442A-A69D-FDFD8BC37B31")) IFakeObject
	: public IUnknown {
...
void GetUnknownArray(_Out_ size_t *count,
					 _Outptr_result_buffer_(*count) IFakeObject ***objects) {
	const size_t size = 5;
	auto ptr = reinterpret_cast<IFakeObject **>(
		::CoTaskMemAlloc(sizeof(IFakeObject *) * size));
	REQUIRE(ptr);
	for (UINT i = 0; i < size; ++i) {
		Microsoft::WRL::ComPtr<IFakeObject> obj;
		REQUIRE_SUCCEEDED(
			Microsoft::WRL::MakeAndInitialize<ArrayTestObject>(&obj, i));
		ptr[i] = obj.Detach();
	}
	*objects = ptr;
	*count = size;
...
TEST_CASE("WindowsInternalTests::VerifyMakeAgileCallback", "[wrl]") {
	...
	class CallbackClient {
	  public:
		HRESULT On(IMemoryBufferReference *, IInspectable *) { return S_OK; }
	};
	CallbackClient callbackClient;

#ifdef WIL_ENABLE_EXCEPTIONS
	auto cbAgile = wil::MakeAgileCallback<
		ITypedEventHandler<IMemoryBufferReference *, IInspectable *>>(
		[](IMemoryBufferReference *, IInspectable *) -> HRESULT {
			return S_OK;
		});
	REQUIRE(wil::is_agile(cbAgile));

	auto cbAgileWithMember = wil::MakeAgileCallback<
		ITypedEventHandler<IMemoryBufferReference *, IInspectable *>>(
		&callbackClient, &CallbackClient::On);
	REQUIRE(wil::is_agile(cbAgileWithMember));
#endif
	auto cbAgileNoThrow = wil::MakeAgileCallbackNoThrow<
		ITypedEventHandler<IMemoryBufferReference *, IInspectable *>>(
		[](IMemoryBufferReference *, IInspectable *) -> HRESULT {
			return S_OK;
		});
	...
	auto cbAgileWithMemberNoThrow = wil::MakeAgileCallbackNoThrow<
		ITypedEventHandler<IMemoryBufferReference *, IInspectable *>>(
		&callbackClient, &CallbackClient::On);
...
TEST_CASE("WindowsInternalTests::HStringTests", "[resource][unique_any]") {
...

	{
		...
		REQUIRE_SUCCEEDED(::WindowsPreallocateStringBuffer(
			ARRAYSIZE(kittens), &bufferStorage, &theBuffer));
		REQUIRE_SUCCEEDED(
			StringCchCopyW(bufferStorage, ARRAYSIZE(kittens), kittens));

		// Promote sets the promoted-to value but resets theBuffer
		wil::unique_hstring promoted;
		REQUIRE_SUCCEEDED(wil::make_hstring_from_buffer_nothrow(
			wistd::move(theBuffer), &promoted));
...

	{
		...
		REQUIRE_FAILED(wil::make_hstring_from_buffer_nothrow(
			wistd::move(theBuffer), nullptr));
...
#ifdef WIL_ENABLE_EXCEPTIONS
	{
		wchar_t *bufferStorage = nullptr;
		wil::unique_hstring_buffer theBuffer;
		THROW_IF_FAILED(::WindowsPreallocateStringBuffer(
			ARRAYSIZE(kittens), &bufferStorage, &theBuffer));
		THROW_IF_FAILED(
			StringCchCopyW(bufferStorage, ARRAYSIZE(kittens), kittens));
		...
		wil::unique_hstring promoted;
		REQUIRE_NOTHROW(
			promoted = wil::make_hstring_from_buffer(wistd::move(theBuffer)));
		REQUIRE(static_cast<bool>(promoted));
		REQUIRE_FALSE(static_cast<bool>(theBuffer));
	}
#endif
...
static void __stdcall ThreadPoolWaitTestCallback(
	_Inout_ PTP_CALLBACK_INSTANCE /*instance*/, _Inout_opt_ void *context,
	_Inout_ PTP_WAIT wait, _In_ TP_WAIT_RESULT /*waitResult*/) {
	ThreadPoolWaitTestContext &myContext =
		*reinterpret_cast<ThreadPoolWaitTestContext *>(context);
...
void ThreadPoolWaitTestHelper(bool requireExactCallbackCount) {
	...
	WaitResourceT wait;
	wait.reset(
		CreateThreadpoolWait(ThreadPoolWaitTestCallback, &myContext, nullptr));
	REQUIRE(wait);

	SetThreadpoolWait(wait.get(), myContext.Event.get(), nullptr);

	constexpr int loopCount = 5;
	for (int currCallbackCount = 0; currCallbackCount != loopCount;
		 ++currCallbackCount) {
		// Signal event.
		myContext.Event.SetEvent();

		// Wait until 'myContext.Counter' increments by 1.
		for (int itr = 0; itr != 50 && currCallbackCount == myContext.Counter;
			 ++itr) {
			Sleep(10);
		}
	...
	if (requireExactCallbackCount) {
		...
		REQUIRE((loopCount + 1 == myContext.Counter ||
				 loopCount == myContext.Counter));
	}
}

TEST_CASE("WindowsInternalTests::ThreadPoolWaitTest",
		  "[resource][unique_threadpool_wait]") {
...
static void __stdcall ThreadPoolWaitWorkCallback(
	...
	ThreadPoolWaitWorkContext &myContext =
		*reinterpret_cast<ThreadPoolWaitWorkContext *>(context);
	::InterlockedIncrement(&myContext.Counter);
}
...
void ThreadPoolWaitWorkHelper(bool requireExactCallbackCount) {
	...
	WaitResourceT work;
	work.reset(
		CreateThreadpoolWork(ThreadPoolWaitWorkCallback, &myContext, nullptr));
	REQUIRE(work);
...
TEST_CASE("WindowsInternalTests::ThreadPoolWorkTest",
		  "[resource][unique_threadpool_work]") {
...
static void __stdcall ThreadPoolTimerWorkCallback(
	...
	ThreadPoolTimerWorkContext &myContext =
		*reinterpret_cast<ThreadPoolTimerWorkContext *>(context);
	myContext.Event.SetEvent();
	::InterlockedIncrement(&myContext.Counter);
}

template <typename TimerResourceT, typename DueTimeT,
		  typename SetThreadpoolTimerT>
void ThreadPoolTimerWorkHelper(SetThreadpoolTimerT const &setThreadpoolTimerFn,
							   bool requireExactCallbackCount) {
	ThreadPoolTimerWorkContext myContext;
	REQUIRE_SUCCEEDED(myContext.Event.create());

	TimerResourceT timer;
	timer.reset(CreateThreadpoolTimer(ThreadPoolTimerWorkCallback, &myContext,
									  nullptr));
	REQUIRE(timer);
	...
	for (int currCallbackCount = 0; currCallbackCount != loopCount;
		...
		setThreadpoolTimerFn(timer.get(),
							 reinterpret_cast<DueTimeT *>(&dueTime), 0,
							 allowedWindow);
	...
	const auto allowedWindow = 0;
	LONGLONG dueTime = -5 * 10000I64; // 5ms
	setThreadpoolTimerFn(timer.get(), reinterpret_cast<DueTimeT *>(&dueTime), 0,
						 allowedWindow);
...
TEST_CASE("WindowsInternalTests::ThreadPoolTimerTest",
		  "[resource][unique_threadpool_timer]") {
	static_assert(sizeof(FILETIME) == sizeof(LONGLONG),
				  "FILETIME and LONGLONG must be same size");
	ThreadPoolTimerWorkHelper<wil::unique_threadpool_timer, FILETIME>(
		SetThreadpoolTimer, false);
	ThreadPoolTimerWorkHelper<wil::unique_threadpool_timer_nocancel, FILETIME>(
...
static void __stdcall SlimEventTrollCallback(
	...
	// Note: This relies on the fact that the 'wil::slim_event' class only has a
...
TEST_CASE("WindowsInternalTests::SlimEventTests", "[resource][slim_event]") {
	{
		...
		wil::unique_threadpool_timer trollTimer(
			CreateThreadpoolTimer(SlimEventTrollCallback, &event, nullptr));
		REQUIRE(trollTimer);

		FILETIME trollDueTime = wil::filetime::from_int64(0);
		SetThreadpoolTimer(trollTimer.get(), &trollDueTime, /*period(ms)*/ 5,
						   /*window(ms)*/ 0);

		// Ensure we timeout in spite of being constantly woken up
		// unnecessarily.
		REQUIRE_FALSE(event.wait(/*timeout(ms)*/ 100));

		wil::unique_threadpool_timer friendlyTimer(
			CreateThreadpoolTimer(SlimEventFriendlyCallback, &event, nullptr));
		REQUIRE(friendlyTimer);

		FILETIME friendlyDueTime = wil::filetime::from_int64(UINT64(
			-100 * wil::filetime_duration::one_millisecond)); // 100ms (relative
															  // to now)
		SetThreadpoolTimer(friendlyTimer.get(), &friendlyDueTime,
						   /*period(ms)*/ 0, /*window(ms)*/ 0);
...
static void __stdcall ConditionVariableCallback(
	_Inout_ PTP_CALLBACK_INSTANCE /*Instance*/, _In_ void *Context) {
	auto callbackContext = reinterpret_cast<T *>(Context);

	// Acquire the lock to ensure we don't notify the condition variable before
	// the other thread has gone to sleep.
...
TEST_CASE("WindowsInternalTests::ConditionVariableTests",
	...
	SECTION("Test 'wil::condition_variable' with 'wil::critical_section'") {
		...
		REQUIRE(TrySubmitThreadpoolCallback(
			ConditionVariableCallback<ConditionVariableCSCallbackContext>,
	...
	SECTION("Test 'wil::condition_variable' with 'wil::srwlock'") {
		...
		// Test exclusive lock.
		{
			...
			// Schedule the thread that will wake up this thread.
			REQUIRE(TrySubmitThreadpoolCallback(
				ConditionVariableCallback<ConditionVariableSRWCallbackContext>,
				&callbackContext, nullptr));
...
TEST_CASE("WindowsInternalTests::ReturnWithExpectedTests", "[result_macros]") {
	...
	REQUIRE_RETURNS_EXPECTED(E_UNEXPECTED, [] {
		RETURN_IF_FAILED_WITH_EXPECTED(E_UNEXPECTED, E_FAIL, E_UNEXPECTED,
	...
	REQUIRE_RETURNS_EXPECTED(E_FAIL, [] {
		RETURN_IF_FAILED_WITH_EXPECTED(E_FAIL, E_UNEXPECTED, E_POINTER,
									   E_INVALIDARG);
		return S_OK;
	});
}
...
TEST_CASE("WindowsInternalTests::LogWithExpectedTests", "[result_macros]") {
	...
	// Succeeded
	REQUIRE_LOG(S_OK, [] {
		REQUIRE(S_OK == LOG_IF_FAILED_WITH_EXPECTED(MDEC(hrOKRef()), E_FAIL,
													E_INVALIDARG));
	});

	// Expected
	REQUIRE_LOG(S_OK, [] {
		REQUIRE(E_UNEXPECTED == LOG_IF_FAILED_WITH_EXPECTED(
									E_UNEXPECTED, E_UNEXPECTED, E_INVALIDARG));
	});
	REQUIRE_LOG(S_OK, [] {
		REQUIRE(E_UNEXPECTED ==
				LOG_IF_FAILED_WITH_EXPECTED(E_UNEXPECTED, E_FAIL, E_UNEXPECTED,
											E_POINTER, E_INVALIDARG));
	});
...
template <template <typename> class Wrapper> void VerifyAlignment() {
	// Some of the wrappers require a method called ProcessShutdown(), so we'll
	// give it one.
	...
	static_assert(alignof(alignment_sensitive_struct) != alignof(char),
				  "Need to choose a better alignment-sensitive type");
	...
	static_assert(alignof(attempted_misalignment) ==
					  alignof(alignment_sensitive_struct),
				  "Wrapper type does not respect alignment");

	// Verify that the wrapper type placed the inner object at proper alignment.
	// Note: use std::addressof in case the alignment_sensitive_struct overrides
	// the & operator.
	REQUIRE(reinterpret_cast<uintptr_t>(
				std::addressof(possibly_misaligned.wrapper.get())) %
				alignof(alignment_sensitive_struct) ==
			0);
}
...
TEST_CASE("WindowsInternalTests::ModuleReference", "[wrl]") {
	...
	struct FakeModuleBase : Microsoft::WRL::Details::ModuleBase {
		...
		STDMETHOD_(unsigned long, GetObjectCount)() const { return count; }
		// Dummy implementations of everything else (never called).
		STDMETHOD_(const Microsoft::WRL::Details::CreatorMap **,
		...
		(const wchar_t *, const wchar_t **, _Inout_ RO_REGISTRATION_COOKIE *,
		 unsigned int) {
			return E_NOTIMPL;
		}
		STDMETHOD(UnregisterWinRTObject)
		(const wchar_t *, _In_ RO_REGISTRATION_COOKIE) {
			return E_NOTIMPL;
		}
		STDMETHOD(RegisterCOMObject)
		(const wchar_t *, _In_ IID *, _In_ IClassFactory **, _Inout_ DWORD *,
		 unsigned int) {
			return E_NOTIMPL;
		}
		STDMETHOD(UnregisterCOMObject)
		(const wchar_t *, _Inout_ DWORD *, unsigned int) {
			return E_NOTIMPL;
		}
	};
	FakeModuleBase fake;
	...
	(defined(NTDDI_WIN10_CO)                                                   \
		 ...
		 : WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP |                  \
								   WINAPI_PARTITION_SYSTEM |                   \
								   WINAPI_PARTITION_GAMES))
TEST_CASE("WindowsInternalTests::VerifyModuleReferencesForThread",
		  "[win32_helpers]") {
	bool success = true;
	std::thread([&] {

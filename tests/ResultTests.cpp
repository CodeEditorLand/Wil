...
struct SharedObject {
	SharedObject() { ::InterlockedIncrement(&objectCount); }

	~SharedObject() { ::InterlockedDecrement(&objectCount); }
...
TEST_CASE("ResultTests::ExceptionHandling", "[result]") {
	...
	SECTION("Test 'what()' implementation on ResultException") {
		auto swap = witest::AssignTemporaryValue(
			&wil::g_fResultThrowPlatformException, false);
		try {
	...
	SECTION("Test messaging from an unhandled std exception") {
		// #pragma warning(suppress: 28931)  // unused assignment -- it IS being
		// used... seems like a tool issue.
		auto hr = []() {
			try {
		...
		REQUIRE(failures.size() == 1);
		REQUIRE(failures[0].hr ==
				HRESULT_FROM_WIN32(ERROR_UNHANDLED_EXCEPTION));
		REQUIRE(wcsstr(failures[0].pszMessage, L"runtime") !=
				nullptr); // should get the exception what() string...
	...
	SECTION("Test messaging from bad_alloc") {
		...
		REQUIRE(wcsstr(failures[0].pszMessage, L"alloc") !=
				nullptr); // should get the exception what() string...
	...
	SECTION("Test messaging from a WIL exception") {
		...
		REQUIRE(wcsstr(failures[1].pszMessage, L"Exception") !=
				nullptr); // should get the exception debug string...
	...
	SECTION("Exception test (different exception type thrown...)") {
		...
		try {
			...
			REQUIRE(exception.GetFailureInfo().uLineNumber ==
					line); // should have thrown new, so we should have the
						   // rethrow line number
	...
	SECTION("Exception test (rethrow same exception type...)") {
		...
		try {
			...
			REQUIRE(exception.GetFailureInfo().uLineNumber ==
					line); // should have re-thrown the original exception (with
	...
	SECTION("Test catch message") {
		...
		REQUIRE(wcsstr(failures[0].pszMessage, L"alloc") !=
				nullptr); // should get the exception what() string...
		REQUIRE(wcsstr(failures[0].pszMessage, L"train") !=
				nullptr); // should *also* get the message...
	...
	SECTION("Test ResultFromException...") {
		...
		auto hr = wil::ResultFromException([&] { throw std::bad_alloc(); });
		REQUIRE(failures.empty());
		REQUIRE(hr == E_OUTOFMEMORY);
	}
	failures.clear();
	...
	SECTION("Explicit failfast for unrecognized") {
		REQUIRE_CRASH(wil::ResultFromException([&] { throw E_FAIL; }));
	}
	failures.clear();
	...
	SECTION("Manual debug-only validation of the SEH failfast") {
		...
		auto hr2 = wil::ResultFromExceptionDebug(
			WI_DIAGNOSTICS_INFO, wil::SupportedExceptions::Thrown, [&] {
				// Uncomment to test SEH fail-fast
	...
	SECTION("Standard") {
		auto line = __LINE__;
		auto hr = wil::ResultFromExceptionDebug(
			WI_DIAGNOSTICS_INFO, [&] { THROW_HR(E_INVALIDARG); });
		REQUIRE(failures.size() == 2);
	...
	SECTION("bad_alloc") {
		auto hr = wil::ResultFromExceptionDebug(
			WI_DIAGNOSTICS_INFO, [&] { throw std::bad_alloc(); });
		REQUIRE(failures.size() == 1);
		REQUIRE(hr == E_OUTOFMEMORY);
	}
	failures.clear();
	...
	SECTION("std::exception") {
		auto hr = wil::ResultFromExceptionDebug(
			WI_DIAGNOSTICS_INFO, [&] { throw std::range_error("range"); });
		REQUIRE(failures.size() == 1);
...
void ExceptionHandlingCompilationTest() {
	...
	wil::ResultFromExceptionDebug(WI_DIAGNOSTICS_INFO,
								  wil::SupportedExceptions::All,
								  [&] { THROW_HR(E_FAIL); });

	wil::ResultFromException(WI_DIAGNOSTICS_INFO,
							 wil::SupportedExceptions::None, [&] {});
...
TEST_CASE("ResultTests::NoOriginationByDefault", "[result]") {
	...
	// We can't guarantee test order, so clear the error payload prior to
	// starting
	SetRestrictedErrorInfo(nullptr);

	[]() -> HRESULT { RETURN_HR(S_OK); }();
...
TEST_CASE("ResultTests::OriginatedWithMessagePreserved", "[result]") {
	...
	witest::RequireRestrictedErrorInfo(
		HRESULT_FROM_WIN32(ERROR_UNHANDLED_EXCEPTION),
		L"std::exception: Puppies not allowed");
...
TEST_CASE("ResultTests::ReportDoesNotChangeLastError", "[result]") {
	decltype(wil::details::g_pfnLoggingCallback) oopsie =
		[](wil::FailureInfo const &) noexcept {
			::SetLastError(ERROR_ABANDON_HIBERFILE);
		};
	auto swap = witest::AssignTemporaryValue(
		&wil::details::g_pfnLoggingCallback, oopsie);

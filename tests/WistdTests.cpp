...
TEST_CASE("WistdFunctionTests::StdFunctionConstructionTest", "[wistd]") {
...

	{
		...
		std::function<int()> stdFn = [holder]() { return holder.value; };
...
TEST_CASE("WistdFunctionTests::CopyAssignmentTest", "[wistd]") {
	object_counter_state state;
	{
		...
		{
			wistd::function<int()> copyFrom = [counter =
												   object_counter{state}]() {
				return counter.state->copy_count;
			};
			REQUIRE(0 == copyFrom());
...
TEST_CASE("WistdFunctionTests::MoveConstructionTest", "[wistd]") {
	object_counter_state state;
	{
		...
		// Because we move the underlying function object, we _must_ invalidate
		// the moved from function
...
TEST_CASE("WistdFunctionTests::MoveAssignmentTest", "[wistd]") {
	object_counter_state state;
	{
		...
		{
			wistd::function<int()> moveFrom = [counter =
												   object_counter{state}]() {
				return counter.state->copy_count;
			};
			REQUIRE(0 == moveFrom());
...
// MSVC's optimizer has had issues with wistd::function in the past when
// forwarding wistd::function objects to a function that accepts the arguments
// by value. This test exercises the workaround that we have in place. Note that
// this of course requires building with optimizations enabled
void ForwardingTest(wistd::function<int()> getValue,
...
template <typename... Args> void CallForwardingTest(Args &&...args) {
	ForwardingTest(wistd::forward<Args>(args)...);
}

TEST_CASE("WistdFunctionTests::OptimizationRegressionTest", "[wistd]") {
	CallForwardingTest(wistd::function<int()>(GetValue),
					   wistd::function<int(int)>(Negate),
					   wistd::function<int(int, int)>(Add));
}

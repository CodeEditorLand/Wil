...
TEST_CASE("NtResultTests::NtReturn", "[result]") {
	auto status = []() { NT_RETURN_NTSTATUS(STATUS_INVALID_CONNECTION); }();
	REQUIRE(status == STATUS_INVALID_CONNECTION);

	status = []() {
		NT_RETURN_NTSTATUS_MSG(STATUS_INVALID_CONNECTION,
							   "Test NT_RETURN_NTSTATUS_MSG");
	}();
	REQUIRE(status == STATUS_INVALID_CONNECTION);

	status = []() {
		NT_RETURN_NTSTATUS_MSG(STATUS_INVALID_CONNECTION,
							   "Test NT_RETURN_NTSTATUS_MSG %s",
							   L"with parameter");
	}();
	REQUIRE(status == STATUS_INVALID_CONNECTION);
	...
	status = []() {
		NT_RETURN_IF_NTSTATUS_FAILED_MSG(STATUS_INVALID_CONNECTION,
										 "Test NT_RETURN_NTSTATUS_MSG %s",
										 L"with parameter");
		return STATUS_SUCCESS;
	}();
...
TEST_CASE("NtResultTests::NtThrowCatch", "[result]") {
	// Throw NTSTATUS with immediate conversion to HRESULT. HRESULT would appear
	// in the logs.
	auto hr = []() {
		try {
	...
	// Throw HRESULT with conversion to NTSTATUS on a best effort. NTSTATUS
	...
	// Throw HRESULT with conversion to NTSTATUS on a best effort that maps to
	// generic error. NTSTATUS would appear in the logs.
	status = []() {
		try {
	...
	status = []() {
		try {
			THROW_NTSTATUS_MSG(STATUS_INVALID_CONNECTION,
							   "Throw STATUS_INVALID_CONNECTION as NTSTATUS");
		}
		NT_CATCH_RETURN();
	}();
	REQUIRE(status == STATUS_INVALID_CONNECTION);
	...
	status = []() {
		...
		NT_CATCH_RETURN_MSG("Catching STATUS_INVALID_CONNECTION thrown by "
							"NT_THROW_NTSTATUS_MSG");
	}();
	REQUIRE(status == STATUS_INVALID_CONNECTION);
}
#endif

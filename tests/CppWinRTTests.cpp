...
// HRESULT values that C++/WinRT throws as something other than
// winrt::hresult_error - e.g. a type derived from winrt::hresult_error, std::*,
// etc.
...
void CheckMapVector(
	std::vector<
		winrt::Windows::Foundation::Collections::IKeyValuePair<T, K>> const
		&test,
	std::map<T, K> const &src) {
...
TEST_CASE("CppWinRTTests::VectorToVector", "[cppwinrt]") {
	winrt::init_apartment();
	{
		...
		REQUIRE(wil::to_vector(sv.First()) == src_vector);
		REQUIRE(wil::to_vector(
					sv.as<winrt::Windows::Foundation::Collections::IIterable<
						winrt::hstring>>()) == src_vector);
	}
	{
	...
	{
		std::map<winrt::hstring, winrt::hstring> src_map{
			{L"kittens", L"fluffy"}, {L"puppies", L"cute"}};
...
TEST_CASE("CppWinRTTests::WilToCppWinRTExceptionTranslationTest",
		  "[cppwinrt]") {
...
TEST_CASE("CppWinRTTests::CppWinRTToWilExceptionTranslationTest",
		  "[cppwinrt]") {
	auto test = [](HRESULT hr) {
		try {
...
TEST_CASE("CppWinRTTests::ResultFromExceptionDebugTest", "[cppwinrt]") {
	auto test = [](HRESULT hr, wil::SupportedExceptions supportedExceptions) {
		auto result = wil::ResultFromExceptionDebug(
			WI_DIAGNOSTICS_INFO, supportedExceptions,
			[&]() { winrt::check_hresult(hr); });
		REQUIRE(hr == result);
	};
...
TEST_CASE("CppWinRTTests::CppWinRTConsistencyTest", "[cppwinrt]") {
	// Since setting 'winrt_to_hresult_handler' opts us into _all_ C++/WinRT
	// exception translation handling, we need to make sure that we preserve
	// behavior, at least with 'check_hresult', especially when C++/WinRT maps a
	// particular HRESULT value to a different exception type
	auto test = [](HRESULT hr) {
		try {
			winrt::check_hresult(hr);
		} catch (...) {
	...
	// C++/WinRT also maps a few std::* exceptions to various HRESULTs. We
	...
	// NOTE: C++/WinRT maps other 'std::exception' derived exceptions to E_FAIL,
	// however we preserve the WIL behavior that such exceptions become
...
template <bool value> struct EnabledTraits {
...
wil::com_task<void> test_sta_task(HANDLE e) {
	...
	// Remember original thread so we can return to it at the start of each test
	// (if desired).
	winrt::apartment_context context;
	...
	signal_later(sync.get()); // prevent void_com_task from completing before we
							  // call await_ready
	co_await void_com_task(value, sync.get());
	REQUIRE(*value == 2);
	REQUIRE(on_ui_thread());

	// Fancier version that produces a reference (which PPL and C++/WinRT don't
	// support).
	co_await context;		  // start on UI thread
	signal_later(sync.get()); // prevent intref_com_task from completing before
							  // we call await_ready
	...
	// Test that co_await of a com_task from a threadpool thread stays on the
	// threadpool. Also test move-only type.
	co_await winrt::resume_background(); // start on non-UI thread
	signal_later(sync.get()); // prevent string_com_task from completing before
							  // we call await_ready
	...
	// Test exceptions.
	co_await context;		  // start on UI thread
	signal_later(sync.get()); // prevent exception_com_task from completing
							  // before we call await_ready
	REQUIRE_THROWS_AS(co_await exception_com_task(sync.get()), int);
	REQUIRE(on_ui_thread());

	// Test forced apartment awareness via task conversion.
	signal_later(sync.get()); // prevent string_task from completing before we
							  // call await_ready
...
TEST_CASE("CppWinRTTests::SimpleTaskTest", "[cppwinrt]") {
	std::thread([] {
		...
		auto done = wil::shared_event(
			wil::unique_event(wil::EventOptions::ManualReset));
		auto handle = done.get();
		auto task = test_sta_task(handle);
		DWORD waitResult;
		while ((waitResult = MsgWaitForMultipleObjects(
					1, &handle, false, INFINITE, QS_ALLEVENTS)) ==
			   WAIT_OBJECT_0 + 1) {
			MSG msg;
...
// Define our own custom dispatcher that we can force it to behave in certain
// ways. wil::resume_foreground supports any dispatcher that has a
// dispatcher_traits.
...
struct TestDispatcher {
	...
	void TryEnqueue(TestDispatcherPriority priority,
					TestDispatcherHandler const &handler) const {
...
template <> struct dispatcher_traits<test::TestDispatcher> {
...
TEST_CASE("CppWinRTTests::ResumeForegroundTests", "[cppwinrt]") {
	...
	using Verify = decltype(wil::resume_foreground(
		winrt::Windows::System::DispatcherQueue{nullptr}));
	...
	[]() -> winrt::Windows::Foundation::IAsyncAction {
		...
		try {
			...
			seen = e.code() ==
				   HRESULT_FROM_WIN32(HRESULT_FROM_WIN32(ERROR_NO_TASK_QUEUE));
		}
		REQUIRE(seen);
		...
		co_await wil::resume_foreground(dispatcher,
										test::TestDispatcherPriority::Weird);
	}()
				.get();
}
#endif // coroutines
...
TEST_CASE("CppWinRTTests::ThrownExceptionWithMessage", "[cppwinrt]") {
	...
	witest::RequireRestrictedErrorInfo(E_INVALIDARG,

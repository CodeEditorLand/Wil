...
// IMallocSpy requires you to implement all methods, but we often only want one
// or two...
struct MallocSpy
	...
	virtual SIZE_T STDMETHODCALLTYPE PreRealloc(void *ptr, SIZE_T requestSize,
												void **newPtr,
												BOOL wasSpyed) override {
		*newPtr = ptr;
	...
	virtual void *STDMETHODCALLTYPE PostRealloc(void *ptr,
												BOOL wasSpyed) override {
	...
	virtual void *STDMETHODCALLTYPE PreGetSize(void *ptr,
											   BOOL wasSpyed) override {
	...
	virtual SIZE_T STDMETHODCALLTYPE PostGetSize(SIZE_T size,
												 BOOL wasSpyed) override {
	...
	virtual void *STDMETHODCALLTYPE PreDidAlloc(void *ptr,
												BOOL wasSpyed) override {
	...
	virtual int STDMETHODCALLTYPE PostDidAlloc(void * /*ptr*/,
											   BOOL /*wasSpyed*/,
											   int result) override {
		return result;
	}

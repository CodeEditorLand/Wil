...
// Required for pinterface template specializations that we depend on in this
// test
...
TEST_CASE("WinRTTests::VerifyTraitsTypes", "[winrt]") {
	static_assert(
		wistd::is_same_v<bool,
						 typename wil::details::LastType<int, bool>::type>,
		"");
	static_assert(
		wistd::is_same_v<int, typename wil::details::LastType<int>::type>, "");

	static_assert(
		wistd::is_same_v<IAsyncAction *,
						 decltype(wil::details::GetReturnParamPointerType(
							 &IFileIOStatics::WriteTextAsync))>,
		"");
	static_assert(
		wistd::is_same_v<IAsyncOperation<bool> *,
						 decltype(wil::details::GetReturnParamPointerType(
							 &ILauncherStatics::LaunchUriAsync))>,
		"");

	static_assert(
		wistd::is_same_v<void, decltype(wil::details::GetAsyncResultType(
								   static_cast<IAsyncAction *>(nullptr)))>,
		"");
	static_assert(
		wistd::is_same_v<boolean,
						 decltype(wil::details::GetAsyncResultType(
							 static_cast<IAsyncOperation<bool> *>(nullptr)))>,
		"");
	static_assert(
		wistd::is_same_v<IStorageFile *,
						 decltype(wil::details::GetAsyncResultType(
							 static_cast<IAsyncOperation<StorageFile *> *>(
								 nullptr)))>,
		"");
}

template <bool InhibitArrayReferences, bool IgnoreCase, typename LhsT,
		  typename RhsT>
void DoHStringComparisonTest(LhsT &&lhs, RhsT &&rhs, int relation) {
	using compare =
		wil::details::hstring_compare<InhibitArrayReferences, IgnoreCase>;
	...
	// We wish to test with both const and non-const values. We can do this for
	// free here so long as the type is not an array since changing the
	// const-ness of an array may change the expected results
#pragma warning(suppress : 4127)
	if (!wistd::is_array<wistd::remove_reference_t<LhsT>>::value &&
		...
		DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
			constLhs, rhs, relation);
	}
...
#pragma warning(suppress : 4127)
	if (!wistd::is_array<wistd::remove_reference_t<RhsT>>::value &&
		...
		DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
			lhs, constRhs, relation);
	}
}

// The two string arguments are expected to compare equal to one another using
// the specified IgnoreCase argument and contain at least one embedded null
// character
template <bool InhibitArrayReferences, bool IgnoreCase, size_t Size>
void DoHStringSameValueComparisonTest(const wchar_t (&lhs)[Size],
									  const wchar_t (&rhs)[Size]) {
	...
	// For non-const arrays, we should never deduce length, so even though we
	// append different values to each string, we do so after the last null
	// character, so they should never be read
	...
	// Const array - embedded nulls are included only if InhibitArrayReferences
	// is false
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhs, rhs, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsNonConstArray, InhibitArrayReferences ? 0 : 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsCstr, InhibitArrayReferences ? 0 : 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsRef, InhibitArrayReferences ? -1 : 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsStr, InhibitArrayReferences ? -1 : 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsHstr, InhibitArrayReferences ? -1 : 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsUniqueStr, InhibitArrayReferences ? -1 : 0);

	// Non-const array - *never* deduce length
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsNonConstArray, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsCstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsRef, -1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsStr, -1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsHstr, -1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsUniqueStr, -1);

	// C string - impossible to deduce length
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsCstr,
																rhsCstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsCstr, rhsRef,
																-1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsCstr, rhsStr,
																-1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsCstr,
																rhsHstr, -1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsCstr, rhsUniqueStr, -1);

	// HStringReference
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsRef, rhsRef,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsRef, rhsStr,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsRef, rhsHstr,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsRef, rhsUniqueStr, 0);

	// HString
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsStr, rhsStr,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsStr, rhsHstr,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsStr, rhsUniqueStr, 0);

	// Raw HSTRING
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsHstr,
																rhsHstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsHstr, rhsUniqueStr, 0);

	// wil::unique_hstring
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsUniqueStr, rhsUniqueStr, 0);

#ifdef WIL_ENABLE_EXCEPTIONS
	std::wstring lhsWstr(lhs, 7);
	std::wstring rhsWstr(rhs, 7);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr,
																rhsWstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstr, rhs, InhibitArrayReferences ? 1 : 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstr, rhsNonConstArray, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr,
																rhsCstr, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr, rhsRef,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr, rhsStr,
																0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr,
																rhsHstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstr, rhsUniqueStr, 0);
#endif
#ifdef __WI_HAS_STD_WSTRING_VIEW
	std::wstring_view lhsWstrview(lhs, Size - 1);
	std::wstring_view rhsWstrview(rhs, Size - 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstrview,
																rhsWstrview, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstrview, rhs, InhibitArrayReferences ? 1 : 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstrview, rhsNonConstArray, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstrview,
																rhsCstr, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstrview,
																rhsRef, 0);
	...
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstrview,
																rhsHstr, 0);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstrview, rhsUniqueStr, 0);
#endif
}

// It's expected that the first argument (lhs) compares greater than the second
// argument (rhs)
template <bool InhibitArrayReferences, bool IgnoreCase, size_t LhsSize,
		  size_t RhsSize>
void DoHStringDifferentValueComparisonTest(const wchar_t (&lhs)[LhsSize],
										   const wchar_t (&rhs)[RhsSize]) {
	...
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsNonConstArray, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhs, rhsCstr,
																1);
	...
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhs, rhsHstr,
																1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhs, rhsUniqueStr, 1);
	...
	// Non-const array
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsNonConstArray, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsCstr, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsRef, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsNonConstArray, rhsStr, 1);
	...
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsStr, rhsHstr,
																1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsStr, rhsUniqueStr, 1);

	// Raw HSTRING
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsHstr,
																rhsHstr, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsHstr, rhsUniqueStr, 1);

	// wil::unique_hstring
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsUniqueStr, rhsUniqueStr, 1);
	...
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr,
																rhsWstr, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr, rhs,
																1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(
		lhsWstr, rhsNonConstArray, 1);
	DoHStringComparisonTest<InhibitArrayReferences, IgnoreCase>(lhsWstr,
...
TEST_CASE("WinRTTests::HStringComparison", "[winrt][hstring_compare]") {
	...
	SECTION("Empty string") {
		...
		// str may end up referencing a null HSTRING. That's fine; we'll just
		// test null HSTRING twice
		HString str;
...
TEST_CASE("WinRTTests::HStringMapTest", "[winrt][hstring_compare]") {
	...
	for (auto &pair : wstringMap) {
		HString str;
		THROW_IF_FAILED(str.Set(pair.first.c_str(),
								static_cast<UINT>(pair.first.length())));
	...
	for (auto &pair : wstringMap) {
		...
		REQUIRE(itr->first ==
				HStringReference(pair.first.c_str(),
								 static_cast<UINT>(pair.first.length())));
...
TEST_CASE("WinRTTests::HStringCaseInsensitiveMapTest",
...
// This is not a test method, nor should it be called. This is a
// compilation-only test.
#ifdef WIL_ENABLE_EXCEPTIONS
void RunWhenCompleteCompilationTest() {
	{
		ComPtr<IAsyncOperation<HSTRING>> stringOp;
		wil::run_when_complete(
			stringOp.Get(), [](HRESULT /* result */, HSTRING /* value */) {});
	...
	{
		ComPtr<IAsyncOperationWithProgress<HSTRING, UINT64>>
			stringOpWithProgress;
		wil::run_when_complete(
			stringOpWithProgress.Get(),
			[](HRESULT /* result */, HSTRING /* value */) {});
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM)
...
TEST_CASE("WinRTTests::RunWhenCompleteMoveOnlyTest",
		  "[winrt][run_when_complete]") {
	auto op = Make<FakeAsyncOperation<int>>();
	REQUIRE(op);

	bool gotEvent = false;
	auto hr = wil::run_when_complete_nothrow(
		op.Get(), [&gotEvent, enforce = cannot_copy{}](HRESULT hr, int result) {
			(void)enforce;
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM)
TEST_CASE("WinRTTests::WaitForCompletionTimeout",
		  "[winrt][wait_for_completion]") {
	auto op = Make<FakeAsyncOperation<bool, boolean>>();
	REQUIRE(op);

	// The wait_for_completion* functions don't properly deduce the "decayed"
	// async type, so force it here
	auto asyncOp = static_cast<IAsyncOperation<bool> *>(op.Get());

	bool timedOut = false;
	REQUIRE_SUCCEEDED(
		wil::wait_for_completion_or_timeout_nothrow(asyncOp, 1, &timedOut));
	REQUIRE(timedOut);
}
...
void WaitForCompletionCompilationTest() {
	...
	FAIL_FAST_HR_MSG(E_UNEXPECTED,
					 "This is a compilation test, and should not be called");
	...
	// HRESULT wait_for_completion_nothrow(_In_
	// ABI::Windows::Foundation::IAsyncOperation<TResult>* operation,
	//     _Out_ typename wil::details::MapAsyncOpResultType<TResult>::type*
	//     result, COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS, DWORD timeout =
	//     INFINITE);
	...
	// TProgress>* operation,
	//     _Out_ typename wil::details::MapAsyncOpProgressResultType<TResult,
	//     TProgress>::type* result, COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS,
	...
	// template <typename TAsync = ABI::Windows::Foundation::IAsyncAction>
	// inline HRESULT wait_for_completion_or_timeout_nothrow(_In_ TAsync*
	// operation,
	//     DWORD timeoutValue, _Out_ bool* timedOut, COWAIT_FLAGS flags =
	//     COWAIT_DISPATCH_CALLS);
	wil::wait_for_completion_or_timeout_nothrow(action, timeoutValue,
												&timedOut);
	wil::wait_for_completion_or_timeout_nothrow(action, timeoutValue, &timedOut,
												COWAIT_DEFAULT);

	// template <typename TResult>
	// HRESULT wait_for_completion_or_timeout_nothrow(_In_
	// ABI::Windows::Foundation::IAsyncOperation<TResult>* operation,
	//     _Out_ typename wil::details::MapAsyncOpResultType<TResult>::type*
	//     result, DWORD timeoutValue, _Out_ bool* timedOut, COWAIT_FLAGS flags
	//     = COWAIT_DISPATCH_CALLS);
	wil::wait_for_completion_or_timeout_nothrow(operation, timeoutValue,
												&timedOut);
	wil::wait_for_completion_or_timeout_nothrow(operation, timeoutValue,
												&timedOut, COWAIT_DEFAULT);
	...
	// TProgress>* operation,
	//     _Out_ typename wil::details::MapAsyncOpProgressResultType<TResult,
	//     TProgress>::type* result, DWORD timeoutValue, _Out_ bool* timedOut,
	//     COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS);
	wil::wait_for_completion_or_timeout_nothrow(
		operationWithResult.Get(), &result, timeoutValue, &timedOut);
	wil::wait_for_completion_or_timeout_nothrow(operationWithResult.Get(),
												&result, timeoutValue,
												&timedOut, COWAIT_DEFAULT);

#ifdef WIL_ENABLE_EXCEPTIONS
	// template <typename TAsync = ABI::Windows::Foundation::IAsyncAction>
	// inline void wait_for_completion(_In_ TAsync* operation, COWAIT_FLAGS
	// flags = COWAIT_DISPATCH_CALLS, DWORD timeout = INFINITE);
	wil::wait_for_completion(action);
	wil::wait_for_completion(action, COWAIT_DEFAULT);

	// template <typename TResult, typename TReturn = typename
	// wil::details::MapToSmartType<typename
	// wil::details::MapAsyncOpResultType<TResult>::type>::type> TReturn
	//     wait_for_completion(_In_
	//     ABI::Windows::Foundation::IAsyncOperation<TResult>* operation,
	//     COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS, DWORD timeout =
	//     INFINITE);
	wil::wait_for_completion(operation);
	wil::wait_for_completion(operation, COWAIT_DEFAULT);

	// template <typename TResult, typename TProgress, typename TReturn =
	// typename wil::details::MapToSmartType<typename
	// wil::details::MapAsyncOpResultType<TResult>::type>::type> TReturn
	//     wait_for_completion(_In_
	//     ABI::Windows::Foundation::IAsyncOperationWithProgress<TResult,
	//     TProgress>* operation, COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS,
	//     DWORD timeout = INFINITE);
	result = wil::wait_for_completion(operationWithResult.Get());
	result =
		wil::wait_for_completion(operationWithResult.Get(), COWAIT_DEFAULT);
#endif
}
#pragma warning(pop)
#endif
...
TEST_CASE("WinRTTests::TimeTTests", "[winrt][time_t]") {
	// Verifying that converting DateTime variable set as the date that means 0
	// as time_t works
	DateTime time1 = {wil::SecondsToStartOf1970 *
					  wil::HundredNanoSecondsInSecond};
...
ComPtr<IVector<IInspectable *>> MakeSampleInspectableVector(UINT32 count = 5) {
	...
	REQUIRE_SUCCEEDED(GetActivationFactory(
...
ComPtr<IVector<HSTRING>> MakeSampleStringVector() {
	...
	const HStringReference items[] = {HStringReference(L"one"),
									  HStringReference(L"two"),
									  HStringReference(L"three")};

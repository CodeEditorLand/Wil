...
// Included first and then again later to ensure that we're able to "light up"
// new functionality based off new includes
...
TEST_CASE("ResourceTests::TestLastErrorContext",
		  "[resource][last_error_context]") {
	...
	// The context can be moved.
	{
		...
		REQUIRE(GetLastError() == 42);
		SetLastError(0);
		// error42 has been moved-from and should not do anything at
		// destruction.
	}
	...
	// The context can be dismissed, which cause it to do nothing at
	// destruction.
	{
		SetLastError(42);
		auto error42 = wil::last_error_context();
	...
	// The value in the context is unimpacted by other things changing the last
	// error
	{
		SetLastError(42);
		auto error42 = wil::last_error_context();
...
interface __declspec(uuid("ececcc6a-5193-4d14-b38e-ed1460c20b00")) ITest
	: public IUnknown {
...
TEST_CASE("ResourceTests::TestOperationsOnGenericSmartPointerClasses",
		  "[resource]") {
#ifdef WIL_ENABLE_EXCEPTIONS
	{
		...
		// wil::com_ptr
		wil::com_ptr<PointerTestObject> ptr3 =
			Microsoft::WRL::Make<PointerTestObject>();
		...
		static_assert(
			wistd::is_same<
				typename wil::smart_pointer_details<decltype(ptr2)>::pointer,
				HANDLE>::value,
			"type-mismatch");
		static_assert(
			wistd::is_same<
				typename wil::smart_pointer_details<decltype(ptr3)>::pointer,
				PointerTestObject *>::value,
			"type-mismatch");

		auto p2 = wil::detach_from_smart_pointer(ptr2);
		auto p3 = wil::detach_from_smart_pointer(ptr3);
		// auto p4 = wil::detach_from_smart_pointer(ptr4); // wil::shared_any_t
		// and std::shared_ptr do not support release().
		HANDLE p4{};
	...
	Microsoft::WRL::ComPtr<PointerTestObject> ptr4 =
		Microsoft::WRL::Make<PointerTestObject>();

	static_assert(
		wistd::is_same<
			typename wil::smart_pointer_details<decltype(ptr1)>::pointer,
			int *>::value,
		"type-mismatch");
	static_assert(
		wistd::is_same<
			typename wil::smart_pointer_details<decltype(ptr4)>::pointer,
			PointerTestObject *>::value,
		"type-mismatch");
...
void StlAdlTest() {
	// This test has exposed some Argument Dependent Lookup issues in wistd /
	// stl.  Primarily we're just looking for clean compilation.
...
void UniqueProcessInfo() {
	...
	CreateProcessW(nullptr, nullptr, nullptr, nullptr, FALSE, 0, nullptr,
				   nullptr, nullptr, &process);
...
void NoexceptConstructibleTest() {
	using BaseStorage =
		wil::details::unique_storage<wil::details::handle_resource_policy>;
	...
	// wil::unique_handle is one of the many types which are expected to be
	// noexcept constructible since they don't perform any "advanced"
	// initialization.
	static_assert(
		wistd::is_nothrow_default_constructible_v<wil::unique_handle>,
		"wil::unique_any_t should always be nothrow default constructible");
	static_assert(wistd::is_nothrow_constructible_v<wil::unique_handle, HANDLE>,
				  "wil::unique_any_t should be noexcept if the storage is");

	// The inverse: A throwing storage constructor.
	static_assert(
		wistd::is_nothrow_default_constructible_v<
			wil::unique_any_t<ThrowingConstructor>>,
		"wil::unique_any_t should always be nothrow default constructible");
	static_assert(
		!wistd::is_nothrow_constructible_v<
			wil::unique_any_t<ThrowingConstructor>, HANDLE>,
		"wil::unique_any_t shouldn't be noexcept if the storage isn't");

	// With a protected constructor wil::unique_any_t will be unable to
	// correctly "forward" the noexcept attribute, but the code should still
	// compile.
...
using unique_fakeclose_call =
	wil::unique_com_call<FakeComInterface, decltype(&CloseFakeComInterface),
						 CloseFakeComInterface>;
...
static void __stdcall FakeCall() { g_called = true; }
...
static void TestStringMaker(VerifyContents &&verifyContents) {
	PCWSTR values[] = {L"", L"value",
					   // 300 chars
					   L"012345678901234567890123456789012345678901234567890123"
					   L"4567890123456789012345678901234567890123456789"
					   L"012345678901234567890123456789012345678901234567890123"
					   L"4567890123456789012345678901234567890123456789"
					   L"012345678901234567890123456789012345678901234567890123"
					   L"4567890123456789012345678901234567890123456789"};
	...
	for (const auto &value : values) {
		...
		// In the case of the wil::unique_hstring and the empty string the
		// buffer is in a read only section and can't be written to, so
		// StringCchCopy(maker.buffer(), valueLength + 1, value) will fault
		// adding the nul terminator. Use memcpy_s specifying exact size that
		// will be zero in this case instead.
		memcpy_s(maker.buffer(), valueLength * sizeof(*value), value,
				 valueLength * sizeof(*value));
		result = maker.release();
		verifyContents(value, valueLength, result);

		{
			// no promote, ensure no leaks (not tested here, inspect in the
			// debugger)
...
static void VerifyMakeUniqueString(bool nullValueSupported = true) {
	...
	struct {
		...
		{L"value", L"value", 5}, {L"value", L"value"}, {L"va", L"va\0ue", 5},
		{L"v", L"value", 1},	 {L"\0", L"", 5},	   {L"\0", nullptr, 5},
	};

	using maker = wil::details::string_maker<StringType>;
	for (auto const &entry : testCaseEntries) {
		bool shouldSkipNullString =
			((wcscmp(entry.expectedValue, L"\0") == 0) && !nullValueSupported);
		if (!shouldSkipNullString) {
			auto desiredValue =
				wil::make_unique_string<StringType>(entry.expectedValue);
			auto stringValue = wil::make_unique_string<StringType>(
				entry.testValue, entry.testLength);
			auto stringValueNoThrow =
				wil::make_unique_string_nothrow<StringType>(entry.testValue,
															entry.testLength);
			auto stringValueFailFast =
				wil::make_unique_string_failfast<StringType>(entry.testValue,
															 entry.testLength);
			REQUIRE(wcscmp(maker::get(desiredValue), maker::get(stringValue)) ==
					0);
			REQUIRE(wcscmp(maker::get(desiredValue),
						   maker::get(stringValueNoThrow)) == 0);
			REQUIRE(wcscmp(maker::get(desiredValue),
						   maker::get(stringValueFailFast)) == 0);
		}
	}
}

TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerCoTaskMem",
		  "[resource][string_maker]") {
	VerifyMakeUniqueString<wil::unique_cotaskmem_string>();
	TestStringMaker<wil::unique_cotaskmem_string>(
		[](PCWSTR value, size_t /*valueLength*/,
		   const wil::unique_cotaskmem_string &result) {
			REQUIRE(wcscmp(value, result.get()) == 0);
		});
}

#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerLocalAlloc",
		  "[resource][string_maker]") {
	VerifyMakeUniqueString<wil::unique_hlocal_string>();
	TestStringMaker<wil::unique_hlocal_string>(
		[](PCWSTR value, size_t /*valueLength*/,
		   const wil::unique_hlocal_string &result) {
			REQUIRE(wcscmp(value, result.get()) == 0);
		});
}

TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerGlobalAlloc",
		  "[resource][string_maker]") {
	VerifyMakeUniqueString<wil::unique_hglobal_string>();
	TestStringMaker<wil::unique_hglobal_string>(
		[](PCWSTR value, size_t /*valueLength*/,
		   const wil::unique_hglobal_string &result) {
			REQUIRE(wcscmp(value, result.get()) == 0);
		});
}

TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerProcessHeap",
		  "[resource][string_maker]") {
	VerifyMakeUniqueString<wil::unique_process_heap_string>();
	TestStringMaker<wil::unique_process_heap_string>(
		[](PCWSTR value, size_t /*valueLength*/,
		   const wil::unique_process_heap_string &result) {
			REQUIRE(wcscmp(value, result.get()) == 0);
		});
}
#endif

TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerMidl",
		  "[resource][string_maker]") {
	VerifyMakeUniqueString<wil::unique_midl_string>();
	TestStringMaker<wil::unique_midl_string>(
		[](PCWSTR value, size_t /*valueLength*/,
		   const wil::unique_midl_string &result) {
			REQUIRE(wcscmp(value, result.get()) == 0);
		});
}

TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerHString",
		  "[resource][string_maker]") {
	...
	TestStringMaker<wil::unique_hstring>([](PCWSTR value, size_t valueLength,
											const wil::unique_hstring &result) {
		UINT32 length;
		REQUIRE(wcscmp(value,
					   WindowsGetStringRawBuffer(result.get(), &length)) == 0);
		REQUIRE(valueLength == length);
	});
}

#ifdef WIL_ENABLE_EXCEPTIONS
TEST_CASE("UniqueStringAndStringMakerTests::VerifyStringMakerStdWString",
		  "[resource][string_maker]") {
	std::string s;
...
TEST_CASE("UniqueStringAndStringMakerTests::VerifyLegacySTringMakers",
		  "[resource][string_maker]") {
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
...
TEST_CASE("UniqueMidlStringTests", "[resource][rpc]") {
	wil::unique_midl_ptr<int[]> intArray{
		reinterpret_cast<int *>(::MIDL_user_allocate(sizeof(int) * 10))};
	intArray[2] = 1;

	wil::unique_midl_ptr<int> intSingle{
		reinterpret_cast<int *>(::MIDL_user_allocate(sizeof(int) * 1))};
}
...
TEST_CASE("UniqueInvokeCleanupMembers", "[resource]") {
	...
	wil::unique_any<ThingWithDestroy *, decltype(&ThingWithDestroy::destroy),
					&ThingWithDestroy::destroy>
		p(&toDestroy);
	p.reset();
	REQUIRE(!p);
	...
	{
		wil::unique_struct<ThingToDestroy2, decltype(&ThingToDestroy2::destroy),
						   &ThingToDestroy2::destroy>
			other;
...
TEST_CASE("ComTokenCloser", "[resource]") {
	using token_tester_t = wil::unique_com_token<ITokenTester, DWORD_PTR,
												 decltype(MyTokenTesterCloser),
												 &MyTokenTesterCloser>;
...
TEST_CASE("ComTokenDirectCloser", "[resource]") {
	using token_tester_t =
		wil::unique_com_token<ITokenTester, DWORD_PTR,
							  decltype(&ITokenTester::DirectClose),
							  &ITokenTester::DirectClose>;

...
TEST_CASE("TokenHelpersTests::VerifyOpenCurrentAccessTokenNoThrow",
		  "[token_helpers]") {
	...
	REQUIRE_SUCCEEDED(
		wil::open_current_access_token_nothrow(&token, TOKEN_READ));
	REQUIRE(token != nullptr);

	REQUIRE_SUCCEEDED(wil::open_current_access_token_nothrow(
		&token, TOKEN_READ, wil::OpenThreadTokenAs::Current));
	REQUIRE(token != nullptr);

	REQUIRE_SUCCEEDED(wil::open_current_access_token_nothrow(
		&token, TOKEN_READ, wil::OpenThreadTokenAs::Self));
	REQUIRE(token != nullptr);
}

#ifdef WIL_ENABLE_EXCEPTIONS
TEST_CASE("TokenHelpersTests::VerifyOpenCurrentAccessToken",
		  "[token_helpers]") {
	// Open current thread/process access token
	...
	token = wil::open_current_access_token(TOKEN_READ,
										   wil::OpenThreadTokenAs::Current);
	REQUIRE(token != nullptr);

	token = wil::open_current_access_token(TOKEN_READ,
										   wil::OpenThreadTokenAs::Self);
	REQUIRE(token != nullptr);
}
#endif

#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
TEST_CASE("TokenHelpersTests::VerifyGetTokenInformationNoThrow",
		  "[token_helpers]") {
	SECTION("Passing a null token") {
		wil::unique_tokeninfo_ptr<TOKEN_USER> tokenInfo;
		REQUIRE_SUCCEEDED(
			wil::get_token_information_nothrow(tokenInfo, nullptr));
		REQUIRE(tokenInfo != nullptr);
	}

	SECTION("Passing a non null token, since it a fake token there is no "
			"tokenInfo and hence should fail, code path is correct") {
		HANDLE faketoken = GetStdHandle(STD_INPUT_HANDLE);
		wil::unique_tokeninfo_ptr<TOKEN_USER> tokenInfo;
		REQUIRE_FAILED(
			wil::get_token_information_nothrow(tokenInfo, faketoken));
	}
}

// Pseudo tokens can be passed to token APIs and avoid the handle allocations
// making use more efficient.
TEST_CASE("TokenHelpersTests::DemonstrateUseWithPseudoTokens",
		  "[token_helpers]") {
	wil::unique_tokeninfo_ptr<TOKEN_USER> tokenInfo;
	REQUIRE_SUCCEEDED(wil::get_token_information_nothrow(
		tokenInfo, GetCurrentProcessToken()));
	REQUIRE(tokenInfo != nullptr);

	REQUIRE_SUCCEEDED(wil::get_token_information_nothrow(
		tokenInfo, GetCurrentThreadEffectiveToken()));
	REQUIRE(tokenInfo != nullptr);

	// No thread token by default, this should fail
	REQUIRE_FAILED(
		wil::get_token_information_nothrow(tokenInfo, GetCurrentThreadToken()));
	REQUIRE(tokenInfo == nullptr);
}
...
TEST_CASE("TokenHelpersTests::VerifyGetTokenInformation", "[token_helpers]") {
	...
	wil::unique_tokeninfo_ptr<TOKEN_USER> tokenInfo(
		wil::get_token_information<TOKEN_USER>(nullptr));
	REQUIRE(tokenInfo != nullptr);
}
#endif
...
wil::unique_handle GetTokenToImpersonate() {
	...
	FAIL_FAST_IF_WIN32_BOOL_FALSE(::OpenProcessToken(
		GetCurrentProcess(), TOKEN_ALL_ACCESS, &processToken));

	wil::unique_handle impersonateToken;
	FAIL_FAST_IF_WIN32_BOOL_FALSE(::DuplicateToken(
		processToken.get(), SecurityImpersonation, &impersonateToken));

	return impersonateToken;
}

TEST_CASE("TokenHelpersTests::VerifyResetThreadTokenNoThrow",
		  "[token_helpers]") {
	...
	REQUIRE_SUCCEEDED(wil::impersonate_token_nothrow(impersonateToken.get(),
													 setThreadToken1));
	REQUIRE(IsImpersonating());

	SECTION(
		"Clear the token again, should be not impersonating, explicit reset") {
...
template <
	typename T,
	wistd::enable_if_t<!wil::details::MapTokenStructToInfoClass<T>::FixedSize>
		* = nullptr>
...
template <typename T, wistd::enable_if_t<
						  wil::details::MapTokenStructToInfoClass<T>::FixedSize>
						  * = nullptr>
void TestGetTokenInfoForCurrentThread() {
	T tokenInfo{};
	const auto hr = wil::get_token_information_nothrow(&tokenInfo, nullptr);
...
TEST_CASE("TokenHelpersTests::VerifyGetTokenInformation2", "[token_helpers]") {
	...
	// Fixed size and reports size using ERROR_INSUFFICIENT_BUFFER (perf
	// opportunity, ignore second allocation)
...
TEST_CASE("TokenHelpersTests::VerifyGetTokenInformationBadLength",
		  "[token_helpers]") {
	// Fixed size and reports size using ERROR_BAD_LENGTH (bug)
	TestGetTokenInfoForCurrentThread<TOKEN_ELEVATION>();
}

TEST_CASE("TokenHelpersTests::"
		  "VerifyGetTokenInformationSecurityImpersonationLevelErrorCases",
		  "[token_helpers]") {
	SECURITY_IMPERSONATION_LEVEL tokenInfo{};

	// SECURITY_IMPERSONATION_LEVEL does not support the effective token when it
	// is implicit. Demonstrate the error return in that case.
	REQUIRE(E_INVALIDARG == wil::get_token_information_nothrow(
								&tokenInfo, GetCurrentThreadEffectiveToken()));

	// Using an explicit token is supported but returns ERROR_NO_TOKEN when
	// there is no impersonation token be sure to use
	// RETURN_IF_FAILED_EXPECTED() and don't use the exception forms if this
	// case is not expected.
	REQUIRE(HRESULT_FROM_WIN32(ERROR_NO_TOKEN) ==
			wil::get_token_information_nothrow(&tokenInfo,
	...
	REQUIRE(S_OK == wil::get_token_information_nothrow(
						&tokenInfo, GetCurrentThreadToken()));

	RevertToSelf();
}
#endif

bool operator==(const SID_IDENTIFIER_AUTHORITY &left,
				const SID_IDENTIFIER_AUTHORITY &right) {
...
TEST_CASE("TokenHelpersTests::StaticSid", "[token_helpers]") {
	...
	auto staticSid =
		wil::make_static_sid(SECURITY_NT_AUTHORITY, SECURITY_BUILTIN_DOMAIN_RID,
							 DOMAIN_ALIAS_RID_GUESTS);
	auto largerSid = wil::make_static_sid(
		SECURITY_NT_AUTHORITY, SECURITY_BUILTIN_DOMAIN_RID,
		DOMAIN_ALIAS_RID_GUESTS, DOMAIN_ALIAS_RID_BACKUP_OPS);
	...
	REQUIRE(*GetSidSubAuthority(staticSid.get(), 0) ==
...
TEST_CASE("TokenHelpersTests::TestMembership", "[token_helpers]") {
	...
	REQUIRE_SUCCEEDED(wil::test_token_membership_nothrow(
		&member, GetCurrentThreadEffectiveToken(), SECURITY_NT_AUTHORITY,
...
TEST_CASE("TokenHelpersTests::VerifyGetTokenInfo", "[token_helpers]") {
	REQUIRE_NOTHROW(
		wil::get_token_information<TOKEN_APPCONTAINER_INFORMATION>());
	...
	GetTokenInformation(GetCurrentThreadEffectiveToken(), TokenSource, &ts,
						sizeof(ts), &resultSize);
...
TEST_CASE("TokenHelpersTests::VerifyGetTokenInfoFailFast", "[token_helpers]") {
	// fixed size
	REQUIRE_NOTHROW(
		wil::get_token_information_failfast<TOKEN_APPCONTAINER_INFORMATION>());
	// variable size

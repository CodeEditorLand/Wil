...
TEST_CASE("SafeCastTests::SafeCastThrowsTemplateCheck", "[safecast]") {
	// In all cases, a value of '1' should be cast-able to any signed or
	// unsigned integral type without error
	SECTION("Unqualified char") {
		char orig = 1;
		wil::safe_cast<char>(orig);
		wil::safe_cast<signed char>(orig);
		// wil::safe_cast<unsigned char>     (orig); // No available conversion
		// in intsafe
		wil::safe_cast<short>(orig);
		wil::safe_cast<signed short>(orig);
		// wil::safe_cast<unsigned short>    (orig); // No available conversion
		// in intsafe
		wil::safe_cast<int>(orig);
		wil::safe_cast<signed int>(orig);
		// wil::safe_cast<unsigned int>      (orig); // No available conversion
		// in intsafe
		wil::safe_cast<long>(orig);
		// wil::safe_cast<unsigned long>     (orig); // No available conversion
		// in intsafe
		wil::safe_cast<__int64>(orig);
		wil::safe_cast<signed __int64>(orig);
		// wil::safe_cast<unsigned __int64>  (orig); // No available conversion
		// in intsafe
		wil::safe_cast<__int3264>(orig);
		// wil::safe_cast<unsigned __int3264>(orig); // No available conversion
		// in intsafe wil::safe_cast<wchar_t>           (orig); // No available
		// conversion in intsafe
	}
...
TEST_CASE("SafeCastTests::SafeCastNoThrowSyntaxCheck", "[safecast]") {
	...
	SECTION("safe_cast_nothrow two parameter potentially unsafe due to usage "
	...
	// safe_cast_nothrow two parameter known safe, throws compiler error as
	// expected
	// {
	//     unsigned char uc = 0;
	//     unsigned short us;
...
TEST_CASE("SafeCastTests::SafeCastNoFailures",
		  "[safecast]"){SECTION("INT -> LONG"){INT i = INT_MAX;
LONG l = wil::safe_cast_nothrow<LONG>(i);
REQUIRE(l == INT_MAX);
}

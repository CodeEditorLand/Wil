...
// Useful for validating that the copy constructor is never called (e.g. to
// validate perfect forwarding). Note that the copy constructor/assignment
// operator are not deleted since we want to be able to validate in scenarios
// that require CopyConstructible (e.g. for wistd::function)
struct fail_on_copy {
...
// Useful for validating that objects get copied e.g. as opposed to capturing a
// reference
struct value_holder {
	int value = 0xbadf00d;

	~value_holder() { value = 0xbadf00d; }
};
...
// State for object_counter type. This has the unfortunate side effect that the
// object_counter type cannot be used in contexts that require a default
// constructible type, but has the nice property that it allows for tests to run
// concurrently
struct object_counter_state {
	...
	LONG instance_count() { return constructed_count - destructed_count; }
};

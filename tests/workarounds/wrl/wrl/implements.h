...
#pragma warning(push)
#pragma warning(disable : 4584) // 'class1' : base-class 'class2' is already a
								// base-class of 'class3'
...
template <unsigned int flags> struct RuntimeClassFlags {
...
template <typename Derived, typename MixInType,
...
template <typename FactoryInterface = IInspectable> class ComposableBase {};
// Back-compat indicator for RuntimeClass to not support IWeakReferenceSource
typedef RuntimeClassFlags<WinRt | InhibitWeakReference>
	InhibitWeakReferencePolicy;

template <unsigned int RuntimeClassTypeT> struct ErrorHelper {
...
template <> struct ErrorHelper<InhibitRoOriginateError> {
	static void OriginateError(HRESULT hr, HSTRING message) {
		UNREFERENCED_PARAMETER(hr);
		UNREFERENCED_PARAMETER(message);
		// No-Op
...
// Sections automatically generate a list of pointers to CreatorMap through the
// linker Sections a and z are used as a terminators
...
extern "C" {
// Location of the first and last entries for the linker generated list of
// pointers to CreatorMapEntry
__declspec(selectany) __declspec(allocate("minATL$__a")) const
	CreatorMap *__pobjectentryfirst = nullptr;
...
__declspec(selectany) __declspec(allocate("minATL$__m")) const
	CreatorMap *__pobjectentrymid = nullptr;
__declspec(selectany) __declspec(allocate("minATL$__z")) const
	CreatorMap *__pobjectentrylast = nullptr;
}
...
class __declspec(novtable) ModuleBase {
  private:
	...
	static_assert(sizeof(moduleLock_) == sizeof(SRWLOCK),
  ...
  public:
	...
	ModuleBase() throw() {
#ifdef _DEBUG
		// WRLs support for activatable classes requires there is only one
		// instance of Module<>, this assert ensures there is only one. Since
		// Module<> is templatized, using different template parameters will
		// result in multiple instances, avoid this by making sure all code in a
		// component uses the same parameters. Note that the C++ CX runtime
		// creates an instance; Module<InProc, Platform::Details::InProcModule>,
		// so mixing it with non CX code can result in this assert.
		// WRL supports static and dynamically allocated Module<>, choose
		// dynamic by defining __WRL_DISABLE_STATIC_INITIALIZE__ and allocate
		// that instance with new but only once, for example in the main() entry
		// point of an application.
		__WRL_ASSERT__(::InterlockedCompareExchangePointer(
						   reinterpret_cast<void *volatile *>(&module_), this,
						   nullptr) == nullptr &&
					   "The module was already instantiated");

		SRWLOCK initSRWLOCK = SRWLOCK_INIT;
		__WRL_ASSERT__(reinterpret_cast<SRWLOCK *>(&moduleLock_)->Ptr ==
						   initSRWLOCK.Ptr &&
...
#ifdef _DEBUG
		__WRL_ASSERT__(::InterlockedCompareExchangePointer(
						   reinterpret_cast<void *volatile *>(&module_),
						   nullptr, this) == this &&
					   "The module was already instantiated");
#else
		module_ = nullptr;
	...
	STDMETHOD_(unsigned long, GetObjectCount)() const { return objectCount_; }
	...
	(_In_opt_z_ const wchar_t *, _In_z_ const wchar_t **activatableClassIds,
	 _Inout_ RO_REGISTRATION_COOKIE *cookie, unsigned int) = 0;
	STDMETHOD(UnregisterWinRTObject)
	(_In_opt_z_ const wchar_t *, _In_ RO_REGISTRATION_COOKIE) = 0;
	STDMETHOD(RegisterCOMObject)
	(_In_opt_z_ const wchar_t *, _In_ IID *, _In_ IClassFactory **,
	 _Inout_ DWORD *, unsigned int) = 0;
	STDMETHOD(UnregisterCOMObject)
	(_In_opt_z_ const wchar_t *, _Inout_ DWORD *, unsigned int) = 0;
};
...
class RuntimeClassBaseT : private RuntimeClassBase {
  protected:
	template <typename T>
	static HRESULT
	AsIID(_In_ T *implements, REFIID riid,
		  _Outptr_result_nullonfailure_ void **ppvObject) throw() {
		*ppvObject = nullptr;
#pragma warning(push)
...
#pragma warning(disable : 6326)
		// Conditional check using template parameter is constant and can be
		// used to optimize the code
		bool isRefDelegated = false;
		// Prefer InlineIsEqualGUID over other forms since it's better perf on
		// 4-byte aligned data, which is almost always the case.
		if (InlineIsEqualGUID(riid, __uuidof(IUnknown)) ||
			((RuntimeClassTypeT & WinRt) != 0 &&
...
#pragma warning(                                                               \
	suppress : 6102) // '*ppvObject' is used but may not be initialized
	...
	template <typename T>
	static HRESULT
	GetImplementedIIDS(_In_ T *implements, _Out_ ULONG *iidCount,
		...
		*iidCount = 0;
		...
		IID *iidArray =
			reinterpret_cast<IID *>(::CoTaskMemAlloc(sizeof(IID) * count));
		if (iidArray == nullptr) {
  ...
  public:
	HRESULT RuntimeClassInitialize() throw() { return S_OK; }
};
...
template <unsigned int type, typename I, bool doStrictCheck = true,
...
struct VerifyInterfaceHelper<ClassicCom, I, doStrictCheck, false> {
	static void Verify() throw() {
		...
		// Make sure that your interfaces inherit from IUnknown and are not
		// IUnknown and/or IInspectable based The IUnknown is allowed only on
		// RuntimeClass as first template parameter
		static_assert(__is_base_of(IUnknown, I) &&
						  !__is_base_of(IInspectable, I) &&
						  !(doStrictCheck && IsSame<IUnknown, I>::value),
					  "'I' has to derive from 'IUnknown' and not from "
					  "'IInspectable'. 'I' must not be IUnknown.");
#else
		static_assert(__is_base_of(IUnknown, I),
...
#ifdef __WRL_STRICT__
		// Make sure that your interfaces inherit from IUnknown and are not
		// IUnknown and/or IInspectable except when IInspectable is the first
		// template parameter
		static_assert(__is_base_of(IUnknown, I) &&
						  (doStrictCheck ? !(IsSame<IInspectable, I>::value ||
											 IsSame<IUnknown, I>::value)
										 : __is_base_of(IInspectable, I)),
					  "'I' has to derive from 'IUnknown' and must not be "
					  "IUnknown and/or IInspectable.");
#else
		static_assert(__is_base_of(IUnknown, I),
					  "'I' has to derive from 'IUnknown'.");
#endif
...
struct VerifyInterfaceHelper<WinRt, I, doStrictCheck, false> {
	static void Verify() throw() {
#ifdef __WRL_STRICT__
		// IWeakReferenceSource is exception for WinRt and can be used however
		// it cannot be first templated interface Make sure that your interfaces
		// inherit from IInspectable and are not IInspectable The IInspectable
		// is allowed only on RuntimeClass as first template parameter
		static_assert(
			(__is_base_of(IWeakReferenceSource, I) && doStrictCheck) ||
				(__is_base_of(IInspectable, I) &&
				 !(doStrictCheck && IsSame<IInspectable, I>::value)),
			"'I' has to derive from 'IWeakReferenceSource' or 'IInspectable' "
			"and must not be IInspectable");
#else
		...
		static_assert(__is_base_of(IWeakReference, I) ||
						  ...
						  __is_base_of(IInspectable, I),
					  "'I' has to derive from 'IWeakReference', "
					  "'IWeakReferenceSource' or 'IInspectable'");
#endif
	}
};
...
struct VerifyInterfaceHelper<type, I, true, true> {
	static void Verify() throw() {
		...
		// Allow using FtmBase on classes configured with
		// RuntimeClassFlags<WinRt> (Default configuration)
		static_assert(
			I::ClassFlags::value == type || type == WinRtClassicComMix ||
				__is_base_of(::Microsoft::WRL::Details::FtmBaseMarker, I),
			"Implements class must have the same and/or compatibile flags "
			"configuration");
#endif
	}
};
...
struct VerifyInterfaceHelper<type, I, false, true> {
	static void Verify() throw() {
		...
		static_assert(I::ClassFlags::value == type ||
					  ...
					  "Implements class must have the same and/or compatible "
					  "flags configuration."
					  "If you use WRL::FtmBase it cannot be specified as first "
					  "template parameter on RuntimeClass");

		// Besides make sure that the first interface on Implements meet flags
		// requirement
...
template <typename I0> struct __declspec(novtable) InterfaceTraits {
	typedef I0 Base;
	static const unsigned long IidCount = 1;

	template <unsigned int ClassType> static void Verify() throw() {
		VerifyInterfaceHelper<ClassType & WinRtClassicComMix, Base>::Verify();
	}

	template <typename T> static Base *CastToBase(_In_ T *ptr) throw() {
		return static_cast<Base *>(ptr);
	}

	template <typename T> static IUnknown *CastToUnknown(_In_ T *ptr) throw() {
		return static_cast<IUnknown *>(static_cast<Base *>(ptr));
	}

	template <typename T>
	_Success_(return ==
					 true) static bool CanCastTo(_In_ T *ptr, REFIID riid,
	...
	static void FillArrayWithIid(_Inout_ unsigned long *index,
...
struct __declspec(novtable) InterfaceTraits<CloakedIid<CloakedType>> {
	...
	static void FillArrayWithIid(_Inout_ unsigned long *,
								 _Inout_ IID *) throw() {}
};

// Specialization for Nil parameter
template <> struct __declspec(novtable) InterfaceTraits<Nil> {
	typedef Nil Base;
	static const unsigned long IidCount = 0;
	...
	template <typename T>
	_Success_(return == true) static bool CanCastTo(_In_ T *, REFIID,
													_Outptr_ void **) throw() {
		return false;
	}
};
...
// Verify inheritance
template <typename I, typename Base> struct VerifyInheritanceHelper {
	static void Verify() throw() {
		static_assert(
			Details::IsBaseOfStrict<typename InterfaceTraits<Base>::Base,
									typename InterfaceTraits<I>::Base>::value,
			"'I' needs to inherit from 'Base'.");
	}
};

template <typename I> struct VerifyInheritanceHelper<I, Nil> {
	static void Verify() throw() {}
};
...
// should always be checked for null on reference counting and cleanup
// operations.
inline Details::ModuleBase *GetModuleBase() throw() {
	return Details::ModuleBase::module_;
}

// ChainInterfaces - template allows specifying a derived COM interface along
// with its class hierarchy to allow QI for the base interfaces
template <typename I0, typename I1, typename I2 = Details::Nil,
		  typename I3 = Details::Nil, typename I4 = Details::Nil,
		  typename I5 = Details::Nil, typename I6 = Details::Nil,
		  typename I7 = Details::Nil, typename I8 = Details::Nil,
...
struct ChainInterfaces : I0 {
  protected:
	...
	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv) throw() {
		typename Details::InterfaceTraits<I0>::Base *ptr =
			Details::InterfaceTraits<I0>::CastToBase(this);
...
template <typename DerivedType, typename BaseType, bool hasImplements,
		  ...
		  typename I6, typename I7, typename I8, typename I9>
struct ChainInterfaces<MixIn<DerivedType, BaseType, hasImplements>, I1, I2, I3,
					   I4, I5, I6, I7, I8, I9> {
	static_assert(!hasImplements,
				  "Cannot use ChainInterfaces<MixIn<...>> to Mix a class "
  ...
  protected:
	...
	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv) throw() {
		BaseType *ptr =
			static_cast<BaseType *>(static_cast<DerivedType *>(this));
	...
	// It's not possible to cast to IUnknown when Base interface inherit more
	// interfaces The RuntimeClass is taking always the first interface as
	// IUnknown thus it's required to list IInspectable or IUnknown class before
	// MixIn<Derived, MixInType> parameter, such as: struct MyRuntimeClass :
	// RuntimeClass<IInspectable,
	// ChainInterfaces<MixIn<MyRuntimeClass,MyIndependentImplementation>, IFoo,
	// IBar>, MyIndependentImplementation  {}
...
// Helper template used by Implements. This template traverses a list of
// interfaces and adds them as base class and information to enable QI.
// doStrictCheck is typically false only for the first interface, allowing
// IInspectable to be explicitly specified only as the first interface.
template <typename RuntimeClassFlagsT, bool doStrictCheck,
		  typename... TInterfaces>
struct __declspec(novtable) ImplementsHelper;

template <typename T> struct __declspec(novtable) ImplementsMarker {};

template <typename I0, bool isImplements>
struct __declspec(novtable) MarkImplements;

template <typename I0> struct __declspec(novtable) MarkImplements<I0, false> {
	typedef I0 Type;
};

template <typename I0> struct __declspec(novtable) MarkImplements<I0, true> {
...
struct __declspec(novtable)
	MarkImplements<MixIn<DerivedType, BaseType, hasImplements>, true> {
...
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename I0,
		  typename... Bases>
struct __declspec(novtable)
	AdjustImplements<RuntimeClassFlagsT, doStrictCheck, I0, Bases...> {
	typedef ImplementsHelper<
		RuntimeClassFlagsT, doStrictCheck,
		typename MarkImplements<I0, __is_base_of(ImplementsBase, I0)>::Type,
		Bases...>
		Type;
};

// Use AdjustImplements to remove instances of "Details::Nil" from the type
// list.
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename... Bases>
struct __declspec(novtable) AdjustImplements<RuntimeClassFlagsT, doStrictCheck,
											 typename Details::Nil, Bases...> {
	typedef typename AdjustImplements<RuntimeClassFlagsT, doStrictCheck,
									  Bases...>::Type Type;
};

template <typename RuntimeClassFlagsT, bool doStrictCheck>
struct __declspec(novtable)
	AdjustImplements<RuntimeClassFlagsT, doStrictCheck> {
...
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename I0,
		  typename... TInterfaces>
struct __declspec(novtable)
	ImplementsHelper<RuntimeClassFlagsT, doStrictCheck, I0, TInterfaces...>
	: I0, AdjustImplements<RuntimeClassFlagsT, true, TInterfaces...>::Type {
	template <typename OtherRuntimeClassFlagsT, bool OtherDoStrictCheck,
  ...
  protected:
	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv,
					  bool *pRefDelegated = nullptr) throw() {
		VerifyInterfaceHelper<RuntimeClassFlagsT::value & WinRtClassicComMix,
							  I0, doStrictCheck>::Verify();
		// Prefer InlineIsEqualGUID over other forms since it's better perf on
		// 4-byte aligned data, which is almost always the case.
		...
		return AdjustImplements<RuntimeClassFlagsT, true,
								TInterfaces...>::Type::CanCastTo(riid, ppv,
																 pRefDelegated);
	}
	...
	unsigned long GetIidCount() throw() {
		return 1 + AdjustImplements<RuntimeClassFlagsT, true,
									TInterfaces...>::Type::GetIidCount();
	}

	// FillArrayWithIid
	void FillArrayWithIid(_Inout_ unsigned long *index,
						  _Inout_ IID *iids) throw() {
		*(iids + *index) = __uuidof(I0);
		(*index)++;
		AdjustImplements<RuntimeClassFlagsT, true,
						 TInterfaces...>::Type::FillArrayWithIid(index, iids);
	}
};

// Selector is used to "tag" base interfaces to be used in casting, since a
// runtime class may indirectly derive from the same interface or Implements<>
// template multiple times
template <typename base, typename disciminator>
struct __declspec(novtable) Selector : public base {};

// Specialization handles types that derive from ImplementsHelper (e.g. nested
// Implements).
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename I0,
		  typename... TInterfaces>
struct __declspec(novtable) ImplementsHelper<
	RuntimeClassFlagsT, doStrictCheck, ImplementsMarker<I0>, TInterfaces...>
	: Selector<I0, ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
									ImplementsMarker<I0>, TInterfaces...>>,
	  Selector<typename AdjustImplements<RuntimeClassFlagsT, true,
										 TInterfaces...>::Type,
			   ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
								ImplementsMarker<I0>, TInterfaces...>> {
	template <typename OtherRuntimeClassFlagsT, bool OtherDoStrictCheck,
			  typename... TOtherInterfaces>
	friend struct ImplementsHelper;
  ...
  protected:
	typedef Selector<I0, ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
										  ImplementsMarker<I0>, TInterfaces...>>
		CurrentType;
	typedef Selector<typename AdjustImplements<RuntimeClassFlagsT, true,
											   TInterfaces...>::Type,
					 ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
									  ImplementsMarker<I0>, TInterfaces...>>
		BaseType;
...
// CloakedIid instance. Since the first "real" interface should be checked
// against doStrictCheck, pass this through unchanged. Two specializations for
// cloaked prevent the need to use the Selector used in the Implements<> case.
// The same can't be done there because some type ambiguities are unavoidable.
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename I0,
		  typename I1, typename... TInterfaces>
struct __declspec(novtable) ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
											 CloakedIid<I0>, I1, TInterfaces...>
  ...
  protected:
	typedef
		typename AdjustImplements<RuntimeClassFlagsT, doStrictCheck, I0>::Type
			CurrentType;
	typedef typename AdjustImplements<RuntimeClassFlagsT, true, I1,
									  TInterfaces...>::Type BaseType;
	...
	IUnknown *CastToUnknown() throw() { return CurrentType::CastToUnknown(); }

	// Don't expose the cloaked IID(s), but continue processing the rest of the
	// interfaces
	unsigned long GetIidCount() throw() { return BaseType::GetIidCount(); }
...
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename I0>
struct __declspec(novtable)
	ImplementsHelper<RuntimeClassFlagsT, doStrictCheck, CloakedIid<I0>>
	: AdjustImplements<RuntimeClassFlagsT, doStrictCheck, I0>::Type {
	template <typename OtherRuntimeClassFlagsT, bool OtherDoStrictCheck,
			  typename... TInterfaces>
	friend struct ImplementsHelper;
...
struct __declspec(novtable)
  ...
  protected:
	...
	HRESULT CanCastTo(_In_ REFIID /*riid*/, _Outptr_ void ** /*ppv*/,
					  bool * /*pRefDelegated*/ = nullptr) throw() {
...
template <typename RuntimeClassFlagsT, bool doStrictCheck, typename C0,
		  typename C1, typename C2, typename C3, typename C4, typename C5,
		  typename C6, typename C7, typename C8, typename C9,
		  typename... TInterfaces>
struct __declspec(novtable) ImplementsHelper<
	RuntimeClassFlagsT, doStrictCheck,
	ChainInterfaces<C0, C1, C2, C3, C4, C5, C6, C7, C8, C9>, TInterfaces...>
  ...
  protected:
	template <unsigned int RuntimeClassTypeT>
	friend class Details::RuntimeClassBaseT;
	typedef typename AdjustImplements<RuntimeClassFlagsT, true,
									  TInterfaces...>::Type BaseType;

	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv,
					  bool *pRefDelegated = nullptr) throw() {
		ChainInterfaces<C0, C1, C2, C3, C4, C5, C6, C7, C8,
						C9>::template Verify<RuntimeClassFlagsT::value>();

		HRESULT hr =
			ChainInterfaces<C0, C1, C2, C3, C4, C5, C6, C7, C8, C9>::CanCastTo(
				riid, ppv);
		if (FAILED(hr)) {
			hr = BaseType::CanCastTo(riid, ppv, pRefDelegated);
		}
	...
	IUnknown *CastToUnknown() throw() {
		return ChainInterfaces<C0, C1, C2, C3, C4, C5, C6, C7, C8,
							   C9>::CastToUnknown();
	}
	...
	void FillArrayWithIid(_Inout_ unsigned long *index,
						  _Inout_ IID *iids) throw() {
		ChainInterfaces<C0, C1, C2, C3, C4, C5, C6, C7, C8,
						C9>::FillArrayWithIid(index, iids);
		BaseType::FillArrayWithIid(index, iids);
	}
};

// Mixin specialization
template <typename RuntimeClassFlagsT, typename DerivedType,
		  typename BaseMixInType, bool hasImplements, typename... TInterfaces,
		  bool doStrictCheck>
struct __declspec(novtable) ImplementsHelper<
	RuntimeClassFlagsT, doStrictCheck,
	MixIn<DerivedType, BaseMixInType, hasImplements>, TInterfaces...>
	: AdjustImplements<RuntimeClassFlagsT, true, TInterfaces...>::Type {
	static_assert(
		hasImplements,
  ...
  protected:
	...
	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv,
					  bool *pRefDelegated = nullptr) throw() {
		VerifyInterfaceHelper<RuntimeClassFlagsT::value & WinRtClassicComMix,
							  BaseMixInType, doStrictCheck>::Verify();

		HRESULT hr =
			static_cast<BaseMixInType *>(static_cast<DerivedType *>(this))
				->CanCastTo(riid, ppv);
		if (FAILED(hr)) {
			hr = BaseType::CanCastTo(riid, ppv, pRefDelegated);
		}
	...
	IUnknown *CastToUnknown() throw() {
		return static_cast<BaseMixInType *>(static_cast<DerivedType *>(this))
			->CastToUnknown();
	}
	...
	void FillArrayWithIid(_Inout_ unsigned long *index,
						  _Inout_ IID *iids) throw() {
		static_cast<BaseMixInType *>(static_cast<DerivedType *>(this))
			->FillArrayWithIid(index, iids);
		BaseType::FillArrayWithIid(index, iids);
	}
};

// Specialization handles inheriting COM objects. ComposableBase must be the
// last non-nil interface in the list. Trailing nil's are allowed for
// compatibility with some tools that pad out the list.
template <typename I0, typename...> struct AreAllNil {
...
template <> struct AreAllNil<Microsoft::WRL::Details::Nil> {
	static const bool value = true;
};

template <typename RuntimeClassFlagsT, typename FactoryInterface,
		  bool doStrictCheck, typename... TInterfaces>
struct __declspec(novtable)
	ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
					 ComposableBase<FactoryInterface>, TInterfaces...>
	: ImplementsHelper<RuntimeClassFlagsT, true,
					   ComposableBase<FactoryInterface>> {
	template <typename OtherRuntimeClassFlagsT, bool OtherDoStrictCheck,
			  typename... TOtherInterfaces>
	friend struct ImplementsHelper;
	template <unsigned int RuntimeClassTypeT> friend class RuntimeClassBaseT;
  ...
  protected:
	...
	typedef ImplementsHelper<RuntimeClassFlagsT, true,
							 ComposableBase<FactoryInterface>>
		BaseType;

	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv,
					  bool *pRefDelegated = nullptr) throw() {
		static_assert(AreAllNil<TInterfaces...>::value,
					  "ComposableBase should be the last template parameter to "
					  "RuntimeClass");
		return BaseType::CanCastTo(riid, ppv, pRefDelegated);
	}
...
struct __declspec(novtable) ImplementsHelper<RuntimeClassFlagsT, doStrictCheck,
  ...
  protected:
	...
	HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv,
					  bool *pRefDelegated) throw() {
		*pRefDelegated = true;
		return composableBase_.CopyTo(riid, ppv);
	}

	IUnknown *CastToUnknown() throw() { return nullptr; }

	unsigned long GetIidCount() throw() { return iidCount_; }
  ...
  public:
	HRESULT SetComposableBasePointers(
		_In_ IInspectable *base,
		_In_opt_ FactoryInterface *baseFactory = nullptr) throw() {
		if (composableBase_ != nullptr) {
#if (NTDDI_VERSION >= NTDDI_WINBLUE)
			ErrorHelper<RuntimeClassFlagsT::value &
						InhibitRoOriginateError>::OriginateError(E_UNEXPECTED,
																 nullptr);
#endif // (NTDDI_VERSION >= NTDDI_WINBLUE)
			return E_UNEXPECTED;
		}
...
// Implements - template implementing QI using the information provided through
// its template parameters Each template parameter has to be one of the
// following:
// * COM Interface
...
struct __declspec(novtable) Implements
	: Details::AdjustImplements<RuntimeClassFlags<WinRt>, true, I0,
								TInterfaces...>::Type,
  ...
  protected:
	typedef
		typename Details::AdjustImplements<RuntimeClassFlags<WinRt>, true, I0,
										   TInterfaces...>::Type BaseType;
	template <typename RuntimeClassFlagsT, bool doStrictCheck,
			  typename... TOtherInterfaces>
	...
	IUnknown *CastToUnknown() throw() { return BaseType::CastToUnknown(); }
...
struct __declspec(novtable)
	Implements<RuntimeClassFlags<flags>, I0, TInterfaces...>
	: Details::AdjustImplements<RuntimeClassFlags<flags>, true, I0,
								TInterfaces...>::Type,
	  Details::ImplementsBase {
  public:
  ...
  protected:
	typedef
		typename Details::AdjustImplements<RuntimeClassFlags<flags>, true, I0,
										   TInterfaces...>::Type BaseType;
	template <typename RuntimeClassFlagsT, bool doStrictCheck,
			  typename... TOtherInterfaces>
	friend struct Details::ImplementsHelper;
...
class FtmBase
	: public Implements<::Microsoft::WRL::RuntimeClassFlags<WinRtClassicComMix>,
						::Microsoft::WRL::CloakedIid<::IMarshal>>,
	  // Inheriting from FtmBaseMarker allows using FtmBase on classes
	  // configured with RuntimeClassFlags<WinRt> (Default configuration)
	  private ::Microsoft::WRL::Details::FtmBaseMarker {
	// defining type 'Super' for other compilers since '__super' is a
	// VC++-specific language extension
	using Super =
		Implements<::Microsoft::WRL::RuntimeClassFlags<WinRtClassicComMix>,
				   ::Microsoft::WRL::CloakedIid<::IMarshal>>;
  ...
  public:
	...
	// IMarshal Methods
#pragma warning(suppress : 6101) // PREFast cannot see through the smart-pointer
								 // invocation
	STDMETHOD(GetUnmarshalClass)
	(_In_ REFIID riid, _In_opt_ void *pv, _In_ DWORD dwDestContext,
	 _Reserved_ void *pvDestContext, _In_ DWORD mshlflags,
	 _Out_ CLSID *pCid) override {
		if (marshaller_) {
			return marshaller_->GetUnmarshalClass(
...
#pragma warning(suppress : 6101) // PREFast cannot see through the smart-pointer
	...
	(_In_ REFIID riid, _In_opt_ void *pv, _In_ DWORD dwDestContext,
	 _Reserved_ void *pvDestContext, _In_ DWORD mshlflags,
	 _Out_ DWORD *pSize) override {
		if (marshaller_) {
			return marshaller_->GetMarshalSizeMax(
	...
	(_In_ IStream *pStm, _In_ REFIID riid, _In_opt_ void *pv,
	 _In_ DWORD dwDestContext, _Reserved_ void *pvDestContext,
	 _In_ DWORD mshlflags) override {
		if (marshaller_) {
			return marshaller_->MarshalInterface(pStm, riid, pv, dwDestContext,
...
#pragma warning(suppress : 6101) // PREFast cannot see through the smart-pointer
	...
	(_In_ IStream *pStm, _In_ REFIID riid, _Outptr_ void **ppv) override {
		if (marshaller_) {
	...
	static HRESULT
	CreateGlobalInterfaceTable(_Out_ IGlobalInterfaceTable **git) throw() {
		*git = nullptr;
	...
	::Microsoft::WRL::ComPtr<IMarshal>
		marshaller_; // Holds a reference to the free threaded marshaler
};
...
class __declspec(novtable) PerfCountersBase {
  public:
	ULONG GetAddRefCount() throw() { return addRefCount_; }

	ULONG GetReleaseCount() throw() { return releaseCount_; }

	ULONG GetQueryInterfaceCount() throw() { return queryInterfaceCount_; }
  ...
  protected:
	PerfCountersBase() throw()
		: addRefCount_(0), releaseCount_(0), queryInterfaceCount_(0) {}

	void IncrementAddRefCount() throw() { InterlockedIncrement(&addRefCount_); }
...
#define UnknownInterlockedCompareExchangePointer                               \
	InterlockedCompareExchangePointer
#define UnknownInterlockedCompareExchangePointerForIncrement                   \
	InterlockedCompareExchangePointer
#define UnknownInterlockedCompareExchangePointerForRelease                     \
	InterlockedCompareExchangePointer
...
// Since variadic templates can't have a parameter pack after default arguments,
// provide a convenient helper for defaults.
...
template <class RuntimeClassFlagsT, bool implementsWeakReferenceSource,
		  bool implementsInspectable, bool implementsFtmBase,
		  typename... TInterfaces>
...
// Reference counting functions that check overflow. If overflow is detected,
// ref count value will stop at LONG_MAX, and the object being reference-counted
// will be leaked.
inline unsigned long
SafeUnknownIncrementReference(long volatile &refcount) throw() {
	long oldValue = refcount;
	while (oldValue != LONG_MAX &&
		   (UnknownInterlockedCompareExchangeForIncrement(
				&refcount, oldValue + 1, oldValue) != oldValue)) {
		oldValue = refcount;
	}
...
inline unsigned long
SafeUnknownDecrementReference(long volatile &refcount) throw() {
	long oldValue = refcount;
	while (oldValue != LONG_MAX &&
		   (UnknownInterlockedCompareExchangeForRelease(
				&refcount, oldValue - 1, oldValue) != oldValue)) {
		oldValue = refcount;
	}
...
template <class RuntimeClassFlagsT, bool implementsWeakReferenceSource,
		  bool implementsFtmBase, typename... TInterfaces>
class __declspec(novtable)
	RuntimeClassImpl<RuntimeClassFlagsT, implementsWeakReferenceSource, false,
					 implementsFtmBase, TInterfaces...>
	: public Details::AdjustImplements<RuntimeClassFlagsT, false,
									   TInterfaces...>::Type,
...
#endif
{
  public:
	...
	(REFIID riid, _Outptr_result_nullonfailure_ void **ppvObject) {
#ifdef _PERF_COUNTERS
  ...
  protected:
	...
	unsigned long InternalRelease() throw() {
		...
		if (newValue == 0) {
			...
			// that the destructor cannot observe values changing on other
			// threads.
	...
	unsigned long GetRefCount() const throw() { return refcount_; }
...
template <typename I> struct HasIInspectable<I, false> {
	static const bool isIInspectable = __is_base_of(IInspectable, I);
};

template <typename I> struct HasIInspectable<I, true> {
	static const bool isIInspectable =
		HasIInspectable<typename I::FirstInterface>::isIInspectable;
};
...
#else
template <typename I0,
		  bool isIInspectable = HasIInspectable<I0>::isIInspectable>
#endif
struct IInspectableInjector;

template <typename I0> struct IInspectableInjector<I0, true> {
	typedef Details::Nil InspectableIfNeeded;
};

template <typename I0> struct IInspectableInjector<I0, false> {
...
class __declspec(novtable)
	RuntimeClassImpl<RuntimeClassFlagsT, false, true, false, I0, TInterfaces...>
	: public Details::AdjustImplements<
		  RuntimeClassFlagsT, false,
		  typename IInspectableInjector<I0>::InspectableIfNeeded, I0,
		  TInterfaces...>::Type,
	  public RuntimeClassBaseT<RuntimeClassFlagsT::value>,
	  protected RuntimeClassFlags<InhibitWeakReference>,
	  public DontUseNewUseMake
  ...
  public:
	...
	STDMETHOD(GetRuntimeClassName)(_Out_ HSTRING *runtimeClassName) {
		*runtimeClassName = nullptr;
		...
		__WRL_ASSERT__(false && "Use InspectableClass macro to set runtime "
								"class name and trust level.");

#if (NTDDI_VERSION >= NTDDI_WINBLUE)
		ErrorHelper<RuntimeClassFlagsT::value &
					InhibitRoOriginateError>::OriginateError(E_NOTIMPL,
															 nullptr);
#endif // (NTDDI_VERSION >= NTDDI_WINBLUE)
		return E_NOTIMPL;
	}
...
#endif // !defined(__WRL_STRICT__) ||
	   // !defined(__WRL_FORCE_INSPECTABLE_CLASS_MACRO__)
...
class StrongReference {
  public:
	...
	unsigned long GetStrongReferenceCount() throw() { return strongRefCount_; }
...
// To support storing, encoding and decoding reference-count/pointers regardless
// of the target platform. In a RuntimeClass, the refCount_ member can mean
// either
...
//    2. If it is an encoded pointer to the weak reference, the most significant
...
//    2. If it is, shift the value to the left and cast it to a
//    WeakReferenceImpl*
//
const UINT_PTR EncodeWeakReferencePointerFlag = static_cast<UINT_PTR>(1)
												<< ((sizeof(UINT_PTR) * 8) - 1);

union ReferenceCountOrWeakReferencePointer {
	// Represents the count when it is a count (in practice only the least
	// significant 4 bytes)
	UINT_PTR refCount;
...
inline bool IsValueAPointerToWeakReference(INT_PTR value) { return value < 0; }

// Forward declaration
class WeakReferenceImpl;
inline INT_PTR
EncodeWeakReferencePointer(Microsoft::WRL::Details::WeakReferenceImpl *value);
inline Microsoft::WRL::Details::WeakReferenceImpl *
DecodeWeakReferencePointer(INT_PTR value);

// Helper functions, originally from winnt.h, needed to get the semantics right
// when fetching values in multi-threaded scenarios. This is in order to
// guarantee the compiler emits exactly one read (compiler may decide to
// re-fetch the value later on, which in some cases can break code)
#if defined(_ARM_)

FORCEINLINE
LONG ReadULongPtrNoFence(
...
inline T ReadValueFromPointerNoFence(_In_ const volatile T *value) {
	ULONG_PTR currentValue = ReadULongPtrNoFence(
		reinterpret_cast<const volatile ULONG_PTR *>(value));
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
class __declspec(novtable)
	RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0, TInterfaces...>
	: public Details::AdjustImplements<
		  RuntimeClassFlagsT, false,
		  typename IInspectableInjector<I0>::InspectableIfNeeded, I0,
		  IWeakReferenceSource, TInterfaces...>::Type,
	  public RuntimeClassBaseT<RuntimeClassFlagsT::value>,
	  public DontUseNewUseMake
#ifdef _PERF_COUNTERS
	,
...
#endif
{
  public:
	...
	RuntimeClassImpl() throw() { refCount_.rawValue = 1; }
...
inline INT_PTR
EncodeWeakReferencePointer(Microsoft::WRL::Details::WeakReferenceImpl *value) {
	return ((reinterpret_cast<INT_PTR>(value) >> 1) |
			EncodeWeakReferencePointerFlag);
}

inline Microsoft::WRL::Details::WeakReferenceImpl *
DecodeWeakReferencePointer(INT_PTR value) {
	return reinterpret_cast<Microsoft::WRL::Details::WeakReferenceImpl *>(
		static_cast<UINT_PTR>(value) << 1);
}
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
class __declspec(novtable)
	RuntimeClassImpl<RuntimeClassFlagsT, false, true, true, I0, TInterfaces...>
	: public RuntimeClassImpl<RuntimeClassFlagsT, false, true, false, I0,
							  TInterfaces...> {};

template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
class __declspec(novtable)
	RuntimeClassImpl<RuntimeClassFlagsT, true, true, true, I0, TInterfaces...>
	: public RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0,
							  FtmBase, TInterfaces...> {};

// To minimize breaks with code written against WRL before variadic support was
// added, this form is maintained.
template <typename... TInterfaces> struct InterfaceListHelper {
...
template <typename RuntimeClassFlagsT, bool implementsWeakReferenceSource,
		  bool implementsInspectable, bool implementsFtmBase,
		  typename... TInterfaces>
class RuntimeClass<InterfaceListHelper<TInterfaces...>, RuntimeClassFlagsT,
				   implementsWeakReferenceSource, implementsInspectable,
				   implementsFtmBase>
	: public RuntimeClassImpl<RuntimeClassFlagsT, implementsWeakReferenceSource,
							  implementsInspectable, implementsFtmBase,
							  TInterfaces...> {
  protected:
#pragma warning(                                                               \
	suppress : 6101) // Function only used internally and the value of
					 // 'ppvObject' is only used if *handled is true
	HRESULT
	CustomQueryInterface(REFIID /*riid*/,
...
// It inherits from Details::RuntimeClass that provides helper methods for
// reference counting and collecting IIDs
template <typename... TInterfaces>
class RuntimeClass
	: public Details::RuntimeClassImpl<DETAILS_RTCLASS_FLAGS_ARGUMENTS(
...
class WeakReferenceImpl final
  ...
  public:
	WeakReferenceImpl(_In_ IUnknown *unk) throw()
		: StrongReference(LONG_MAX / 2), unknown_(unk) {
		// Set ref count to 2 to avoid unnecessary interlocked increment
		// operation while returning WeakReferenceImpl from GetWeakReference
		// method. One reference is hold by the object the second is hold by the
		// caller of GetWeakReference method.
		refcount_ = 2;
	}
	...
	STDMETHOD(Resolve)
	(REFIID riid, _Outptr_result_maybenull_ _Result_nullonfailure_
		...
		*ppvObject = nullptr;
		...
		for (;;) {
			...
			// InterlockedCompareExchange calls _InterlockedCompareExchange
			// intrinsic thus we call directly _InterlockedCompareExchange to
			// save the call
			if (::_InterlockedCompareExchange(&this->strongRefCount_, ref + 1,
											  ref) == ref) {
#ifdef _PERF_COUNTERS
				// This artificially manipulates the strong ref count via AddRef
				// to account for the resolve interlocked operation above when
				// tallying reference counting operations.
				unknown_->AddRef();
		...
		HRESULT hr = unknown_->QueryInterface(
			riid, reinterpret_cast<void **>(ppvObject));
		unknown_->Release();
		return hr;
	}
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0,
				 TInterfaces...>::~RuntimeClassImpl() throw() {
	if (IsValueAPointerToWeakReference(refCount_.rawValue)) {
		WeakReferenceImpl *weakRef =
			DecodeWeakReferencePointer(refCount_.rawValue);
		weakRef->Release();
		weakRef = nullptr;
	}
}
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
unsigned long RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0,
							   TInterfaces...>::GetRefCount() const throw() {
	ReferenceCountOrWeakReferencePointer currentValue =
		ReadValueFromPointerNoFence<ReferenceCountOrWeakReferencePointer>(
			&refCount_);

	if (IsValueAPointerToWeakReference(currentValue.rawValue)) {
		WeakReferenceImpl *weakRef =
			DecodeWeakReferencePointer(currentValue.rawValue);
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
unsigned long RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0,
							   TInterfaces...>::InternalAddRef() throw() {
#ifdef _PERF_COUNTERS
	IncrementAddRefCount();
#endif
	...
	for (;;) {
		if (!IsValueAPointerToWeakReference(currentValue.rawValue)) {
			...
			INT_PTR previousValue = reinterpret_cast<INT_PTR>(
...
template <class RuntimeClassFlagsT, typename I0, typename... TInterfaces>
unsigned long RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0,
							   TInterfaces...>::InternalRelease() throw() {
#ifdef _PERF_COUNTERS
	IncrementReleaseCount();
#endif
...
RuntimeClassImpl<RuntimeClassFlagsT, true, true, false, I0, TInterfaces...>::
	GetWeakReference(_Outptr_ IWeakReference **weakReference) {
	...
	*weakReference = nullptr;
	...
	// WeakReferenceImpl is created with ref count 2 to avoid interlocked
	// increment
	...
	for (;;) {
		...
		weakRef->SetStrongReference(
			static_cast<unsigned long>(currentValue.refCount));

#ifdef __WRL_UNITTEST__
		OnBeforeGetWeakReferenceSwap();
#endif

		previousValue = reinterpret_cast<
			INT_PTR>(UnknownInterlockedCompareExchangePointer(
			reinterpret_cast<PVOID *>(&(
				this->refCount_
					.ifHighBitIsSetThenShiftLeftToYieldPointerToWeakReference)),
			reinterpret_cast<PVOID>(encodedWeakRef),
			currentValue
				.ifHighBitIsSetThenShiftLeftToYieldPointerToWeakReference));
		if (previousValue == currentValue.rawValue) {
			// No need to call AddRef in this case, WeakReferenceImpl is created
			// with ref count 2 to avoid interlocked increment
			*weakReference = weakRef;
			return S_OK;
...
template <typename T> class MakeAllocator {
  public:
	...
	void *Allocate() throw() {
		...
		if constexpr (alignof(T) > __STDCPP_DEFAULT_NEW_ALIGNMENT__) {
			return buffer_ = (char *)operator new(
	...
	void Detach() throw() { buffer_ = nullptr; }
...
// Make and MakeAndInitialize functions must not be marked as throw() as the
// constructor is allowed to throw exceptions.
template <typename T, typename... TArgs> ComPtr<T> Make(TArgs &&...args) {
	static_assert(
...
#pragma warning(push)
#pragma warning(                                                               \
	disable : 6387 6388 28196) // PREFast does not understand call to
							   // ComPtr<T>::CopyTo() is safe here

template <typename T, typename I, typename... TArgs>
HRESULT MakeAndInitialize(_Outptr_result_nullonfailure_ I **result,
						  TArgs &&...args) {
	static_assert(
		__is_base_of(Details::RuntimeClassBase, T),
		"Make can only instantiate types that derive from RuntimeClass");
	static_assert(__is_base_of(I, T),
...
#define InspectableClass(runtimeClassName, trustLevel)                         \
  public:                                                                      \
	...
	InternalGetRuntimeClassName() throw() {                                    \
		static_assert((RuntimeClassT::ClassFlags::value &                      \
					   ::Microsoft::WRL::WinRtClassicComMix) ==                \
							  ::Microsoft::WRL::WinRt ||                       \
						  (RuntimeClassT::ClassFlags::value &                  \
						   ::Microsoft::WRL::WinRtClassicComMix) ==            \
							  ::Microsoft::WRL::WinRtClassicComMix,            \
					  "'InspectableClass' macro must not be used with "        \
					  "ClassicCom clasess.");                                  \
		static_assert(                                                         \
			__is_base_of(::Microsoft::WRL::Details::RuntimeClassBase,          \
						 RuntimeClassT),                                       \
			"'InspectableClass' macro can only be used with "                  \
			"::Windows::WRL::RuntimeClass types");                             \
		static_assert(                                                         \
			!__is_base_of(IActivationFactory, RuntimeClassT),                  \
			"Incorrect usage of IActivationFactory interface. Make sure that " \
			"your RuntimeClass doesn't implement IActivationFactory "          \
			"interface use ::Windows::WRL::ActivationFactory instead or "      \
			"'InspectableClass' macro is not used on "                         \
			"::Windows::WRL::ActivationFactory");                              \
		...
		*runtimeName = nullptr;                                                \
		...
		if (name != nullptr) {                                                 \
			hr = ::WindowsCreateString(                                        \
				name, static_cast<UINT32>(::wcslen(name)), runtimeName);       \
		}                                                                      \
		return hr;                                                             \
	}                                                                          \
	...
	STDMETHOD(QueryInterface)                                                  \
	(REFIID riid, _Outptr_result_nullonfailure_ void **ppvObject) override {   \
		bool handled = false;                                                  \
	...
	STDMETHOD_(ULONG, Release)() override { return RuntimeClassT::Release(); } \
	STDMETHOD_(ULONG, AddRef)() override { return RuntimeClassT::AddRef(); }   \
                                                                               \
  private:
  ...
  public:                                                                      \
	...
	(REFIID riid, _Outptr_result_nullonfailure_ void **ppvObject) {            \
		...
		static_assert(                                                         \
			__is_base_of(::Microsoft::WRL::Details::RuntimeClassBase,          \
						 RuntimeClassT),                                       \
			"'MixInHelper' macro can only be used with "                       \
			"::Windows::WRL::RuntimeClass types");                             \
		static_assert(                                                         \
			!__is_base_of(IClassFactory, RuntimeClassT),                       \
			"Incorrect usage of IClassFactory interface. Make sure that your " \
			"RuntimeClass doesn't implement IClassFactory interface use "      \
			"::Windows::WRL::ClassFactory instead or 'MixInHelper' macro is "  \
			"not used on ::Windows::WRL::ClassFactory");                       \
		return RuntimeClassT::QueryInterface(riid, ppvObject);                 \
	}                                                                          \
	STDMETHOD_(ULONG, Release)() { return RuntimeClassT::Release(); }          \
	STDMETHOD_(ULONG, AddRef)() { return RuntimeClassT::AddRef(); }            \
                                                                               \
  private:

// Please make sure that those macros are in sync with those ones from
// 'wrl/module.h'
#ifndef WrlCreatorMapIncludePragmaEx
#define WrlCreatorMapIncludePragmaEx(className, group)                         \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'WrlCreatorMapIncludePragmaEx' macro");
#endif

#ifndef WrlCreatorMapIncludePragma
#define WrlCreatorMapIncludePragma(className)                                  \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'WrlCreatorMapIncludePragma' macro");
#endif

#ifndef ActivatableClassWithFactoryEx
#define ActivatableClassWithFactoryEx(className, factory, groupId)             \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'ActivatableClassWithFactoryEx' macro");
#endif

#ifndef ActivatableClassWithFactory
#define ActivatableClassWithFactory(className, factory)                        \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'ActivatableClassWithFactory' macro");
#endif

#ifndef ActivatableClass
#define ActivatableClass(className)                                            \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'ActivatableClass' macro");
#endif

#ifndef ActivatableStaticOnlyFactoryEx
#define ActivatableStaticOnlyFactoryEx(factory, serverName)                    \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'ActivatableStaticOnlyFactoryEx' macro");
#endif

#ifndef ActivatableStaticOnlyFactory
#define ActivatableStaticOnlyFactory(factory)                                  \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'ActivatableStaticOnlyFactory' macro");
#endif

#ifndef CoCreatableClassWithFactoryEx
#define CoCreatableClassWithFactoryEx(className, factory, groupId)             \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'CoCreatableClassWithFactory' macro");
#endif
...
#define CoCreatableClass(className)                                            \
	static_assert(false, "It's required to include 'wrl/module.h' to be able " \
						 "to use 'CoCreatableClass' macro");
#endif

#ifndef CoCreatableClassWrlCreatorMapInclude
#define CoCreatableClassWrlCreatorMapInclude(className)                        \
	static_assert(false,                                                       \
				  "It's required to include 'wrl/module.h' to be able to use " \
				  "'CoCreatableClassWrlCreatorMapInclude' macro");
#endif

...
TEST_CASE("EventWatcherTests::Construction", "[resource][event_watcher]") {
	...
	SECTION("Create unique_event_watcher_nothrow with unique_event_nothrow") {
		...
		auto watcher =
			wil::make_event_watcher_nothrow(wistd::move(eventToPass), [] {});
		REQUIRE(watcher != nullptr);
	...
	SECTION("Create unique_event_watcher_nothrow with handle") {
		...
		auto watcher =
			wil::make_event_watcher_nothrow(eventToDupe.get(), [] {});
		REQUIRE(watcher != nullptr);
	...
	SECTION("Create unique_event_watcher shared watcher") {
		wil::shared_event_watcher sharedWatcher =
			wil::make_event_watcher([] {});
	}
#endif
}
...
TEST_CASE("EventWatcherTests::VerifyLastChangeObserved",
		  "[resource][event_watcher]") {
	...
	for (auto const &eventOption : eventOptions) {
		auto allChangesMade = make_event(
			wil::EventOptions::ManualReset); // ManualReset to avoid hang in
											 // case where 2 callbacks are
											 // generated (a test failure).
		...
		int volatile countObserved = 0;
		auto watcher =
			wil::make_event_watcher_nothrow(make_event(eventOption), [&] {
				allChangesMade.wait();
				countObserved = countObserved + 1;
		...
		REQUIRE(
			(countObserved == 1 ||
			 countObserved == 2)); // ensure the race worked how we wanted it to
		REQUIRE(lastObservedState == stateToObserve);
	}
}

#define ROOT_KEY_PAIR                                                          \
	HKEY_CURRENT_USER, L"Software\\Microsoft\\RegistryWatcherTest"

TEST_CASE("RegistryWatcherTests::Construction",
		  "[registry][registry_watcher]") {
	SECTION("Create unique_registry_watcher_nothrow with string") {
		auto watcher = wil::make_registry_watcher_nothrow(
			ROOT_KEY_PAIR, true, [&](wil::RegistryChangeKind) {});
		REQUIRE(watcher);
	}
	...
	SECTION("Create unique_registry_watcher_nothrow with unique_hkey") {
		...
		REQUIRE_SUCCEEDED(HRESULT_FROM_WIN32(
			RegCreateKeyExW(ROOT_KEY_PAIR, 0, nullptr, 0, KEY_NOTIFY, nullptr,
							&keyToMove, nullptr)));

		auto watcher = wil::make_registry_watcher_nothrow(
			wistd::move(keyToMove), true, [&](wil::RegistryChangeKind) {});
		REQUIRE(watcher);
	...
	SECTION("Create unique_registry_watcher_nothrow with handle") {
		...
		auto watcher = wil::make_registry_watcher_nothrow(
			rootKey.get(), L"", true, [&](wil::RegistryChangeKind) {});
		REQUIRE(watcher);
	}

#ifdef WIL_ENABLE_EXCEPTIONS
	SECTION("Create unique_registry_watcher with string") {
		REQUIRE_NOTHROW(wil::make_registry_watcher(
			ROOT_KEY_PAIR, true, [&](wil::RegistryChangeKind) {}));
	}

	SECTION("Create unique_registry_watcher with unique_hkey") {
		wil::unique_hkey keyToMove;
		THROW_IF_FAILED(HRESULT_FROM_WIN32(
			RegCreateKeyExW(ROOT_KEY_PAIR, 0, nullptr, 0, KEY_NOTIFY, nullptr,
							&keyToMove, nullptr)));
...
void SetRegistryValue(_In_ HKEY hKey, _In_opt_ LPCWSTR lpSubKey,
					  _In_opt_ LPCWSTR lpValueName, _In_ DWORD dwType,
					  _In_reads_bytes_opt_(cbData) LPCVOID lpData,
					  _In_ DWORD cbData) {
	wil::unique_hkey key;
	REQUIRE(RegOpenKeyExW(hKey, lpSubKey, 0, KEY_WRITE, &key) == ERROR_SUCCESS);
	REQUIRE(RegSetValueExW(key.get(), lpValueName, 0, dwType,
						   static_cast<BYTE const *>(lpData),
						   cbData) == ERROR_SUCCESS);
}

TEST_CASE("RegistryWatcherTests::VerifyDelivery",
		  "[registry][registry_watcher]") {
	...
	auto watcher = wil::make_registry_watcher_nothrow(
		ROOT_KEY_PAIR, true, [&](wil::RegistryChangeKind changeType) {
			countObserved = countObserved + 1;
...
TEST_CASE("RegistryWatcherTests::VerifyLastChangeObserved",
		  "[registry][registry_watcher]") {
	RegDeleteTreeW(ROOT_KEY_PAIR);
	auto allChangesMade = make_event(
		wil::EventOptions::ManualReset); // ManualReset for the case where both
										 // registry operations result in a
										 // callback.
	auto processedChange = make_event();
	...
	auto watcher = wil::make_registry_watcher_nothrow(
		ROOT_KEY_PAIR, true,
		[&, called = false](wil::RegistryChangeKind) mutable {
			// This callback may be called more than once (since we modify the
			// key twice), but we're holding references to local variables.
			// Therefore, bail out if this is not the first time we're called
			if (called) {
				return;
			}
			called = true;
			...
			RegGetValueW(ROOT_KEY_PAIR, L"value", RRF_RT_REG_DWORD, nullptr,
						 &value, &cbValue);
	...
	REQUIRE(
		countObserved >=
		1); // Sometimes 2 events are observed, see if this can be eliminated.
	REQUIRE(lastObservedState == stateToObserve);
	REQUIRE(lastObservedValue == 1);
}

TEST_CASE("RegistryWatcherTests::VerifyDeleteBehavior",
		  "[registry][registry_watcher]") {
	auto notificationReceived = make_event();
	...
	RegDeleteTreeW(ROOT_KEY_PAIR); // delete the key to signal the watcher with
								   // the special error case
	REQUIRE(notificationReceived.wait(5000));
	REQUIRE(countObserved == 1);
	REQUIRE(observedChangeType == wil::RegistryChangeKind::Delete);
}

TEST_CASE("RegistryWatcherTests::VerifyResetInCallback",
		  "[registry][registry_watcher]") {
	auto notificationReceived = make_event();
	...
	wil::unique_registry_watcher_nothrow watcher =
		wil::make_registry_watcher_nothrow(
			ROOT_KEY_PAIR, TRUE, [&](wil::RegistryChangeKind) {
				watcher.reset();
				DWORD value = 2;
				SetRegistryValue(ROOT_KEY_PAIR, L"value", REG_DWORD, &value,
								 sizeof(value));
				notificationReceived.SetEvent();
			});
	REQUIRE(watcher);
...
TEST_CASE("RegistryWatcherTests::VerifyResetInCallbackStress",
		  "[LocalOnly][registry][registry_watcher][stress]") {
				...
				ROOT_KEY_PAIR, TRUE, [&](wil::RegistryChangeKind) {
					{
						...
						watcher.reset(); // get m_refCount to 1 to ensure the
										 // Release happens on the background
										 // thread
					}
					++value;
					SetRegistryValue(ROOT_KEY_PAIR, L"value", REG_DWORD, &value,
...
TEST_CASE("RegistryWatcherTests::VerifyResetAfterDelete",
		  "[registry][registry_watcher]") {
	auto notificationReceived = make_event();
	...
	wil::unique_registry_watcher_nothrow watcher =
		wil::make_registry_watcher_nothrow(
			ROOT_KEY_PAIR, true, [&](wil::RegistryChangeKind changeType) {
				...
				observedChangeType = changeType;
				notificationReceived.SetEvent();
				watcher = wil::make_registry_watcher_nothrow(
					ROOT_KEY_PAIR, true,
...
TEST_CASE("RegistryWatcherTests::VerifyCallbackFinishesBeforeFreed",
		  "[registry][registry_watcher]") {
	auto notificationReceived = make_event();
	...
	auto watcher = wil::make_registry_watcher_nothrow(
		ROOT_KEY_PAIR, true, [&](wil::RegistryChangeKind) {
			notificationReceived.SetEvent();
			// ensure that the callback is still being executed while the
			// watcher is reset().
...
TEST_CASE("FileSystemWatcherTests::Construction", "[resource][folder_watcher]"){
	SECTION("Create unique_folder_watcher_nothrow with valid path"){
		auto watcher = wil::make_folder_watcher_nothrow(
			L"C:\\Windows\\System32", true, wil::FolderChangeEvents::All,
			[] {});
REQUIRE(watcher);
}
...
SECTION("Create unique_folder_watcher_nothrow with invalid path") {
	auto watcher = wil::make_folder_watcher_nothrow(
		L"X:\\invalid path", true, wil::FolderChangeEvents::All, [] {});
	REQUIRE(!watcher);
}

#ifdef WIL_ENABLE_EXCEPTIONS
SECTION("Create unique_folder_watcher with valid path") {
	REQUIRE_NOTHROW(wil::make_folder_watcher(
		L"C:\\Windows\\System32", true, wil::FolderChangeEvents::All, [] {}));
}

SECTION("Create unique_folder_watcher with invalid path") {
	REQUIRE_THROWS(wil::make_folder_watcher(
		L"X:\\invalid path", true, wil::FolderChangeEvents::All, [] {}));
}
#endif
}

TEST_CASE("FileSystemWatcherTests::VerifyDelivery",
		  "[resource][folder_watcher]") {
	...
	int observedCount = 0;
	auto watcher = wil::make_folder_watcher_nothrow(
		folder.Path(), true, wil::FolderChangeEvents::All, [&] {
			++observedCount;
...
TEST_CASE("FolderChangeReaderTests::Construction",
		  "[resource][folder_change_reader]"){
...
SECTION("Create folder_change_reader with valid path") {
	REQUIRE_NOTHROW(wil::make_folder_change_reader(
		L"C:\\Windows\\System32", true, wil::FolderChangeEvents::All,
		[](auto, auto) {}));
}

SECTION("Create folder_change_reader with invalid path") {
	REQUIRE_THROWS(wil::make_folder_change_reader(L"X:\\invalid path", true,
...
TEST_CASE("FolderChangeReaderTests::VerifyDelivery",
		  "[resource][folder_change_reader]") {
	witest::TestFolder folder;
	REQUIRE(folder);
	...
	auto reader = wil::make_folder_change_reader_nothrow(
		...
		[&](wil::FolderChangeEvent event, PCWSTR fileName) {
			observedEvent = event;
			StringCchCopyW(observedFileName, ARRAYSIZE(observedFileName),
						   fileName);
			notificationEvent.SetEvent();
		});
	REQUIRE(reader);

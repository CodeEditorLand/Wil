...
// Uncomment this line to do a more exhaustive test of the concepts covered by
// this file.  By default we don't fully compile every combination of tests as
// this test can substantially impact build times with template expansion.

// #define WIL_EXHAUSTIVE_TEST

// Helper objects / functions
class __declspec(uuid("a817e7a2-43fa-11d0-9e44-00aa00b6770a")) IUnknownFake
	: public IUnknown {
  public:
...
class __declspec(uuid("a817e7a2-43fa-11d0-9e44-00aa00b6770b")) IUnknownFake2
	: public IUnknownFake {};
...
TEST_CASE("ComTests::Test_Constructors", "[com][com_ptr]") {
	...
	SECTION("Null/default construction") {
		...
		wil::com_ptr_nothrow<IUnknown> ptr2(
			nullptr); // default explicit null constructor
	...
	SECTION("Move construction") {
		...
		wil::com_ptr_nothrow<IUnknownFake> ptrMove(
			reinterpret_cast<wil::com_ptr_nothrow<IUnknownFake> &&>(ptr));
		REQUIRE(IUnknownFake::GetAddRef() == 1);
		...
		wil::com_ptr_nothrow<IUnknownFake> ptrMove2(
			reinterpret_cast<wil::com_ptr_nothrow<IUnknownFake2> &&>(ptr2));
		REQUIRE(IUnknownFake::GetAddRef() == 1);
...
TEST_CASE("ComTests::Test_Make", "[com][com_ptr]") {
	...
	auto ptr =
		wil::make_com_ptr_nothrow(&helper); // CTAD workaround for pre-C++20
	REQUIRE(IUnknownFake::GetAddRef() == 1);
	REQUIRE(ptr.get() == &helper);
}
...
TEST_CASE("ComTests::Test_Assign", "[com][com_ptr]") {
	...
	SECTION("Self assignment") {
		...
		REQUIRE(ptr.get() == &helper);
		// wil::com_ptr<T> can do self-assignment without blowing up -- and
		// chooses NOT to preserve the this comparison for performance as this
		// should be a rare/never operation...
...
TEST_CASE("ComTests::Test_Helpers", "[com][com_ptr]") {
	...
	SECTION("attach") {
		ptrHelper = &helper;
		wil::com_ptr_nothrow<IUnknownFake> ptr2(
			&helper2); // have some non null pointer
...
TEST_CASE("ComTests::Test_As", "[com][com_ptr]") {
	...
	SECTION("query by IID") {
		...
		REQUIRE(S_OK == ptr.query_to(__uuidof(IUnknown),
	...
	SECTION("query by invalid IID") {
		...
		REQUIRE(S_OK != ptr.query_to(__uuidof(IDispatch),
									 reinterpret_cast<void **>(&ptr2)));
		REQUIRE(ptr2 == nullptr);
	}
...
TEST_CASE("ComTests::Test_CopyTo", "[com][com_ptr]") {
	...
	SECTION("copy by IID") {
		wil::com_ptr_nothrow<IUnknown> ptr2;
		REQUIRE(S_OK == ptr.copy_to(__uuidof(IUnknown),
									reinterpret_cast<void **>(&ptr2)));
		REQUIRE(ptr2 != nullptr);
	}
	...
	SECTION("copy by invalid IID") {
		wil::com_ptr_nothrow<IUnknown> ptr2;
		REQUIRE(S_OK != ptr.copy_to(__uuidof(IDispatch),
									reinterpret_cast<void **>(&ptr2)));
		REQUIRE(ptr2 == nullptr);
	}
...
// Helps with testing wil::com_ptr<const ExtensionHelper> configuration when the
// operator -> is used
class ExtensionHelper {
  public:
	HRESULT Extend() const { return S_OK; }
	STDMETHOD_(ULONG, AddRef)() const { return 0; }
	STDMETHOD_(ULONG, Release)() const { return 0; }
};
...
// Make sure that the pointer can be defined just with forward declaration of
// the class
...
interface __declspec(uuid("ececcc6a-5193-4d14-b38e-ed1460c20a00")) ITest
	: public IUnknown {
	STDMETHOD_(void, Test)() = 0;
};

interface __declspec(uuid("ececcc6a-5193-4d14-b38e-ed1460c20a01")) IDerivedTest
	: public ITest {
...
interface __declspec(uuid("ececcc6a-5193-4d14-b38e-ed1460c20a04")) INever
	: public IUnknown {
	STDMETHOD_(void, Never)() = 0;
};

interface __declspec(uuid("ececcc6a-5193-4d14-b38e-ed1460c20a05")) IAlways
	: public IUnknown {
	STDMETHOD_(void, Always)() = 0;
};

class __declspec(empty_bases) __declspec(
	uuid("ececcc6a-5193-4d14-b38e-ed1460c20b00")) // non-implemented to allow QI
												  // for the class to be
												  // attempted (and fail)
	ComObject
	: witest::AllocatedObject,
	  public Microsoft::WRL::RuntimeClass<
		  ...
		  Microsoft::WRL::ChainInterfaces<IDerivedTest, ITest>, IAlways> {
  public:
...
class __declspec(empty_bases) __declspec(
	uuid("ececcc6a-5193-4d14-b38e-ed1460c20b01")) // non-implemented to allow QI
												  // for the class to be
												  // attempted (and fail)
	WinRtObject
	: witest::AllocatedObject,
	  public Microsoft::WRL::RuntimeClass<
		  ...
		  ITest, IDerivedTest, ITestInspectable, IDerivedTestInspectable,
		  IAlways, Microsoft::WRL::FtmBase> {
  public:
	COM_DECLSPEC_NOTHROW IFACEMETHODIMP_(void) Test() {}
	COM_DECLSPEC_NOTHROW IFACEMETHODIMP_(void) TestDerived() {}
...
class __declspec(empty_bases) NoCom : witest::AllocatedObject {
  public:
	ULONG __stdcall AddRef() { return m_ref++; }
...
template <typename T, typename U,
		  typename = wistd::enable_if_t<!wistd::is_same_v<T, U>>>
T *cast_object(U *) {
	FAIL_FAST();
}

template <typename T> T *cast_object(T *ptr) { return ptr; }

template <typename IFace, typename Object> static IFace *make_object() {
	auto obj = Microsoft::WRL::Make<Object>();

	IFace *result = nullptr;
	if (FAILED(obj.Get()->QueryInterface(__uuidof(IFace),
										 reinterpret_cast<void **>(&result)))) {
		// The QI only fails when we're asking for a CFoo from a CFoo
		// (equivalent types)... in this case just return the original pointer
		// -- the reinterpret_cast is needed as the code is shared and the other
		// (nonuniform) cases also compile it (but do not execute it).
...
template <> NoCom *make_object<NoCom, NoCom>() { return new NoCom(); }
...
#endif
	TestSmartPointer(wil::com_ptr_failfast<IFace>(p1),
					 wil::com_ptr_failfast<IFace>(p2));
	TestSmartPointer(wil::com_ptr_nothrow<IFace>(p1),
					 wil::com_ptr_nothrow<IFace>(p2));
}

template <typename IFace, typename Object> static void TestPointer() {
...
TEST_CASE("ComTests::Test_MemberFunctions", "[com][com_ptr]") {
	// avoid overwhelming debug logging, perhaps the COM helpers are over
	// reporting
...
static void TestPointerConversionCombination(IFace1 *p1, IFace2 *p2) {
#ifdef WIL_ENABLE_EXCEPTIONS
	TestSmartPointerConversion(wil::com_ptr<IFace1>(p1),
							   wil::com_ptr_nothrow<IFace2>(p2));
#endif
	TestSmartPointerConversion(wil::com_ptr_failfast<IFace1>(p1),
							   wil::com_ptr_nothrow<IFace2>(p2));
	TestSmartPointerConversion(wil::com_ptr_nothrow<IFace1>(p1),
							   wil::com_ptr_nothrow<IFace2>(p2));

#ifdef WIL_EXHAUSTIVE_TEST
#ifdef WIL_ENABLE_EXCEPTIONS
	TestSmartPointerConversion(wil::com_ptr<IFace1>(p1),
							   wil::com_ptr<IFace2>(p2));
	TestSmartPointerConversion(wil::com_ptr_failfast<IFace1>(p1),
							   wil::com_ptr<IFace2>(p2));
	TestSmartPointerConversion(wil::com_ptr_nothrow<IFace1>(p1),
							   wil::com_ptr<IFace2>(p2));
	...
	TestSmartPointerConversion(wil::com_ptr<IFace1>(p1),
							   wil::com_ptr_failfast<IFace2>(p2));
#endif
	TestSmartPointerConversion(wil::com_ptr_failfast<IFace1>(p1),
							   wil::com_ptr_failfast<IFace2>(p2));
	TestSmartPointerConversion(wil::com_ptr_nothrow<IFace1>(p1),
...
static void TestPointerConversion() {
	...
	TestPointerConversionCombination(static_cast<IFace1 *>(p2),
									 p2); // same object
...
TEST_CASE("ComTests::Test_PointerConversion", "[com][com_ptr]") {
	...
	TestPointerConversion<ITestInspectable, IDerivedTestInspectable,
						  WinRtObject>();
	TestPointerConversion<IDerivedTest, IDerivedTest, WinRtObject>();
	TestPointerConversion<IDerivedTestInspectable, IDerivedTestInspectable,
						  WinRtObject>();
...
void TestGlobalQueryIidPpv(wistd::true_type, const Ptr &source) // interface
{
	...
	SECTION("com_query_to(iid, ppv)") {
		if (source) {
			...
			REQUIRE_ERROR(
				wil::com_query_to_failfast(source, IID_PPV_ARGS(&never)));
			wil::com_query_to_nothrow(source, IID_PPV_ARGS(&dest3));
			REQUIRE_ERROR(
				wil::com_query_to_nothrow(source, IID_PPV_ARGS(&never)));
...
static void TestGlobalQuery(const Ptr &source) {
	...
	TestGlobalQueryIidPpv<TargetIFace, Ptr>(
		typename wistd::is_abstract<TargetIFace>::type(), source);
}

// Test fluent query functions for types that support them (exception and fail
// fast)
template <typename IFace, typename Ptr>
void TestSmartPointerQueryFluent(
	wistd::true_type, const Ptr &source) // void return (non-error based)
{
	SECTION("query") {
		if (source) {
...
template <typename IFace, typename Ptr>
void TestSmartPointerQueryFluent(
	wistd::false_type, const Ptr & /*source*/) // error-code based return
{
	// error code based code cannot call the fluent error methods
}

// Test iid, ppv queries for types that support them (interfaces yes, classes
// no)
template <typename IFace, typename Ptr>
void TestSmartPointerQueryIidPpv(wistd::true_type,
								 const Ptr &source) // interface
{
	wil::com_ptr_nothrow<INever> never;
...
// "Test" iid, ppv queries for types that support them for a class (unsupported
// same (interfaces yes, classes no)
template <typename IFace, typename Ptr>
void TestSmartPointerQueryIidPpv(wistd::false_type,
								 const Ptr & /*source*/) // class
{
	// we can't compile against iid, ppv with a class
}

// Test the various query and copy methods against the given source pointer
// (trying produce the given dest pointer)
template <typename IFace, typename Ptr>
void TestSmartPointerQuery(const Ptr &source) {
	...
	TestSmartPointerQueryFluent<IFace, Ptr>(
		typename wistd::is_same<void, typename Ptr::result>::type(), source);

	TestSmartPointerQueryIidPpv<IFace, Ptr>(
		typename wistd::is_abstract<IFace>::type(), source);
}
...
TEST_CASE("ComTests::Test_Query", "[com][com_ptr]") {
	...
	// This adds a significant amount of time to the compilation duration, so
	// most tests are disabled by default...
#ifdef WIL_EXHAUSTIVE_TEST
	...
	TestQuery<IDerivedTestInspectable, IUnknown,
			  WinRtObject>(); // IDerivedTestInspectable
...
template <typename Ptr> void TestAgile(const Ptr &source) {
...
template <typename IFace> void TestAgileCombinations() {
...
template <typename Ptr> void TestWeak(const Ptr &source) {
	bool supports_weak =
		(source && (wil::try_com_query_nothrow<IInspectable>(source)));
...
template <typename TargetIFace, typename Ptr>
void TestSmartPointerQueryFluentWithFailedResolve(wistd::false_type,
												  const Ptr & /*source*/) {}

template <typename TargetIFace, typename Ptr>
void TestSmartPointerQueryFluentWithFailedResolve(wistd::true_type,
												  const Ptr &source) {
...
void TestSmartPointerQueryWithFailedResolve(const Ptr source) {
	...
	SECTION("try_query") { REQUIRE(!source.template try_query<TargetIFace>()); }
	...
	SECTION("try_copy") { REQUIRE(!source.template try_copy<TargetIFace>()); }
	...
	TestSmartPointerQueryFluentWithFailedResolve<TargetIFace, Ptr>(
		typename wistd::is_same<void, typename Ptr::result>::type(), source);
...
#endif
	TestGlobalQueryWithFailedResolve<TargetIFace>(
		wil::com_ptr_failfast<IFace>(ptr));
#endif
	TestGlobalQueryWithFailedResolve<TargetIFace>(
		wil::com_ptr_nothrow<IFace>(ptr));
	TestGlobalQueryWithFailedResolve<TargetIFace>(
		Microsoft::WRL::ComPtr<IFace>(ptr));

#ifdef WIL_ENABLE_EXCEPTIONS
	TestSmartPointerQueryWithFailedResolve<TargetIFace>(
...
template <typename IFace> void TestWeakCombinations() {
	auto ptr = make_object<IFace, WinRtObject>();
	...
	// On the final release of the pointer, the weak reference will no longer
	// resolve
	ptr->Release();
...
TEST_CASE("ComTests::VerifyCoCreate", "[com][CoCreateInstance]") {
	...
	REQUIRE_FALSE(static_cast<bool>(
		wil::CoCreateInstanceNoThrow<ShellLink, IStream>().get()));
}
...
TEST_CASE("ComTests::VerifyCoGetClassObject", "[com][CoGetClassObject]") {
	...
	REQUIRE_FALSE(
		static_cast<bool>(wil::CoGetClassObjectNoThrow<ShellLink, IStream>()));
}
#endif

#if defined(__IBackgroundCopyManager_INTERFACE_DEFINED__) &&                   \
	(__WI_LIBCPP_STD_VER >= 17)
TEST_CASE("ComTests::VerifyCoCreateEx", "[com][CoCreateInstance]") {
...

	{
#ifdef WIL_ENABLE_EXCEPTIONS
		auto [sp1, ps1] =
			wil::CoCreateInstanceEx<IBackgroundCopyManager, IUnknown>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		REQUIRE((sp1 && ps1));
#endif
		auto [hr, unk] =
			wil::CoCreateInstanceExNoThrow<IBackgroundCopyManager, IUnknown>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		...
		REQUIRE((sp && ps));
		auto [sp3, ps3] =
			wil::CoCreateInstanceExFailFast<IBackgroundCopyManager, IUnknown>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		REQUIRE((sp3 && ps3));
	}
...
#ifdef WIL_ENABLE_EXCEPTIONS
	{
		auto [ps, pf] = wil::CoCreateInstanceEx<IPersistStream, IPersistFile>(
			__uuidof(ShellLink), CLSCTX_INPROC_SERVER);
...
TEST_CASE("ComTests::VerifyTryCoCreateInstanceMissingInterface",
		  "[com][CoCreateInstance]") {
	auto init = wil::CoInitializeEx_failfast();
	...
	{
		auto [sp, pb] =
			wil::TryCoCreateInstanceEx<IBackgroundCopyManager, IPropertyBag>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		REQUIRE(sp != nullptr);
		REQUIRE(pb == nullptr);
	}
	{
		auto [sp, pb] =
			wil::TryCoCreateInstanceExNoThrow<IBackgroundCopyManager,
											  IPropertyBag>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		REQUIRE(sp != nullptr);
		REQUIRE(pb == nullptr);
	...
	{
		auto [sp, pb] =
			wil::TryCoCreateInstanceExFailFast<IBackgroundCopyManager,
											   IPropertyBag>(
				__uuidof(BackgroundCopyManager), CLSCTX_LOCAL_SERVER);
		REQUIRE(sp != nullptr);
		REQUIRE(pb == nullptr);
...
TEST_CASE("ComTests::VerifyQueryMultipleInterfaces", "[com][com_multi_query]") {
	...
	auto mgr =
		wil::CoCreateInstance<BackgroundCopyManager>(CLSCTX_LOCAL_SERVER);
	auto [sp, ps] =
		wil::com_multi_query<IBackgroundCopyManager, IUnknown>(mgr.get());
	REQUIRE(sp);
	REQUIRE(ps);
	auto [sp1, pb] =
		wil::try_com_multi_query<IBackgroundCopyManager, IPropertyBag>(
			mgr.get());
	REQUIRE(sp1);
	REQUIRE(!pb);
}
...
TEST_CASE("ComTests::VerifyComSetSite", "[com][com_set_site]") {
	...
	class ServiceObject WrlFinal
		: public RuntimeClass<RuntimeClassFlags<ClassicCom>, IServiceProvider> {
	  public:
		ServiceObject(IServiceProvider *site = nullptr) { m_site = site; }

		STDMETHODIMP QueryService(REFIID /*sid*/, REFIID /*riid*/,
	...
	{
		auto cleanupSite =
			wil::com_set_site(objWithSite.Get(), serviceObj2.Get());
		...
		auto siteCount = 0;
		wil::for_each_site(objWithSite.Get(),
						   [&](IUnknown * /*site*/) { siteCount++; });
		REQUIRE(siteCount == 2);
	}
...
class FakeStream : public IStream {
  public:
	...
	// ISequentialStream
	STDMETHOD(Read)
	(_Out_writes_bytes_to_(cb, *pcbRead) void *pv, _In_ ULONG cb,
	...
	STDMETHOD(Write)
	(_In_reads_bytes_(cb) const void *, _In_ ULONG cb,
	 ...
	 _Out_opt_ ULARGE_INTEGER *plibNewPosition) {
		if (dwOrigin == STREAM_SEEK_CUR) {
			if ((dlibMove.QuadPart < 0) &&
				(static_cast<unsigned long long>(-dlibMove.QuadPart) >
				 Position)) {
				Position = 0;
			} else {
	...
	STDMETHOD(Revert)(void) override { return E_NOTIMPL; }
	...
	STDMETHOD(CopyTo)
	(_In_ IStream *pstm, ULARGE_INTEGER cb, _Out_opt_ ULARGE_INTEGER *pcbRead,
	...
	void SetPosition(unsigned long long position,
					 unsigned long long positionMax) {
		Position = position;
...
TEST_CASE("StreamTests::ReadPartial", "[com][IStream]") {
	...
	REQUIRE_SUCCEEDED(
		wil::stream_read_partial_nothrow(&stream, buffer, 32, &readSize));
	REQUIRE(stream.MaxReadSize == readSize);

	// Reading less than what's available is OK
	REQUIRE_SUCCEEDED(
		wil::stream_read_partial_nothrow(&stream, buffer, 5, &readSize));
	REQUIRE(5 == readSize);

#ifdef WIL_ENABLE_EXCEPTIONS
	REQUIRE(stream.MaxReadSize ==
			wil::stream_read_partial(&stream, buffer, 32));
...
TEST_CASE("StreamTests::Write", "[com][IStream]") {
	...
	REQUIRE_SUCCEEDED(
		wil::stream_write_nothrow(&stream, buffer, sizeof(buffer)));
...
TEST_CASE("StreamTests::SeekCur", "[com][IStream]") {
	...
	REQUIRE_SUCCEEDED(
		wil::stream_seek_from_current_position_nothrow(&stream, 10, &landed));
	...
	REQUIRE_SUCCEEDED(wil::stream_seek_from_current_position_nothrow(
		&stream, -1000, &landed));
	REQUIRE(0ULL == landed);

	REQUIRE_SUCCEEDED(
		wil::stream_seek_from_current_position_nothrow(&stream, 6000, &landed));

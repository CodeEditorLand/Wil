...
struct my_async_status
	: winrt::implements<my_async_status,
						winrt::Windows::Foundation::IAsyncInfo> {
	wil::single_threaded_property<winrt::Windows::Foundation::AsyncStatus>
		Status{winrt::Windows::Foundation::AsyncStatus::Started};
...
struct my_pointer_args
	...
	wil::single_threaded_property<winrt::Windows::System::VirtualKeyModifiers>
		KeyModifiers{winrt::Windows::System::VirtualKeyModifiers::None};
	wil::single_threaded_property<winrt::Windows::UI::Xaml::Input::Pointer>
		Pointer{nullptr};

	winrt::Windows::UI::Input::PointerPoint
	GetCurrentPoint(winrt::Windows::UI::Xaml::UIElement const &) {
		throw winrt::hresult_not_implemented();
	}
	winrt::Windows::Foundation::Collections::IVector<
		winrt::Windows::UI::Input::PointerPoint>
	GetIntermediatePoints(winrt::Windows::UI::Xaml::UIElement const &) {
		throw winrt::hresult_not_implemented();
	}
};
...
TEST_CASE("CppWinRTAuthoringTests::Events", "[property]") {
	struct Test {
		...
		wil::typed_event<winrt::Windows::Foundation::IInspectable, int>
			MyTypedEvent;
	} test;

	auto token = test.MyEvent([](winrt::Windows::Foundation::IInspectable,
								 int args) { REQUIRE(args == 42); });
	test.MyEvent.invoke(nullptr, 42);
	test.MyEvent(token);

	auto token2 = test.MyTypedEvent([](winrt::Windows::Foundation::IInspectable,
									   int args) { REQUIRE(args == 42); });
...
TEST_CASE("CppWinRTAuthoringTests::EventsAndCppWinRt", "[property]") {
	struct Test
		: winrt::implements<
			  Test, winrt::Windows::Foundation::IMemoryBufferReference> {
		wil::single_threaded_property<uint32_t> Capacity{0};
		wil::typed_event<winrt::Windows::Foundation::IMemoryBufferReference,
	...
	bool invoked = false;
	auto token = test.Closed(
		[&](winrt::Windows::Foundation::IMemoryBufferReference,
			winrt::Windows::Foundation::IInspectable) { invoked = true; });
...
// This test seems to have issues on the CI machines, so enabling only when run
// locally
TEST_CASE("CppWinRTAuthoringTests::NotifyPropertyChanged",
		  "[property][LocalOnly]") {
#if defined(WIL_ENABLE_EXCEPTIONS)
	auto uninit = wil::RoInitialize_failfast(RO_INIT_MULTITHREADED);
	// We need to initialize the XAML core in order to instantiate a
	// PropertyChangedEventArgs. Do all the work on a separate DispatcherQueue
	// thread so we can shut it down cleanly and make sure COM is completely
	// cleaned up before we finish the test.

	auto controller = winrt::Windows::System::DispatcherQueueController::
	...
	auto enqueueResult = controller.DispatcherQueue().TryEnqueue([&] {
		winrt::check_bool(DuplicateHandle(
			...
			dispatcherThreadHandle.put(), SYNCHRONIZE, FALSE, 0));
		auto manager = winrt::Windows::UI::Xaml::Hosting::WindowsXamlManager::
	...
	REQUIRE(WaitForSingleObject(dispatcherThreadHandle.get(), 5000) ==
			WAIT_OBJECT_0);
#endif
}
#endif // msvc

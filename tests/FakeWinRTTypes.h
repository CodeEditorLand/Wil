...
template <typename T> struct WinRTStorage {
	T value = {};
	...
	bool Equals(T val) {
		// NOTE: Padding can through this off, but this isn't intended to be a
		// robust solution...
		return memcmp(&value, &val, sizeof(T)) == 0;
	}
};

template <> struct WinRTStorage<HSTRING> {
	Microsoft::WRL::Wrappers::HString value;

	HRESULT CopyTo(HSTRING *result) const { return value.CopyTo(result); }

	HRESULT Set(HSTRING val) { return value.Set(val); }
	...
	bool Equals(HSTRING val) const { return value == val; }
};

template <typename T> struct WinRTStorage<T *> {
	...
	bool Equals(T *val) { return value.Get() == val; }
};

// Very minimal IAsyncOperation implementation that gives calling tests control
...
struct FakeAsyncOperation
	...
	using Handler =
		ABI::Windows::Foundation::IAsyncOperationCompletedHandler<Logical>;

	// IAsyncInfo
	IFACEMETHODIMP get_Id(unsigned int *) override { return E_NOTIMPL; }
	...
	IFACEMETHODIMP Close() override { return E_NOTIMPL; }
	...
	void Complete(HRESULT hr, Abi result) {
		...
		Handler *handler = nullptr;
		{
			...
			if (m_status == AsyncStatus::Started) {
				...
				m_status =
					SUCCEEDED(hr) ? AsyncStatus::Completed : AsyncStatus::Error;
...
struct FakeVector
	...
	IFACEMETHODIMP
	GetView(ABI::Windows::Foundation::Collections::IVectorView<Logical> **view)
		override {
		this->AddRef();
		*view = this;
		return S_OK;
	...
	IFACEMETHODIMP IndexOf(Abi value, unsigned *index,
						   boolean *found) override {
	...
	IFACEMETHODIMP GetMany(unsigned startIndex, unsigned capacity, Abi *value,

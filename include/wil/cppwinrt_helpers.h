...
struct dispatcher_handler {
	...
	dispatcher_handler(dispatcher_handler &&other) noexcept
...
namespace wil {
//! Resumes coroutine execution on the thread associated with the dispatcher, or
//! throws an exception (from an arbitrary thread) if unable. Supported
//! dispatchers are Windows.System.DispatcherQueue,
//! Microsoft.System.DispatcherQueue, Microsoft.UI.Dispatching.DispatcherQueue,
//! and Windows.UI.Core.CoreDispatcher, but you must include the corresponding
//! <winrt/Namespace.h> header before including wil\cppwinrt_helpers.h. It is
//! okay to include wil\cppwinrt_helpers.h multiple times: support will be
//! enabled for any winrt/Namespace.h headers that were included since the
//! previous inclusion of wil\cppwinrt_headers.h.
...
[[nodiscard]] auto resume_foreground(
	...
	struct awaitable {
		...
		void await_suspend(details::coroutine_handle<> handle) {
			...
			try {
				// The return value of Schedule is not reliable. Use the
				// dispatcher_handler destructor to detect whether the work item
				// failed to run.
				Traits::Scheduler::Schedule(m_dispatcher, m_priority, handler);
			} catch (...) {
				m_state.handle =
					nullptr; // the exception will resume the coroutine, so the
							 // handler shouldn't do it
				throw;
			}
		}
		...
		void await_resume() const {
			if (m_state.orphaned) {
				throw winrt::hresult_error(static_cast<winrt::hresult>(
					0x800701ab)); // HRESULT_FROM_WIN32(ERROR_NO_TASK_QUEUE)
			}
		}
...
/// @cond
#if defined(WINRT_Windows_UI_Core_H) &&                                        \
	!defined(__WIL_CPPWINRT_WINDOWS_UI_CORE_HELPERS)
...
#if defined(WINRT_Windows_System_H) &&                                         \
	!defined(__WIL_CPPWINRT_WINDOWS_SYSTEM_HELPERS)
...
#if defined(WINRT_Microsoft_System_H) &&                                       \
	!defined(__WIL_CPPWINRT_MICROSOFT_SYSTEM_HELPERS)
...
#if defined(WINRT_Microsoft_UI_Dispatching_H) &&                               \
	!defined(__WIL_CPPWINRT_MICROSOFT_UI_DISPATCHING_HELPERS)
...
/** Converts C++ / WinRT vectors, iterators, and iterables to std::vector by
requesting the collection's data in bulk. This can be more efficient in terms of
IPC cost than iteratively processing the collection.
~~~
winrt::IVector<winrt::hstring> collection = GetCollection();
std::vector<winrt::hstring> allData = wil::to_vector(collection); // read all
data from collection for (winrt::hstring const& item : allData)
{
	// use item
}
~~~
Can be used for IVector<T>, IVectorView<T>, IIterable<T>, IIterator<T>, and any
type or interface that C++/WinRT projects those interfaces for (PropertySet,
IMap<T,K>, etc.) Iterable-only types fetch content in units of 64. When used
with an iterator, the returned vector contains the iterator's current position
...
template <typename TSrc> auto to_vector(TSrc const &src) {
	if constexpr (details::is_winrt_vector_like<TSrc>::value) {
		...
		while (true) {
			...
			auto fetched = src.GetMany({result.data() + lastSize,
										result.data() + lastSize + chunkSize});
...
#if !defined(____x_ABI_CWindows_CFoundation_CIClosable_FWD_DEFINED__) &&       \
	!defined(MIDL_NS_PREFIX)
...
//! <Windows.UI.Interop.h> before including wil/cppwinrt_helpers.h, and that
//! NTDDI_VERSION is at least NTDDI_WIN10_CU. It is okay to include
//! wil\cppwinrt_helpers.h multiple times: support will be enabled for any
//! headers that were included since the previous inclusion of
//! wil\cppwinrt_headers.h.

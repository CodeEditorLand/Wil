...
The `wil::secure_allocator` allocator calls `SecureZeroMemory` before
deallocating memory. This provides a mechanism for secure STL containers such as
`wil::secure_vector`, `wil::secure_string`, and `wil::secure_wstring`. */
...
using secure_string = std::basic_string<char, std::char_traits<char>,
										wil::secure_allocator<char>>;

/// @cond
namespace details {
template <> struct string_maker<std::wstring> {
	HRESULT make(_In_reads_opt_(length) PCWSTR source,
				 size_t length) WI_NOEXCEPT try {
		m_value =
			source ? std::wstring(source, length) : std::wstring(length, L'\0');
		return S_OK;
	} catch (...) {
...
// first character in a string's buffer. This is the overload for std::wstring.
// Other overloads available in resource.h.
inline PCWSTR str_raw_ptr(const std::wstring &str) { return str.c_str(); }

#if _HAS_CXX17
/**
	zstring_view. A zstring_view is identical to a std::string_view except it is
   always nul-terminated (unless empty).
	* zstring_view can be used for storing string literals without "forgetting"
   the length or that it is nul-terminated.
	* A zstring_view can be converted implicitly to a std::string_view because
   it is always safe to use a nul-terminated string_view as a plain string view.
	* A zstring_view can be constructed from a std::string because the data in
   std::string is nul-terminated.
*/
...
class basic_zstring_view : public std::basic_string_view<TChar> {
  ...
  public:
	...
	operator=(const basic_zstring_view &) noexcept = default;

	constexpr basic_zstring_view(const TChar *pStringData,
								 size_type stringLength) noexcept
	...
	constexpr basic_zstring_view(
		const TChar (&stringArray)[stringArrayLength]) noexcept
		: std::basic_string_view<TChar>(
			  &stringArray[0], length_n(&stringArray[0], stringArrayLength)) {}

	// Construct from nul-terminated char ptr. To prevent this from
	// overshadowing array construction, we disable this constructor if the
	// value is an array (including string literal).
	template <
		typename TPtr,
		std::enable_if_t<std::is_convertible<TPtr, const TChar *>::value &&
	...
	// basic_string_view [] precondition won't let us read view[view.size()]; so
	// we define our own.
	WI_NODISCARD constexpr const TChar &
  ...
  private:
	// Bounds-checked version of char_traits::length, like strnlen. Requires
	// that the input contains a null terminator.
	static constexpr size_type length_n(_In_reads_opt_(buf_size)
											const TChar *str,
										size_type buf_size) noexcept {
	...
	// The following basic_string_view methods must not be allowed because they
	// break the nul-termination.
...
constexpr zstring_view operator"" _zv(const char *str,
									  std::size_t len) noexcept {
	return zstring_view(str, len);
}

constexpr zwstring_view operator"" _zv(const wchar_t *str,
									   std::size_t len) noexcept {

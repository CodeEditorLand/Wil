...
// want these concepts, but including STL headers directly introduces friction
...
// This header mimics libc++'s '__config' header to the extent necessary to get
// the wistd::* definitions compiling. Note that this has a few key differences
// since libc++'s MSVC compatability is currently not functional and a bit
// behind
...
#define __WI_CLANG_DISABLE_WARNING(warning)                                    \
	__pragma(clang diagnostic ignored #warning)
#else
#define __WI_CLANG_DISABLE_WARNING(warning)
#endif

// NOTE: MSVC, which is what we primarily target, is severly underrepresented in
// libc++ and checks such as
// __has_feature(...) are always false for MSVC, even when the feature being
// tested _is_ present in MSVC. Therefore, we instead modify all checks to be
// __WI_HAS_FEATURE_IS_UNION, etc., which provides the correct value for MSVC
// and falls back to the __has_feature(...), etc. value otherwise. We
// intentionally leave '__has_feature', etc. undefined for MSVC so that we don't
// accidentally use the incorrect behavior
...
#define __WI_ALIGNAS(x) __attribute__((__aligned__(x)))
	...
	(!defined(__WI_LIBCPP_CXX03_LANG) &&                                       \
	 defined(__GNUC__)) // All supported GCC versions
...
#define __WI_HAS_FEATURE_HAS_VIRTUAL_DESTRUCTOR                                \
...
#define __WI_HAS_FEATURE_IS_TRIVIALLY_CONSTRUCTIBLE                            \
	__has_feature(is_trivially_constructible)
#define __WI_HAS_FEATURE_IS_TRIVIALLY_ASSIGNABLE                               \
	__has_feature(is_trivially_assignable)
#define __WI_HAS_FEATURE_HAS_TRIVIAL_DESTRUCTOR                                \
...
#define __WI_HAS_FEATURE_IS_TRIVIALLY_COPYABLE                                 \
	__has_feature(is_trivially_copyable)
#define __WI_HAS_FEATURE_IS_TRIVIAL __has_feature(is_trivial)
#define __WI_HAS_FEATURE_HAS_TRIVIAL_CONSTRUCTOR                               \
	__has_feature(has_trivial_constructor) || (__WI_GNUC_VER >= 403)
#define __WI_HAS_FEATURE_HAS_NOTHROW_CONSTRUCTOR                               \
	__has_feature(has_nothrow_constructor) || (__WI_GNUC_VER >= 403)
#define __WI_HAS_FEATURE_HAS_NOTHROW_COPY                                      \
	__has_feature(has_nothrow_copy) || (__WI_GNUC_VER >= 403)
#define __WI_HAS_FEATURE_HAS_NOTHROW_ASSIGN                                    \
	__has_feature(has_nothrow_assign) || (__WI_GNUC_VER >= 403)
...
// NOTE: Much of the following assumes a decently recent version of MSVC. Past
// versions can be supported, but will need
...
#define __WI_LIBCPP_SUPPRESS_NOEXCEPT_ANALYSIS                                 \
	__pragma(warning(suppress : 26439))
...
#endif
#endif // !defined(__WI_LIBCPP_LITTLE_ENDIAN) &&
	   // !defined(__WI_LIBCPP_BIG_ENDIAN)
...
#if __WI_LIBCPP_STD_VER > 14 && defined(__cpp_inline_variables) &&             \
...
#if !defined(__WI_LIBCPP_DISABLE_VISIBILITY_ANNOTATIONS) &&                    \
	!defined(__WI_LIBCPP_COMPILER_MSVC)
...
#define __WI_LIBCPP_INLINE_VISIBILITY                                          \
	__WI_LIBCPP_HIDDEN __WI_LIBCPP_INTERNAL_LINKAGE
...
template <class _T1, class _T2 = _T1> struct __less {
	...
	__WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX11 bool
...
// These are added to wistd to enable use of min/max without having to use the
// windows.h min/max macros that some clients might not have access to. Note:
// the STL versions of these have debug checking for the less than operator and
// support for iterators that these implementations lack. Use the STL versions
// when you require use of those features.

// min

template <class _Tp, class _Compare>
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX11 const
	_Tp &(min)(const _Tp &__a, const _Tp &__b, _Compare __comp) {
	return __comp(__b, __a) ? __b : __a;
}

template <class _Tp>
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX11 const
	_Tp &(min)(const _Tp &__a, const _Tp &__b) {
...
template <class _Tp, class _Compare>
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX11 const
	_Tp &(max)(const _Tp &__a, const _Tp &__b, _Compare __comp) {
	return __comp(__a, __b) ? __b : __a;
}

template <class _Tp>
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX11 const
	_Tp &(max)(const _Tp &__a, const _Tp &__b) {

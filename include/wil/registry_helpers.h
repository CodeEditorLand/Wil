...
#define __WIL_REGISTRY_HELPERS_INCLUDED
	...
	(defined(__cpp_lib_optional) && defined(_OPTIONAL_))
#include <functional>
#include <iterator>
#endif
...
namespace reg {
/**
 * \brief Helper function to translate registry return values if the value was
 * not found \param hr HRESULT to test from registry APIs \return boolean if the
 * HRESULT indicates the registry value was not found
 */
...
/**
 * \brief Helper function to translate registry return values if the buffer was
 * too small \param hr HRESULT to test from registry APIs \return boolean if the
 * HRESULT indicates the buffer was too small for the value being read
 */
...
/**
 * \brief A utility function that walks a contiguous wchar_t container looking
 * for strings within a multi-string \tparam InputIt An iterator type that
 * reference a container that holds wchar_t characters to translate into
 * individual strings \tparam Fn A callback function to be called each time a
 * string is found - given the [begin, end] iterators referencing the found
 * string \param first An iterator referencing to the beginning of the target
 * container (like a std::begin iterator) \param last An iterator referencing
 * one-past-the-end of the target container (like a std::end iterator) \param
 * func A callback function to be called each time a string is found - given the
 * [begin, end] iterators referencing the found string
 */
...
void walk_multistring(const InputIt &first, const InputIt &last, Fn func) {
	...
	while (current != end_iterator) {
		...
		if (next != end_iterator) {
			...
			if (next != last_null) {
				// call the function provided with the [begin, end] pair
				// referencing a string found
				func(current, next);
			}
			current = next + 1;
		} else {
...
/**
 * \brief A translation function taking iterators referencing std::wstring
 * objects and returns a corresponding std::vector<wchar_t> to be written to a
 * MULTI_SZ registry value The translation follows the rules for how MULTI_SZ
 * registry values should be formatted, notably how null characters should be
 * embedded within the returned vector \tparam InputIt An iterator type that
 * references a container that holds std::wstring objects to translate into a
 * wchar_t buffer \param first An iterator referencing to the beginning of the
 * target container (like a std::begin iterator) \param last An iterator
 * referencing one-past-the-end of the target container (like a std::end
 * iterator) \return A std::vector<wchar_t> with the raw wchar_t buffer of bytes
 * prepared to write to a MULTI_SZ registry value
 */
template <class InputIt>
::std::vector<wchar_t> get_multistring_from_wstrings(const InputIt &first,
													 const InputIt &last) {
	...
	for (const auto &wstr : ::wil::make_range(first, last)) {
		multistring.insert(multistring.end(), ::std::begin(wstr),
						   ::std::end(wstr));
		multistring.push_back(L'\0');
	}
...
/**
 * \brief A translation function taking iterators referencing wchar_t characters
 * and returns extracted individual std::wstring objects The translation follows
 * the rules for how MULTI_SZ registry value can be formatted, notably with
 * embedded null characters Note that this conversion avoids returning empty
 * std::wstring objects even though the input may contain contiguous null
 * wchar_t values \tparam InputIt An iterator type that reference a container
 * that holds wchar_t characters to translate into individual strings \param
 * first An iterator referencing to the beginning of the target container (like
 * a std::begin iterator) \param last An iterator referencing one-past-the-end
 * of the target container (like a std::end iterator) \return A
 * std::vector<std::wstring> of the extracted strings from the input container
 * of wchar_t characters
 */
template <class InputIt>
::std::vector<::std::wstring>
get_wstring_vector_from_multistring(const InputIt &first, const InputIt &last) {
	if (last - first < 3) {
		// it doesn't have the required 2 terminating null characters - return
		// an empty string
...
void get_multistring_bytearray_from_strings_nothrow(
	...
	for (size_t i = 0; i < C; ++i) {
		...
		total_array_length_bytes +=
			sizeof(wchar_t); // plus one for the null-terminator
	}
	total_array_length_bytes +=
		sizeof(wchar_t); // plus one for the ending double-null-terminator

	*multistring.addressof() =
		static_cast<uint8_t *>(::CoTaskMemAlloc(total_array_length_bytes));
	if (!multistring.get()) {
		multistring.reset();
		return;
	}
	*multistring.size_address() = total_array_length_bytes;
	...
	for (size_t i = 0; i < C; ++i) {
		...
		static_assert(sizeof(nullTermination) == sizeof(wchar_t),
					  "null terminator must be a wchar");
		memcpy(multistring.get() + array_offset, nullTermination,
			   sizeof(nullTermination));
...
/**
 * \brief A translation function taking iterators referencing wchar_t characters
 * and returns extracted individual wil::unique_cotaskmem_string objects The
 * translation follows the rules for how MULTI_SZ registry value can be
 * formatted, notably with embedded null characters Note that this conversion
 * avoids returning empty wil::unique_cotaskmem_string objects even though the
 * input may contain contiguous null wchar_t values \tparam InputIt An iterator
 * type that reference a container that holds wchar_t characters to translate
 * into individual strings \param first An iterator referencing to the beginning
 * of the target container (like a std::begin iterator) \param last An iterator
 * referencing one-past-the-end of the target container (like a std::end
 * iterator) \param cotaskmem_array The [out]
 * wil::unique_cotaskmem_array_ptr<wil::unique_cotaskmem_string> to contain the
 * array of strings A
 * wil::unique_cotaskmem_array_ptr<wil::unique_cotaskmem_string> of the
 * extracted strings from the input container of wchar_t characters An empty
 * wil::unique_cotaskmem_array_ptr should be translated as out-of-memory as
...
void get_cotaskmemstring_array_from_multistring_nothrow(
	...
	::wil::unique_cotaskmem_array_ptr<::wil::unique_cotaskmem_string>
		&cotaskmem_array) WI_NOEXCEPT {
	if (last - first < 3) {
		// it doesn't have the required 2 terminating null characters - return
		// an empty string
		*cotaskmem_array.addressof() =
			static_cast<PWSTR *>(::CoTaskMemAlloc(sizeof(PWSTR) * 1));
		if (cotaskmem_array) {
	...
	size_t arraySize = 0;
	walk_multistring(first, last,
					 [&](const InputIt &, const InputIt &) { ++arraySize; });

	// allocate the array size necessary to hold all the
	// unique_cotaskmem_strings
	*cotaskmem_array.addressof() =
		static_cast<PWSTR *>(::CoTaskMemAlloc(sizeof(PWSTR) * arraySize));
	if (!cotaskmem_array) {
		// oom will return an empty array
		cotaskmem_array.reset();
		return;
	...
	*cotaskmem_array.size_address() = arraySize;
	...
	walk_multistring(
		...
		[&](const InputIt &string_first, const InputIt &string_last) {
			...
			auto new_string = ::wil::make_cotaskmem_string_nothrow(
				&(*string_first), stringSize);
			if (!new_string) {
				// oom will return an empty array
				cotaskmem_array.reset();
				return;
...
// registry value should be prepared before the first call to the registry read
// API
...
// supports_resize_buffer is used to determine if the input buffer to read a
// registry value can be resized for those cases if the error from the registry
// read API indicates it needs a larger buffer
template <typename T> constexpr bool supports_resize_buffer() WI_NOEXCEPT {
	return false;
}
template <typename T> constexpr HRESULT resize_buffer(T &, DWORD) WI_NOEXCEPT {
	return E_NOTIMPL;
}

// supports_trim_buffer is used to determine if the input buffer to read a
// registry value must be trimmed after the registry read API has successfully
// written into the supplied buffer note that currently only std::wstring
// requires this as it cannot have embedded nulls
template <typename T> constexpr bool supports_trim_buffer() WI_NOEXCEPT {
	return false;
}
template <typename T> constexpr void trim_buffer(T &) WI_NOEXCEPT {}
...
inline DWORD
get_buffer_size_bytes(const ::std::vector<uint8_t> &value) WI_NOEXCEPT {
...
inline HRESULT resize_buffer(::std::vector<uint8_t> &buffer,
							 DWORD byteSize) WI_NOEXCEPT try {
...
inline DWORD
get_buffer_size_bytes(const ::std::vector<wchar_t> &value) WI_NOEXCEPT {
...
inline void *get_buffer(const BSTR &value) WI_NOEXCEPT { return value; }
...
inline DWORD get_buffer_size_bytes(const BSTR &value) WI_NOEXCEPT {
	...
	if (length > 0) {
		...
		// including the last null buffer space in the returned
		// buffer-size-bytes as the registry API we call guarantees null
		// termination
		length += 1;
	}
...
inline HRESULT prepare_buffer(const BSTR &value) WI_NOEXCEPT {
	if (value) {
		// zero out the buffer if pre-allocated
		for (auto &string_char : ::wil::make_range(
				 value, get_buffer_size_bytes(value) / sizeof(WCHAR))) {
			string_char = L'\0';
		}
	}
	return S_OK;
...
template <> constexpr bool supports_resize_buffer<BSTR>() WI_NOEXCEPT {
	return true;
}
// transferringOwnership is only set to false if this is a 'shallow' copy of the
// BSTR and the caller maintained ownership of the original BSTR.
inline HRESULT resize_buffer(BSTR &string, DWORD byteSize,
							 bool transferringOwnership = true) WI_NOEXCEPT {
	// convert bytes to length (number of WCHAR's)
	DWORD length = byteSize / sizeof(WCHAR);
	// SysAllocStringLen adds a null, so subtract a wchar_t from the input
	// length
	length = length > 0 ? length - 1 : length;
	const BSTR new_bstr{::SysAllocStringLen(string, length)};
	RETURN_IF_NULL_ALLOC(new_bstr);

	// if not transferring ownership, the caller will still own the original
	// BSTR
...
inline DWORD
get_buffer_size_bytes(const ::wil::unique_bstr &value) WI_NOEXCEPT {
...
inline HRESULT prepare_buffer(const ::wil::unique_bstr &value) WI_NOEXCEPT {
	if (value) {
		// zero out the buffer if pre-allocated
		for (auto &string_char : ::wil::make_range(
				 value.get(), get_buffer_size_bytes(value) / sizeof(WCHAR))) {
			string_char = L'\0';
		}
	}
	return S_OK;
...
inline HRESULT resize_buffer(::wil::unique_bstr &string,
	...
	// if succeeded in creating a new BSTR, move ownership of the new BSTR into
	// string
...
inline DWORD
get_buffer_size_bytes(const ::wil::shared_bstr &value) WI_NOEXCEPT {
	return get_buffer_size_bytes(value.get());
}
...
inline HRESULT resize_buffer(::wil::shared_bstr &string,
							 DWORD byteSize) WI_NOEXCEPT {
	BSTR temp_bstr = string.get();
...
#if defined(__WIL_OBJBASE_H_)
inline void *
get_buffer(const ::wil::unique_cotaskmem_string &value) WI_NOEXCEPT {
	return value.get();
}

constexpr DWORD
get_buffer_size_bytes(const ::wil::unique_cotaskmem_string &) WI_NOEXCEPT {
	// wil::unique_cotaskmem_string does not intrinsically track its internal
	// buffer size thus the caller must track the buffer size it requested to be
	// allocated
	return 0;
}
...
template <>
constexpr bool
supports_resize_buffer<::wil::unique_cotaskmem_string>() WI_NOEXCEPT {
	return true;
}
inline HRESULT resize_buffer(::wil::unique_cotaskmem_string &string,
							 DWORD byteSize) WI_NOEXCEPT {
	// convert bytes to length (number of WCHAR's)
	size_t length = byteSize / sizeof(wchar_t);
	// ::wil::make_unique_string_nothrow adds one to the length when it
	// allocates, so subtracting 1 from the input length
	length = length > 0 ? length - 1 : length;
	auto new_string =
		::wil::make_unique_string_nothrow<::wil::unique_cotaskmem_string>(
			string.get(), length);
...
inline void *get_buffer(const ::wil::unique_cotaskmem_array_ptr<uint8_t> &value)
	WI_NOEXCEPT {
	return value.get();
}

inline DWORD get_buffer_size_bytes(
...
template <>
constexpr bool
supports_resize_buffer<::wil::unique_cotaskmem_array_ptr<uint8_t>>()
	WI_NOEXCEPT {
	return true;
}
inline HRESULT
resize_buffer(::wil::unique_cotaskmem_array_ptr<uint8_t> &arrayValue,
			  DWORD byteSize) WI_NOEXCEPT {
	...
	*tempValue.size_address() = byteSize;
	...
	const auto bytesToCopy =
		arrayValue.size() < byteSize ? arrayValue.size() : byteSize;
...
inline void *
get_buffer(const ::wil::shared_cotaskmem_string &value) WI_NOEXCEPT {
	return value.get();
}

constexpr DWORD
get_buffer_size_bytes(const ::wil::shared_cotaskmem_string &) WI_NOEXCEPT {
	// wil::shared_cotaskmem_string does not intrinsically track its internal
	// buffer size thus the caller must track the buffer size it requested to be
	// allocated
	return 0;
}
...
template <>
constexpr bool
supports_resize_buffer<::wil::shared_cotaskmem_string>() WI_NOEXCEPT {
	return true;
}
inline HRESULT resize_buffer(::wil::shared_cotaskmem_string &string,
							 DWORD byteSize) WI_NOEXCEPT {
	// convert bytes to length (number of WCHAR's)
	size_t length = byteSize / sizeof(wchar_t);
	// ::wil::make_unique_string_nothrow adds one to the length when it
...
inline void *
get_buffer(const ::wil::unique_process_heap_string &value) WI_NOEXCEPT {
	return value.get();
}

constexpr DWORD
get_buffer_size_bytes(const ::wil::unique_process_heap_string &) WI_NOEXCEPT {
	// wil::unique_process_heap_string does not intrinsically track its internal
	// buffer size thus the caller must track the buffer size it requested to be
	// allocated
	return 0;
}
...
template <>
constexpr bool
supports_resize_buffer<::wil::unique_process_heap_string>() WI_NOEXCEPT {
	return true;
}
inline HRESULT resize_buffer(::wil::unique_process_heap_string &string,
							 DWORD byteSize) WI_NOEXCEPT {
	// convert bytes to length (number of WCHAR's)
	size_t length = byteSize / sizeof(wchar_t);
	// ::wil::make_unique_string_nothrow adds one to the length when it
	...
	length = length > 0 ? length - 1 : length;
	auto new_string =
		::wil::make_unique_string_nothrow<::wil::unique_process_heap_string>(
			string.get(), length);
	RETURN_IF_NULL_ALLOC(new_string.get());
...
template <typename T> DWORD get_value_type() WI_NOEXCEPT {
	static_assert(sizeof(T) != sizeof(T),
				  "Unsupported type for get_value_type");
}

template <typename T> DWORD set_value_type() WI_NOEXCEPT {
	static_assert(sizeof(T) != sizeof(T),
				  "Unsupported type for set_value_type");
}
...
template <>
constexpr DWORD
set_value_type<const ::wil::unique_cotaskmem_string>() WI_NOEXCEPT {
	return REG_SZ;
}
...
template <>
constexpr DWORD
set_value_type<const ::wil::shared_cotaskmem_string>() WI_NOEXCEPT {
	return REG_SZ;
}
#endif // #if defined(__WIL_OBJBASE_H_STL)
} // namespace reg_value_type_info
...
template <typename err_policy = ::wil::err_exception_policy> class reg_view_t {
  public:
	...
	typename err_policy::result open_key(
		_In_opt_ _In_opt_ PCWSTR subKey, _Out_ HKEY *hkey,
		::wil::reg::key_access access = ::wil::reg::key_access::read) const {
		constexpr DWORD zero_options{0};
		return err_policy::HResult(HRESULT_FROM_WIN32(::RegOpenKeyExW(
			m_key, subKey, zero_options, get_access_flags(access), hkey)));
	}

	typename err_policy::result create_key(
		PCWSTR subKey, _Out_ HKEY *hkey,
		::wil::reg::key_access access = ::wil::reg::key_access::read) const {
		*hkey = nullptr;
		...
		return err_policy::HResult(HRESULT_FROM_WIN32(
			::RegCreateKeyExW(m_key, subKey, zero_reserved, null_class,
							  zero_options, get_access_flags(access),
							  null_security_attributes, hkey, &disposition)));
	}
	...
	typename err_policy::result delete_value(_In_opt_ PCWSTR value_name) const {
		return err_policy::HResult(
			HRESULT_FROM_WIN32(::RegDeleteValueW(m_key, value_name)));
	}

	template <typename R>
	typename err_policy::result
	get_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
			  R &return_value,
			  DWORD type = reg_value_type_info::get_value_type<R>()) const {
		return get_value_with_type(subkey, value_name, return_value, type);
	}

	// typename D supports unsigned 32-bit values; i.e. allows the caller to
	// pass a DWORD* as well as uint32_t*
	template <size_t Length, typename DwordType,
			  wistd::enable_if_t<wistd::is_same_v<DwordType, uint32_t> ||
								 wistd::is_same_v<DwordType, unsigned long>> * =
				  nullptr>
	typename err_policy::result
	get_value_char_array(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
		...
		const auto hr = HRESULT_FROM_WIN32(::RegGetValueW(
			m_key, subkey, value_name,
			::wil::reg::reg_view_details::get_value_flags_from_value_type(type),
	...
	// intended for err_exception_policy as err_returncode_policy will not get
	// an error code
	template <typename R>
	::std::optional<R>
	try_get_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
				  DWORD type = reg_value_type_info::get_value_type<R>()) const {
		R value{};
		const auto hr = get_value_with_type<R, ::wil::err_returncode_policy>(
	...
	template <typename R>
	typename err_policy::result
	set_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
			  const R &value,
  ...
  private:
	...
	template <typename R>
	typename err_policy::result
	set_value_with_type(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
						const R &value, DWORD type) const {
		return err_policy::HResult(HRESULT_FROM_WIN32(::RegSetKeyValueW(
			m_key, subkey, value_name, type,
	...
	typename get_value_with_type_policy::result get_value_with_type(
		...
		{
			const auto prepare_buffer_hr =
				reg_value_type_info::prepare_buffer(return_value);
			if (FAILED(prepare_buffer_hr)) {
				return get_value_with_type_policy::HResult(prepare_buffer_hr);
			}
		}

		// get_buffer_size_bytes should include the null terminator when used
		// for strings.
		DWORD bytes_allocated{
			reg_value_type_info::get_buffer_size_bytes(return_value)};
		...
		for (;;) {
			...
			get_value_hresult = HRESULT_FROM_WIN32(
				::RegGetValueW(m_key, subkey, value_name,
							   get_value_flags_from_value_type(type), null_type,
							   reg_value_type_info::get_buffer(return_value),
							   &data_size_bytes));

			// some return types we can grow as needed - e.g. when writing to a
			// std::wstring only compile and resize_buffer for those types that
			// support dynamically growing the buffer
			if
#if defined(__cpp_if_constexpr)
				constexpr
#endif
				(reg_value_type_info::supports_resize_buffer<R>()) {
				// Attempt to grow the buffer with the data_size_bytes returned
				// from GetRegValueW GetRegValueW will indicate the caller
				// allocate the returned number of bytes in one of two cases:
				// 1. returns ERROR_MORE_DATA
				// 2. returns ERROR_SUCCESS when we gave it a nullptr for the
				// out buffer
				const bool shouldReallocate =
					(::wil::reg::is_registry_buffer_too_small(
						get_value_hresult)) ||
				...
				if (shouldReallocate) {
					...
					const auto resize_buffer_hr =
						reg_value_type_info::resize_buffer(return_value,
														   data_size_bytes);
					if (FAILED(resize_buffer_hr)) {
						// if resize fails, return this error back to the caller
						return get_value_with_type_policy::HResult(
							resize_buffer_hr);
					}
				...
				// if the RegGetValueW call succeeded with a non-null [out]
				// param, and the type supports resize_buffer and the bytes we
				// allocated don't match data_size_bytes returned from
				// RegGetValueW resize the buffer to match what RegGetValueW
				// returned
				if (SUCCEEDED(get_value_hresult)) {
					const auto current_byte_size =
						reg_value_type_info::get_buffer_size_bytes(
							return_value);
					if (current_byte_size != data_size_bytes) {
						...
						const auto resize_buffer_hr =
							reg_value_type_info::resize_buffer(return_value,
															   data_size_bytes);
						if (FAILED(resize_buffer_hr)) {
							// if resize fails, return this error back to the
							// caller
							return get_value_with_type_policy::HResult(
								resize_buffer_hr);
						}
			...
			// we don't need to reallocate and retry the call to RegGetValueW so
			// breaking out of the loop
			break;
		}

		// some types (generally string types) require trimming its internal
		// buffer after RegGetValueW successfully wrote into its buffer
		if
#if defined(__cpp_if_constexpr)
			constexpr
#endif
...
using reg_view_nothrow =
	::wil::reg::reg_view_details::reg_view_t<::wil::err_returncode_policy>;
#if defined(WIL_ENABLE_EXCEPTIONS)
using reg_view =
	::wil::reg::reg_view_details::reg_view_t<::wil::err_exception_policy>;
...
// function overloads to allow *_enumerator objects to be constructed from all 3
// types of HKEY representatives
inline HKEY get_hkey(HKEY h) WI_NOEXCEPT { return h; }
...
// these overloads must be declared before the template functions below, as some
// of those template functions reference these overload functions
inline void clear_name(::std::wstring &name, size_t) WI_NOEXCEPT {
	name.assign(name.size(), L'\0');
}
inline ::std::wstring copy_name(const ::std::wstring &str,
...
template <typename T> PWSTR address_of_name(const T &name) WI_NOEXCEPT {
	return static_cast<PWSTR>(
		::wil::reg::reg_view_details::reg_value_type_info::get_buffer(name));
}

template <typename T> bool is_valid(const T &name) WI_NOEXCEPT {
...
template <typename T>
size_t resize_name(T &name, size_t current_length,
				   size_t new_length) WI_NOEXCEPT {
	if (new_length > current_length) {
		// resize_buffer takes size in bytes
		if (FAILED(
				::wil::reg::reg_view_details::reg_value_type_info::
					resize_buffer(name, static_cast<DWORD>(new_length *
														   sizeof(wchar_t))))) {
			return 0;
		}
		return new_length;
	...
	// continue to use the existing buffer since the requested length is less
	// than or equals to the current length
...
#if defined(__WIL_OLEAUTO_H_)
// overloads for some of the above string functions - specific for
// wil::unique_bstr these should come after the template functions - as they
// reference some of those functions
inline size_t resize_name(::wil::unique_bstr &name, size_t current_length,
						  size_t new_length) WI_NOEXCEPT {
	if (new_length > current_length) {
		// SysAllocStringLen adds a null, so subtract a wchar_t from the input
		// length
		...
		const BSTR new_bstr{
			::SysAllocStringLen(nullptr, static_cast<UINT>(new_length))};
		if (!new_bstr) {
			return 0;
		}
		name.reset(new_bstr);
...
inline ::wil::unique_bstr copy_name(const ::wil::unique_bstr &name,
									size_t length) WI_NOEXCEPT {
	if (!is_valid(name)) {
		return {};
	}
	...
	return ::wil::unique_bstr{
		::SysAllocStringLen(name.get(), static_cast<UINT>(length))};
}
#endif // #if defined(__WIL_OLEAUTO_H_)
};	   // namespace reg_iterator_details
...
// all methods must be noexcept - to be usable with any iterator type (throwing
// or non-throwing)
template <typename T> class key_iterator_data {
  public:
	...
	key_iterator_data(const key_iterator_data &rhs) WI_NOEXCEPT {
		...
		name = ::wil::reg::reg_iterator_details::copy_name(rhs.name,
														   rhs.m_name_length);
		m_hkey = rhs.m_hkey;
		m_index = rhs.m_index;
		m_name_length = ::wil::reg::reg_iterator_details::is_valid(name)
							? rhs.m_name_length
							: 0;
	}
  ...
  private:
	...
	bool resize(size_t new_length) WI_NOEXCEPT {
		m_name_length = ::wil::reg::reg_iterator_details::resize_name(
			name, m_name_length, new_length);
	...
	HRESULT enumerate_current_index() WI_NOEXCEPT {
		...
		for (auto string_length = static_cast<DWORD>(m_name_length);;) {
			...
			const auto error = ::RegEnumKeyExW(
				...
				string_length == 0
					? nullptr
					: ::wil::reg::reg_iterator_details::address_of_name(
						  name), // lpName
			...
			if (error == ERROR_SUCCESS) {
				...
				::wil::reg::reg_view_details::reg_value_type_info::trim_buffer(
					name);
				break;
			}
			...
			if (error == ERROR_MORE_DATA) {
				...
				string_length += ::wil::reg::reg_iterator_details::
					iterator_default_buffer_length;
				continue;
			}
...
template <typename T> class value_iterator_data {
  public:
	...
	value_iterator_data &
	operator=(value_iterator_data &&rhs) WI_NOEXCEPT = default;
...
template <typename T> class iterator_t {
  public:
	// defining iterator_traits allows STL <algorithm> functions to be used with
	// this iterator class. Notice this is a forward_iterator
	...
	iterator_t(HKEY hkey) : m_data(hkey) {
		if (hkey != nullptr) {
			m_data.resize(::wil::reg::reg_iterator_details::
							  iterator_default_buffer_length);
			m_data.m_index = 0;
	...
	bool operator==(const iterator_t &rhs) const WI_NOEXCEPT {
		if (m_data.at_end() || rhs.m_data.at_end()) {
			// if either is not initialized (or end), both must not be
			// initialized (or end) to be equal
			return m_data.m_index == rhs.m_data.m_index;
		}
		return m_data.m_hkey == rhs.m_data.m_hkey &&
			   m_data.m_index == rhs.m_data.m_index;
	}
...
template <typename T> class iterator_nothrow_t {
  public:
	...
	iterator_nothrow_t(HKEY hkey) WI_NOEXCEPT : m_data(hkey) {
		if (hkey != nullptr) {
			m_data.m_index = 0;
			if (!m_data.resize(::wil::reg::reg_iterator_details::
								   iterator_default_buffer_length)) {
	...
	iterator_nothrow_t &
	operator=(const iterator_nothrow_t &) WI_NOEXCEPT = default;
	...
	bool at_end() const WI_NOEXCEPT { return m_data.at_end(); }

	HRESULT last_error() const WI_NOEXCEPT { return m_last_error; }
	...
	HRESULT move_next() WI_NOEXCEPT {
		...
		if (newIndex < m_data.m_index) {
			...
			m_last_error = E_INVALIDARG;
				   ...
				   ::wil::reg::reg_iterator_details::iterator_end_offset) {
			// fail if this creates an end iterator
			m_last_error = E_INVALIDARG;
		} else {
	...
	// operator support
	const T &operator*() const WI_NOEXCEPT { return m_data; }
	const T &operator*() WI_NOEXCEPT { return m_data; }
	const T *operator->() const WI_NOEXCEPT { return &m_data; }
	const T *operator->() WI_NOEXCEPT { return &m_data; }

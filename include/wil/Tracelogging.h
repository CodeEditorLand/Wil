...
// Note that we avoid pulling in STL's memory header from TraceLogging.h through
// Resource.h as we have TraceLogging customers who are still on older versions
// of STL (without std::shared_ptr<>).
...
#pragma warning(push)
#pragma warning(disable : 26135) // Missing locking annotation, Caller failing
								 // to hold lock
...
#define __TRACELOGGING_TEST_HOOK_VERIFY_API_TELEMETRY(                         \
	nameSpace, apiList, specializationList, countArray, numCounters)
...
#define __TRACELOGGING_DEFINE_PROVIDER_STORAGE_LINK(name1, name2)              \
	__annotation(L"_TlgProviderLink:|" _wiltlg_LSTRINGIZE(                     \
		__LINE__) L"|Key|" _wiltlg_LSTRINGIZE(name1) L"=" _wiltlg_LSTRINGIZE(name2))

// Utility macro for writing relevant fields from a wil::FailureInfo structure
// into a TraceLoggingWrite statement.  Most fields are relevant for telemetry
// or for simple ETW, but there are a few additional fields reported via ETW.
...
#define __RESULT_TELEMETRY_COMMON_FAILURE_PARAMS(failure)                      \
	TraceLoggingUInt32((failure).hr, "hresult", "Failure error code"),         \
		TraceLoggingString((failure).pszFile, "fileName",                      \
						   "Source code file name where the error occurred"),  \
		TraceLoggingUInt32((failure).uLineNumber, "lineNumber",                \
						   "Line number within the source code file where "    \
						   "the error occurred"),                              \
		TraceLoggingString((failure).pszModule, "module",                      \
						   "Name of the binary where the error occurred"),     \
		TraceLoggingUInt32(                                                    \
			static_cast<DWORD>((failure).type), "failureType",                 \
			"Indicates what type of failure was observed (exception, "         \
			"returned error, logged error or fail fast"),                      \
		TraceLoggingWideString(                                                \
			(failure).pszMessage, "message",                                   \
			"Custom message associated with the failure (if any)"),            \
		TraceLoggingUInt32((failure).threadId, "threadId",                     \
						   "Identifier of the thread the error occurred on"),  \
		TraceLoggingString(                                                    \
			(failure).pszCallContext, "callContext",                           \
			"List of telemetry activities containing this error"),             \
		TraceLoggingUInt32((failure).callContextOriginating.contextId,         \
						   "originatingContextId",                             \
						   "Identifier for the oldest telemetry activity "     \
		...
		TraceLoggingWideString(                                                \
			...
			"Custom message associated with the oldest telemetry activity "    \
			"containing this error (if any)"),                                 \
		TraceLoggingUInt32((failure).callContextCurrent.contextId,             \
						   "currentContextId",                                 \
						   "Identifier for the newest telemetry activity "     \
						   "containing this error"),                           \
		TraceLoggingString(                                                    \
			(failure).callContextCurrent.contextName, "currentContextName",    \
			"Name of the newest telemetry activity containing this error"),    \
		TraceLoggingWideString(                                                \
			(failure).callContextCurrent.contextMessage,                       \
			"currentContextMessage",                                           \
			"Custom message associated with the newest telemetry activity "    \
			"containing this error (if any)")

#define __RESULT_TRACELOGGING_COMMON_FAILURE_PARAMS(failure)                   \
	__RESULT_TELEMETRY_COMMON_FAILURE_PARAMS(failure),                         \
		TraceLoggingUInt32(static_cast<DWORD>((failure).failureId),            \
						   "failureId",                                        \
						   "Identifier assigned to this failure"),             \
		TraceLoggingUInt32(static_cast<DWORD>((failure).cFailureCount),        \
						   "failureCount",                                     \
						   "Number of failures seen within the binary where "  \
						   "the error occurred"),                              \
...
#define __ACTIVITY_START_PARAMS()                                              \
	TraceLoggingStruct(1, "wilActivity"),                                      \
		TraceLoggingUInt32(::GetCurrentThreadId(), "threadId",                 \
						   "Identifier of the thread the activity was run on")

// Activity Stop Event (SUCCESSFUL or those WITHOUT full failure info -- just
// hr) Also utilized for intermediate stop events (a successful call to 'Stop()'
// from a Split activity
...
// "ActivityError" tagged event (all distinct FAILURES occurring within the
// outer activity scope)
...
// "ActivityFailure" tagged event (only comes through on TELEMETRY for
// CallContext activities that have FAILED)
...
#define __ACTIVITY_FAILURE_TELEMETRY_PARAMS(hr, contextName, contextMessage)   \
	TelemetryPrivacyDataTag(PDT_ProductAndServicePerformance),                 \
		...
		TraceLoggingString(contextName, "currentContextName",                  \
						   "Name of the activity containing this error"),      \
		TraceLoggingWideString(                                                \
			contextMessage, "currentContextMessage",                           \
			"Custom message for the activity containing this error (if any)")

// "FallbackError" events (all FAILURE events happening outside of ANY activity
// context)
...
template <typename ActivityTraceLoggingType, ActivityOptions options,
		  UINT64 keyword, UINT8 level, UINT64 privacyTag,
		  typename TlgReflectorTag>
class ActivityBase;

/// @cond
namespace details {
// Lazy static initialization helper for holding a singleton telemetry class to
// maintain the provider handle.
...
template <class T> class static_lazy {
  public:
	void __cdecl cleanup() WI_NOEXCEPT {
		...
		// If object is being constructed on another thread, wait until
		// construction completes. Need a memory barrier here (see get() and
		// ~Completer below) so use the result that we get from
		// InitOnceBeginInitialize(..., &pVoid, ...)
		if (::InitOnceBeginInitialize(&m_initOnce, INIT_ONCE_CHECK_ONLY,
									  &pending, &pVoid) &&
			!pending) {
	...
	T *get(void(__cdecl *cleanupFunc)(void)) WI_NOEXCEPT {
		void *pVoid{};
		BOOL pending;
		if (::InitOnceBeginInitialize(&m_initOnce, 0, &pending, &pVoid) &&
			pending) {
			// Don't do anything non-trivial from DllMain, fail fast.
			// Some 3rd party code in IE calls shell functions this way, so we
			// can only enforce this in DEBUG.
#ifdef DEBUG
			...
			pVoid = &m_storage;
			::new (pVoid) T();
			atexit(cleanupFunc); // ignore failure (that's what the C runtime
								 // does, too)
			completer.Succeed();
		}
  ...
  private:
	...
	struct Completer {
		...
		explicit Completer(static_lazy *pSelf) WI_NOEXCEPT
			: m_pSelf(pSelf),
			  m_flags(INIT_ONCE_INIT_FAILED) {}
		...
		~Completer() WI_NOEXCEPT {
			...
			::InitOnceComplete(&m_pSelf->m_initOnce, m_flags,
							   &m_pSelf->m_storage);
		}
	};
};

// This class serves as a simple RAII wrapper around CallContextInfo.  It
// presumes that the contextName parameter is always a static string, but copies
// or allocates the contextMessage as needed.

class StoredCallContextInfo : public wil::CallContextInfo {
  public:
	StoredCallContextInfo() WI_NOEXCEPT { ::ZeroMemory(this, sizeof(*this)); }
	...
	StoredCallContextInfo &
	operator=(StoredCallContextInfo &&other) WI_NOEXCEPT {
	...
	StoredCallContextInfo(PCSTR staticContextName,
	...
	void SetMessage(_Printf_format_string_ PCSTR formatString,
		...
		PrintLoggingMessage(loggingMessage, ARRAYSIZE(loggingMessage),
	...
	void AssignMessage(PCWSTR message) {
		...
		if (length > 0) {
			...
			if (contextMessage != nullptr) {
				m_ownsMessage = true;
				memcpy_s(const_cast<PWSTR>(contextMessage), sizeBytes, message,
						 sizeBytes);
			}
		}
	}
...
void SetRelatedActivityId(
	wil::ActivityBase<ActivityTraceLoggingType, options, keyword, level,
					  privacyTag, TlgReflectorTag> &activity) {
...
// This class acts as a simple RAII class returned by a call to
// ContinueOnCurrentThread() for an activity or by a call to
// WatchCurrentThread() on a provider.  The result is meant to be a stack local
// variable whose scope controls the lifetime of an error watcher on the given
// thread.  That error watcher re-directs errors occurrent within the object's
// lifetime to the associated provider or activity.
...
class ActivityThreadWatcher {
  public:
	...
	ActivityThreadWatcher(
		_In_ details::IFailureCallback *pCallback,
...
// This is the base-class implementation of a TraceLogging class.  TraceLogging
// classes are defined with BEGIN_TRACELOGGING_CLASS and automatically derive
// from this class

enum class ErrorReportingType { None = 0, Telemetry, TraceLogging };
...
class TraceLoggingProvider : public details::IFailureCallback {
  ...
  protected:
	// This can be overridden to provide specific initialization code for any
	// individual provider. It will be ran once when the single static singleton
	// instance of this class is created.
	virtual void Initialize() WI_NOEXCEPT {}

	// This method can be overridden by a provider to more tightly control what
	// happens in the event of a failure in a CallContext activity,
	// WatchCurrentThread() object, or attributed to a specific failure.
	virtual void OnErrorReported(bool alreadyReported,
								 FailureInfo const &failure) WI_NOEXCEPT {
		if (!alreadyReported &&
			...
			} else if (m_errorReportingType ==
					   ErrorReportingType::TraceLogging) {
  ...
  protected:
	...
	WI_NODISCARD bool IsEnabled_(
		...
		return ((m_providerHandle != nullptr) &&
				TraceLoggingProviderEnabled(m_providerHandle, eventLevel,
											eventKeywords)) ||
	...
	void ReportTelemetryFailure(FailureInfo const &failure) WI_NOEXCEPT {
		...
		TraceLoggingWrite(
			m_providerHandle, "FallbackError",
			TelemetryPrivacyDataTag(PDT_ProductAndServicePerformance),
	...
	// It prints out a trace message for debug purposes. The message does not go
	// into the telemetry.
	void ReportTraceLoggingError(_In_ _Printf_format_string_ PCSTR formatString,
								 va_list argList) WI_NOEXCEPT {
		if (IsEnabled_(WINEVENT_LEVEL_ERROR, 0)) {
			wchar_t loggingMessage[2048];
			details::PrintLoggingMessage(loggingMessage,
										 ARRAYSIZE(loggingMessage),
										 formatString, argList);
			TraceLoggingWrite(
				m_providerHandle, "TraceLoggingError",
				TraceLoggingLevel(WINEVENT_LEVEL_ERROR),
	...
	// into the telemetry.
	void
	ReportTraceLoggingMessage(_In_ _Printf_format_string_ PCSTR formatString,
							  va_list argList) WI_NOEXCEPT {
		if (IsEnabled_(WINEVENT_LEVEL_VERBOSE, 0)) {
			...
			details::PrintLoggingMessage(loggingMessage,
										 ...
										 formatString, argList);
			TraceLoggingWrite(
				m_providerHandle, "TraceLoggingInfo",
				TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE),
				TraceLoggingWideString(loggingMessage, "traceLoggingMessage"));
		}
	}

	void Register(TraceLoggingHProvider const providerHandle,
				  TLG_PENABLECALLBACK callback = nullptr) WI_NOEXCEPT {
	...
	void
	AttachProvider(TraceLoggingHProvider const providerHandle) WI_NOEXCEPT {
		m_providerHandle = providerHandle;
...
template <typename TraceLoggingType, UINT64 keyword = 0,
  ...
  public:
	...
	*/
	...
	May only be called once. If used, must be called before starting the
	activity.
	*/
...
template <typename TraceLoggingType, UINT64 keyword = 0,
	...
	: public _TlgActivityBase<BasicThreadActivity<TraceLoggingType, keyword,
							  ...
							  keyword, level> {
	using BaseTy = _TlgActivityBase<
		BasicThreadActivity<TraceLoggingType, keyword, level, TlgReflectorTag>,
		keyword, level>;
	friend BaseTy;

	void OnStarted() { this->PushThreadActivityId(); }

	void OnStopped() { this->PopThreadActivityId(); }
...
#define __WI_TraceLoggingWriteTagged(activity, name, ...)                      \
	__pragma(warning(push)) __pragma(warning(disable : 4127)) do {             \
		_tlgActivityDecl(activity) TraceLoggingWriteActivity(                  \
			TraceLoggingType::Provider(), (name),                              \
			_tlgActivityRef(activity).Id(), NULL, __VA_ARGS__);                \
	}                                                                          \
	while (0)                                                                  \
	__pragma(warning(pop))

// This is the ultimate base class implementation for all activities.  Activity
// classes are defined with DEFINE_TRACELOGGING_ACTIVITY,
// DEFINE_CALLCONTEXT_ACTIVITY, DEFINE_TELEMETRY_ACTIVITY and others

template <typename ActivityTraceLoggingType,
		  ActivityOptions options = ActivityOptions::None, UINT64 keyword = 0,
  ...
  public:
	...
	ActivityBase(PCSTR contextName, bool shouldWatchErrors = false) WI_NOEXCEPT
		: m_activityData(contextName),
		  ...
		  m_callbackHolder(this, m_activityData.GetCallContext(),
						   shouldWatchErrors) {}
	...
	ActivityBase(ActivityBase &&other, bool shouldWatchErrors) WI_NOEXCEPT
		...
		m_pActivityData =
			m_sharedActivityData ? m_sharedActivityData.get() : &m_activityData;
	...
	ActivityBase(ActivityBase &&other) WI_NOEXCEPT
		: ActivityBase(wistd::move(other),
					   other.m_callbackHolder.IsWatching()) {}
	...
	ActivityBase(ActivityBase const &other) WI_NOEXCEPT
		: m_activityData(),
		  ...
		  m_callbackHolder(
			  this, nullptr,
			  false) // false = do not automatically watch for failures
	{
		operator=(other);
	}
	...
	ActivityBase &operator=(ActivityBase const &other) WI_NOEXCEPT {
		...
		if (other.m_sharedActivityData) {
		...
		} else if (m_sharedActivityData.create(
					   wistd::move(other.m_activityData))) {
			// Locking should not be required as the first copy should always
			// take place on the owning thread...
			...
			other.m_callbackHolder.SetCallContext(
				m_pActivityData->GetCallContext());
		}
		m_callbackHolder.SetCallContext(m_pActivityData->GetCallContext());
		return *this;
	}

	// These calls all result in setting a message to associate with any
	// failures that might occur while running the activity.  For example, you
	// could associate a filename with a call context activity so that the file
	// name is only reported if the activity fails with the failure.
	...
	// This call stops watching for errors on the thread that the activity was
	// originally created on.  Use it when moving the activity into a
	// thread-agnostic class or moving an activity across threads.
	...
	// Call this API to retrieve an RAII object to watch events on the current
	// thread.  The returned object should only be used on the stack.

	WI_NODISCARD ActivityThreadWatcher ContinueOnCurrentThread() WI_NOEXCEPT {
		if (IsRunning()) {
			return ActivityThreadWatcher(this,
										 *m_pActivityData->GetCallContext());
		}
		return ActivityThreadWatcher();
	}

	// This is the 'default' Stop routine that accepts an HRESULT and completes
	// the activity...
	...
	void Stop(HRESULT hr = S_OK) WI_NOEXCEPT {
		...
		if (stopActivity) {
			...
			__WI_TraceLoggingWriteTagged(
				*this, "ActivityIntermediateStop", TraceLoggingKeyword(Keyword),
				TelemetryPrivacyDataTag(PDT_ProductAndServicePerformance),
	...
	bool NotifyFailure(FailureInfo const &failure) WI_NOEXCEPT override {
		// We always report errors to the ETW stream, but we hold-back the
		// telemetry keyword if we've already reported this error to this
		// particular telemetry provider.

		__TRACELOGGING_TEST_HOOK_ACTIVITY_ERROR(failure);

		if (WI_IsFlagClear(failure.flags,
						   FailureFlags::RequestSuppressTelemetry)) {
#ifdef __clang__
#pragma clang diagnostic push
				...
				__WI_TraceLoggingWriteTagged(
					*this, "ActivityError",
					...
					__ACTIVITY_ERROR_TELEMETRY_FAILURE_PARAMS(failure));
					   ...
					   WI_IsFlagSet(options,
									ActivityOptions::TraceLoggingOnFailure)) {
				__WI_TraceLoggingWriteTagged(
					*this, "ActivityError", TraceLoggingKeyword(0),
					TraceLoggingLevel(WINEVENT_LEVEL_ERROR),
					__ACTIVITY_ERROR_TRACELOGGING_FAILURE_PARAMS(failure));
			} else {
				__WI_TraceLoggingWriteTagged(
					*this, "ActivityError", TraceLoggingKeyword(Keyword),
					TraceLoggingLevel(WINEVENT_LEVEL_ERROR),
					__ACTIVITY_ERROR_TRACELOGGING_FAILURE_PARAMS(failure));
			}
#pragma warning(pop)
...
#pragma clang diagnostic pop
	...
	// This is the base TraceLoggingActivity<> contract...  we implement it so
	// that this class can be used by all of the activity macros and we re-route
  ...
  protected:
	...
	void SetStopResult(HRESULT hr,
					   _Out_opt_ HRESULT *phr = nullptr) WI_NOEXCEPT {
		auto lock = LockExclusive();
	...
	// Locking should not be required on these accessors as we only use this at
	// reporting (which will only happen from the final stop)
	...
	WI_NODISCARD details::StoredCallContextInfo *
	GetCallContext() const WI_NOEXCEPT {
		return m_pActivityData->GetCallContext();
	}

	// Think of this routine as the destructor -- since we need to call virtual
	// derived methods, we can't use it as a destructor without a pure virtual
	// method call, so we have the derived class call it in its destructor...

	void Destroy() WI_NOEXCEPT {
		bool fStop = true;
		if (m_sharedActivityData) {
			// The lock unifies the 'unique()' check and the 'reset()' of any
			// non-unique activity so that we can positively identify the final
			// release of the internal data
  ...
  private:
	void ReportStopActivity(HRESULT hr) WI_NOEXCEPT {
		if (FAILED(hr) &&
			WI_AreAllFlagsClear(Keyword, (MICROSOFT_KEYWORD_TELEMETRY |
										  MICROSOFT_KEYWORD_MEASURES |
										  MICROSOFT_KEYWORD_CRITICAL_DATA)) &&
			WI_IsFlagSet(options, ActivityOptions::TelemetryOnFailure)) {
			wil::FailureInfo const *pFailure = GetFailureInfo();
			if (pFailure != nullptr) {
				__TRACELOGGING_TEST_HOOK_CALLCONTEXT_ERROR(pFailure,
														   pFailure->hr);
				...
				__WI_TraceLoggingWriteTagged(
					*this, "ActivityFailure",
					...
					__ACTIVITY_FAILURE_TELEMETRY_PARAMS(
						hr, m_pActivityData->GetCallContext()->contextName,
	...
	class ActivityData
		: public _TlgActivityBase<
			  ActivityData<ActivityTraceLoggingType, TlgReflectorTag>, keyword,
			  level> {
		using BaseTy = _TlgActivityBase<
			ActivityData<ActivityTraceLoggingType, TlgReflectorTag>, keyword,
			level>;
		...
		// SFINAE dispatching on presence of
		// ActivityTraceLoggingType::CreateActivityId(_Out_ GUID&
		...
		auto CreateActivityIdByProviderType(int, _Out_ GUID &childActivityId)
			-> decltype(ProviderType::CreateActivityId(childActivityId,
													   this->GetRelatedId()),
						(void)0) {
			ProviderType::CreateActivityId(childActivityId,
										   this->GetRelatedId());
		}
		...
		auto CreateActivityIdByProviderType(long, _Out_ GUID &childActivityId)
			-> void {
			EventActivityIdControl(EVENT_ACTIVITY_CTRL_CREATE_ID,
								   &childActivityId);
		}

		void CreateActivityId(_Out_ GUID &childActivityId) {
			CreateActivityIdByProviderType<ActivityTraceLoggingType>(
				0, childActivityId);
		}
	  ...
	  public:
		...
		void NotifyFailure(FailureInfo const &failure) WI_NOEXCEPT {
			if ((failure.hr != m_failure.GetFailureInfo()
								   .hr) && // don't replace with the same error
										   // (likely propagation up the stack)
				((failure.hr != m_result) ||
				 SUCCEEDED(
					 m_result))) // don't replace if we've already got the
								 // current explicitly supplied failure code
			{
		...
		bool SetStopResult(HRESULT hr, _Out_opt_ HRESULT *phr) WI_NOEXCEPT {
			// We must be expecting at least one Stop -- otherwise the caller is
			// calling Stop() more times than it can (normally once, or +1 for
			// each call to Split())
		...
		HRESULT SetUnhandledException() WI_NOEXCEPT {
			...
			SetStopResult(
				FAILED(hr) ? hr : HRESULT_FROM_WIN32(ERROR_UNHANDLED_EXCEPTION),
				&hr);
			return hr;
		}

		void IncrementExpectedStopCount() WI_NOEXCEPT { m_stopCountExpected++; }

		WI_NODISCARD FailureInfo const *GetFailureInfo() const WI_NOEXCEPT {
			return (FAILED(m_result) &&
					(m_result == m_failure.GetFailureInfo().hr))
	...
	mutable ActivityData<ActivityTraceLoggingType, TlgReflectorTag>
		m_activityData;
	mutable ActivityData<ActivityTraceLoggingType, TlgReflectorTag>
		*m_pActivityData;
	mutable details::shared_object<
		ActivityData<ActivityTraceLoggingType, TlgReflectorTag>>
		m_sharedActivityData;
...
#define __WI_TraceLoggingWriteStart(activity, name, ...)                       \
	__pragma(warning(push)) __pragma(warning(disable : 4127)) do {             \
		_tlgActivityDecl(activity) static const UINT64 _tlgActivity_Keyword =  \
			_tlgActivityRef(activity).Keyword;                                 \
		static const UINT8 _tlgActivity_Level =                                \
			_tlgActivityRef(activity).Level;                                   \
		static const UINT64 _tlgActivityPrivacyTag =                           \
			_tlgActivityRef(activity).PrivacyTag;                              \
		static_assert(_tlgActivity_Keyword ==                                  \
						  (_tlgActivity_Keyword _tlg_FOREACH(_tlgKeywordVal,   \
															 __VA_ARGS__)),    \
					  "Do not use TraceLoggingKeyword in "                     \
					  "TraceLoggingWriteStart. Keywords for START events are " \
					  "specified in the activity type, e.g. "                  \
					  "TraceLoggingActivity<Provider,Keyword,Level>.");        \
		static_assert(_tlgActivity_Level == (_tlgActivity_Level _tlg_FOREACH(  \
												_tlgLevelVal, __VA_ARGS__)),   \
					  "Do not use TraceLoggingLevel in "                       \
					  "TraceLoggingWriteStart. The Level for START events is " \
					  "specified in the activity type, e.g. "                  \
					  "TraceLoggingActivity<Provider,Keyword,Level>.");        \
...
#define __WRITE_ACTIVITY_START(EventId, ...)                                   \
	...
	__WI_TraceLoggingWriteStart(*this, #EventId, __ACTIVITY_START_PARAMS(),    \
								__VA_ARGS__);                                  \
	...
	__pragma(warning(push)) __pragma(warning(disable : 4127)) do {             \
		...
		static_assert(_tlgActivity_Keyword ==                                  \
						  (_tlgActivity_Keyword _tlg_FOREACH(_tlgKeywordVal,   \
															 __VA_ARGS__)),    \
					  "Do not use TraceLoggingKeyword in "                     \
					  "TraceLoggingWriteStop. Keywords for STOP events are "   \
					  "specified in the activity type, e.g. "                  \
					  "TraceLoggingActivity<Provider,Keyword,Level>.");        \
		static_assert(_tlgActivity_Level == (_tlgActivity_Level _tlg_FOREACH(  \
												_tlgLevelVal, __VA_ARGS__)),   \
					  "Do not use TraceLoggingLevel in "                       \
					  "TraceLoggingWriteStop. The Level for STOP events is "   \
					  "specified in the activity type, e.g. "                  \
					  "TraceLoggingActivity<Provider,Keyword,Level>.");        \
		_tlgActivityRef(activity).zInternalStop();                             \
		TraceLoggingWriteActivity(                                             \
			TraceLoggingType::Provider(), (name),                              \
			_tlgActivityRef(activity).Id(), NULL,                              \
	...
	if (pFailure != nullptr) {                                                 \
		...
		if (false,                                                             \
			WI_IsAnyFlagSet(Keyword, (MICROSOFT_KEYWORD_TELEMETRY |            \
									  MICROSOFT_KEYWORD_MEASURES |             \
									  MICROSOFT_KEYWORD_CRITICAL_DATA))) {     \
			__WI_TraceLoggingWriteStop(                                        \
				*this, #EventId,                                               \
				__ACTIVITY_STOP_TELEMETRY_FAILURE_PARAMS(failure),             \
				__VA_ARGS__);                                                  \
		} else {                                                               \
			__WI_TraceLoggingWriteStop(                                        \
				*this, #EventId,                                               \
				__ACTIVITY_STOP_TRACELOGGING_FAILURE_PARAMS(failure),          \
				__VA_ARGS__);                                                  \
		}                                                                      \
	} else {                                                                   \
		__TRACELOGGING_TEST_HOOK_ACTIVITY_STOP(nullptr, GetResult());          \
		__WI_TraceLoggingWriteStop(*this, #EventId,                            \
								   __ACTIVITY_STOP_PARAMS(GetResult()),        \
								   __VA_ARGS__);                               \
	}                                                                          \
	IgnoreCurrentThread();
...
#define __BEGIN_TRACELOGGING_ACTIVITY_CLASS(ActivityClassName, ...)            \
	class ActivityClassName final                                              \
		: public wil::ActivityBase<TraceLoggingType, __VA_ARGS__> {            \
	  protected:                                                               \
		void StopActivity() WI_NOEXCEPT override {                             \
			__WRITE_ACTIVITY_STOP(ActivityClassName);                          \
		}                                                                      \
		bool                                                                   \
		WasAlreadyReportedToTelemetry(long failureId) WI_NOEXCEPT override {   \
	  ...
	  public:                                                                  \
		...
		ActivityClassName(ActivityClassName const &other) WI_NOEXCEPT          \
			: ActivityBase(other) {}                                           \
		ActivityClassName(ActivityClassName &&other) WI_NOEXCEPT               \
			: ActivityBase(wistd::move(other)) {}                              \
		ActivityClassName(ActivityClassName &&other,                           \
						  bool shouldWatchErrors) WI_NOEXCEPT                  \
			: ActivityBase(wistd::move(other), shouldWatchErrors) {}           \
		ActivityClassName &                                                    \
		operator=(ActivityClassName const &other) WI_NOEXCEPT {                \
		...
		void StopWithResult(HRESULT hr) { ActivityBase::Stop(hr); }            \
		template <typename... TArgs>                                           \
		void StopWithResult(HRESULT hr, TArgs &&...args) {                     \
			SetStopResult(hr);                                                 \
			Stop(wistd::forward<TArgs>(args)...);                              \
		}                                                                      \
		void Stop(HRESULT hr = S_OK) WI_NOEXCEPT { ActivityBase::Stop(hr); }   \
		...
		void StartActivityWithCorrelationVector(PCSTR correlationVector)       \
			WI_NOEXCEPT {                                                      \
			__WRITE_ACTIVITY_START(                                            \
				ActivityClassName,                                             \
...
#define __IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)                          \
  private:                                                                     \
	template <typename... TArgs>                                               \
	ActivityClassName(wil::details::tag_start, TArgs &&...args) WI_NOEXCEPT    \
		: ActivityBase(#ActivityClassName) {                                   \
		StartActivity(wistd::forward<TArgs>(args)...);                         \
		__TRACELOGGING_DEFINE_PROVIDER_STORAGE_LINK("this",                    \
													ActivityClassName);        \
	}                                                                          \
	template <typename... TArgs>                                               \
	ActivityClassName(wil::details::tag_start_cv,                              \
					  _In_opt_ PCSTR correlationVector, TArgs &&...args)       \
		WI_NOEXCEPT : ActivityBase(#ActivityClassName) {                       \
		StartActivityWithCorrelationVector(correlationVector,                  \
										   wistd::forward<TArgs>(args)...);    \
		__TRACELOGGING_DEFINE_PROVIDER_STORAGE_LINK("this",                    \
													ActivityClassName);        \
	}                                                                          \
                                                                               \
  public:                                                                      \
	ActivityClassName() WI_NOEXCEPT                                            \
		: ActivityBase(#ActivityClassName, false) {}                           \
	template <typename... TArgs>                                               \
	WI_NODISCARD static ActivityClassName Start(TArgs &&...args) {             \
		return ActivityClassName(wil::details::tag_start(),                    \
								 wistd::forward<TArgs>(args)...);              \
	}                                                                          \
	template <typename... TArgs>                                               \
	WI_NODISCARD static ActivityClassName StartWithCorrelationVector(          \
		_In_ PCSTR correlationVector, TArgs &&...args) {                       \
		return ActivityClassName(wil::details::tag_start_cv(),                 \
								 correlationVector,                            \
								 wistd::forward<TArgs>(args)...);              \
	}
...
#define __IMPLEMENT_CALLCONTEXT_CLASS(ActivityClassName)                       \
  ...
  public:                                                                      \
	...
	ActivityClassName(PCSTR contextName,                                       \
					  _Printf_format_string_ PCSTR formatString, ...)          \
		WI_NOEXCEPT : ActivityClassName(contextName) {                         \
		va_list argList;                                                       \
		va_start(argList, formatString);                                       \
		GetCallContext()->SetMessage(formatString, argList);                   \
	}                                                                          \
	WI_NODISCARD static ActivityClassName Start(PCSTR contextName)             \
		WI_NOEXCEPT {                                                          \
		return ActivityClassName(static_cast<void **>(__nullptr),              \
								 contextName);                                 \
	}                                                                          \
	WI_NODISCARD static ActivityClassName Start(                               \
		PCSTR contextName, _Printf_format_string_ PCSTR formatString, ...)     \
		WI_NOEXCEPT {                                                          \
		va_list argList;                                                       \
		va_start(argList, formatString);                                       \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT(EventId, ...)                         \
	void EventId() {                                                           \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId, _GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);      \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT(EventId, ...)                         \
	void EventId() {                                                           \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_CV(EventId, ...)                      \
	void EventId(PCSTR correlationVector) {                                    \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId, _GENERIC_PARTB_FIELDS_ENABLED,                    \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(EventId, VarType1, varName1,   \
	...
	template <typename T1> void EventId(T1 &&varName1) {                       \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
				_wiltlg_STRINGIZE(varName1)),                                  \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1_CV(EventId, VarType1,          \
												   varName1, ...)              \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(EventId, VarType1, varName1,   \
												VarType2, varName2, ...)       \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName1)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType2>(wistd::forward<T2>(varName2)),           \
				_wiltlg_STRINGIZE(varName2)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(EventId, VarType1, varName1,   \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2_CV(                            \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                               \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3, ...)  \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName2)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType3>(wistd::forward<T3>(varName3)),           \
				_wiltlg_STRINGIZE(varName3)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                               \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName3)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType4>(wistd::forward<T4>(varName4)),           \
				_wiltlg_STRINGIZE(varName4)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM4(                               \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName4)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType5>(wistd::forward<T5>(varName5)),           \
				_wiltlg_STRINGIZE(varName5)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM5(                               \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM5_CV(                            \
	...
	template <typename T1, typename T2, typename T3, typename T4, typename T5> \
	void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3, T4 &&varName4,   \
				 T5 &&varName5, PCSTR correlationVector) {                     \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName5)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType6>(wistd::forward<T6>(varName6)),           \
				_wiltlg_STRINGIZE(varName6)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM6(                               \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM6_CV(                            \
	...
	template <typename T1, typename T2, typename T3, typename T4, typename T5, \
			  typename T6>                                                     \
	void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3, T4 &&varName4,   \
				 T5 &&varName5, T6 &&varName6, PCSTR correlationVector) {      \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM7(                               \
	...
	void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3, T4 &&varName4,   \
				 T5 &&varName5, T6 &&varName6, T7 &&varName7) {                \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName6)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType7>(wistd::forward<T7>(varName7)),           \
				_wiltlg_STRINGIZE(varName7)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM7(                               \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM8(                               \
	...
	void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3, T4 &&varName4,   \
				 T5 &&varName5, T6 &&varName6, T7 &&varName7, T8 &&varName8) { \
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName7)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType8>(wistd::forward<T8>(varName8)),           \
				_wiltlg_STRINGIZE(varName8)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM8(                               \
		...
		__WI_TraceLoggingWriteTagged(                                          \
			*this, #EventId,                                                   \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName8)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType9>(wistd::forward<T9>(varName9)),           \
				_wiltlg_STRINGIZE(varName9)),                                  \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM9(                               \
...
#define DEFINE_TAGGED_TRACELOGGING_EVENT_UINT32(EventId, varName, ...)         \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(EventId, UINT32, varName,          \
											__VA_ARGS__)
...
#define __IMPLEMENT_TRACELOGGING_CLASS_BASE(                                   \
  ...
  public:                                                                      \
	...
	static void SetErrorReportingType(wil::ErrorReportingType type)            \
		WI_NOEXCEPT {                                                          \
		return Instance()->SetErrorReportingType_(type);                       \
	}                                                                          \
	static void __stdcall FallbackTelemetryCallback(                           \
		bool alreadyReported, wil::FailureInfo const &failure) WI_NOEXCEPT {   \
		return Instance()->OnErrorReported(alreadyReported, failure);          \
	}                                                                          \
	WI_NODISCARD static wil::ActivityThreadWatcher WatchCurrentThread(         \
		PCSTR contextName) WI_NOEXCEPT {                                       \
	...
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		CallContext, wil::ActivityOptions::TelemetryOnFailure)                 \
	...
	static CallContext Start(PCSTR contextName,                                \
							 _Printf_format_string_ PCSTR formatString, ...)   \
		WI_NOEXCEPT {                                                          \
		va_list argList;                                                       \
		va_start(argList, formatString);                                       \
	...
	static void TraceLoggingInfo(_Printf_format_string_ PCSTR formatString,    \
								 ...) WI_NOEXCEPT {                            \
		va_list argList;                                                       \
		va_start(argList, formatString);                                       \
		return Instance()->ReportTraceLoggingMessage(formatString, argList);   \
	}                                                                          \
	static void TraceLoggingError(_Printf_format_string_ PCSTR formatString,   \
								  ...) WI_NOEXCEPT {                           \
		va_list argList;                                                       \
		va_start(argList, formatString);                                       \
...
#define __IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                             \
	TraceLoggingClassName, ProviderName, ProviderId, TraceLoggingOption)       \
	__IMPLEMENT_TRACELOGGING_CLASS_BASE(TraceLoggingClassName,                 \
	...
	struct StaticHandle {                                                      \
		...
		StaticHandle() WI_NOEXCEPT {                                           \
			TRACELOGGING_DEFINE_PROVIDER_STORAGE(                              \
				__hInner, ProviderName, ProviderId, TraceLoggingOption);       \
			_tlg_DefineProvider_annotation(TraceLoggingClassName,              \
										   _Tlg##TraceLoggingClassName##Prov,  \
										   0, ProviderName);                   \
			handle = &__hInner;                                                \
		}                                                                      \
	} m_staticHandle;                                                          \
                                                                               \
  protected:                                                                   \
	void Create() WI_NOEXCEPT { Register(m_staticHandle.handle); }             \
                                                                               \
  public:

#define __IMPLEMENT_TRACELOGGING_CLASS(TraceLoggingClassName, ProviderName,    \
									   ProviderId)                             \
	__IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                                 \
		TraceLoggingClassName, ProviderName, ProviderId,                       \
		TraceLoggingOptionMicrosoftTelemetry())

#define __IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP_CB(                          \
	TraceLoggingClassName, ProviderName, ProviderId, TraceLoggingOption)       \
	__IMPLEMENT_TRACELOGGING_CLASS_BASE(TraceLoggingClassName,                 \
										TraceLoggingClassName)                 \
  private:                                                                     \
	...
	static VOID NTAPI Callback(                                                \
		_In_ const GUID *SourceId, ULONG ControlCode, UCHAR Level,             \
		ULONGLONG MatchAnyKeyword, ULONGLONG MatchAllKeyword,                  \
		_In_opt_ EVENT_FILTER_DESCRIPTOR *FilterData, void *CallbackContext);  \
                                                                               \
  protected:                                                                   \
...
#define __IMPLEMENT_TRACELOGGING_CLASS_WITHOUT_TELEMETRY(                      \
	TraceLoggingClassName, ProviderName, ProviderId)                           \
	__IMPLEMENT_TRACELOGGING_CLASS_BASE(TraceLoggingClassName,                 \
										TraceLoggingClassName)                 \
  private:                                                                     \
	struct StaticHandle {                                                      \
		TraceLoggingHProvider handle;                                          \
		StaticHandle() WI_NOEXCEPT {                                           \
			TRACELOGGING_DEFINE_PROVIDER_STORAGE(__hInner, ProviderName,       \
												 ProviderId);                  \
			_tlg_DefineProvider_annotation(TraceLoggingClassName,              \
										   _Tlg##TraceLoggingClassName##Prov,  \
										   0, ProviderName);                   \
...
#define DEFINE_TRACELOGGING_EVENT_PARAM1(EventId, VarType1, varName1, ...)     \
	template <typename T1> static void EventId(T1 &&varName1) {                \
		TraceLoggingWrite(TraceLoggingType::Provider(), #EventId,              \
						  TraceLoggingValue(static_cast<VarType1>(             \
												wistd::forward<T1>(varName1)), \
											_wiltlg_STRINGIZE(varName1)),      \
						  _GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);         \
	}
#else
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM2(EventId, VarType1, varName1,          \
										 VarType2, varName2, ...)              \
	template <typename T1, typename T2>                                        \
	static void EventId(T1 &&varName1, T2 &&varName2) {                        \
		TraceLoggingWrite(TraceLoggingType::Provider(), #EventId,              \
						  TraceLoggingValue(static_cast<VarType1>(             \
												wistd::forward<T1>(varName1)), \
											_wiltlg_STRINGIZE(varName1)),      \
						  TraceLoggingValue(static_cast<VarType2>(             \
												wistd::forward<T2>(varName2)), \
											_wiltlg_STRINGIZE(varName2)),      \
						  _GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);         \
	}
#else
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM2_CV(EventId, VarType1, varName1,       \
											VarType2, varName2, ...)           \
	template <typename T1, typename T2>                                        \
	static void EventId(T1 &&varName1, T2 &&varName2,                          \
						PCSTR correlationVector) {                             \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM3(                                      \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3, ...)  \
	template <typename T1, typename T2, typename T3>                           \
	static void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3) {         \
		TraceLoggingWrite(TraceLoggingType::Provider(), #EventId,              \
						  ...
						  TraceLoggingValue(static_cast<VarType2>(             \
												wistd::forward<T2>(varName2)), \
											_wiltlg_STRINGIZE(varName2)),      \
						  TraceLoggingValue(static_cast<VarType3>(             \
												wistd::forward<T3>(varName3)), \
											_wiltlg_STRINGIZE(varName3)),      \
						  _GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);         \
	}
#else
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                   \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3, ...)  \
	template <typename T1, typename T2, typename T3>                           \
	static void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3,           \
						PCSTR correlationVector) {                             \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM4(EventId, VarType1, varName1,          \
										 VarType2, varName2, VarType3,         \
...
#ifdef _GENERIC_PARTB_FIELDS_ENABLED
#define DEFINE_TRACELOGGING_EVENT_PARAM4_CV(EventId, VarType1, varName1,       \
											VarType2, varName2, VarType3,      \
											varName3, VarType4, varName4, ...) \
	template <typename T1, typename T2, typename T3, typename T4>              \
	static void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3,           \
						T4 &&varName4, PCSTR correlationVector) {              \
		TraceLoggingWrite(                                                     \
			TraceLoggingType::Provider(), #EventId,                            \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
...
#define DEFINE_TRACELOGGING_EVENT_PARAM6(                                      \
	...
	template <typename T1, typename T2, typename T3, typename T4, typename T5, \
			  typename T6>                                                     \
	static void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3,           \
						T4 &&varName4, T5 &&varName5, T6 &&varName6) {         \
		TraceLoggingWrite(                                                     \
			TraceLoggingType::Provider(), #EventId,                            \
			TraceLoggingValue(                                                 \
				static_cast<VarType1>(wistd::forward<T1>(varName1)),           \
	...
	static void EventId(T1 &&varName1, T2 &&varName2, T3 &&varName3,           \
		...
		TraceLoggingWrite(                                                     \
			...
			TraceLoggingValue(                                                 \
				...
				_wiltlg_STRINGIZE(varName9)),                                  \
			TraceLoggingValue(                                                 \
				static_cast<VarType10>(wistd::forward<T10>(varName10)),        \
				_wiltlg_STRINGIZE(varName10)),                                 \
			_GENERIC_PARTB_FIELDS_ENABLED, __VA_ARGS__);                       \
	}
#else
#define DEFINE_TRACELOGGING_EVENT_PARAM10(                                     \
...
// To declare a tracelogging class, declare your class derived from
// wil::TraceLoggingProvider, populate the uuid attribute of the class with the
// GUID of your provider, and then include the
// IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY macro within your
// class.
//
// If you want to register a provider using a callback to log events, you can
// instead use the
// IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY_AND_CALLBACK
// Additionally your tracelogging class will have to implement a static Callback
// method. See the declaration within
// __IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP_CB.
//
// If you don't need or use telemetry, you can instead use the
// IMPLEMENT_TRACELOGGING_CLASS_WITHOUT_TELEMETRY. This prevents telemetry from
// enabling your provider even if you're not using telemetry.

#define IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                               \
	TraceLoggingClassName, ProviderName, ProviderId, GroupName)                \
	__IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                                 \
		TraceLoggingClassName, ProviderName, ProviderId, GroupName)

#define IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP_CB(                            \
	TraceLoggingClassName, ProviderName, ProviderId, GroupName)                \
	__IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP_CB(                              \
		TraceLoggingClassName, ProviderName, ProviderId, GroupName)

#define IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY(                 \
	TraceLoggingClassName, ProviderName, ProviderId)                           \
	IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                                   \
		TraceLoggingClassName, ProviderName, ProviderId,                       \
		TraceLoggingOptionMicrosoftTelemetry())
#define IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY_AND_CALLBACK(    \
	TraceLoggingClassName, ProviderName, ProviderId)                           \
	IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP_CB(                                \
		TraceLoggingClassName, ProviderName, ProviderId,                       \
		TraceLoggingOptionMicrosoftTelemetry())
#define IMPLEMENT_TRACELOGGING_CLASS_WITH_WINDOWS_CORE_TELEMETRY(              \
	TraceLoggingClassName, ProviderName, ProviderId)                           \
	IMPLEMENT_TRACELOGGING_CLASS_WITH_GROUP(                                   \
		TraceLoggingClassName, ProviderName, ProviderId,                       \
		TraceLoggingOptionWindowsCoreTelemetry())
#define IMPLEMENT_TRACELOGGING_CLASS_WITHOUT_TELEMETRY(                        \
	TraceLoggingClassName, ProviderName, ProviderId)                           \
	__IMPLEMENT_TRACELOGGING_CLASS_WITHOUT_TELEMETRY(TraceLoggingClassName,    \
...
#define IMPLEMENT_TRACELOGGING_CLASS                                           \
	IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY
#endif

// [Optional] Externally using a Tracelogging class
// Use TraceLoggingProviderWrite to directly use the trace logging provider
// externally from the class in code. This is recommended only for simple
// TraceLogging events.  Telemetry events and activities are better defined
...
#define TraceLoggingProviderWrite(TraceLoggingClassName, EventId, ...)         \
	TraceLoggingWrite(TraceLoggingClassName::TraceLoggingType::Provider(),     \
					  EventId, __VA_ARGS__)

#define TraceLoggingProviderWriteTelemetry(TraceLoggingClassName, EventId,     \
										   ...)                                \
	TraceLoggingWrite(                                                         \
		TraceLoggingClassName::TraceLoggingType::Provider(), EventId,          \
...
// Use these macros to define a Custom Event for a Provider.  Use the
// TraceLoggingClassWrite or TraceLoggingClassWriteTelemetry from within a
// custom event to issue the event.  Methods will be a no-op (and not be called)
...
// Use these macros to define very simple telemetry events for a Provider.  The
// events can be TELEMETRY events or TRACELOGGING events.

#define DEFINE_TELEMETRY_EVENT(EventId)                                        \
	DEFINE_TRACELOGGING_EVENT(                                                 \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))

#define DEFINE_TELEMETRY_EVENT_PARAM1(EventId, VarType1, varName1)             \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM2(EventId, VarType1, varName1, VarType2,   \
									  varName2)                                \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM3(EventId, VarType1, varName1, VarType2,   \
									  varName2, VarType3, varName3)            \
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM4(EventId, VarType1, varName1, VarType2,   \
									  varName2, VarType3, varName3, VarType4,  \
									  varName4)                                \
	DEFINE_TRACELOGGING_EVENT_PARAM4(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		VarType4, varName4, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM5(EventId, VarType1, varName1, VarType2,   \
									  varName2, VarType3, varName3, VarType4,  \
...
#define DEFINE_TELEMETRY_EVENT_PARAM6(                                         \
	...
	DEFINE_TRACELOGGING_EVENT_PARAM6(                                          \
		...
		VarType4, varName4, VarType5, varName5, VarType6, varName6,            \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM7(EventId, VarType1, varName1, VarType2,   \
									  varName2, VarType3, varName3, VarType4,  \
									  varName4, VarType5, varName5, VarType6,  \
...
#define DEFINE_TELEMETRY_EVENT_PARAM1_CV(EventId, VarType1, varName1)          \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM2_CV(EventId, VarType1, varName1,          \
										 VarType2, varName2)                   \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM3_CV(                                      \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TELEMETRY_EVENT_PARAM4_CV(EventId, VarType1, varName1,          \
										 VarType2, varName2, VarType3,         \
										 varName3, VarType4, varName4)         \
...
#define DEFINE_TELEMETRY_EVENT_UINT32(EventId, varName)                        \
	DEFINE_TELEMETRY_EVENT_PARAM1(EventId, UINT32, varName)
#define DEFINE_TELEMETRY_EVENT_BOOL(EventId, varName)                          \
	DEFINE_TELEMETRY_EVENT_PARAM1(EventId, bool, varName)
#define DEFINE_TELEMETRY_EVENT_STRING(EventId, varName)                        \
	DEFINE_TELEMETRY_EVENT_PARAM1(EventId, PCWSTR, varName)
...
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag, VarType1, \
												varName1)                      \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM2(EventId, PrivacyTag, VarType1, \
												varName1, VarType2, varName2)  \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM3(EventId, PrivacyTag, VarType1, \
												varName1, VarType2, varName2,  \
												VarType3, varName3)            \
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM4(                               \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3, VarType4, varName4)                                              \
	DEFINE_TRACELOGGING_EVENT_PARAM4(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		VarType4, varName4, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),  \
...
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_CV(EventId, PrivacyTag)               \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),             \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM1_CV(EventId, PrivacyTag,        \
												   VarType1, varName1)         \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM2_CV(                            \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM3_CV(                            \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_EVENTTAGGED_TELEMETRY_EVENT_CV(EventId, PrivacyTag,   \
														EventTag)              \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),             \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_TELEMETRY_EVENT_PARAM1_CV(                \
	EventId, PrivacyTag, EventTag, VarType1, varName1)                         \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_TELEMETRY_EVENT_PARAM2_CV(                \
...
#define DEFINE_COMPLIANT_EVENTTAGGED_TELEMETRY_EVENT_PARAM6_CV(                \
	EventId, PrivacyTag, EventTag, VarType1, varName1, VarType2, varName2,     \
	VarType3, varName3, VarType4, varName4, VarType5, varName5, VarType6,      \
	varName6)                                                                  \
	DEFINE_TRACELOGGING_EVENT_PARAM6_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		VarType4, varName4, VarType5, varName5, VarType6, varName6,            \
...
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_UINT32(EventId, PrivacyTag, varName)  \
	DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag, UINT32,       \
											varName)
...
#define DEFINE_COMPLIANT_TELEMETRY_EVENT_STRING(EventId, PrivacyTag, varName)  \
	DEFINE_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag, PCWSTR,       \
											varName)
...
#define DEFINE_MEASURES_EVENT(EventId)                                         \
	DEFINE_TRACELOGGING_EVENT(EventId,                                         \
							  TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM1(EventId, VarType1, varName1)              \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM2(EventId, VarType1, varName1, VarType2,    \
									 varName2)                                 \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM3(EventId, VarType1, varName1, VarType2,    \
									 varName2, VarType3, varName3)             \
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM4(EventId, VarType1, varName1, VarType2,    \
									 varName2, VarType3, varName3, VarType4,   \
									 varName4)                                 \
	DEFINE_TRACELOGGING_EVENT_PARAM4(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		VarType4, varName4, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM5(EventId, VarType1, varName1, VarType2,    \
									 varName2, VarType3, varName3, VarType4,   \
									 varName4, VarType5, varName5)             \
	DEFINE_TRACELOGGING_EVENT_PARAM5(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_MEASURES_EVENT_PARAM6(                                          \
	...
	DEFINE_TRACELOGGING_EVENT_PARAM6(                                          \
		...
		VarType4, varName4, VarType5, varName5, VarType6, varName6,            \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM7(EventId, VarType1, varName1, VarType2,    \
									 varName2, VarType3, varName3, VarType4,   \
									 varName4, VarType5, varName5, VarType6,   \
									 varName6, VarType7, varName7)             \
	DEFINE_TRACELOGGING_EVENT_PARAM7(                                          \
...
#define DEFINE_MEASURES_EVENT_CV(EventId)                                      \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM1_CV(EventId, VarType1, varName1)           \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM2_CV(EventId, VarType1, varName1, VarType2, \
										varName2)                              \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM3_CV(EventId, VarType1, varName1, VarType2, \
										varName2, VarType3, varName3)          \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_MEASURES_EVENT_PARAM4_CV(EventId, VarType1, varName1, VarType2, \
										varName2, VarType3, varName3,          \
										VarType4, varName4)                    \
	DEFINE_TRACELOGGING_EVENT_PARAM4_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_MEASURES_EVENT_UINT32(EventId, varName)                         \
	DEFINE_MEASURES_EVENT_PARAM1(EventId, UINT32, varName)
#define DEFINE_MEASURES_EVENT_BOOL(EventId, varName)                           \
	DEFINE_MEASURES_EVENT_PARAM1(EventId, bool, varName)
#define DEFINE_MEASURES_EVENT_STRING(EventId, varName)                         \
	DEFINE_MEASURES_EVENT_PARAM1(EventId, PCWSTR, varName)

#define DEFINE_COMPLIANT_MEASURES_EVENT(EventId, PrivacyTag)                   \
	DEFINE_TRACELOGGING_EVENT(EventId,                                         \
							  TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES), \
							  TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM1(EventId, PrivacyTag, VarType1,  \
											   varName1)                       \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM2(EventId, PrivacyTag, VarType1,  \
											   varName1, VarType2, varName2)   \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM3(EventId, PrivacyTag, VarType1,  \
											   varName1, VarType2, varName2,   \
											   VarType3, varName3)             \
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM10(                               \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3, VarType4, varName4, VarType5, varName5, VarType6, varName6,      \
	VarType7, varName7, VarType8, varName8, VarType9, varName9, VarType10,     \
	varName10)                                                                 \
	DEFINE_TRACELOGGING_EVENT_PARAM10(                                         \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		VarType4, varName4, VarType5, varName5, VarType6, varName6, VarType7,  \
		varName7, VarType8, varName8, VarType9, varName9, VarType10,           \
		varName10, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),            \
		TelemetryPrivacyDataTag(PrivacyTag))
...
#define DEFINE_COMPLIANT_MEASURES_EVENT_CV(EventId, PrivacyTag)                \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),              \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM1_CV(EventId, PrivacyTag,         \
												  VarType1, varName1)          \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM2_CV(                             \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_MEASURES_EVENT_PARAM3_CV(                             \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_EVENTTAGGED_MEASURES_EVENT_CV(EventId, PrivacyTag,    \
													   EventTag)               \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),              \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_MEASURES_EVENT_PARAM1_CV(                 \
	EventId, PrivacyTag, EventTag, VarType1, varName1)                         \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_MEASURES_EVENT_PARAM2_CV(                 \
	EventId, PrivacyTag, EventTag, VarType1, varName1, VarType2, varName2)     \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
...
#define DEFINE_COMPLIANT_EVENTTAGGED_MEASURES_EVENT_PARAM9_CV(                 \
	EventId, PrivacyTag, EventTag, VarType1, varName1, VarType2, varName2,     \
	VarType3, varName3, VarType4, varName4, VarType5, varName5, VarType6,      \
	varName6, VarType7, varName7, VarType8, varName8, VarType9, varName9)      \
...
#define DEFINE_CRITICAL_DATA_EVENT_PARAM1(EventId, VarType1, varName1)         \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM2(EventId, VarType1, varName1,         \
										  VarType2, varName2)                  \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM3(                                     \
	...
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM4(EventId, VarType1, varName1,         \
										  VarType2, varName2, VarType3,        \
										  varName3, VarType4, varName4)        \
	DEFINE_TRACELOGGING_EVENT_PARAM4(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_CRITICAL_DATA_EVENT_PARAM1_CV(EventId, VarType1, varName1)      \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM2_CV(EventId, VarType1, varName1,      \
											 VarType2, varName2)               \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM3_CV(                                  \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_CRITICAL_DATA_EVENT_PARAM4_CV(EventId, VarType1, varName1,      \
											 VarType2, varName2, VarType3,     \
											 varName3, VarType4, varName4)     \
	DEFINE_TRACELOGGING_EVENT_PARAM4_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_CRITICAL_DATA_EVENT_UINT32(EventId, varName)                    \
	DEFINE_CRITICAL_DATA_EVENT_PARAM1(EventId, UINT32, varName)
#define DEFINE_CRITICAL_DATA_EVENT_BOOL(EventId, varName)                      \
	DEFINE_CRITICAL_DATA_EVENT_PARAM1(EventId, bool, varName)
#define DEFINE_CRITICAL_DATA_EVENT_STRING(EventId, varName)                    \
	DEFINE_CRITICAL_DATA_EVENT_PARAM1(EventId, PCWSTR, varName)

#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT(EventId, PrivacyTag)              \
	DEFINE_TRACELOGGING_EVENT(                                                 \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),         \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag,       \
													VarType1, varName1)        \
	DEFINE_TRACELOGGING_EVENT_PARAM1(                                          \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM2(                           \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TRACELOGGING_EVENT_PARAM2(                                          \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM3(                           \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TRACELOGGING_EVENT_PARAM3(                                          \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_CV(EventId, PrivacyTag)           \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),         \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1_CV(EventId, PrivacyTag,    \
													   VarType1, varName1)     \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM2_CV(                        \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM3_CV(                        \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TRACELOGGING_EVENT_PARAM3_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_COMPLIANT_EVENTTAGGED_CRITICAL_DATA_EVENT_CV(                   \
	EventId, PrivacyTag, EventTag)                                             \
	DEFINE_TRACELOGGING_EVENT_CV(                                              \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),         \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_CRITICAL_DATA_EVENT_PARAM1_CV(            \
	EventId, PrivacyTag, EventTag, VarType1, varName1)                         \
	DEFINE_TRACELOGGING_EVENT_PARAM1_CV(                                       \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag), TraceLoggingEventTag(EventTag))
#define DEFINE_COMPLIANT_EVENTTAGGED_CRITICAL_DATA_EVENT_PARAM2_CV(            \
	EventId, PrivacyTag, EventTag, VarType1, varName1, VarType2, varName2)     \
	DEFINE_TRACELOGGING_EVENT_PARAM2_CV(                                       \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
...
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_UINT32(EventId, PrivacyTag,       \
													varName)                   \
	DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag, UINT32,   \
												varName)
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_BOOL(EventId, PrivacyTag,         \
												  varName)                     \
	DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag, bool,     \
												varName)
#define DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_STRING(EventId, PrivacyTag,       \
													varName)                   \
	DEFINE_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag, PCWSTR,   \
												varName)

// Custom Activities
// For these you pair the appropriate BEGIN and END macros to define your
// activity.  Within the pair you can use the (TODO: LIST MACRO NAMES) macros to
// add behavior.
...
#define BEGIN_TRACELOGGING_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName, Level) \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(ActivityClassName,                     \
										wil::ActivityOptions::None, 0, Level)  \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_TRACELOGGING_ACTIVITY_CLASS(ActivityClassName,         \
													PrivacyTag)                \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(ActivityClassName,                     \
										wil::ActivityOptions::None, 0,         \
										WINEVENT_LEVEL_VERBOSE, PrivacyTag)    \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_TRACELOGGING_ACTIVITY_CLASS_WITH_LEVEL(                \
	ActivityClassName, PrivacyTag, Level)                                      \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::None, 0, Level, PrivacyTag)   \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)

// [optional] param is: Level
#define BEGIN_CALLCONTEXT_ACTIVITY_CLASS(ActivityClassName)                    \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure)           \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_CALLCONTEXT_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName, Level)  \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure, 0, Level) \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_CALLCONTEXT_ACTIVITY_CLASS(ActivityClassName,          \
												   PrivacyTag)                 \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure, 0,        \
		WINEVENT_LEVEL_VERBOSE, PrivacyTag)                                    \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_CALLCONTEXT_ACTIVITY_CLASS_WITH_LEVEL(                 \
	ActivityClassName, PrivacyTag, Level)                                      \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure, 0, Level, \
		PrivacyTag)                                                            \
...
#define BEGIN_COMPLIANT_TELEMETRY_ACTIVITY_CLASS(ActivityClassName,            \
	...
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_TELEMETRY_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName, \
															PrivacyTag, Level) \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure,           \
...
#define BEGIN_CRITICAL_DATA_ACTIVITY_CLASS(ActivityClassName)                  \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		...
		MICROSOFT_KEYWORD_CRITICAL_DATA)                                       \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_CRITICAL_DATA_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName,       \
													  Level)                   \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure,           \
		MICROSOFT_KEYWORD_CRITICAL_DATA, Level)                                \
	__IMPLEMENT_ACTIVITY_CLASS(ActivityClassName)
#define BEGIN_COMPLIANT_CRITICAL_DATA_ACTIVITY_CLASS(ActivityClassName,        \
													 PrivacyTag)               \
	__BEGIN_TRACELOGGING_ACTIVITY_CLASS(                                       \
		ActivityClassName, wil::ActivityOptions::TelemetryOnFailure,           \
...
// Simple Activities
// For these you just use the appropriate macro to define the KIND of activity
// you want and specify the name (for tracelogging you can give other options)
...
#define DEFINE_COMPLIANT_TELEMETRY_ACTIVITY(ActivityClassName, PrivacyTag)     \
	...
	END_ACTIVITY_CLASS()
#define DEFINE_COMPLIANT_TELEMETRY_ACTIVITY_WITH_LEVEL(ActivityClassName,      \
													   PrivacyTag, Level)      \
	BEGIN_COMPLIANT_TELEMETRY_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName,     \
														PrivacyTag, Level)     \
	END_ACTIVITY_CLASS()
...
#define DEFINE_COMPLIANT_MEASURES_ACTIVITY(ActivityClassName, PrivacyTag)      \
	...
	END_ACTIVITY_CLASS()
#define DEFINE_COMPLIANT_MEASURES_ACTIVITY_WITH_LEVEL(ActivityClassName,       \
													  PrivacyTag, Level)       \
	BEGIN_COMPLIANT_MEASURES_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName,      \
													   PrivacyTag, Level)      \
	END_ACTIVITY_CLASS()
...
#define DEFINE_COMPLIANT_CRITICAL_DATA_ACTIVITY(ActivityClassName, PrivacyTag) \
	BEGIN_COMPLIANT_CRITICAL_DATA_ACTIVITY_CLASS(ActivityClassName,            \
												 PrivacyTag)                   \
	END_ACTIVITY_CLASS()
#define DEFINE_COMPLIANT_CRITICAL_DATA_ACTIVITY_WITH_LEVEL(ActivityClassName,  \
														   PrivacyTag, Level)  \
	BEGIN_COMPLIANT_CRITICAL_DATA_ACTIVITY_CLASS_WITH_LEVEL(ActivityClassName, \
															PrivacyTag, Level) \
	END_ACTIVITY_CLASS()

// [Optional] Custom Start or Stop Events for Activities
// Use these macros to define custom start or custom stop methods for an
// activity.  Any activity can have multiple start or stop methods.  To add
// custom start or stop events, define a StartActivity instance method or a Stop
// instance method within the BEGIN/END pair of a custom activity.  Within that
// function, use TraceLoggingClassWriteStart or TraceLoggingClassWriteStop.
...
// Use these macros to define a Custom Tagged Event for a Custom Activity.  Use
// the TraceLoggingClassWriteTagged or TraceLoggingClassWriteTaggedTelemetry
// macros from within a custom event to write the event.
...
#define TraceLoggingClassWriteTaggedTelemetry(EventId, ...)                    \
	__WI_TraceLoggingWriteTagged(                                              \
		*this, #EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),     \
		__VA_ARGS__)

#define TraceLoggingClassWriteTaggedMeasure(EventId, ...)                      \
	__WI_TraceLoggingWriteTagged(                                              \
		*this, #EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),      \
		__VA_ARGS__)
...
#define DEFINE_TAGGED_TELEMETRY_EVENT(EventId)                                 \
	DEFINE_TAGGED_TRACELOGGING_EVENT(                                          \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM1(EventId, VarType1, varName1)      \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM2(EventId, VarType1, varName1,      \
											 VarType2, varName2)               \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM3(                                  \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM4(EventId, VarType1, varName1,      \
											 VarType2, varName2, VarType3,     \
											 varName3, VarType4, varName4)     \
...
#define DEFINE_TAGGED_TELEMETRY_EVENT_CV(EventId)                              \
	DEFINE_TAGGED_TRACELOGGING_EVENT_CV(                                       \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM1_CV(EventId, VarType1, varName1)   \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1_CV(                                \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM2_CV(EventId, VarType1, varName1,   \
												VarType2, varName2)            \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM3_CV(                               \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY))
#define DEFINE_TAGGED_TELEMETRY_EVENT_PARAM4_CV(EventId, VarType1, varName1,   \
												VarType2, varName2, VarType3,  \
												varName3, VarType4, varName4)  \
...
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT(EventId, PrivacyTag)           \
	DEFINE_TAGGED_TRACELOGGING_EVENT(                                          \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),             \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag,    \
													   VarType1, varName1)     \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM2(                        \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_TELEMETRY),                      \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM3(                        \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_UINT32(EventId, PrivacyTag,    \
													   varName)                \
	DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag,        \
												   UINT32, varName)
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_BOOL(EventId, PrivacyTag,      \
													 varName)                  \
	DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag, bool,  \
												   varName)
#define DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_STRING(EventId, PrivacyTag,    \
													   varName)                \
	DEFINE_TAGGED_COMPLIANT_TELEMETRY_EVENT_PARAM1(EventId, PrivacyTag,        \
												   PCWSTR, varName)

// [Optional] Simple Tagged Events for Activities
// Use these methods to define very simple tagged measures events for a Custom
// Activity.
...
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM1(EventId, VarType1, varName1)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM2(EventId, VarType1, varName1,       \
											VarType2, varName2)                \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM3(                                   \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM4(EventId, VarType1, varName1,       \
											VarType2, varName2, VarType3,      \
											varName3, VarType4, varName4)      \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM4(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM1_CV(EventId, VarType1, varName1)    \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1_CV(                                \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM2_CV(EventId, VarType1, varName1,    \
											   VarType2, varName2)             \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM3_CV(                                \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES))
#define DEFINE_TAGGED_MEASURES_EVENT_PARAM4_CV(EventId, VarType1, varName1,    \
											   VarType2, varName2, VarType3,   \
											   varName3, VarType4, varName4)   \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM4_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_MEASURES_EVENT_BOOL(EventId, varName)                    \
	DEFINE_TAGGED_MEASURES_EVENT_PARAM1(EventId, bool, varName)
...
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT(EventId, PrivacyTag)            \
	DEFINE_TAGGED_TRACELOGGING_EVENT(                                          \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),              \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM1(EventId, PrivacyTag,     \
													  VarType1, varName1)      \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM2(                         \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_MEASURES),                       \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM3(                         \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_UINT32(EventId, PrivacyTag,     \
													  varName)                 \
	DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM1(EventId, PrivacyTag, UINT32, \
												  varName)
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_BOOL(EventId, PrivacyTag,       \
													varName)                   \
	DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM1(EventId, PrivacyTag, bool,   \
												  varName)
#define DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_STRING(EventId, PrivacyTag,     \
													  varName)                 \
	DEFINE_TAGGED_COMPLIANT_MEASURES_EVENT_PARAM1(EventId, PrivacyTag, PCWSTR, \
												  varName)

// [Optional] Simple Tagged Events for Activities
// Use these methods to define very simple tagged CRITICAL_DATA events for a
// Custom Activity.
...
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM1(EventId, VarType1, varName1)  \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM2(EventId, VarType1, varName1,  \
												 VarType2, varName2)           \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM3(                              \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM4(EventId, VarType1, varName1,  \
												 VarType2, varName2, VarType3, \
												 varName3, VarType4, varName4) \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM4(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_CV(EventId)                          \
	DEFINE_TAGGED_TRACELOGGING_EVENT_CV(                                       \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM1_CV(EventId, VarType1,         \
													varName1)                  \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1_CV(                                \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM2_CV(                           \
	EventId, VarType1, varName1, VarType2, varName2)                           \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM3_CV(                           \
	...
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA))
#define DEFINE_TAGGED_CRITICAL_DATA_EVENT_PARAM4_CV(                           \
	EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,       \
	VarType4, varName4)                                                        \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM4_CV(                                \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT(EventId, PrivacyTag)       \
	DEFINE_TAGGED_TRACELOGGING_EVENT(                                          \
		EventId, TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),         \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(                    \
	EventId, PrivacyTag, VarType1, varName1)                                   \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM1(                                   \
		EventId, VarType1, varName1,                                           \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_PARAM2(                    \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2)               \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM2(                                   \
		EventId, VarType1, varName1, VarType2, varName2,                       \
		TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA),                  \
		TelemetryPrivacyDataTag(PrivacyTag))
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_PARAM3(                    \
	EventId, PrivacyTag, VarType1, varName1, VarType2, varName2, VarType3,     \
	varName3)                                                                  \
	DEFINE_TAGGED_TRACELOGGING_EVENT_PARAM3(                                   \
		EventId, VarType1, varName1, VarType2, varName2, VarType3, varName3,   \
...
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_UINT32(                    \
	...
	DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag,    \
													   UINT32, varName)
#define DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_BOOL(EventId, PrivacyTag,  \
														 varName)              \
	DEFINE_TAGGED_COMPLIANT_CRITICAL_DATA_EVENT_PARAM1(EventId, PrivacyTag,    \
													   bool, varName)
...
// These are desktop only and are not recommended by the fundamentals team.
// These activities lag behind regular activities in their ability to use
// CallContext or to be cross-thread portable, so their usage should be limited.

#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
#define BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD_LEVEL(    \
	ActivityClassName, keyword, level)                                         \
	class ActivityClassName final                                              \
		: public _TlgActivityBase<ActivityClassName, keyword, level> {         \
	  ...
	  protected:                                                               \
		...
		wil::FailureInfo const *GetFailureInfo() {                             \
			return (FAILED(m_result) && (m_cache.GetFailure() != nullptr) &&   \
					(m_result == m_cache.GetFailure()->hr))                    \
					   ? m_cache.GetFailure()                                  \
					   : nullptr;                                              \
		}                                                                      \
...
#define BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD(          \
	ActivityClassName, keyword)                                                \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD_LEVEL(        \
		ActivityClassName, keyword, WINEVENT_LEVEL_VERBOSE)

#define BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_LEVEL(            \
	ActivityClassName, level)                                                  \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD_LEVEL(        \
		ActivityClassName, 0, level)

#define BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS(ActivityClassName)     \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD_LEVEL(        \
		ActivityClassName, 0, WINEVENT_LEVEL_VERBOSE)
...
#define BEGIN_DEFINE_TELEMETRY_THREAD_ACTIVITY_CLASS(ActivityClassName)        \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD(              \
		ActivityClassName, MICROSOFT_KEYWORD_TELEMETRY)

#define END_DEFINE_TELEMETRY_THREAD_ACTIVITY_CLASS()                           \
	END_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS()

#define DEFINE_TRACELOGGING_THREAD_ACTIVITY_WITH_KEYWORD_LEVEL(                \
	ActivityClassName, keyword, level)                                         \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD_LEVEL(        \
		ActivityClassName, keyword, level)                                     \
	END_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS()

#define DEFINE_TRACELOGGING_THREAD_ACTIVITY_WITH_KEYWORD(ActivityClassName,    \
														 keyword)              \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_KEYWORD(              \
		ActivityClassName, keyword)                                            \
	END_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS()

#define DEFINE_TRACELOGGING_THREAD_ACTIVITY_WITH_LEVEL(ActivityClassName,      \
													   level)                  \
	BEGIN_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS_WITH_LEVEL(                \
		ActivityClassName, level)                                              \
	END_DEFINE_TRACELOGGING_THREAD_ACTIVITY_CLASS()
...
#define BEGIN_DEFINE_TRACELOGGING_ACTIVITY_CLASS                               \
	BEGIN_TRACELOGGING_ACTIVITY_CLASS
...
#define DECLARE_TRACELOGGING_CLASS(TraceLoggingClassName, ProviderName,        \
								   ProviderId)                                 \
...
#define IMPLEMENT_TELEMETRY_CLASS(TelemetryClassName, TraceLoggingClassName)   \
	__IMPLEMENT_TRACELOGGING_CLASS_BASE(TelemetryClassName,                    \
										TraceLoggingClassName)                 \
  protected:                                                                   \
	void Create() {                                                            \
		AttachProvider(TraceLoggingClassName::Provider());                     \
		__TRACELOGGING_DEFINE_PROVIDER_STORAGE_LINK(TelemetryClassName,        \
													TraceLoggingClassName);    \
	}                                                                          \
                                                                               \
  public:
...
#pragma detect_mismatch(                                                       \
	"ODR_violation_WIL_API_TELEMETRY_SUSPEND_HANDLER_mismatch", "1")
#else
#pragma detect_mismatch(                                                       \
	"ODR_violation_WIL_API_TELEMETRY_SUSPEND_HANDLER_mismatch", "0")
#endif

class ApiTelemetryLogger : public wil::TraceLoggingProvider {
	// {fb7fcbc6-7156-5a5b-eabd-0be47b14f453}
	IMPLEMENT_TRACELOGGING_CLASS_WITH_MICROSOFT_TELEMETRY(
		ApiTelemetryLogger, "Microsoft.Windows.ApiTelemetry",
		(0xfb7fcbc6, 0x7156, 0x5a5b, 0xea, 0xbd, 0x0b, 0xe4, 0x7b, 0x14, 0xf4,
		 0x53));

  public:
	// Used to store of list of APIs (with namespace, class, custom and call
	// count data per API). This is public so that it can be unit tested.
	class ApiDataList {
	  public:
		struct ApiData {
			...
			ApiData(PCWSTR className_, PCWSTR apiName_, PCSTR specialization_,
					volatile long *counterReference_)
				: className(className_), apiName(apiName_),
				  specialization(specialization_),
				  counterReference(counterReference_) {}
		};

		// Inserts a new Api call counter into the list, keeping the list sorted
		// by className
		void Insert(PCWSTR className, PCWSTR apiName,
					_In_opt_ PCSTR specialization,
					volatile long *counterReference) {
			wistd::unique_ptr<ApiData> newApiData(new (std::nothrow) ApiData(
				className, apiName, specialization, counterReference));
			if (newApiData) {
		...
		// For each distinct namespace, calls the provided flushCallback
		// function. After returning, it will have deleted all ApiData elements,
		// and zeroed the *counterReference stored in each ApiData.
		void
		Flush(wistd::function<void(PCWSTR, PCWSTR, PCSTR, UINT32 *, UINT16)>
				  flushCallback) {
			...
			while (root) {
				// First find the number of characters we need to allocate for
				// each string, and the number of items in the counter array to
				// allocate
				size_t totalApiListLength =
					1; // Init to 1 to account for null terminator
				size_t totalSpecializationsLength =
					1; // Init to 1 to account for null terminator
				UINT16 numCounts = 0;

				ProcessSingleNamespace(&root, [&](wistd::unique_ptr<ApiData>
													  &node) {
					// Get the length needed for the class string
					const wchar_t *strAfterNamespace =
						GetClassStringPointer(node->className);
					size_t classStrLen =
						wcslen(strAfterNamespace ? strAfterNamespace
												 : node->className);

					totalApiListLength +=
						(classStrLen + wcslen(node->apiName) +
						 1); // We add 1 to account for the comma delimeter
					if (node->specialization) {
						totalSpecializationsLength +=
							strlen(node->specialization) +
							1; // We add 1 to account for the comma delimeter
					} else {
						totalSpecializationsLength +=
							2; // '-' plus comma delimeter
					}
					numCounts++;
				});

				// Fill arrays with the API data, and then pass it to the
				// callback function
				wistd::unique_ptr<wchar_t[]> apiList(
					new (std::nothrow) wchar_t[totalApiListLength]);
				wistd::unique_ptr<char[]> specializationList(
					new (std::nothrow) char[totalSpecializationsLength]);
				wistd::unique_ptr<UINT32[]> countArray(new (std::nothrow)
														   UINT32[numCounts]);
				size_t nameSpaceLength =
					GetNameSpaceLength(root->className) + 1;
				wistd::unique_ptr<wchar_t[]> nameSpace(
					new (std::nothrow) wchar_t[nameSpaceLength]);
				if (!apiList || !specializationList || !countArray ||
					!nameSpace) {
					return;
				}

				ZeroMemory(apiList.get(), totalApiListLength * sizeof(wchar_t));
				ZeroMemory(specializationList.get(),
						   totalSpecializationsLength * sizeof(char));
				ZeroMemory(countArray.get(), numCounts * sizeof(UINT32));
				ZeroMemory(nameSpace.get(), nameSpaceLength * sizeof(wchar_t));

				StringCchCopyNW(nameSpace.get(), STRSAFE_MAX_CCH,
								root->className, nameSpaceLength - 1);
				...
				wistd::unique_ptr<ApiData> *lastNamespaceNode =
					ProcessSingleNamespace(&root, [&](wistd::unique_ptr<ApiData>
						...
						countArray[countArrayIndex] =
							static_cast<UINT32>(::InterlockedExchangeNoFence(
								node->counterReference, 0));

						// Prepend the portion of the apiName group string
						// that's after the '.'. So for example, if the
						// className is "Windows.System.Launcher", then we
						// prepend "Launcher." to the apiName string.
						const wchar_t *strAfterNamespace =
							GetClassStringPointer(node->className);
						if (strAfterNamespace) {
							FAIL_FAST_IF_FAILED(
								StringCchCatW(apiList.get(), totalApiListLength,
											  strAfterNamespace + 1));
							FAIL_FAST_IF_FAILED(StringCchCatW(
								apiList.get(), totalApiListLength, L"."));
						}

						FAIL_FAST_IF_FAILED(StringCchCatW(
							apiList.get(), totalApiListLength, node->apiName));
						if (node->specialization) {
							FAIL_FAST_IF(
								strncat_s(specializationList.get(),
										  totalSpecializationsLength,
										  node->specialization,
										  strlen(node->specialization)) != 0);
						} else {
							FAIL_FAST_IF(strncat_s(specializationList.get(),
												   totalSpecializationsLength,
												   "-", 1) != 0);
						}
				...
				// Call the callback function with the data we've collected for
				// this namespace
				flushCallback(nameSpace.get(), apiList.get(),
							  specializationList.get(), countArray.get(),
							  numCounts);
	  ...
	  private:
		static wistd::unique_ptr<ApiData> *ProcessSingleNamespace(
			wistd::unique_ptr<ApiData> *root,
			wistd::function<void(wistd::unique_ptr<ApiData> &)>
				workerCallback) {
			...
			while (*currentNode) {
				...
				// Check if our next node would be a new namespace; if so, then
				// break out
				if (node->next &&
					!IsSameNameSpace(node->className, node->next->className)) {
					break;
				}
		...
		static bool IsSameNameSpace(PCWSTR namespaceClass1,
									PCWSTR namespaceClass2) {
			return (wcsncmp(namespaceClass1, namespaceClass2,
							GetNameSpaceLength(namespaceClass2) + 1) == 0);
		}

		static size_t GetNameSpaceLength(PCWSTR nameSpaceClass) {
			const wchar_t *strAfterNamespace =
				GetClassStringPointer(nameSpaceClass);
			return (strAfterNamespace ? (strAfterNamespace - nameSpaceClass)
									  : wcslen(nameSpaceClass));
		}

		static const wchar_t *GetClassStringPointer(PCWSTR nameSpaceClass) {
			// Note: Usage of wcsrchr can cause build errors in some components,
			// so we implement a way of getting the pointer to the 'class'
			// portion of the string ourselves.
			int retIndex = 0;
  ...
  public:
	// Initializes an entry that holds the className.apiName, along with a
	// counter for that className.apiName. The counterReference passed to this
	// should later be passed to LogApiInfo.
	//
	// A separate entry will be created for each apiName that has a distinct
	// specialization value.
	//
	// This function only needs to be called once for each API, although it
	// doesn't hurt if it gets called more than once.
	//
	// The apiName, className, and specialization parameters should be compile
	// time constants. specialization can be null.
	DEFINE_EVENT_METHOD(InitApiData)
	(PCWSTR className, PCWSTR apiName, _In_opt_ PCSTR specialization,
	 volatile long *counterReference) {
		// TODO: Validate that apiName and className are a compile-time
		// constants; validate that specialization is either compile-time
		// constant or nullptr; validate that counterReference points to static
		// variable. Can do this by making sure address is <= (GetModuleHandle()
		// + DLL size).
		m_apiDataList.Insert(className, apiName, specialization,
							 counterReference);
	}

	// Fires a telemetry event that contains the method call apiName that has
	// been logged by the component, since the last FireEvent() call, or since
	// the component was loaded.
	DEFINE_EVENT_METHOD(FireEvent)() {
		m_apiDataList.Flush([](PCWSTR nameSpace, PCWSTR apiList,
							   PCSTR specializationList, UINT32 *countArray,
							   UINT16 numCounters) {
			if (::wil::details::IsDebuggerPresent()) {
				TraceLoggingWrite(
					Provider(), "ApiCallCountsWithDebuggerPresent",
					...
					TraceLoggingUInt32Array(countArray, numCounters,
											"HitCounts"),
					TraceLoggingBoolean(TRUE, "UTCReplace_AppSessionGuid"),
					TraceLoggingKeyword(MICROSOFT_KEYWORD_CRITICAL_DATA));
			} else {
				TraceLoggingWrite(
					Provider(), "ApiCallCounts",
					TraceLoggingValue(nameSpace, "Namespace"),
					TraceLoggingValue(apiList, "ApiDataList"),
					TraceLoggingValue(specializationList, "CustomList"),
					TraceLoggingUInt32Array(countArray, numCounters,
			...
			__TRACELOGGING_TEST_HOOK_VERIFY_API_TELEMETRY(
				nameSpace, apiList, specializationList, countArray,
				numCounters);
		});

		if (m_fireEventDelay < c_fireEventDelayLimit) {
			// Double the exponential backoff timer, until it reaches the
			// maximum
			m_fireEventDelay *= 2;
			if (m_fireEventDelay > c_fireEventDelayLimit) {
	...
	// Used to declare that the component will handle calling FireEvent() in its
	// own suspend handler. This optimizes the frequency at which the event will
	// be fired.
  ...
  private:
...
#else
		m_fireEventDelay =
			__TRACELOGGING_TEST_HOOK_API_TELEMETRY_EVENT_DELAY_MS;
#endif
		m_fireEventThreadPoolTimer.reset(::CreateThreadpoolTimer(
			[](PTP_CALLBACK_INSTANCE, PVOID, PTP_TIMER) { FireEvent(); },
			nullptr, nullptr));
		ScheduleFireEventCallback();
	}
	...
	~ApiTelemetryLogger() WI_NOEXCEPT override {
		...
		// release handle to thread pool timer instead of its destructor being
		// call, if process is being terminated and dll is not being unloaded
		// dynamically destruction of threadpool timer is considered invalid
	...
	void ScheduleFireEventCallback() {
		// do not schedule thread pool timer callback, if process is being
		// terminated and dll is not being unloaded dynamically
		if (m_fireEventThreadPoolTimer && !ProcessShutdownInProgress()) {
			...
			*reinterpret_cast<PLONGLONG>(&dueTime) =
				-static_cast<LONGLONG>(m_fireEventDelay) * 10000;
			SetThreadpoolTimer(m_fireEventThreadPoolTimer.get(), &dueTime, 0,
							   0);
		}
	}
	...
	// The timer used to determine when to fire the next telemetry event (when
	// it's fired based on a timer).
...
// Insert WI_LOG_API_USE near the top of a WinRT method to log that a method was
// called. The parameter should be the method name, for example:
//  - WI_LOG_API_USE(L"LaunchUriAsync");
//
// To log that the WinRT method reached a certain line of code, pass an override
// string:
//  - WI_LOG_API_USE(L"LaunchUriAsync", "PointA");
//
// If the class name can't be obtained at runtime, or if instrumenting a
// non-WinRT API, use the below macro, and pass the fully qualified class name
// (in the case of WinRT), or a string identifying the group of the non-WinRT
// API:
//  - WI_LOG_CLASS_API_USE(RuntimeClass_Windows_System_Launcher,
//  L"LaunchUriAsync");
//
// Note: If the component can have a suspend handler, the following line should
// be added before including TraceLogging.h:
//  - #define WIL_API_TELEMETRY_SUSPEND_HANDLER
// This will optimize the component's ability to upload telemetry, as it will
// upload on suspend. It will also disable frequent telemetry upload early in
// process execution.
//
// Alternatively, a component can call
// wil::details:ApiTelemetryLogger::FireEvent() from it's own suspend handler.
// If this is done, then in DLLMain it should also call
// wil::details::ApiTelemetryLogger::UsingOwnSuspendHandler().
//
...
//
// Adding this code snippet ensures that during process termination, thread pool
// timer destructor or SetThreadPoolTimer methods are not called, because they
// are invalid to call when dll is not getting dynamically unloaded. Skipping
// this code block will result in a continuable exception being thrown if
// process is getting terminated and dll in which ApiTelemetryLogger is not
// getting dynamically unloaded. For more details about lpReserved parameter,
...
#define __WI_LOG_CLASS_API_USE3(className, apiName, specialization)            \
	do {                                                                       \
		...
		if (1 == ::InterlockedIncrementNoFence(&__wil_apiCallCounter)) {       \
			::wil::details::ApiTelemetryLogger::InitApiData(                   \
				className, apiName, specialization, &__wil_apiCallCounter);    \
		}                                                                      \
	} while (0, 0)
...
#define __WI_LOG_API_USE2(apiName, specialization)                             \
	__WI_LOG_CLASS_API_USE3(InternalGetRuntimeClassName(), apiName,            \
							specialization)

...
/*
 * A wil::task<T> / com_task<T> is a coroutine with the following
 * characteristics:
 *
 ...
 * not to replace them. It provides coroutine implementations for scenarios that
 * PPL and C++/WinRT do not support, but it does not support everything that PPL
 * and C++/WinRT do.
 *
 ...
 * | | T can be void                                       | Yes       | Yes |
 * Yes           | | T can be reference                                  | No |
 * No        | Yes           | | T can be WinRT object | Yes       | Yes       |
 * Yes           | | T can be non-WinRT object                           | Yes
 * | No        | Yes           | | T can be move-only | No        | No        |
 * Yes           | | Coroutine can be cancelled                          | Yes
 * | Yes       | No            | | Coroutine can throw arbitrary exceptions |
 * Yes       | No        | Yes           | | Can co_await more than once | Yes
 * | No        | No            | | Can have multiple clients waiting for
 * completion    | Yes       | No        | No            | | co_await resumes in
 * same COM context                | Sometimes | Yes       | You choose [1]| |
 * Can force co_await to resume in same context        | Yes       | N/A       |
 * Yes [1]       | | Can force co_await to resume in any thread          | Yes
 * | No        | Yes           | | Can change coroutine's resumption model | No
 * | No        | Yes           | | Can wait synchronously | Yes       | Yes |
 * Yes [2]       | | Can be consumed by non-C++ languages                | No |
 * Yes       | No            | | Implementation is small and efficient | No |
 * Yes       | Yes           | | Can abandon coroutine (fail to co_await) | Yes
 * | Yes       | Yes           | | Exception in abandoned coroutine | Crash |
 * Ignored   | Crash         | | Coroutine starts automatically | Yes       |
 * Yes       | Yes           | | Coroutine starts synchronously | No        |
 * Yes       | Yes           | | Integrates with C++/WinRT coroutine callouts |
 * No        | Yes       | No            |
 *
 * [1] Resumption in the same COM apartment requires that you include COM
 * headers. [2] Synchronous waiting requires that you include <synchapi.h>
 * (usually via <windows.h>).
 *
 ...
 *
 * co_await wil::com_task(GetNameAsync()); // now defaults to
 * resume_same_apartment();
 *
...
template <typename T> struct result_holder {
	...
	template <typename... Args> void emplace_value(Args &&...args) {
		...
		new (wistd::addressof(result.wrap))
			result_wrapper<T>{wistd::forward<Args>(args)...};
		status = result_status::value;
	}

	void unhandled_exception() noexcept {
		WI_ASSERT(status == result_status::empty);
		new (wistd::addressof(result.error))
...
template <typename T> struct promise_base {
	...
	auto as_promise() noexcept { return static_cast<Promise *>(this); }

	// Make it easier to access the coroutine handle.
	auto as_handle() noexcept {
		return __WI_COROUTINE_NAMESPACE::coroutine_handle<
			Promise>::from_promise(*as_promise());
	}
	...
	void destroy() { as_handle().destroy(); }

	// The client lost interest in the coroutine, either because they are
	// discarding the result without awaiting (risky!), or because they have
	// finished awaiting. Discarding the result without awaiting is risky
	// because any exception in the coroutine will be unobserved and result in a
	// crash. If you want to disallow it, then raise an exception if waiting ==
	// running_ptr.
	void abandon() {
		auto waiting =
			m_waiting.exchange(abandoned_ptr(), std::memory_order_acq_rel);
	...
	void unhandled_exception() noexcept { m_holder.unhandled_exception(); }

	void resume_waiting_coroutine(void *waiting) const {
		if (m_resumer) {
			m_resumer(waiting);
		} else {
			__WI_COROUTINE_NAMESPACE::coroutine_handle<>::from_address(waiting)
				.resume();
		}
	}
	...
	auto final_suspend() noexcept {
		struct awaiter : __WI_COROUTINE_NAMESPACE::suspend_always {
			promise_base &self;
			void await_suspend(
				__WI_COROUTINE_NAMESPACE::coroutine_handle<>) const noexcept {
				// Need acquire so we can read from m_resumer.
				// Need release so that the results are published in the case
				// that nobody is awaiting right now, so that the eventual
				// awaiter (possibly on another thread) can read the results.
				auto waiting = self.m_waiting.exchange(
					completed_ptr(), std::memory_order_acq_rel);
	...
	auto client_await_suspend(void *waiting, void(__stdcall *resumer)(void *)) {
		...
		// Acquire to ensure that we can read the results of the return value,
		// if the coroutine is completed. Release to ensure that our resumption
		// state is published, if the coroutine is not completed.
		auto previous = m_waiting.exchange(waiting, std::memory_order_acq_rel);

		// Suspend if the coroutine is still running.
		// Otherwise, the coroutine is completed: Nobody will resume us, so we
		// will have to resume ourselves.
		WI_ASSERT((previous == running_ptr()) || (previous == completed_ptr()));
		return previous == running_ptr();
	}

	T client_await_resume() { return m_holder.get_value(); }
};
...
template <> struct task_promise<void> : promise_base<void> {
	void return_void() { this->emplace_value(); }
};
...
template <typename T> struct task_base {
	...
	// You must #include <ole2.h> before <wil\coroutine.h> to enable
	// apartment-aware awaiting.
	...
	// You must #include <synchapi.h> (usually via <windows.h>) to enable
	// synchronous waiting.
	decltype(auto) get() &&;
...
template <typename T> struct task : details::coro::task_base<T> {
	...
	// Constructing from task_promise<T>* cannot be explicit because
	// get_return_object relies on implicit conversion.
	task(details::coro::task_promise<T> *initial = nullptr) noexcept
		: base(initial) {}
...
template <typename T> struct com_task : details::coro::task_base<T> {
	...
	// Constructing from task_promise<T>* cannot be explicit because
	// get_return_object relies on implicit conversion.
	com_task(details::coro::task_promise<T> *initial = nullptr) noexcept
		: base(initial) {}
...
// Can re-include this header after including synchapi.h (usually via windows.h)
// to enable synchronous wait.
...
// Can re-include this header after including COM header files to enable
// non-agile tasks.
#if defined(_COMBASEAPI_H_) && defined(_THREADPOOLAPISET_H_) &&                \
	!defined(__WIL_COROUTINE_NON_AGILE_INCLUDED)
...
struct apartment_resumer {
	...
	static wil::com_ptr<IContextCallback> current_context() {
		...
		// Do not use IID_PPV_ARGS to avoid ambiguity between ::IUnknown and
		// winrt::IUnknown.
		CoGetObjectContext(__uuidof(IContextCallback),
	...
	static void __stdcall resume_in_context(void *parameter) {
		...
		if (self->context == nullptr || self->context == current_context()) {
			self->context =
				nullptr; // removes the context cleanup from the resume path
			self->waiter();
		} else if (is_sta()) {
	...
	static void submit_threadpool_callback(PTP_SIMPLE_CALLBACK callback,
										   void *context) {
		THROW_IF_WIN32_BOOL_FALSE(
			TrySubmitThreadpoolCallback(callback, context, nullptr));
	}

	static void CALLBACK resume_context(PTP_CALLBACK_INSTANCE /*instance*/,
										void *parameter) {
	...
	void resume_context_sync() {
		...
		auto result = local_context->ContextCallback(
	...
	resume_apartment_callback(ComCallData *data) noexcept {
		as_self(data->pUserDefined)->waiter();
		return S_OK;
	}

	void check() { THROW_IF_FAILED(resume_result); }
};
...
// This follows the same algorithm employed by C++/WinRT, which has features
// like avoiding stack buildup and proper handling of the neutral apartment. It
// does, however, introduce fail-fast code paths if thread pool tasks cannot be
// submitted. (Those fail-fasts could be removed by preallocating the tasks, but
// that means paying an up-front cost for something that may never end up used,
// as well as introducing extra cleanup code in the fast-path.)
template <typename T> struct com_awaiter : agile_awaiter<T> {
	com_awaiter(promise_ptr<T> &&initial)
		: agile_awaiter<T>(wistd::move(initial)) {}
	apartment_resumer resumer;

	auto await_suspend(__WI_COROUTINE_NAMESPACE::coroutine_handle<> handle) {
		resumer.capture_context(handle);
		return this->promise->client_await_suspend(
			wistd::addressof(resumer), apartment_resumer::resume_in_context);
	}

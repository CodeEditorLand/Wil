...
// wistd::function
//
// All of the code below is in direct support of wistd::function.  This class is
// identical to std::function with the following exceptions:
//
// 1) It never allocates and is safe to use from exception-free code (custom
// allocators are not supported) 2) It's slightly bigger on the stack (64 bytes,
// rather than 24 for 32bit) 3) There is an explicit static-assert if a lambda
// becomes too large to hold in the internal buffer (rather than an allocation)
...
__WI_LIBCPP_NORETURN inline __WI_LIBCPP_INLINE_VISIBILITY void
...
template <class _Fp, class _Rp, class... _ArgTypes>
void __func<_Fp, _Rp(_ArgTypes...)>::__clone(
...
// 'wistd::function' is most similar to 'inplace_function' in that it _only_
// permits holding function objects that can fit within its internal buffer.
// Therefore, we expand this size to accommodate space for at least 12 pointers
// (__base vtable takes an additional one).
constexpr const size_t __buffer_size = 13 * sizeof(void *);

} // namespace __function

// NOTE: The extra 'alignas' here is to work around the x86 compiler bug
// mentioned in https://github.com/microsoft/STL/issues/1533 to force alignment
// on the stack
template <class _Rp, class... _ArgTypes>
class __WI_LIBCPP_TEMPLATE_VIS
__WI_ALIGNAS(typename aligned_storage<__function::__buffer_size>::type)
	...
	template <class _Fp> struct __callable {
		static const bool value = __callable_imp<
			_Fp, __lazy_and<integral_constant<bool, !is_same<__uncvref_t<_Fp>,
															 function>::value>,
  ...
  public:
	...
	// NOTE: type_info is very compiler specific, and on top of that, we're
	// operating in a namespace other than 'std' so all functions requiring RTTI
	// have been removed
};
...
function<_Rp(_ArgTypes...)>::function(_Fp __f)
	...
	if (__function::__not_null(__f)) {
		...
		static_assert(
			...
			"buffer (12 pointers).  Refactor to reduce size of the capture.");
...
function<_Rp(_ArgTypes...)> &function<_Rp(_ArgTypes...)>::operator=(_Fp &&__f) {
	*this = nullptr;
	if (__function::__not_null(__f)) {
		typedef __function::__func<typename decay<_Fp>::type, _Rp(_ArgTypes...)>
			_FF;
		static_assert(
			sizeof(_FF) <= sizeof(__buf_),
			"The sizeof(wistd::function) has grown too large for the reserved "
...
template <class _Rp, class... _ArgTypes>
inline __WI_LIBCPP_INLINE_VISIBILITY bool
operator==(nullptr_t, const function<_Rp(_ArgTypes...)> &__f) WI_NOEXCEPT {
	return !__f;
}
...
typename __invoke_of<_Fn, _Args...>::type invoke(_Fn &&__f, _Args &&...__args)
	...
	return wistd::__invoke(wistd::forward<_Fn>(__f),
						   wistd::forward<_Args>(__args)...);
}

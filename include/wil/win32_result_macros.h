...
#define __WIN32_RETURN_GLE_FAIL(str)                                           \
	return __R_FN(Win32_Return_GetLastError)(__R_INFO_ONLY(str))
...
#define WIN32_RETURN_WIN32(error)                                              \
	__WIN32_RETURN_WIN32(wil::verify_win32(error), #error)
...
// Conditionally returns failures (WIN32 error code) - use for failures that are
// expected in common use - failures are not logged - macros are only for
...
#define WIN32_RETURN_CAUGHT_EXCEPTION()                                        \
	return __R_FN(Win32_Return_CaughtException)(__R_INFO_ONLY(nullptr))
...
// Win32ErrorFromCaughtException is a function that is meant to be called from
// within a catch(...) block. Internally it re-throws and catches the exception
// to convert it to a WIN32 error code. If an exception is of an unrecognized
...
// }
_Always_(_Post_satisfies_(return > 0)) __declspec(
	noinline) inline long Win32ErrorFromCaughtException() WI_NOEXCEPT {
	return __WIN32_FROM_HRESULT(ResultFromCaughtException());
}

namespace details::__R_NS_NAME {
#ifdef WIL_ENABLE_EXCEPTIONS
__R_DIRECT_METHOD(long, Win32_Return_CaughtException)
(__R_DIRECT_FN_PARAMS_ONLY) WI_NOEXCEPT {
	__R_FN_LOCALS;
	return __WIN32_FROM_HRESULT(
		wil::details::ReportFailure_CaughtException<FailureType::Return>(
			__R_DIRECT_FN_CALL_ONLY));
}
#endif

__R_DIRECT_METHOD(long, Win32_Return_GetLastError)
(__R_DIRECT_FN_PARAMS_ONLY) WI_NOEXCEPT {
	__R_FN_LOCALS;
	return __WIN32_FROM_HRESULT(
		wil::details::ReportFailure_GetLastErrorHr<FailureType::Return>(
			__R_DIRECT_FN_CALL_ONLY));
}
} // namespace details::__R_NS_NAME
} // namespace wil

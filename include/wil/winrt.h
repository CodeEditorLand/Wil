...
#define __WI_HAS_STD_LESS 1
#pragma warning(push)
#pragma warning(disable : 4643) // Forward declaring '...' in namespace std is
								// not permitted by the C++ Standard.
namespace std {
...
// Code using the public SDK and C++ CX code has the ABI prefix, windows
// internal is built in a way that does not.
#if !defined(MIDL_NS_PREFIX) &&                                                \
	!defined(____x_ABI_CWindows_CFoundation_CIClosable_FWD_DEFINED__)
// Internal .idl files use the namespace without the ABI prefix. Macro out ABI
// for that case
...
inline __time64_t
DateTime_to_time_t(ABI::Windows::Foundation::DateTime dateTime) {
	// DateTime is the number of 100 - nanosecond intervals since January 1,
	// 1601.
	return (dateTime.UniversalTime / HundredNanoSecondsInSecond -
...
inline ABI::Windows::Foundation::DateTime time_t_to_DateTime(__time64_t timeT) {
	...
	dateTime.UniversalTime =
		(timeT + SecondsToStartOf1970) * HundredNanoSecondsInSecond;
	return dateTime;
}
...
/// @cond
namespace details {
// hstring_compare is used to assist in HSTRING comparison of two potentially
// non-similar string types. E.g. comparing a raw HSTRING with WRL's
// HString/HStringReference/etc. The consumer can optionally inhibit the
// deduction of array sizes by providing 'true' for the 'InhibitStringArrays'
// template argument. This is generally done in scenarios where the consumer
// cannot guarantee that the input argument types are perfectly preserved from
// end-to-end. E.g. if a single function in the execution path captures an array
// as const T&, then it is impossible to differentiate const arrays (where we
// generally do want to deduce length) from non-const arrays (where we generally
// do not want to deduce length). The consumer can also optionally choose to
// perform case-insensitive comparison by providing 'true' for the 'IgnoreCase'
// template argument.
template <bool InhibitStringArrays, bool IgnoreCase> struct hstring_compare {
	...
	get_buffer(const Microsoft::WRL::Wrappers::HString &hstr,
			   UINT32 *length) WI_NOEXCEPT {
		return hstr.GetRawBuffer(length);
	}

	static const wchar_t *
	get_buffer(const Microsoft::WRL::Wrappers::HStringReference &hstr,
			   UINT32 *length) WI_NOEXCEPT {
		return hstr.GetRawBuffer(length);
	}

	static const wchar_t *get_buffer(const unique_hstring &str,
									 UINT32 *length) WI_NOEXCEPT {
		return ::WindowsGetStringRawBuffer(str.get(), length);
	}

	template <bool..., bool Enable = InhibitStringArrays>
	static wistd::enable_if_t<Enable, const wchar_t *>
	get_buffer(const wchar_t *str, UINT32 *length) WI_NOEXCEPT {
	...
	static wistd::enable_if_t<
		wistd::conjunction<
			...
			wistd::is_same<wistd::decay_t<wistd::remove_pointer_t<StringT>>,
						   wchar_t>,
	...
	static wistd::enable_if_t<Enable, const wchar_t *>
	get_buffer(const wchar_t (&str)[Size], UINT32 *length) WI_NOEXCEPT {
		*length = Size - 1;
		return str;
	}
	...
	template <size_t Size, bool..., bool Enable = !InhibitStringArrays>
	static wistd::enable_if_t<Enable, const wchar_t *>
	get_buffer(wchar_t (&str)[Size], UINT32 *length) WI_NOEXCEPT {
		*length = static_cast<UINT32>(wcslen(str));
		return str;
	}

	// Overload for std::wstring, or at least things that behave like
	// std::wstring, without adding a dependency on STL headers
	template <typename StringT>
	static wistd::enable_if_t<
		wistd::conjunction_v<
			wistd::is_constructible<StringT, wchar_t *>,
			wistd::is_convertible<decltype(wistd::declval<StringT>().data()),
								  const wchar_t *>,
			wistd::is_same<typename StringT::size_type,
						   decltype(wistd::declval<StringT>().size())>>,
		const wchar_t *>
	...
	static auto compare(LhsT &&lhs, RhsT &&rhs)
		-> decltype(get_buffer(lhs, wistd::declval<UINT32 *>()),
		...
		const auto result =
			::CompareStringOrdinal(lhsBuffer, lhsLength, rhsBuffer, rhsLength,
	...
	static auto equals(LhsT &&lhs, RhsT &&rhs)
		WI_NOEXCEPT->decltype(compare(wistd::forward<LhsT>(lhs),
									  wistd::forward<RhsT>(rhs)),
							  bool()) {
		return compare(wistd::forward<LhsT>(lhs), wistd::forward<RhsT>(rhs)) ==
			   CSTR_EQUAL;
	}
	...
	template <typename LhsT, typename RhsT>
	static auto less(LhsT &&lhs, RhsT &&rhs)
		WI_NOEXCEPT->decltype(compare(wistd::forward<LhsT>(lhs),
									  wistd::forward<RhsT>(rhs)),
							  bool()) {
		return compare(wistd::forward<LhsT>(lhs), wistd::forward<RhsT>(rhs)) ==
...
/** TwoPhaseHStringConstructor help using the 2 phase constructor pattern for
HSTRINGs.
~~~
...
struct TwoPhaseHStringConstructor {
	...
	//! Returns the HSTRING after it has been populated like Detatch() or
	//! release(); be sure to put this in a RAII type to manage its lifetime.
	HSTRING Promote() {
		m_characterLength = 0;
	...
	/** Ensure that the size of the data provided is consistent with the
	pre-allocated buffer. It seems that WindowsPreallocateStringBuffer()
	provides the null terminator in the buffer (based on testing) so this can be
	called before populating the buffer.
	*/
	WI_NODISCARD HRESULT Validate(ULONG bytesRead) const {
		// Null termination is required for the buffer before calling
  ...
  private:
	TwoPhaseHStringConstructor(UINT32 characterLength)
		: m_characterLength(characterLength) {
...
//! A transparent less-than comparison function object that enables comparison
//! of various string types intended for use with associative containers (such
//! as `std::set`, `std::map`, etc.) that use
//! `Microsoft::WRL::Wrappers::HString` as the key type. This removes the need
//! for the consumer to explicitly create an `HString` object when using lookup
//! functions such as `find`, `lower_bound`, etc. For example, the following
//! scenarios would all work exactly as you would expect them to:
//! ~~~
...
//! ~~~
//! The first four calls in the example above use `WindowsGetStringRawBuffer`
//! (or equivalent) to get the string buffer and length for the comparison. The
//! fifth example uses `std::wstring::c_str` and `std::wstring::length` for
//! getting these two values. The remaining three examples use only the string
//! buffer and call `wcslen` for the length. That is, the length is *not*
//! deduced for either array. This is because argument types are not always
//! perfectly preserved by container functions and in fact are often captured as
//! const references making it impossible to differentiate const arrays - where
//! we can safely deduce length - from non const arrays - where we cannot safely
//! deduce length since the buffer may be larger than actually needed (e.g.
//! creating a `char[MAX_PATH]` array, but only filling it with 10 characters).
//! The implications of this behavior is that string literals that contain
//! embedded null characters will only include the part of the buffer up to the
//! first null character. For example, the following example will result in all
//! calls to `find` returning an end iterator.
//! ~~~
//! std::map<HString, int, wil::hstring_less> map;
...
//! map.emplace(std::move(key), 42);
//!
//! // All of the following return map.end() since they look for the string
//! "foo" map.find(constArray); map.find(nonConstArray);
//! map.find(static_cast<const wchar_t*>(constArray));
//! ~~~
//! In order to search using a string literal that contains embedded null
//! characters, a simple alternative is to first create an `HStringReference`
...
struct hstring_less {
	...
	template <typename LhsT, typename RhsT>
	WI_NODISCARD auto operator()(const LhsT &lhs,
								 const RhsT &rhs) const WI_NOEXCEPT
		->decltype(details::hstring_compare<true, false>::less(lhs, rhs)) {
		return details::hstring_compare<true, false>::less(lhs, rhs);
	}
};

//! A transparent less-than comparison function object whose behavior is
//! equivalent to that of @ref hstring_less with the one difference that
//! comparisons are case-insensitive. That is, the following example will
//! correctly find the inserted value:
//! ~~~
...
struct hstring_insensitive_less {
	...
	WI_NODISCARD auto operator()(const LhsT &lhs, const RhsT &rhs) const
		WI_NOEXCEPT->decltype(details::hstring_compare<true, true>::less(lhs,
																		 rhs)) {
...
// of it. This is needed when lifetime management of the type is needed, for
// example when holding them as a value produced in an iterator. This type has a
// common set of methods used to abstract the access to the value that is
// similar to ComPtr<> and the WRL Wrappers: Get(), GetAddressOf() and other
// operators. Clients of the smart type must use those to access the value.

// TODO: Having the base definition defined will result in creating leaks if a
// type that needs resource management (e.g. PROPVARIANT) that has not
// specialized is used.
//
// One fix is to use std::is_enum to cover that case and leave the base
// definition undefined. That base should use static_assert to inform clients
// how to fix the lack of specialization.
...
#pragma warning(disable : 4702) // https://github.com/Microsoft/wil/issues/2
	struct type					// T holder
	{
		...
		// In case of absence of T::operator=(T&&) a call to T::operator=(const
		// T&) will happen
...
struct MapToSmartType<
	T, typename wistd::enable_if<wistd::is_base_of<
		   IUnknown, typename wistd::remove_pointer<T>::type>::value>::type> {
	typedef Microsoft::WRL::ComPtr<typename wistd::remove_pointer<T>::type>
		type;
};
...
template <> struct MapToSmartType<HSTRING, void> {
	class HStringWithRelease : public Microsoft::WRL::Wrappers::HString {
	  public:
		// Unlike all other WRL types HString does not have
		// ReleaseAndGetAddressOf and GetAddressOf() has non-standard behavior,
		// calling Release().
...
// WinRT interfaces like IVector<>, IAsyncOperation<> and IIterable<> can be
// templated on a runtime class (instead of an interface or primitive type). In
// these cases the objects produced by those interfaces implement an interface
// defined by the runtime class default interface.
//
...
template <typename VectorType> struct MapVectorResultType {
	...
	static TResult PeekGetAtType(
		HRESULT (STDMETHODCALLTYPE TVector::*)(unsigned, TResult *));
...
template <typename T> struct MapIteratorResultType {
	...
	static TResult
		PeekCurrentType(HRESULT (STDMETHODCALLTYPE TIterable::*)(TResult *));
	typedef decltype(PeekCurrentType(
		&ABI::Windows::Foundation::Collections::IIterator<T>::get_Current))
		type;
};
...
template <typename T> struct MapAsyncOpResultType {
	...
	static TResult PeekGetResultsType(
		HRESULT (STDMETHODCALLTYPE TAsyncOperation::*)(TResult *));
	typedef decltype(PeekGetResultsType(
		&ABI::Windows::Foundation::IAsyncOperation<T>::GetResults)) type;
};
...
/** Range base for and STL algorithms support for WinRT ABI collection types,
IVector<T>, IVectorView<T>, IIterable<T> similar to support provided by
<collection.h> for C++ CX. Three error handling policies are supported.
~~~
ComPtr<CollectionType> collection = GetCollection(); // can be IVector<HSTRING>,
IVectorView<HSTRING> or IIterable<HSTRING>

for (auto const& element : wil::get_range(collection.Get()))                //
exceptions for (auto const& element : wil::get_range_nothrow(collection.Get(),
&hr))   // error code for (auto const& element :
wil::get_range_failfast(collection.Get()))       // fail fast
{
   // use element
}
~~~
...
auto itFound = std::find_if(fileRange.begin(), fileRange.end(),
[](ComPtr<IStorageFile> file) -> bool
{
...
class vector_range {
  public:
	...
	class vector_iterator {
	  public:
		...
		vector_iterator(const vector_iterator &other) {
			...
			m_i = other.m_i;
			err_policy::HResult(
				other.m_element.CopyTo(m_element.GetAddressOf()));
		}
		...
		vector_iterator &operator=(const vector_iterator &other) {
			if (this != wistd::addressof(other)) {
				...
				m_i = other.m_i;
				err_policy::HResult(
					other.m_element.CopyTo(m_element.ReleaseAndGetAddressOf()));
			}
			return *this;
		}
		...
		reference operator*() {
			err_policy::HResult(
				m_v->GetAt(m_i, m_element.ReleaseAndGetAddressOf()));
			return m_element;
		}
	...
	vector_iterator begin() { return vector_iterator(m_v, 0); }
...
template <typename VectorType> class vector_range_nothrow {
  public:
	...
	class vector_iterator_nothrow {
	  public:
#ifdef _XUTILITY_
		// must be input_iterator_tag as use (via ++, --, etc.) of one
		...
		vector_iterator_nothrow(vector_range_nothrow<VectorType> *range,
								unsigned int pos)
		...
		WI_NODISCARD bool
		operator==(vector_iterator_nothrow const &other) const {
	...
	void get_at_current(unsigned int i) {
		if (SUCCEEDED(*m_result) && (i < m_size)) {
			*m_result =
				m_v->GetAt(i, m_currentElement.ReleaseAndGetAddressOf());
		}
	}
  ...
  private:
	...
	HRESULT *m_result;
	HRESULT m_resultStorage = S_OK; // for the case where the caller does not
									// provide the location to store the result
...
class iterable_range {
  public:
	...
	explicit iterable_range(
		_In_ ABI::Windows::Foundation::Collections::IIterable<T> *iterable)
		: m_iterable(iterable) {}
	...
	class iterable_iterator {
	  public:
		...
		// for begin()
		explicit iterable_iterator(
			_In_ ABI::Windows::Foundation::Collections::IIterable<T>
				*iterable) {
		...
		reference operator*() {
			err_policy::HResult(
				m_iterator->get_Current(m_element.ReleaseAndGetAddressOf()));
			return m_element;
		}
	...
	iterable_iterator begin() { return iterable_iterator(m_iterable); }

	iterable_iterator end() { return iterable_iterator(); }
...
/** Converts WinRT vectors to std::vector by requesting the collection's data in
a single operation. This can be more efficient in terms of IPC cost than
iteratively processing it.
~~~
...
template <typename VectorType> auto to_vector(VectorType *src) {
	...
	static_assert(sizeof(TResult) == sizeof(TSmart),
				  "result and smart sizes are different");
	...
	if (expected > 0) {
		...
		THROW_IF_FAILED(src->GetMany(0, static_cast<UINT32>(output.size()),
									 reinterpret_cast<TResult *>(output.data()),
									 &fetched));
...
template <typename T> class iterable_range_nothrow {
  public:
	...
	iterable_range_nothrow(iterable_range_nothrow &&other) WI_NOEXCEPT
		: m_iterator(wistd::move(other.m_iterator)),
			...
			*m_result = m_iterator->get_HasCurrent(&hasCurrent);
			if (SUCCEEDED(*m_result) && hasCurrent) {
				*m_result =
					m_iterator->get_Current(m_element.ReleaseAndGetAddressOf());
				if (FAILED(*m_result)) {
					m_iterator = nullptr; // release the iterator if no elements
										  // are found
				}
			} else {
				m_iterator =
					nullptr; // release the iterator if no elements are found
			}
		}
	}
	...
	class iterable_iterator_nothrow {
	  public:
		...
		iterable_iterator_nothrow(_In_ iterable_range_nothrow *range,
								  int currentIndex)
			: m_range(range), m_i(currentIndex) {}

		WI_NODISCARD bool
		operator==(iterable_iterator_nothrow const &other) const {
			return FAILED(*m_range->m_result) || (m_i == other.m_i);
		}
		...
		iterable_iterator_nothrow &operator++() {
			...
			*m_range->m_result = m_range->m_iterator->MoveNext(&hasCurrent);
			if (SUCCEEDED(*m_range->m_result) && hasCurrent) {
				*m_range->m_result = m_range->m_iterator->get_Current(
					m_range->m_element.ReleaseAndGetAddressOf());
  ...
  private:
	Microsoft::WRL::ComPtr<ABI::Windows::Foundation::Collections::IIterator<T>>
		m_iterator;
...
template <typename T>
vector_range<ABI::Windows::Foundation::Collections::IVector<T>>
get_range(ABI::Windows::Foundation::Collections::IVector<T> *v) {
	return vector_range<ABI::Windows::Foundation::Collections::IVector<T>>(v);
}

template <typename T>
vector_range<ABI::Windows::Foundation::Collections::IVectorView<T>>
get_range(ABI::Windows::Foundation::Collections::IVectorView<T> *v) {
	return vector_range<ABI::Windows::Foundation::Collections::IVectorView<T>>(
		v);
}
...
vector_range<ABI::Windows::Foundation::Collections::IVector<T>,
			 err_failfast_policy>
get_range_failfast(ABI::Windows::Foundation::Collections::IVector<T> *v) {
	return vector_range<ABI::Windows::Foundation::Collections::IVector<T>,
						err_failfast_policy>(v);
}

template <typename T>
vector_range<ABI::Windows::Foundation::Collections::IVectorView<T>,
			 err_failfast_policy>
get_range_failfast(ABI::Windows::Foundation::Collections::IVectorView<T> *v) {
	return vector_range<ABI::Windows::Foundation::Collections::IVectorView<T>,
						err_failfast_policy>(v);
}

template <typename T>
vector_range_nothrow<ABI::Windows::Foundation::Collections::IVector<T>>
get_range_nothrow(ABI::Windows::Foundation::Collections::IVector<T> *v,
				  HRESULT *result = nullptr) {
	return vector_range_nothrow<
		ABI::Windows::Foundation::Collections::IVector<T>>(v, result);
}

template <typename T>
vector_range_nothrow<ABI::Windows::Foundation::Collections::IVectorView<T>>
get_range_nothrow(ABI::Windows::Foundation::Collections::IVectorView<T> *v,
				  HRESULT *result = nullptr) {
	return vector_range_nothrow<
		ABI::Windows::Foundation::Collections::IVectorView<T>>(v, result);
}

#ifdef WIL_ENABLE_EXCEPTIONS
template <typename T>
iterable_range<T>
get_range(ABI::Windows::Foundation::Collections::IIterable<T> *v) {
	return iterable_range<T>(v);
}
#endif // WIL_ENABLE_EXCEPTIONS

template <typename T>
iterable_range<T, err_failfast_policy>
get_range_failfast(ABI::Windows::Foundation::Collections::IIterable<T> *v) {
	return iterable_range<T, err_failfast_policy>(v);
}

template <typename T>
iterable_range_nothrow<T>
get_range_nothrow(ABI::Windows::Foundation::Collections::IIterable<T> *v,
				  HRESULT *result = nullptr) {
...
template <typename X>
typename wil::vector_range<IVectorView<X>>::vector_iterator
begin(IVectorView<X> *v) {
...
template <typename TResult, typename TFunc, typename... Args,
		  typename wistd::enable_if<wistd::is_same<HRESULT, TResult>::value,
									int>::type = 0>
...
#ifdef WIL_ENABLE_EXCEPTIONS
template <typename TResult, typename TFunc, typename... Args,
		  typename wistd::enable_if<wistd::is_same<void, TResult>::value,
									int>::type = 0>
HRESULT CallAndHandleErrorsWithReturnType(TFunc &&func, Args &&...args) {
	try {
...
HRESULT CallAndHandleErrors(TFunc &&func, Args &&...args) {
	return CallAndHandleErrorsWithReturnType<decltype(wistd::forward<TFunc>(
		func)(wistd::forward<Args>(args)...))>(wistd::forward<TFunc>(func),
...
// Takes a member function that has an out param like F(..., IAsyncAction**) or
// F(..., IAsyncOperation<bool>**) and returns IAsyncAction* or
// IAsyncOperation<bool>*.
template <typename I, typename... P>
typename wistd::remove_pointer<typename LastType<P...>::type>::type
	GetReturnParamPointerType(HRESULT (STDMETHODCALLTYPE I::*)(P...));

// Use to determine the result type of the async action/operation interfaces or
// example decltype(GetAsyncResultType(action.get())) returns void
void GetAsyncResultType(ABI::Windows::Foundation::IAsyncAction *);
template <typename P>
void GetAsyncResultType(
	ABI::Windows::Foundation::IAsyncActionWithProgress<P> *);
template <typename T>
typename wil::details::MapAsyncOpResultType<T>::type
GetAsyncResultType(ABI::Windows::Foundation::IAsyncOperation<T> *);
template <typename T, typename P>
typename wil::details::MapAsyncOpProgressResultType<T, P>::type
GetAsyncResultType(
	ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *);

// Use to determine the result type of the async action/operation interfaces or
// example decltype(GetAsyncDelegateType(action.get())) returns void
ABI::Windows::Foundation::IAsyncActionCompletedHandler *
GetAsyncDelegateType(ABI::Windows::Foundation::IAsyncAction *);
template <typename P>
ABI::Windows::Foundation::IAsyncActionWithProgressCompletedHandler<P> *
GetAsyncDelegateType(ABI::Windows::Foundation::IAsyncActionWithProgress<P> *);
template <typename T>
ABI::Windows::Foundation::IAsyncOperationCompletedHandler<T> *
GetAsyncDelegateType(ABI::Windows::Foundation::IAsyncOperation<T> *);
template <typename T, typename P>
ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler<T, P> *
GetAsyncDelegateType(
	ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *);

template <typename TBaseAgility, typename TIOperation, typename TFunction>
HRESULT RunWhenCompleteAction(_In_ TIOperation operation,
							  TFunction &&func) WI_NOEXCEPT {
	using namespace Microsoft::WRL;
	typedef wistd::remove_pointer_t<decltype(GetAsyncDelegateType(operation))>
		TIDelegate;

	auto callback = Callback<
		Implements<RuntimeClassFlags<ClassicCom>, TIDelegate, TBaseAgility>>(
		[func = wistd::forward<TFunction>(func)](
			TIOperation operation,
			ABI::Windows::Foundation::AsyncStatus status) mutable -> HRESULT {
			HRESULT hr = S_OK;
			if (status !=
				ABI::Windows::Foundation::AsyncStatus::
					Completed) // avoid a potentially costly marshaled QI / call
							   // if we completed successfully
			{
				// QI to the IAsyncInfo interface.  While all operations
				// implement this, it is possible that the stub has
				// disconnected, causing the QI to fail.
				ComPtr<ABI::Windows::Foundation::IAsyncInfo> asyncInfo;
				hr = operation->QueryInterface(IID_PPV_ARGS(&asyncInfo));
				if (SUCCEEDED(hr)) {
					// Save the error code result in a temporary variable to
					// allow us to also retrieve the result of the COM call.  If
					// the stub has disconnected, this call may fail.
...
template <typename TBaseAgility, typename TIOperation, typename TFunction>
HRESULT RunWhenComplete(_In_ TIOperation operation,
						TFunction &&func) WI_NOEXCEPT {
	using namespace Microsoft::WRL;
	...
	auto callback = Callback<
		...
		[func = wistd::forward<TFunction>(func)](
			TIOperation operation,
			ABI::Windows::Foundation::AsyncStatus status) mutable -> HRESULT {
			typename details::MapToSmartType<
				typename GetAbiType<typename wistd::remove_pointer<
					TIOperation>::type::TResult_complex>::type>::type result;

			HRESULT hr = S_OK;
			// avoid a potentially costly marshaled QI / call if we completed
			// successfully
...
HRESULT WaitForCompletion(_In_ TIOperation operation, COWAIT_FLAGS flags,
						  DWORD timeoutValue,
	...
	class CompletionDelegate
		: public Microsoft::WRL::RuntimeClass<
			  Microsoft::WRL::RuntimeClassFlags<
				  Microsoft::WRL::RuntimeClassType::Delegate>,
	...
	RETURN_IF_FAILED(Microsoft::WRL::MakeAndInitialize<CompletionDelegate>(
		&completedDelegate));
	...
	HRESULT hr = CoWaitForMultipleHandles(
		flags, timeoutValue, ARRAYSIZE(handles), handles, &dwHandleIndex);

	// If the caller is listening for timedOut, and we actually timed out, set
	// the bool and return S_OK. Otherwise, fail.
	...
	if (completedDelegate->GetStatus() !=
		ABI::Windows::Foundation::AsyncStatus::Completed) {
		// QI to the IAsyncInfo interface.  While all operations implement this,
		// it is possible that the stub has disconnected, causing the QI to
		// fail.
...
HRESULT WaitForCompletion(_In_ TIOperation operation, _Out_ TIResults result,
						  COWAIT_FLAGS flags, DWORD timeoutValue,
						  _Out_opt_ bool *timedOut) WI_NOEXCEPT {
	RETURN_IF_FAILED_EXPECTED(
		details::WaitForCompletion(operation, flags, timeoutValue, timedOut));
...
/** Set the completion callback for an async operation to run a caller provided
function. Once complete the function is called with the error code result of the
operation and the async operation result (if applicable). The function parameter
list must be (HRESULT hr) for actions, (HRESULT hr, IResultInterface* object)
for operations that produce interfaces, and (HRESULT hr, TResult value) for
operations that produce value types.
~~~
...
~~~
run_when_complete<FtmBase>(getFileOp.Get(), [](HRESULT hr, IStorageFile* file)
-> void
{
...
~~~
hr = run_when_complete_nothrow<StorageFile*>(getFileOp.Get(), [](HRESULT hr,
IStorageFile* file) -> HRESULT
{
...
//! Run a fuction when an async operation completes. Use Microsoft::WRL::FtmBase
//! for TAgility to make the completion handler agile and run on the async
//! thread.
template <typename TAgility = IUnknown, typename TFunc>
HRESULT run_when_complete_nothrow(
	_In_ ABI::Windows::Foundation::IAsyncAction *operation,
	TFunc &&func) WI_NOEXCEPT {
	return details::RunWhenCompleteAction<TAgility>(
		operation, wistd::forward<TFunc>(func));
}

template <typename TAgility = IUnknown, typename TResult, typename TFunc,
		  typename TAsyncResult =
			  typename wil::details::MapAsyncOpResultType<TResult>::type>
HRESULT run_when_complete_nothrow(
	_In_ ABI::Windows::Foundation::IAsyncOperation<TResult> *operation,
	TFunc &&func) WI_NOEXCEPT {
	return details::RunWhenComplete<TAgility>(operation,
											  wistd::forward<TFunc>(func));
}

template <typename TAgility = IUnknown, typename TResult, typename TProgress,
		  typename TFunc,
		  typename TAsyncResult = typename wil::details::
			  MapAsyncOpProgressResultType<TResult, TProgress>::type>
HRESULT run_when_complete_nothrow(
	_In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<
...
HRESULT run_when_complete_nothrow(
	_In_ ABI::Windows::Foundation::IAsyncActionWithProgress<TProgress>
		*operation,
	TFunc &&func) WI_NOEXCEPT {
	return details::RunWhenCompleteAction<TAgility>(
		operation, wistd::forward<TFunc>(func));
...
//! thread.
template <typename TAgility = IUnknown, typename TFunc>
void run_when_complete(_In_ ABI::Windows::Foundation::IAsyncAction *operation,
					   TFunc &&func) {
	THROW_IF_FAILED((details::RunWhenCompleteAction<TAgility>(
		operation, wistd::forward<TFunc>(func))));
}
...
template <typename TAgility = IUnknown, typename TResult, typename TFunc,
		  typename TAsyncResult =
			  typename wil::details::MapAsyncOpResultType<TResult>::type>
void run_when_complete(
	_In_ ABI::Windows::Foundation::IAsyncOperation<TResult> *operation,
	TFunc &&func) {
	THROW_IF_FAILED((details::RunWhenComplete<TAgility>(
		operation, wistd::forward<TFunc>(func))));
}
...
THROW_IF_FAILED(storageFileStatics->GetFileFromPathAsync(HStringReference(path).Get(),
&op)); auto file = wil::wait_for_completion(op.Get());
~~~
*/
template <typename TAsync = ABI::Windows::Foundation::IAsyncAction>
inline HRESULT wait_for_completion_nothrow(
	_In_ TAsync *operation,
...
HRESULT wait_for_completion_nothrow(
	...
	COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT {
	return details::WaitForCompletion(operation, result, flags, INFINITE,
									  nullptr);
}
...
HRESULT wait_for_completion_nothrow(
	_In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<
		TResult, TProgress> *operation,
	_Out_ typename wil::details::MapAsyncOpProgressResultType<
		TResult, TProgress>::type *result,
	COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT {
	return details::WaitForCompletion(operation, result, flags, INFINITE,
									  nullptr);
}
...
inline HRESULT wait_for_completion_or_timeout_nothrow(
	_In_ TAsync *operation, DWORD timeoutValue, _Out_ bool *timedOut,
	COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT {
...
HRESULT wait_for_completion_or_timeout_nothrow(
	...
	COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT {
	return details::WaitForCompletion(operation, result, flags, timeoutValue,
									  timedOut);
}
...
//! Wait for an asynchronous operation to complete (or be canceled).
template <typename TAsync = ABI::Windows::Foundation::IAsyncAction>
inline void wait_for_completion(_In_ TAsync *operation,
								COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) {
	THROW_IF_FAILED(
		details::WaitForCompletion(operation, flags, INFINITE, nullptr));
}
...
TReturn wait_for_completion(
	...
	COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) {
	TReturn result;
	THROW_IF_FAILED(details::WaitForCompletion(operation, result.GetAddressOf(),
											   flags, INFINITE, nullptr));
	return result;
}

template <typename TResult, typename TProgress,
		  typename TReturn = typename wil::details::MapToSmartType<
			  typename wil::details::MapAsyncOpProgressResultType<
				  TResult, TProgress>::type>::type>
TReturn
wait_for_completion(_In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<
						TResult, TProgress> *operation,
					COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) {
	TReturn result;
	THROW_IF_FAILED(details::WaitForCompletion(operation, result.GetAddressOf(),
...
/** Similar to WaitForCompletion but this function encapsulates the creation of
the async operation making usage simpler.
~~~
ComPtr<ILauncherStatics> launcher; // inited somewhere
auto result = call_and_wait_for_completion(launcher.Get(),
&ILauncherStatics::LaunchUriAsync, uri.Get());
~~~
*/
template <typename I, typename... P, typename... Args>
auto call_and_wait_for_completion(I *object,
								  HRESULT (STDMETHODCALLTYPE I::*func)(P...),
								  Args &&...args) {
	Microsoft::WRL::ComPtr<
		typename wistd::remove_pointer<typename wistd::remove_pointer<
			typename details::LastType<P...>::type>::type>::type>
		op;
...
#ifdef WIL_ENABLE_EXCEPTIONS
//! Get a WinRT activation factory object, usually using a IXXXStatics
//! interface.
...
com_ptr<TInterface> GetActivationFactory(PCWSTR runtimeClass) {
	...
	THROW_IF_FAILED(RoGetActivationFactory(
		Microsoft::WRL::Wrappers::HStringReference(runtimeClass).Get(),
		IID_PPV_ARGS(&result)));
	return result;
}
...
com_ptr<TInterface> ActivateInstance(PCWSTR runtimeClass) {
	...
	THROW_IF_FAILED(RoActivateInstance(
		Microsoft::WRL::Wrappers::HStringReference(runtimeClass).Get(),
		&result));
...
class SyncAsyncOp WrlFinal
  ...
  public:
	...
	IFACEMETHODIMP put_Completed(
		ABI::Windows::Foundation::IAsyncOperationCompletedHandler<TResult>
			*competed) override {
		competed->Invoke(this,
						 ABI::Windows::Foundation::AsyncStatus::Completed);
		return S_OK;
	}
...
extern const __declspec(selectany) wchar_t SyncAsyncActionName[] =
	L"SyncActionAction";
...
#endif
			  >> {
	InspectableClass(InterfaceName_Windows_Foundation_IAsyncAction,
					 TrustLevel::BaseTrust);

  public:
	IFACEMETHODIMP put_Completed(
		ABI::Windows::Foundation::IAsyncActionCompletedHandler *competed)
		override {
		competed->Invoke(this,
						 ABI::Windows::Foundation::AsyncStatus::Completed);
		return S_OK;
	...
	IFACEMETHODIMP GetResults() override { return S_OK; }
...
/// @endcond
//! Creates a WinRT async operation object that implements
//! IAsyncOperation<TResult>. Use mostly for testing and for mocking APIs.
template <typename TResult>
HRESULT make_synchronous_async_operation_nothrow(
	ABI::Windows::Foundation::IAsyncOperation<TResult> **result,
	const TResult &value) {
	return Microsoft::WRL::MakeAndInitialize<details::SyncAsyncOp<TResult>>(
		result, value);
}

//! Creates a WinRT async operation object that implements IAsyncAction. Use
//! mostly for testing and for mocking APIs.
inline HRESULT make_synchronous_async_action_nothrow(
	ABI::Windows::Foundation::IAsyncAction **result) {
	return Microsoft::WRL::MakeAndInitialize<details::SyncAsyncActionOp>(
		result);
}
...
template <
	typename TResult,
	typename TRealResult =
		typename details::MapAsyncOpResultType<TResult>::type,
	typename TSmartResult = typename details::MapToSmartType<TRealResult>::type>
void make_synchronous_async_operation(
	ABI::Windows::Foundation::IAsyncOperation<TResult> **result,
	const TResult &value) {
	THROW_IF_FAILED(
		(Microsoft::WRL::MakeAndInitialize<details::SyncAsyncOp<TResult>>(
			result, value)));
}
...
//! mostly for testing and for mocking APIs.
inline void
make_synchronous_async_action(ABI::Windows::Foundation::IAsyncAction **result) {
	THROW_IF_FAILED((
		Microsoft::WRL::MakeAndInitialize<details::SyncAsyncActionOp>(result)));
}
#endif
#pragma endregion
...
// unique_winrt_event_token[_cx] is an RAII wrapper around
// EventRegistrationToken. When the unique_winrt_event_token[_cx] is destroyed,
// the event is automatically unregistered. Declare a
// wil::unique_winrt_event_token[_cx]<T> at the scope the event should be
// registered for (often they are tied to object lifetime), where T is the type
// of the event sender
//     wil::unique_winrt_event_token_cx<Windows::UI::Xaml::Controls::Button>
//     m_token;
//
// Macros have been defined to register for handling the event and then
// returning an unique_winrt_event_token[_cx]. These macros simply hide the
// function references for adding and removing the event.
//     C++/CX  m_token = WI_MakeUniqueWinRtEventTokenCx(ExampleEventName,
//     sender, handler); ABI     m_token =
//     WI_MakeUniqueWinRtEventToken(ExampleEventName, sender, handler,
//     &m_token);                 // Exception and failfast ABI
//     RETURN_IF_FAILED(WI_MakeUniqueWinRtEventTokenNoThrow(ExampleEventName,
//     sender, handler, &m_token));  // No throw variant
//
// When the wrapper is destroyed, the handler will be unregistered. You can
// explicitly unregister the handler prior.
//     m_token.reset();
//
// You can release the EventRegistrationToken from being managed by the wrapper
// by calling .release()
...
//     m_token.get();
//
// See "onecore\shell\tests\wil\UniqueWinRTEventTokenTests.cpp" for more
// examples of usage in ABI and C++/CX.
...
template <typename T> class unique_winrt_event_token_cx {
	using removal_func =
		void (T::*)(Windows::Foundation::EventRegistrationToken);
  ...
  public:
	...
	unique_winrt_event_token_cx(
		Windows::Foundation::EventRegistrationToken token, T ^ sender,
		removal_func removalFunction) WI_NOEXCEPT
		: m_token(token),
		  m_weakSender(sender),
	...
	unique_winrt_event_token_cx &
	operator=(unique_winrt_event_token_cx &&other) WI_NOEXCEPT {
		if (this != wistd::addressof(other)) {
			reset();
			...
			wistd::swap_wil(m_staticRemovalFunction,
							other.m_staticRemovalFunction);
		}

		return *this;
	}

	~unique_winrt_event_token_cx() WI_NOEXCEPT { reset(); }

	WI_NODISCARD explicit operator bool() const WI_NOEXCEPT {
		return (m_token.Value != 0);
	}

	WI_NODISCARD Windows::Foundation::EventRegistrationToken
	get() const WI_NOEXCEPT {
		return m_token;
	}
			...
			if (m_staticRemovalFunction) {
				...
				auto resolvedSender = [&]() {
					try {
						...
						// Ignore RPC or other failures that are unavoidable in
						// some cases matching wil::unique_winrt_event_token and
						// winrt::event_revoker
	...
	// Stops the wrapper from managing resource and returns the
...
template <typename T> class unique_winrt_event_token {
  ...
  public:
	...
	unique_winrt_event_token(::EventRegistrationToken token, T *sender,
							 removal_func removalFunction) WI_NOEXCEPT
		: m_token(token),
		  m_removalFunction(removalFunction) {
	...
	unique_winrt_event_token &
	operator=(const unique_winrt_event_token &) = delete;
	...
	unique_winrt_event_token &
	operator=(unique_winrt_event_token &&other) WI_NOEXCEPT {
		if (this != wistd::addressof(other)) {
			reset();
	...
	~unique_winrt_event_token() WI_NOEXCEPT { reset(); }
	...
	void reset() WI_NOEXCEPT {
		if (m_token.value != 0) {
			// If T cannot be QI'ed from the weak object then T is not a COM
			// interface.
			auto resolvedSender = m_weakSender.try_query<T>();
			if (resolvedSender) {
				FAIL_FAST_IF_FAILED(
					(resolvedSender.get()->*m_removalFunction)(m_token));
			}
			release();
		}
	}
...
// Handles registration of the event handler to the subscribing object and then
// wraps the EventRegistrationToken in unique_winrt_event_token. Not intended to
// be directly called. Use the WI_MakeUniqueWinRtEventTokenCx macro to abstract
// away specifying the functions that handle addition and removal.
template <typename T, typename addition_func, typename removal_func,
		  typename handler>
inline wil::unique_winrt_event_token_cx<T>
make_unique_winrt_event_token_cx(T ^ sender, addition_func additionFunc,
								 removal_func removalFunc, handler ^ h) {
...
template <typename T, typename addition_func, typename removal_func,
		  typename handler>
inline wil::unique_winrt_event_token_cx<T>
make_unique_winrt_static_event_token_cx(addition_func additionFunc,
										removal_func removalFunc, handler ^ h) {
...
// Handles registration of the event handler to the subscribing object and then
// wraps the EventRegistrationToken in unique_winrt_event_token. Not intended to
// be directly called. Use the WI_MakeUniqueWinRtEventToken macro to abstract
// away specifying the functions that handle addition and removal.
template <typename err_policy = wil::err_returncode_policy, typename T,
		  typename addition_func, typename removal_func, typename handler>
inline auto make_unique_winrt_event_token(
	T *sender, addition_func additionFunc, removal_func removalFunc, handler h,
	wil::unique_winrt_event_token<T> *token_reference) {
	::EventRegistrationToken rawToken;
	err_policy::HResult((sender->*additionFunc)(h, &rawToken));
	*token_reference =
		wil::unique_winrt_event_token<T>(rawToken, sender, removalFunc);
	return err_policy::OK();
}

// Overload make function to allow for returning the constructed object when not
// using HRESULT based code.
template <typename err_policy = wil::err_returncode_policy, typename T,
		  typename addition_func, typename removal_func, typename handler>
inline typename wistd::enable_if<
	!wistd::is_same<err_policy, wil::err_returncode_policy>::value,
	wil::unique_winrt_event_token<T>>::type
make_unique_winrt_event_token(T *sender, addition_func additionFunc,
							  removal_func removalFunc, handler h) {
	::EventRegistrationToken rawToken;
	err_policy::HResult((sender->*additionFunc)(h, &rawToken));
...
#define WI_MakeUniqueWinRtEventTokenCx(_event, _object, _handler)              \
	wil::details::make_unique_winrt_event_token_cx(                            \
		_object,                                                               \
		&wil::details::remove_reference<                                       \
			decltype(_object)>::type::##_event## ::add,                        \
		&wil::details::remove_reference<                                       \
			decltype(_object)>::type::##_event## ::remove,                     \
		_handler)
...
#define WI_MakeUniqueWinRtEventTokenNoThrow(_event, _object, _handler,         \
											_token_reference)                  \
	wil::details::make_unique_winrt_event_token(                               \
		_object,                                                               \
		&wistd::remove_pointer<decltype(_object)>::type::add_##_event,         \
		&wistd::remove_pointer<decltype(_object)>::type::remove_##_event,      \
		_handler, _token_reference)
...
template <>
struct ABI::Windows::Foundation::IAsyncOperation<
	ABI::Windows::Foundation::IAsyncAction *>
	: ABI::Windows::Foundation::IAsyncOperation_impl<
		  ABI::Windows::Foundation::IAsyncAction *> {
...
struct ABI::Windows::Foundation::IAsyncOperationWithProgress<
	...
	: ABI::Windows::Foundation::IAsyncOperationWithProgress_impl<
		  ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *, Z> {
	static const wchar_t *z_get_rc_name_impl() {
		return L"IAsyncOperationWithProgress<IAsyncOperationWithProgress<T,P>*,"
			   L"Z>";
	}
};
...
struct ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler<
	...
	: ABI::Windows::Foundation::
		  IAsyncOperationWithProgressCompletedHandler_impl<
			  ABI::Windows::Foundation::IAsyncOperation<T> *, P> {
	static const wchar_t *z_get_rc_name_impl() {
		return L"IAsyncOperationWithProgressCompletedHandler<IAsyncOperation<T>"
			   L"*,P>";
	}
};
...
struct ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler<
	...
	: ABI::Windows::Foundation::
		  IAsyncOperationWithProgressCompletedHandler_impl<
			  ...
			  Z> {
	static const wchar_t *z_get_rc_name_impl() {
		return L"IAsyncOperationWithProgressCompletedHandler<"
			   L"IAsyncOperationWithProgress<T,P>*,Z>";
	}
};
...
namespace std {
//! Specialization of `std::less` for `Microsoft::WRL::Wrappers::HString` that
//! uses `hstring_less` for the comparison function object.
template <>
struct less<Microsoft::WRL::Wrappers::HString> : public wil::hstring_less {};

//! Specialization of `std::less` for `wil::unique_hstring` that uses
//! `hstring_less` for the comparison function object.

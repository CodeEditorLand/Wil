...
namespace wil {
// Determine if apartment variables are supported in the current process
// context. Prior to build 22365, the APIs needed to create apartment variables
// (e.g. RoGetApartmentIdentifier) failed for unpackaged processes. For MS
// people, see http://task.ms/31861017 for details. APIs needed to implement
// apartment variables did not work in non-packaged processes.
inline bool are_apartment_variables_supported() {
	unsigned long long apartmentId{};
	return RoGetApartmentIdentifier(&apartmentId) !=
		   HRESULT_FROM_WIN32(ERROR_API_UNAVAILABLE);
}

// COM will implicitly rundown the apartment registration when it invokes a
// handler and blocks calling unregister when executing the callback. So be
// careful to release() this when callback is invoked to avoid a double free of
// the cookie.
using unique_apartment_shutdown_registration =
	unique_any<APARTMENT_SHUTDOWN_REGISTRATION_COOKIE,
			   decltype(&::RoUnregisterForApartmentShutdown),
...
struct apartment_variable_platform {
	...
	static auto RegisterForApartmentShutdown(IApartmentShutdown *observer) {
		...
		THROW_IF_FAILED(
			RoRegisterForApartmentShutdown(observer, &id, cookie.put()));
		return cookie;
	}

	static void UnRegisterForApartmentShutdown(
	...
	inline static constexpr unsigned long AsyncRundownDelayForTestingRaces =
		INFINITE;
...
namespace details {
// For the address of data, you can detect global variables by the ability to
// resolve the module from the address.
inline bool IsGlobalVariable(const void *moduleAddress) noexcept {
	...
	return GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
							  static_cast<PCWSTR>(moduleAddress),
							  &moduleHandle) != FALSE;
}
...
struct any_maker_base {
	...
	WI_NODISCARD std::any operator()() const { return adapter(inner); }
};
...
template <apartment_variable_leak_action leak_action =
			  apartment_variable_leak_action::fail_fast,
...
struct apartment_variable_base {
	...
	struct apartment_variable_storage {
		apartment_variable_storage(
			apartment_variable_storage &&other) noexcept = default;
		apartment_variable_storage(const apartment_variable_storage &other) =
			delete;

		apartment_variable_storage(typename test_hook::shutdown_type &&cookie_)
			: cookie(std::move(cookie_)) {}

		winrt::apartment_context context;
		typename test_hook::shutdown_type cookie;
		// Variables are stored using the address of the
		// apartment_variable_base<> as the key.
		std::unordered_map<apartment_variable_base<leak_action, test_hook> *,
						   std::any>
			variables;
	};
	...
	~apartment_variable_base() {
		...
		if constexpr (leak_action ==
					  apartment_variable_leak_action::fail_fast) {
			if (isGlobal && !ProcessShutdownInProgress()) {
				// If you hit this fail fast it means the storage in
				// s_apartmentStorage will be leaked. For apartment variables
				// used in .exes, this is expected and this fail fast should be
				// disabled using wil::apartment_variable<T,
				// wil::apartment_variable_leak_action::ignore>
				//
				// For DLLs, if this is expected, disable this fail fast using
				// wil::apartment_variable<T,
				// wil::apartment_variable_leak_action::ignore>
				//
				// Use of apartment variables in DLLs only loaded by COM will
				// never hit this case as COM will unload DLLs before apartments
				// are rundown, providing the opportunity to empty
				// s_apartmentStorage.
				//
				// But DLLs loaded and unloaded to call DLL entry points
				// (outside of COM) may create variable storage that can't be
				// cleaned up as the DLL lifetime is shorter that the COM
				// lifetime. In these cases either 1) accept the leaks and
				// disable the fail fast as describe above 2) disable module
				// unloading by calling wil::ensure_module_stays_loaded 3)
				// CoCreate an object from this DLL to make COM aware of the DLL
	...
	static apartment_variable_storage *
	get_current_apartment_variable_storage() {
		auto storage =
			s_apartmentStorage.get().find(test_hook::GetApartmentId());
	...
	apartment_variable_storage *ensure_current_apartment_variables() {
		...
		struct ApartmentObserver
			: public winrt::implements<ApartmentObserver, IApartmentShutdown> {
			void STDMETHODCALLTYPE
			OnUninitialize(unsigned long long apartmentId) noexcept override {
				// This code runs at apartment rundown so be careful to avoid
				// deadlocks by extracting the variables under the lock then
				...
				// after invoking the callback and does not allow calling
				// unregister in the callback. So release the reference to the
				// registration.
				variables.mapped().cookie.release();
			}
		};
		auto shutdownRegistration = test_hook::RegisterForApartmentShutdown(
			winrt::make<ApartmentObserver>().get());
		return &s_apartmentStorage.get()
					.insert({test_hook::GetApartmentId(),
							 apartment_variable_storage(
								 std::move(shutdownRegistration))})
					.first->second;
	}
	...
	template <typename T> std::any &get_or_create(any_maker<T> &&creator) {
		...
		// The insertion may fail if creator() recursively caused itself to be
		// created, in which case we return the existing object and the
		// falsely-created one is discarded.
		return variable_storage->variables.insert({this, std::move(value)})
			.first->second;
	}
	...
	// replace or create the current value, fail fasts if the value is not
	// already stored
	...
	winrt::Windows::Foundation::IAsyncAction clear_all_apartments_async() {
		...
		wil::unique_mta_usage_cookie
			mta_reference; // need to extend the MTA due to async cleanup
		FAIL_FAST_IF_FAILED(CoIncrementMTAUsage(mta_reference.put()));

		// From a background thread hop into each apartment to run down the
		// object if it's still there.
		co_await winrt::resume_background();

		// This hook enables testing the case where execution of this method
		// loses the race with apartment rundown by other means.
		...
		for (auto &&context : contexts) {
			try {
				...
				// Ignore failure if apartment ran down before we could clean it
				// up. The object already ran down as part of apartment cleanup.
	...
	static const auto &storage() { return s_apartmentStorage.get(); }
...
template <typename T,
		  apartment_variable_leak_action leak_action =
			  apartment_variable_leak_action::fail_fast,
		  typename test_hook = wil::apartment_variable_platform>
struct apartment_variable
	: details::apartment_variable_base<leak_action, test_hook> {
	...
	template <typename F> T &get_or_create(F &&f) {
		return std::any_cast<T &>(
			base::get_or_create(details::any_maker<T>(std::forward<F>(f))));
	}

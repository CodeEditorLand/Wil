...
namespace wil {
//! Determines if a path is an extended length path that can be used to access
...
//! Find the last segment of a path. Matches the behavior of
//! shlwapi!PathFindFileNameW() note, does not support streams being specified
//! like PathFindFileNameW(), is that a bug or a feature?
inline PCWSTR find_last_path_segment(_In_ PCWSTR path) {
	...
	auto const limitedLength =
		((pathLength > 0) && (path[pathLength - 1] == L'\\')) ? (pathLength - 1)
															  : pathLength;

	PCWSTR result = nullptr;
	auto const offset = FindStringOrdinal(
		FIND_FROMEND, path, static_cast<int>(limitedLength), L"\\", 1, TRUE);
	if (offset == -1) {
...
//! Returns the drive number, if it has one. Returns true if there is a drive
//! number, false otherwise. Supports regular and extended length paths.
inline bool try_get_drive_letter_number(_In_ PCWSTR path,
										_Out_ int *driveNumber) {
...
// returns:
//      true + parentPathLength - path has a parent starting at the beginning
//      path and of parentPathLength length. false, no parent path, the input is
//      a root path.
inline bool try_get_parent_path_range(_In_ PCWSTR path,
									  _Out_ size_t *parentPathLength) {
	*parentPathLength = 0;
	...
	if (::CreateDirectoryW(path, nullptr) == FALSE) {
		...
		if (lastError == ERROR_PATH_NOT_FOUND) {
			...
			if (try_get_parent_path_range(path, &parentLength)) {
				wistd::unique_ptr<wchar_t[]> parent(
					new (std::nothrow) wchar_t[parentLength + 1]);
				RETURN_IF_NULL_ALLOC(parent.get());
				RETURN_IF_FAILED(StringCchCopyNW(parent.get(), parentLength + 1,
												 path, parentLength));
				RETURN_IF_FAILED(
...
HRESULT GetFullPathNameW(PCWSTR file, string_type &path,
						 _Outptr_opt_ PCWSTR *filePart = nullptr) {
	wil::assign_null_to_opt_param(filePart);
	const auto hr = AdaptFixedSizeToAllocatedResult<string_type,
													stackBufferLength>(
		path,
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			*valueLengthNeededWithNull = ::GetFullPathNameW(
				file, static_cast<DWORD>(valueLength), value, nullptr);
			RETURN_LAST_ERROR_IF(*valueLengthNeededWithNull == 0);
			WI_ASSERT((*value != L'\0') ==
					  (*valueLengthNeededWithNull < valueLength));
	...
	if (SUCCEEDED(hr) && filePart) {
		*filePart = wil::find_last_path_segment(
			details::string_maker<string_type>::get(path));
	}
	return hr;
}
...
string_type GetFullPathNameW(PCWSTR file,
							 _Outptr_opt_ PCWSTR *filePart = nullptr) {
	string_type result{};
	THROW_IF_FAILED((GetFullPathNameW<string_type, stackBufferLength>(
		file, result, filePart)));
	return result;
}
#endif
...
/// @cond
namespace details {
// Reparse points should not be traversed in most recursive walks of the file
// system, unless allowed through the appropriate reparse tag.
inline bool CanRecurseIntoDirectory(const FILE_ATTRIBUTE_TAG_INFO &info) {
	return (
		...
		(WI_IsFlagClear(info.FileAttributes, FILE_ATTRIBUTE_REPARSE_POINT) ||
		 (IsReparseTagDirectory(info.ReparseTag) ||
		  (info.ReparseTag == IO_REPARSE_TAG_WCI))));
}
} // namespace details
/// @endcond
...
inline wil::unique_hfile TryCreateFileCanRecurseIntoDirectory(
	PCWSTR path, PWIN32_FIND_DATAW fileFindData,
	DWORD access = GENERIC_READ | /*DELETE*/ 0x00010000L,
	DWORD share = FILE_SHARE_READ) {
	wil::unique_hfile result(CreateFileW(
		path, access, share, nullptr, OPEN_EXISTING,
		FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, nullptr));
	if (result) {
		FILE_ATTRIBUTE_TAG_INFO fati{};
		if (GetFileInformationByHandleEx(result.get(), FileAttributeTagInfo,
										 &fati, sizeof(fati)) &&
			details::CanRecurseIntoDirectory(fati)) {
			if (fileFindData) {
				// Refresh the found file's data now that we have secured the
				// directory from external manipulation.
...
// If inputPath is a non-normalized name be sure to pass an extended length form
// to ensure it can be addressed and deleted.
inline HRESULT RemoveDirectoryRecursiveNoThrow(
	...
	if (is_extended_length_path(inputPath)) {
		...
		// PathAllocCombine will convert extended length paths to regular paths
		// if shorter than MAX_PATH, avoid that behavior to provide access
		// inputPath with non-normalized names.
		combineOptions = PATHCCH_ENSURE_IS_EXTENDED_LENGTH_PATH;
	} else {
		// For regular paths normalize here to get consistent results when
	...
	RETURN_IF_FAILED(
		::PathAllocCombine(path.get(), L"*", combineOptions, &searchPath));
	...
	for (;;) {
		...
		if (!(WI_IsFlagSet(fd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) &&
			  path_is_dot_or_dotdot(fd.cFileName))) {
			// Need to form an extended length path to provide the ability to
			// delete paths > MAX_PATH and files with non-normalized names (dots
			// or spaces at the end).
			wil::unique_hlocal_string pathToDelete;
			RETURN_IF_FAILED(
				::PathAllocCombine(path.get(), fd.cFileName,
								   PATHCCH_ENSURE_IS_EXTENDED_LENGTH_PATH |
									   PATHCCH_DO_NOT_NORMALIZE_SEGMENTS,
								   &pathToDelete));
			if (WI_IsFlagSet(fd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY)) {
				// Get a handle to the directory to delete, preventing it from
				// being replaced to prevent writes which could be used to
				// bypass permission checks, and verify that it is not a name
				// surrogate (e.g. symlink, mount point, etc).
				wil::unique_hfile recursivelyDeletableDirectoryHandle =
					TryCreateFileCanRecurseIntoDirectory(pathToDelete.get(),
														 &fd);
				if (recursivelyDeletableDirectoryHandle) {
					...
					RETURN_IF_FAILED(RemoveDirectoryRecursiveNoThrow(
						pathToDelete.get(),
						WI_ClearFlag(localOptions,
									 RemoveDirectoryOptions::KeepRootDirectory),
						recursivelyDeletableDirectoryHandle.get()));
				} else if (WI_IsFlagSet(fd.dwFileAttributes,
										FILE_ATTRIBUTE_REPARSE_POINT)) {
					// This is a directory reparse point that should not be
					// recursed. Delete it without traversing into it.
					RETURN_IF_WIN32_BOOL_FALSE(
						::RemoveDirectoryW(pathToDelete.get()));
				} else {
					// Failed to grab a handle to the file or to read its
					// attributes. This is not safe to recurse.
					RETURN_WIN32(::GetLastError());
				}
			} else {
				// Try a DeleteFile.  Some errors may be recoverable.
				if (!::DeleteFileW(pathToDelete.get())) {
					// Fail for anything other than ERROR_ACCESS_DENIED with
					// option to RemoveReadOnly available
					bool potentiallyFixableReadOnlyProblem =
						WI_IsFlagSet(options,
									 RemoveDirectoryOptions::RemoveReadOnly) &&
						::GetLastError() == ERROR_ACCESS_DENIED;
					RETURN_LAST_ERROR_IF(!potentiallyFixableReadOnlyProblem);

					// Fail if the file does not have read-only set, likely just
					// an ACL problem
					DWORD fileAttr = ::GetFileAttributesW(pathToDelete.get());
					RETURN_LAST_ERROR_IF(
						!WI_IsFlagSet(fileAttr, FILE_ATTRIBUTE_READONLY));

					// Remove read-only flag, setting to NORMAL if completely
					// empty
					...
					// Set the new attributes and try to delete the file again,
					// returning any failure
					::SetFileAttributesW(pathToDelete.get(), fileAttr);
					RETURN_IF_WIN32_BOOL_FALSE(
						::DeleteFileW(pathToDelete.get()));
				}
			}
		}
	...
	if (WI_IsFlagClear(options, RemoveDirectoryOptions::KeepRootDirectory)) {
		if (deleteHandle != INVALID_HANDLE_VALUE) {
			...
			// DeleteFile and RemoveDirectory use POSIX delete, falling back to
			// non-POSIX on most errors. Do the same here.
			FILE_DISPOSITION_INFO_EX fileInfoEx{};
			fileInfoEx.Flags = FILE_DISPOSITION_FLAG_DELETE |
							   FILE_DISPOSITION_FLAG_POSIX_SEMANTICS;
			if (!SetFileInformationByHandle(deleteHandle, FileDispositionInfoEx,
											&fileInfoEx, sizeof(fileInfoEx))) {
				auto const err = ::GetLastError();
				// The real error we're looking for is STATUS_CANNOT_DELETE, but
				// that's mapped to ERROR_ACCESS_DENIED.
				if (err != ERROR_ACCESS_DENIED) {
#endif
					FILE_DISPOSITION_INFO fileInfo{};
					fileInfo.DeleteFile = TRUE;
					RETURN_IF_WIN32_BOOL_FALSE(SetFileInformationByHandle(
						deleteHandle, FileDispositionInfo, &fileInfo,
						sizeof(fileInfo)));
#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)
				} else {
					RETURN_WIN32(err);
...
// Range based for that supports Win32 structures that use NextEntryOffset as
// the basis of traversing a result buffer that contains data. This is used in
// the following FileIO calls: FileStreamInfo, FILE_STREAM_INFO
...
template <typename T> struct next_entry_offset_iterator {
	...
	// range based for requires operator!=, operator++ and operator* to do its
	// work on the type returned from begin() and end(), provide those here.
	WI_NODISCARD bool
	operator!=(const next_entry_offset_iterator &other) const {
	...
	next_entry_offset_iterator &operator++() {
		current_ = (current_->NextEntryOffset != 0)
					   ? reinterpret_cast<T *>(
							 reinterpret_cast<unsigned char *>(current_) +
...
// auto watcher = wil::make_folder_watcher(folder.Path().c_str(), true,
// wil::allChangeEvents, []()
//     {
//         // respond
//     });
...
enum class FolderChangeEvent : DWORD {
	ChangesLost =
		0, // requies special handling, reset state as events were lost
...
enum class FolderChangeEvents : DWORD {
	...
	All = FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME |
...
struct folder_watcher_state {
	folder_watcher_state(wistd::function<void()> &&callback)
		: m_callback(wistd::move(callback)) {}
	wistd::function<void()> m_callback;
	// Order is important, need to close the thread pool wait before the change
	// handle.
...
inline void
delete_folder_watcher_state(_In_opt_ folder_watcher_state *storage) {
	delete storage;
}

typedef resource_policy<
	folder_watcher_state *, decltype(&details::delete_folder_watcher_state),
...
class folder_watcher_t : public storage_t {
  public:
	...
	explicit folder_watcher_t(args_t &&...args) WI_NOEXCEPT
	...
	folder_watcher_t(PCWSTR folderToWatch, bool isRecursive,
	...
	result create(PCWSTR folderToWatch, bool isRecursive,
				  FolderChangeEvents filter,
				  wistd::function<void()> &&callback) {
		return err_policy::HResult(create_common(
			folderToWatch, isRecursive, filter, wistd::move(callback)));
	}

  private:
	// Factored into a standalone function to support Clang which does not
	// support conversion of stateless lambdas to __stdcall
	static void __stdcall callback(PTP_CALLBACK_INSTANCE /*Instance*/,
								   void *context, TP_WAIT *pThreadPoolWait,
								   TP_WAIT_RESULT /*result*/) {
		auto watcherState =
			static_cast<details::folder_watcher_state *>(context);
		...
		SetThreadpoolWait(pThreadPoolWait,
						  watcherState->m_findChangeHandle.get(), __nullptr);
	}

	// This function exists to avoid template expansion of this code based on
	// err_policy.
	HRESULT create_common(PCWSTR folderToWatch, bool isRecursive,
						  FolderChangeEvents filter,
						  wistd::function<void()> &&callback) {
		wistd::unique_ptr<details::folder_watcher_state> watcherState(new (
			std::nothrow) details::folder_watcher_state(wistd::move(callback)));
		RETURN_IF_NULL_ALLOC(watcherState);

		watcherState->m_findChangeHandle.reset(FindFirstChangeNotificationW(
			folderToWatch, isRecursive, static_cast<DWORD>(filter)));
		RETURN_LAST_ERROR_IF(!watcherState->m_findChangeHandle);

		watcherState->m_threadPoolWait.reset(CreateThreadpoolWait(
			&folder_watcher_t::callback, watcherState.get(), __nullptr));
		RETURN_LAST_ERROR_IF(!watcherState->m_threadPoolWait);
		this->reset(
			watcherState
				.release()); // no more failures after this, pass ownership
		SetThreadpoolWait(this->get()->m_threadPoolWait.get(),
						  this->get()->m_findChangeHandle.get(), __nullptr);
		return S_OK;
	}
};

typedef unique_any_t<folder_watcher_t<
	details::unique_storage<details::folder_watcher_state_resource_policy>,
	err_returncode_policy>>
	unique_folder_watcher_nothrow;

inline unique_folder_watcher_nothrow
make_folder_watcher_nothrow(PCWSTR folderToWatch, bool isRecursive,
							FolderChangeEvents filter,
...
inline unique_folder_watcher
make_folder_watcher(PCWSTR folderToWatch, bool isRecursive,
					FolderChangeEvents filter,
					wistd::function<void()> &&callback) {
	return unique_folder_watcher(folderToWatch, isRecursive, filter,
								 wistd::move(callback));
}
#endif // WIL_ENABLE_EXCEPTIONS
...
// auto reader = wil::make_folder_change_reader(folder.Path().c_str(), true,
...
struct folder_change_reader_state {
	folder_change_reader_state(
		bool isRecursive, FolderChangeEvents filter,
		wistd::function<void(FolderChangeEvent, PCWSTR)> &&callback)
		: m_callback(wistd::move(callback)), m_isRecursive(isRecursive),
		  m_filter(filter) {}
	...
	~folder_change_reader_state() {
		if (m_tpIo != __nullptr) {
			...
			if (m_folderHandle) {
				...
				GetOverlappedResult(m_folderHandle.get(), &m_overlapped,
									&bytesTransferredIgnored, TRUE);
			}
	...
	HRESULT StartIo() {
		...
		HRESULT hr = ReadDirectoryChangesW(m_folderHandle.get(), m_readBuffer,
										   sizeof(m_readBuffer), m_isRecursive,
										   static_cast<DWORD>(m_filter),
										   __nullptr, &m_overlapped, __nullptr)
						 ? S_OK
	...
	unsigned char
		m_readBuffer[4096]{}; // Consider alternative buffer sizes. With 512
							  // byte buffer i was not able to observe overflow.
};

inline void delete_folder_change_reader_state(
	_In_opt_ folder_change_reader_state *storage) {
	delete storage;
}

typedef resource_policy<folder_change_reader_state *,
						decltype(&details::delete_folder_change_reader_state),
						details::delete_folder_change_reader_state,
...
class folder_change_reader_t : public storage_t {
  public:
	// forward all base class constructors...
	template <typename... args_t>
	explicit folder_change_reader_t(args_t &&...args) WI_NOEXCEPT
		: storage_t(wistd::forward<args_t>(args)...) {}
  ...
  private:
	...
	static void __stdcall callback(PTP_CALLBACK_INSTANCE /* Instance */,
								   ...
								   TP_IO * /* Io */) {
		auto readerState =
			static_cast<details::folder_change_reader_state *>(context);
		...
		if (result == ERROR_SUCCESS) {
			for (auto const &info : create_next_entry_offset_iterator(
					 reinterpret_cast<FILE_NOTIFY_INFORMATION *>(
						 readerState->m_readBuffer))) {
				wchar_t relativeFileName[MAX_PATH];
				StringCchCopyNW(relativeFileName, ARRAYSIZE(relativeFileName),
								info.FileName,
								info.FileNameLength / sizeof(info.FileName[0]));

				readerState->m_callback(
					static_cast<FolderChangeEvent>(info.Action),
					relativeFileName);
			}
	...
	// err_policy.
	HRESULT
	create_common(PCWSTR folderToWatch, bool isRecursive,
				  FolderChangeEvents filter,
				  wistd::function<void(FolderChangeEvent, PCWSTR)> &&callback) {
		wistd::unique_ptr<details::folder_change_reader_state> readerState(
			new (std::nothrow) details::folder_change_reader_state(
				isRecursive, filter, wistd::move(callback)));
		RETURN_IF_NULL_ALLOC(readerState);

		readerState->m_folderHandle.reset(CreateFileW(
			folderToWatch, FILE_LIST_DIRECTORY,
			FILE_SHARE_READ | FILE_SHARE_DELETE | FILE_SHARE_WRITE, __nullptr,
			OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED,
			__nullptr));
		RETURN_LAST_ERROR_IF(!readerState->m_folderHandle);

		readerState->m_tpIo = CreateThreadpoolIo(
			readerState->m_folderHandle.get(),
...
typedef unique_any_t<folder_change_reader_t<
	details::unique_storage<
		details::folder_change_reader_state_resource_policy>,
	err_returncode_policy>>
...
inline unique_folder_change_reader make_folder_change_reader(
	PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
	wistd::function<void(FolderChangeEvent, PCWSTR)> &&callback) {
	return unique_folder_change_reader(folderToWatch, isRecursive, filter,
									   wistd::move(callback));
}
#endif // WIL_ENABLE_EXCEPTIONS
#pragma endregion

//! Dos and VolumeGuid paths are always extended length paths with the
//! \\?\ prefix.
enum class VolumePrefix {
	Dos = VOLUME_NAME_DOS, // Extended Dos Device path form, e.g.
						   // \\?\C:\Users\Chris\AppData\Local\Temp\wil8C31.tmp
	VolumeGuid =
		VOLUME_NAME_GUID, // \\?\Volume{588fb606-b95b-4eae-b3cb-1e49861aaf18}\Users\Chris\AppData\Local\Temp\wil8C31.tmp
	// The following are special paths which can't be used with Win32 APIs, but
	// are useful in other scenarios.
	None = VOLUME_NAME_NONE, // Path without the volume root, e.g.
							 // \Users\Chris\AppData\Local\Temp\wil8C31.tmp
	NtObjectName =
		VOLUME_NAME_NT, // Unique name used by Object Manager, e.g.
...
Use this instead + VolumePrefix::None instead of
GetFileInformationByHandleEx(FileNameInfo) to get that path form. */
template <typename string_type, size_t stackBufferLength = 256>
HRESULT GetFinalPathNameByHandleW(
	HANDLE fileHandle, string_type &path,
	...
	return AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		...
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			*valueLengthNeededWithNull = ::GetFinalPathNameByHandleW(
				fileHandle, value, static_cast<DWORD>(valueLength),
...
string_type GetFinalPathNameByHandleW(
	HANDLE fileHandle, wil::VolumePrefix volumePrefix = wil::VolumePrefix::Dos,
	wil::PathOptions options = wil::PathOptions::Normalized) {
	string_type result{};
	THROW_IF_FAILED((GetFinalPathNameByHandleW<string_type, stackBufferLength>(
		fileHandle, result, volumePrefix, options)));
	return result;
}
#endif
	...
	return AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		...
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			*valueLengthNeededWithNull =
				::GetCurrentDirectoryW(static_cast<DWORD>(valueLength), value);
			RETURN_LAST_ERROR_IF(*valueLengthNeededWithNull == 0);
			WI_ASSERT((*value != L'\0') ==
					  (*valueLengthNeededWithNull < valueLength));
			if (*valueLengthNeededWithNull < valueLength) {
...
string_type GetCurrentDirectoryW() {
	string_type result{};
	THROW_IF_FAILED(
		(GetCurrentDirectoryW<string_type, stackBufferLength>(result)));
	return result;
}
#endif
...
struct MapInfoClassToInfoStruct; // failure to map is a usage error caught by
								 // the compiler
...
MAP_INFOCLASS_TO_STRUCT(FileIdBothDirectoryInfo, FILE_ID_BOTH_DIR_INFO, false,
						8192);
MAP_INFOCLASS_TO_STRUCT(FileIdBothDirectoryRestartInfo, FILE_ID_BOTH_DIR_INFO,
						true, 0);
MAP_INFOCLASS_TO_STRUCT(FileIoPriorityHintInfo, FILE_IO_PRIORITY_HINT_INFO,
						true, 0);
MAP_INFOCLASS_TO_STRUCT(FileRemoteProtocolInfo, FILE_REMOTE_PROTOCOL_INFO, true,
						0);
MAP_INFOCLASS_TO_STRUCT(FileFullDirectoryInfo, FILE_FULL_DIR_INFO, false, 8192);
MAP_INFOCLASS_TO_STRUCT(FileFullDirectoryRestartInfo, FILE_FULL_DIR_INFO, true,
						0);
...
MAP_INFOCLASS_TO_STRUCT(FileIdExtdDirectoryInfo, FILE_ID_EXTD_DIR_INFO, false,
						8192);
MAP_INFOCLASS_TO_STRUCT(FileIdExtdDirectoryRestartInfo, FILE_ID_EXTD_DIR_INFO,
						true, 0);
#endif

// Type unsafe version used in the implementation to avoid template bloat.
inline HRESULT GetFileInfo(HANDLE fileHandle,
						   FILE_INFO_BY_HANDLE_CLASS infoClass,
						   size_t allocationSize,
						   _Outptr_result_maybenull_ void **result) {
	*result = nullptr;

	wistd::unique_ptr<char[]> resultHolder(
		new (std::nothrow) char[allocationSize]);
	RETURN_IF_NULL_ALLOC(resultHolder);

	for (;;) {
		if (GetFileInformationByHandleEx(fileHandle, infoClass,
										 resultHolder.get(),
			...
			*result = resultHolder.release();
			...
			if (lastError == ERROR_MORE_DATA) {
				...
				RETURN_IF_NULL_ALLOC(resultHolder);
					   ...
					   ERROR_NO_MORE_FILES) // for folder enumeration cases
			{
				break;
			} else if (lastError ==
					   ERROR_INVALID_PARAMETER) // operation not supported by
												// file system
			{
				return HRESULT_FROM_WIN32(lastError);
			} else if ((lastError == ERROR_HANDLE_EOF) &&
					   (infoClass == FileStreamInfo)) {
				break;
			} else {
				RETURN_WIN32(lastError);
			}
...
template <
	...
	typename wistd::enable_if<
		!details::MapInfoClassToInfoStruct<infoClass>::isFixed, int>::type = 0>
HRESULT GetFileInfoNoThrow(
	HANDLE fileHandle,
	wistd::unique_ptr<
...
HRESULT GetFileInfoNoThrow(
	HANDLE fileHandle,
	_Out_ typename details::MapInfoClassToInfoStruct<infoClass>::type *result)
	WI_NOEXCEPT {
	const HRESULT hr = GetFileInformationByHandleEx(fileHandle, infoClass,
...
// Verifies that the given file path is not a hard or a soft link. If the file
// is present at the path, returns a handle to it without delete permissions to
// block an attacker from swapping the file.
inline HRESULT CreateFileAndEnsureNotLinked(PCWSTR path,
											wil::unique_hfile &fileHandle) {
	// Open handles to the original path and to the final path and compare each
	// file's information to verify they are the same file. If they are
	// different, the file is a soft link.
	...
	// Open a handle without the reparse point flag to get the final path in
	// case it is a soft link.
	wil::unique_hfile finalPathHandle(
		CreateFileW(path, 0, 0, nullptr, OPEN_EXISTING,
					FILE_FLAG_BACKUP_SEMANTICS, nullptr));
	RETURN_LAST_ERROR_IF(!finalPathHandle);
	BY_HANDLE_FILE_INFORMATION finalFileInfo;
	RETURN_IF_WIN32_BOOL_FALSE(
		GetFileInformationByHandle(finalPathHandle.get(), &finalFileInfo));
	finalPathHandle.reset();

	// The low and high indices and volume serial number uniquely identify a
	// file. These must match if they are the same file.
	...
	// Return failure if it is a soft link or a hard link (number of links
	// greater than 1).
	RETURN_HR_IF(HRESULT_FROM_WIN32(ERROR_BAD_PATHNAME),
				 (isSoftLink || fileInfo.nNumberOfLinks > 1));
...
template <
	...
	typename wistd::enable_if<
		!details::MapInfoClassToInfoStruct<infoClass>::isFixed, int>::type = 0>
wistd::unique_ptr<typename details::MapInfoClassToInfoStruct<infoClass>::type>
GetFileInfo(HANDLE fileHandle) {
	wistd::unique_ptr<
		typename details::MapInfoClassToInfoStruct<infoClass>::type>
		result;
...
struct file_and_error_result {
	file_and_error_result(HANDLE file_handle, DWORD error)
		: file(file_handle), last_error(error) {}
...
~~~
*/
inline file_and_error_result
try_open_file(PCWSTR path, DWORD dwDesiredAccess = FILE_READ_ACCESS,
	...
	auto handle =
		CreateFileW(path, dwDesiredAccess, dwShareMode, &secAttributes,
...
~~~
*/
inline wil::unique_hfile
open_file(PCWSTR path, DWORD dwDesiredAccess = FILE_READ_ACCESS,
		  DWORD dwShareMode = FILE_SHARE_READ,
		  DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
		  bool inheritHandle = false) noexcept {
	auto result = try_open_file(path, dwDesiredAccess, dwShareMode,
								inheritHandle, dwFlagsAndAttributes);
...
file_and_error_result
create_file(PCWSTR path, DWORD dwDesiredAccess, DWORD dwShareMode,
			LPSECURITY_ATTRIBUTES lpSecurityAttributes,
			DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) noexcept {
	auto handle =
		CreateFileW(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
...
inline file_and_error_result try_create_new_file(
	PCWSTR path, DWORD dwDesiredAccess = FILE_READ_ACCESS | FILE_WRITE_ACCESS,
	DWORD dwShareMode = FILE_SHARE_READ,
...
*/
inline file_and_error_result
try_truncate_existing_file(PCWSTR path,
...
*/
inline wil::unique_hfile
create_new_file(PCWSTR path,
				DWORD dwDesiredAccess = FILE_READ_ACCESS | FILE_WRITE_ACCESS,
				DWORD dwShareMode = FILE_SHARE_READ,
				LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
				DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
				HANDLE hTemplateFile = nullptr) noexcept {
	auto result = try_create_new_file(path, dwDesiredAccess, dwShareMode,
...
inline wil::unique_hfile open_or_create_file(
	...
	DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
	HANDLE hTemplateFile = nullptr) noexcept {
	auto result = try_open_or_create_file(path, dwDesiredAccess, dwShareMode,
										  lpSecurityAttributes,
										  dwFlagsAndAttributes, hTemplateFile);
	THROW_WIN32_IF(result.last_error, !result.file.is_valid());
	return std::move(result.file);

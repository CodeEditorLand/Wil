...
struct single_threaded_property
	...
	single_threaded_property(TArgs &&...value)
		: base_type(std::forward<TArgs>(value)...) {}

	using base_type =
		std::conditional_t<std::is_scalar_v<T> || std::is_final_v<T>,
						   wil::details::single_threaded_property_storage<T>,
						   T>;
...
struct single_threaded_rw_property : single_threaded_property<T> {
	...
	template <typename... TArgs>
	single_threaded_rw_property(TArgs &&...value)
		: base_type(std::forward<TArgs>(value)...) {}
...
#if !defined(__WIL_CPPWINRT_AUTHORING_INCLUDED_FOUNDATION) &&                  \
	defined(WINRT_Windows_Foundation_H) // WinRT / XAML helpers
 ...
 */
template <typename T>
struct untyped_event
	: wil::details::event_base<winrt::Windows::Foundation::EventHandler<T>> {};
 ...
 *         //   event Windows.Foundation.TypedEventHandler<ModalPage, String>
 * OkClicked; wil::typed_event<MarkupSample::ModalPage, winrt::hstring>
 * OkClicked;
 * @endcode
 */
template <typename TSender, typename TArgs>
struct typed_event
	: wil::details::event_base<
		  winrt::Windows::Foundation::TypedEventHandler<TSender, TArgs>> {};

#endif // !defined(__WIL_CPPWINRT_AUTHORING_INCLUDED_FOUNDATION) &&
	   // defined(WINRT_Windows_Foundation_H)

#if !defined(__WIL_CPPWINRT_AUTHORING_INCLUDED_XAML_DATA) &&                   \
	(defined(WINRT_Microsoft_UI_Xaml_Data_H) ||                                \
	 defined(WINRT_Windows_UI_Xaml_Data_H)) // INotifyPropertyChanged helpers
...
using Xaml_Data_PropertyChangedEventArgs =
 ...
 * @tparam T CRTP type
 * @details When you declare your class, make this class a base class and pass
 * your class as a template parameter:
 * @code
 ...
 */
template <typename T,
		  typename Xaml_Data_PropertyChangedEventHandler =
			  wil::details::Xaml_Data_PropertyChangedEventHandler,
		  typename Xaml_Data_PropertyChangedEventArgs =
			  wil::details::Xaml_Data_PropertyChangedEventArgs>
struct notify_property_changed_base {
	...
	auto RaisePropertyChanged(std::wstring_view name) {
		return m_propertyChanged(self(),
								 Xaml_Data_PropertyChangedEventArgs{name});
	}
 ...
 * @details Use the #INIT_NOTIFY_PROPERTY macro to initialize this property in
 * your class constructor. This will set up the right property name, and bind it
 * to the `notify_property_changed_base` implementation.
 */
template <typename T,
		  typename Xaml_Data_PropertyChangedEventHandler =
			  wil::details::Xaml_Data_PropertyChangedEventHandler,
...
struct single_threaded_notifying_property : single_threaded_rw_property<T> {
	...
	template <typename Q> auto &operator=(Q &&q) {
		if (q != this->operator()()) {
			static_cast<base_type &>(*this) = std::forward<Q>(q);
			if (auto strong = m_sender.get();
				(m_npc != nullptr) && (strong != nullptr)) {
	...
	single_threaded_notifying_property(
		...
		: single_threaded_rw_property<T>(std::forward<TArgs...>(args)...),
		  m_name(name), m_npc(npc), m_sender(sender) {}

	single_threaded_notifying_property(
		const single_threaded_notifying_property &) = default;
	single_threaded_notifying_property(single_threaded_notifying_property &&) =
		default;
 ...
 * @brief use this to stamp out a property that calls RaisePropertyChanged upon
 * changing its value. This is a zero-storage alternative to
 * wil::single_threaded_notifying_property<T>
 * @details You can pass an initializer list for the initial property value in
 * the variadic arguments to this macro.
 */
 ...
 * @brief use this to initialize a wil::single_threaded_notifying_property in

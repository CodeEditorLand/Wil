...
#define NT_RETURN_NTSTATUS(status)                                             \
	__NT_RETURN_NTSTATUS(wil::verify_ntstatus(status), #status)

// Always returns a known failure (NTSTATUS) - always logs a var-arg message on
// failure
#define NT_RETURN_NTSTATUS_MSG(status, fmt, ...)                               \
	__NT_RETURN_NTSTATUS_MSG(wil::verify_ntstatus(status), #status, fmt,       \
							 ##__VA_ARGS__)
...
// Conditionally returns failures (NTSTATUS) - always logs a var-arg message on
// failure
#define NT_RETURN_IF_NTSTATUS_FAILED_MSG(status, fmt, ...)                     \
	__WI_SUPPRESS_4127_S do {                                                  \
		const auto __statusRet = wil::verify_ntstatus(status);                 \
		if (FAILED_NTSTATUS(__statusRet)) {                                    \
			__NT_RETURN_NTSTATUS_MSG(__statusRet, #status, fmt,                \
									 ##__VA_ARGS__);                           \
		}                                                                      \
	}                                                                          \
	__WI_SUPPRESS_4127_E while ((void)0, 0)
...
#define NT_RETURN_CAUGHT_EXCEPTION()                                           \
	return __R_FN(Nt_Return_CaughtException)(__R_INFO_ONLY(nullptr))
#define NT_RETURN_CAUGHT_EXCEPTION_MSG(fmt, ...)                               \
	return __R_FN(Nt_Return_CaughtExceptionMsg)(                               \
...
// StatusFromCaughtException is a function that is meant to be called from
// within a catch(...) block.  Internally it re-throws and catches the exception
// to convert it to an NTSTATUS.  If an exception is of an unrecognized type the
...
// }
_Always_(_Post_satisfies_(return < 0)) __declspec(noinline) inline NTSTATUS
	StatusFromCaughtException() WI_NOEXCEPT {
	bool isNormalized = false;
	NTSTATUS status = STATUS_SUCCESS;
	if (details::g_pfnResultFromCaughtExceptionInternal) {
		status = details::g_pfnResultFromCaughtExceptionInternal(nullptr, 0,
																 &isNormalized)
					 .status;
	}
	...
	return wil::details::HrToNtStatus(
		__HRESULT_FROM_WIN32(ERROR_UNHANDLED_EXCEPTION));
}
...
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtExceptionMsg(__R_FN_PARAMS_FULL,
									_Printf_format_string_ PCSTR formatString,
									va_list argList);

namespace __R_NS_NAME {
#ifdef WIL_ENABLE_EXCEPTIONS
__R_DIRECT_METHOD(NTSTATUS, Nt_Return_CaughtException)
(__R_DIRECT_FN_PARAMS_ONLY) WI_NOEXCEPT {
	__R_FN_LOCALS;
	return wil::details::ReportStatus_CaughtException<FailureType::Return>(
...
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtException(__R_FN_PARAMS_FULL,
...
template <>
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtException<FailureType::FailFast>(
...
template <>
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtException<FailureType::Exception>(
		__R_FN_PARAMS_FULL, SupportedExceptions supported) {
	wchar_t message[2048];
	message[0] = L'\0';
	RESULT_NORETURN_RESULT(
...
template <>
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtExceptionMsg<FailureType::FailFast>(
...
template <>
__declspec(noinline) inline NTSTATUS
	ReportStatus_CaughtExceptionMsg<FailureType::Exception>(
		__R_FN_PARAMS_FULL, _Printf_format_string_ PCSTR formatString,
		va_list argList) {
	// Pre-populate the buffer with our message, the exception message will be
	// added to it...

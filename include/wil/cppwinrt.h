...
// WIL and C++/WinRT use two different exception types for communicating HRESULT
// failures. Thus, both libraries need to understand how to translate these
// exception types into the correct HRESULT values at the ABI boundary. Prior to
// C++/WinRT "2.0" this was accomplished by injecting the
// WINRT_EXTERNAL_CATCH_CLAUSE macro - that WIL defines below - into its
// exception handler (winrt::to_hresult). Starting with C++/WinRT "2.0" this
// mechanism has shifted to a global function pointer - winrt_to_hresult_handler
// - that WIL sets automatically when this header is included and
...
// Prior to C++/WinRT "2.0" this header needed to be included before
// 'winrt/base.h' so that our definition of 'WINRT_EXTERNAL_CATCH_CLAUSE' would
// get picked up in the implementation of 'winrt::to_hresult'. This is no longer
// problematic, so only emit an error when using a version of C++/WinRT prior
// to 2.0
static_assert(
	::wil::details::major_version_from_string(CPPWINRT_VERSION) >= 2,
...
#define WINRT_EXTERNAL_CATCH_CLAUSE                                            \
		...
		return winrt::hresult_error(e.GetErrorCode(),                          \
									winrt::to_hstring(e.what()))               \
			.to_abi();                                                         \
	}
#endif
...
// In C++/WinRT 2.0 and beyond, this function pointer exists. In earlier
// versions it does not. It's much easier to avoid linker errors than it is to
// SFINAE on variable existence, so we declare the variable here, but are
// careful not to use it unless the version of C++/WinRT is high enough
extern std::int32_t(__stdcall *winrt_to_hresult_handler)(void *) noexcept;

// The same is true with this function pointer as well, except that the version
// must be 2.X or higher.
extern void(__stdcall *winrt_throw_hresult_handler)(
	uint32_t, char const *, char const *, void *,
...
inline std::int32_t __stdcall winrt_to_hresult(void *returnAddress) noexcept {
	// C++/WinRT only gives us the return address (caller), so pass along an
	// empty 'DiagnosticsInfo' since we don't have accurate file/line/etc.
	// information
	return static_cast<std::int32_t>(
		details::ReportFailure_CaughtException<FailureType::Return>(
...
inline void __stdcall winrt_throw_hresult(
	...
	wil::details::ReportFailure_Hr<FailureType::Log>(
		__R_FN_CALL_FULL __R_COMMA result);
}

inline void WilInitialize_CppWinRT() {
	details::g_pfnResultFromCaughtException_CppWinRt =
		details::ResultFromCaughtException_CppWinRt;
	if constexpr (details::major_version_from_string(CPPWINRT_VERSION) >= 2) {
		...
		if constexpr (details::minor_version_from_string(CPPWINRT_VERSION) >=
					  210122) {
...
// Provides an overload of verify_hresult so that the WIL macros can recognize
// winrt::hresult as a valid "hresult" type.
inline long verify_hresult(winrt::hresult hr) noexcept { return hr; }

// Provides versions of get_abi and put_abi for genericity that directly use
// HSTRING for convenience.
...
inline ::IUnknown *
com_raw_ptr(const winrt::Windows::Foundation::IUnknown &ptr) noexcept {
	return static_cast<::IUnknown *>(winrt::get_abi(ptr));
}

// Needed to power wil::cx_object_from_abi that requires IInspectable
inline ::IInspectable *
com_raw_ptr(const winrt::Windows::Foundation::IInspectable &ptr) noexcept {
...
template <typename T> T convert_from_abi(::IUnknown *from) {
	...
	winrt::check_hresult(
		from->QueryInterface(winrt::guid_of<T>(), winrt::put_abi(to)));
	return to;
}

// For obtaining an object from an interop method on the factory. Example:
// winrt::InputPane inputPane =
// wil::capture_interop<winrt::InputPane>(&IInputPaneInterop::GetForWindow,
// hwnd); If the method produces something different from the factory type:
// winrt::IAsyncAction action = wil::capture_interop<winrt::IAsyncAction,
// winrt::AccountsSettingsPane>(&IAccountsSettingsPaneInterop::ShowAddAccountForWindow,
// hwnd);
template <typename WinRTResult, typename WinRTFactory = WinRTResult,
		  typename Interface, typename... InterfaceArgs, typename... Args>
auto capture_interop(HRESULT (__stdcall Interface::*method)(InterfaceArgs...),
					 Args &&...args) {
	auto interop = winrt::get_activation_factory<WinRTFactory, Interface>();
	return winrt::capture<WinRTResult>(interop, method,
									   std::forward<Args>(args)...);
}

// For obtaining an object from an interop method on an instance. Example:
// winrt::UserActivitySession session =
// wil::capture_interop<winrt::UserActivitySession>(activity,
// &IUserActivityInterop::CreateSessionForWindow, hwnd);
template <typename WinRTResult, typename Interface, typename... InterfaceArgs,
		  typename... Args>
auto capture_interop(winrt::Windows::Foundation::IUnknown const &o,
					 HRESULT (__stdcall Interface::*method)(InterfaceArgs...),
					 Args &&...args) {
	return winrt::capture<WinRTResult>(o.as<Interface>(), method,
									   std::forward<Args>(args)...);
}

/** Holds a reference to the host C++/WinRT module to prevent it from being
unloaded. Normally, this is done by being in an IAsyncOperation coroutine or by
holding a strong reference to a C++/WinRT object hosted in the same module, but
if you have neither, you will need to hold a reference explicitly. For the WRL
equivalent, see wrl_module_reference.
...
~~~~
std::thread([module_ref = wil::winrt_module_reference()]() {
do_background_work(); });

// Don't do this (race condition)
std::thread([]() { wil::winrt_module_reference module_ref; do_background_work();
}); // WRONG
~~~~

Also useful in coroutines that neither capture DLL-hosted COM objects, nor are
themselves DLL-hosted COM objects. (If the coroutine returns IAsyncAction or
captures a get_strong() of its containing WinRT class, then the IAsyncAction or
strong reference will itself keep a strong reference to the host module.)
~~~~
...
struct [[nodiscard]] winrt_module_reference {
	winrt_module_reference() { ++winrt::get_module_lock(); }

	winrt_module_reference(winrt_module_reference const &)
		: winrt_module_reference() {}

	~winrt_module_reference() { --winrt::get_module_lock(); }
};

/** Implements a C++/WinRT class where some interfaces are conditionally
supported.
~~~~
...
Any interface not listed as conditional is assumed to be enabled
unconditionally.
...
struct Version2_or_greater {
	static bool IsEnabled() { return Version2::IsEnabled() ||
Version3::IsEnabled(); }
};

// This implementation supports IMyThing2 if either Version2 or Version3 is
enabled,
...
struct winrt_conditionally_implements : Implements {
	...
	void *find_interface(winrt::guid const &iid) const noexcept override {
		static_assert(sizeof...(Rest) % 2 == 0,
					  "Extra template parameters should come in groups of two");
  ...
  private:
	...
	static bool is_enabled(winrt::guid const &iid) {
		if constexpr (index >= std::tuple_size_v<Tuple>) {
			...
			return (iid ==
					winrt::guid_of<std::tuple_element_t<index + 1, Tuple>>())
	...
	static constexpr void check_no_duplicates() {
		if constexpr (index < upto) {
			static_assert(
				!std::is_same_v<std::tuple_element_t<index, Tuple>,
								std::tuple_element_t<upto, Tuple>>,

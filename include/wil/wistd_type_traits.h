...
//
// Some aspects of STL are core language concepts that should be used from all
// C++ code, regardless of whether exceptions are enabled in the component.
// Common library code that expects to be used from exception-free components
// want these concepts, but including <type_traits> directly introduces friction
// as it requires components not using STL to declare their STL version.  Doing
// so creates ambiguity around whether STL use is safe in a particular component
// and implicitly brings in a long list of headers (including <new>) which can
// create further ambiguity around throwing new support (some routines pulled in
// may expect it).  Secondarily, pulling in these headers also has the potential
// to create naming conflicts or other implied dependencies.
//
// To promote the use of these core language concepts outside of STL-based
// binaries, this file is selectively pulling those concepts *directly* from
// corresponding STL headers.  The corresponding "std::" namespace STL functions
// and types should be preferred over these in code that is bound to STL.  The
// implementation and naming of all functions are taken directly from STL,
// instead using "wistd" (Windows Implementation std) as the namespace.
//
// Routines in this namespace should always be considered a reflection of the
// *current* STL implementation of those routines.  Updates from STL should be
// taken, but no "bugs" should be fixed here.
//
// New, exception-based code should not use this namespace, but instead should
// prefer the std:: implementation. Only code that is not exception-based and
// libraries that expect to be utilized across both exception and non-exception
// based code should utilize this functionality.

#ifndef _WISTD_TYPE_TRAITS_H_
#define _WISTD_TYPE_TRAITS_H_

// DO NOT add *any* additional includes to this file -- there should be no
...
inline __WI_LIBCPP_CONSTEXPR_AFTER_CXX14
	__WI_LIBCPP_NO_CFI __WI_LIBCPP_INLINE_VISIBILITY _Tp *
...
template <class _Tp>
inline __WI_LIBCPP_NO_CFI __WI_LIBCPP_INLINE_VISIBILITY _Tp *
addressof(_Tp &__x) WI_NOEXCEPT {
...
template <class _Tp> struct __WI_LIBCPP_TEMPLATE_VIS remove_cv {
	typedef
		typename remove_volatile<typename remove_const<_Tp>::type>::type type;
};
#if __WI_LIBCPP_STD_VER > 11
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_void
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS __is_nullptr_t
	: public __is_nullptr_t_impl<typename remove_cv<_Tp>::type> {};

#if __WI_LIBCPP_STD_VER > 11
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_null_pointer
	: public __is_nullptr_t_impl<typename remove_cv<_Tp>::type> {};
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_integral
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_floating_point
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_pointer
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_union
...
// public false_type {}; template <class _Tp, class _Up> struct
// __libcpp_is_member_function_pointer<_Tp _Up::*> : public is_function<_Tp> {};
//

template <class _Mp, bool _IsMemberFunctionPtr, bool _IsMemberObjectPtr>
struct __member_pointer_traits_imp { // forward declaration; specializations
									 // later
};
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_member_pointer
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_compound
...
// MSVC has issues compiling some source code that uses the libc++ definition of
// 'declval'
#ifdef _MSC_VER
...
struct __libcpp_is_signed_impl
	: public __WI_LIBCPP_BOOL_CONSTANT(_Tp(-1) < _Tp(0)) {};
...
struct __libcpp_is_unsigned_impl
	: public __WI_LIBCPP_BOOL_CONSTANT(_Tp(0) < _Tp(-1)) {};
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS rank<_Tp[]>
	: public integral_constant<size_t, rank<_Tp>::value + 1> {};
template <class _Tp, size_t _Np>
struct __WI_LIBCPP_TEMPLATE_VIS rank<_Tp[_Np]>
	: public integral_constant<size_t, rank<_Tp>::value + 1> {};
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS extent<_Tp[], 0>
	: public integral_constant<size_t, 0> {};
template <class _Tp, unsigned _Ip>
struct __WI_LIBCPP_TEMPLATE_VIS extent<_Tp[], _Ip>
	: public integral_constant<size_t, extent<_Tp, _Ip - 1>::value> {};
template <class _Tp, size_t _Np>
struct __WI_LIBCPP_TEMPLATE_VIS extent<_Tp[_Np], 0>
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_abstract
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS __libcpp_is_final
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_final
	: public integral_constant<bool, __is_final(_Tp)> {};
#endif
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_aggregate
...
#if __WI_HAS_FEATURE_IS_CONVERTIBLE_TO &&                                      \
	!defined(__WI_LIBCPP_USE_IS_CONVERTIBLE_FALLBACK)
...
// has_unique_object_representations
	...
	defined(__WI_LIBCPP_HAS_UNIQUE_OBJECT_REPRESENTATIONS)
...
#if __WI_LIBCPP_STD_VER > 11 && !defined(__WI_LIBCPP_HAS_NO_VARIABLE_TEMPLATES)
template <class _Tp>
__WI_LIBCPP_INLINE_VAR __WI_LIBCPP_CONSTEXPR bool
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS alignment_of
...
struct __has_pod_with_align
	: public integral_constant<
		  bool, !is_same<typename __find_pod<__all_types, _Align>::type,
						 void>::value> {};
...
template <size_t _Len, size_t _Align,
		  bool = __has_pod_with_align<_Align>::value>
...
struct __WI_LIBCPP_TEMPLATE_VIS aligned_storage
	: public __aligned_storage<_Len, _Align> {};
...
template <class _Tp> struct __WI_LIBCPP_TEMPLATE_VIS make_unsigned {
	typedef
		typename __apply_cv<_Tp, typename __make_unsigned<
									 typename remove_cv<_Tp>::type>::type>::type
			type;
};
...
struct __WI_LIBCPP_TEMPLATE_VIS common_type {
  public:
	typedef
		typename common_type<typename common_type<_Tp, _Up>::type, _Vp>::type
			type;
};
...
template <class _Tp, class _Up, class _DTp = typename decay<_Tp>::type,
		  class _DUp = typename decay<_Up>::type>
using __common_type2 =
	typename conditional<is_same<_Tp, _DTp>::value && is_same<_Up, _DUp>::value,
						 __common_type2_imp<_Tp, _Up>,
						 common_type<_DTp, _DUp>>::type;
...
template <class _Tp>
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR _Tp &&
...
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR_AFTER_CXX17 _T1
...
template <class _Tp>
inline __WI_LIBCPP_INLINE_VISIBILITY _Tp &
forward(typename remove_reference<_Tp>::type &__t) WI_NOEXCEPT {
	return __t;
}

template <class _T1, class _T2 = _T1>
inline __WI_LIBCPP_INLINE_VISIBILITY _T1 exchange(_T1 &__obj,
...
template <> struct __WI_LIBCPP_TEMPLATE_VIS less<void> {
	template <class _T1, class _T2>
	__WI_LIBCPP_NODISCARD_ATTRIBUTE
		__WI_LIBCPP_CONSTEXPR_AFTER_CXX11 __WI_LIBCPP_INLINE_VISIBILITY auto
		operator()(_T1 &&__t, _T2 &&__u) const
		__WI_NOEXCEPT_(noexcept(wistd::forward<_T1>(__t) <
								wistd::forward<_T2>(__u)))
...
template <class _Tp>
inline __WI_LIBCPP_INLINE_VISIBILITY typename decay<_Tp>::type
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, ...), true,
								   false> {
	typedef _Class _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, ...);
...
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, _P2), true,
								   false> {
	typedef _Class _ClassType;
	typedef _Rp _ReturnType;
...
template <class _Rp, class _Class, class _P0>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, ...) const, true,
								   false> {
	typedef _Class const _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, ...);
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1) const, true,
								   false> {
	typedef _Class const _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1);
...
template <class _Rp, class _Class, class _P0, class _P1, class _P2>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, _P2) const, true,
								   false> {
	typedef _Class const _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, _P2);
...
template <class _Rp, class _Class, class _P0, class _P1, class _P2>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, _P2, ...) const,
								   true, false> {
	typedef _Class const _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, _P2, ...);
...
template <class _Rp, class _Class, class _P0>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, ...) volatile, true,
								   false> {
	typedef _Class volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, ...);
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1) volatile, true,
								   false> {
	typedef _Class volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1);
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, ...) volatile,
								   true, false> {
	typedef _Class volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, ...);
...
template <class _Rp, class _Class, class _P0, class _P1, class _P2>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1, _P2) volatile,
								   true, false> {
	typedef _Class volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, _P2);
...
template <class _Rp, class _Class>
struct __member_pointer_traits_imp<_Rp (_Class::*)() const volatile, true,
								   false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)();
...
template <class _Rp, class _Class, class _P0>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0) const volatile, true,
								   false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0);
...
template <class _Rp, class _Class, class _P0>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, ...) const volatile,
								   true, false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, ...);
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<_Rp (_Class::*)(_P0, _P1) const volatile,
								   true, false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1);
...
template <class _Rp, class _Class, class _P0, class _P1>
struct __member_pointer_traits_imp<
	_Rp (_Class::*)(_P0, _P1, ...) const volatile, true, false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, ...);
};
...
template <class _Rp, class _Class, class _P0, class _P1, class _P2>
struct __member_pointer_traits_imp<
	_Rp (_Class::*)(_P0, _P1, _P2) const volatile, true, false> {
	typedef _Class const volatile _ClassType;
	typedef _Rp _ReturnType;
	typedef _Rp(_FnType)(_P0, _P1, _P2);
};
...
class __result_of<_Fn(_A0, _A1, _A2), true, false> {
  public:
	typedef decltype(declval<_Fn>()(declval<_A0>(), declval<_A1>(),
									declval<_A2>())) type;
};
...
class __result_of<_Fn(_Tp), false, true> // _Fn must be member pointer
	: public __result_of_mp<typename remove_reference<_Fn>::type, _Tp,
							is_member_function_pointer<
								typename remove_reference<_Fn>::type>::value> {
};
...
class __result_of<_Fn(_Tp, _A0, _A1, _A2), false,
				  true> // _Fn must be member pointer
	: public __result_of_mp<typename remove_reference<_Fn>::type, _Tp,
							is_member_function_pointer<
								typename remove_reference<_Fn>::type>::value> {
};
	...
	: public __result_of<
		  ...
		  is_class<typename remove_reference<_Fn>::type>::value ||
			  is_function<typename remove_pointer<
				  typename remove_reference<_Fn>::type>::type>::value,
...
#if !defined(__WI_LIBCPP_CXX03_LANG) &&                                        \
	(!__WI_HAS_FEATURE_IS_CONSTRUCTIBLE ||                                     \
...
template <class _To, class _From> struct __is_invalid_base_to_derived_cast {
	...
	static const bool value =
		__lazy_and<__lazy_not<is_same<_RawFrom, _RawTo>>,
...
struct __is_invalid_lvalue_to_rvalue_cast<_ToRef &&, _FromRef &> {
	...
	static const bool value =
		__lazy_and<__lazy_not<is_function<_RawTo>>,
				   __lazy_or<is_same<_RawFrom, _RawTo>,
							 is_base_of<_RawTo, _RawFrom>>>::value;
};
...
struct __is_constructible_helper {
	...
	static integral_constant<
		bool, !__is_invalid_base_to_derived_cast<_To, _From>::value &&
...
template <class _Tp, class... _Args> struct __libcpp_is_constructible {
	...
	typedef decltype(__is_constructible_helper::__test_nary<_Tp, _Args...>(
		0)) type;
};
...
template <class _Tp, class _A0>
decltype((_Tp(declval<_A0>()), true_type())) __is_constructible1_test(_Tp &,
																	  _A0 &);
...
struct __is_constructible0_imp // false, _Tp is not a scalar
	: public common_type<decltype(__is_constructible0_test(
		  declval<_Tp &>()))>::type {};

template <bool, class _Tp, class _A0>
struct __is_constructible1_imp // false, _Tp is not a scalar
	: public common_type<decltype(__is_constructible1_test(
		  declval<_Tp &>(), declval<_A0 &>()))>::type {};

template <bool, class _Tp, class _A0, class _A1>
struct __is_constructible2_imp // false, _Tp is not a scalar
	: public common_type<decltype(__is_constructible2_test(
		  declval<_Tp &>(), declval<_A0>(), declval<_A1>()))>::type {};

template <bool, class _Tp, class _A0, class _A1, class _A2>
struct __is_constructible3_imp // false, _Tp is not a scalar
	: public common_type<decltype(__is_constructible3_test(
		  declval<_Tp &>(), declval<_A0>(), declval<_A1>(),
		  declval<_A2>()))>::type {};
...
template <class _Tp, class _A0 = __is_construct::__nat,
		  class _A1 = __is_construct::__nat, class _A2 = __is_construct::__nat>
struct __WI_LIBCPP_TEMPLATE_VIS is_constructible
	: public __is_constructible3_void_check<
		  is_void<_Tp>::value || is_abstract<_Tp>::value ||
...
struct __WI_LIBCPP_TEMPLATE_VIS
	is_constructible<_Tp, __is_construct::__nat, __is_construct::__nat>
...
template <class _Tp, class _A0>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_constructible<_Tp, _A0, __is_construct::__nat>
	: public __is_constructible1_void_check<
		  is_void<_Tp>::value || is_abstract<_Tp>::value ||
			  is_function<_Tp>::value || is_void<_A0>::value,
		  _Tp, _A0> {};
...
template <class _Tp, class _A0, class _A1>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_constructible<_Tp, _A0, _A1, __is_construct::__nat>
	: public __is_constructible2_void_check<
		  is_void<_Tp>::value || is_abstract<_Tp>::value ||
			  is_function<_Tp>::value || is_void<_A0>::value ||
			  is_void<_A1>::value,
		  _Tp, _A0, _A1> {};
...
#if __WI_LIBCPP_STD_VER > 11 &&                                                \
	!defined(__WI_LIBCPP_HAS_NO_VARIABLE_TEMPLATES) &&                         \
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_trivially_constructible<
	_Tp, __is_construct::__nat, __is_construct::__nat>
	: integral_constant<bool, __is_trivially_constructible(_Tp)> {};

template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_trivially_constructible<_Tp, _Tp, __is_construct::__nat>
	: integral_constant<bool, __is_trivially_constructible(_Tp, _Tp)> {};

template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_trivially_constructible<_Tp, const _Tp &, __is_construct::__nat>
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_trivially_default_constructible
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_trivially_destructible<_Tp[]>
	: public false_type {};
...
#if !defined(__WI_LIBCPP_HAS_NO_NOEXCEPT) ||                                   \
	(__WI_GNUC_VER >= 407 && __cplusplus >= 201103L)
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_nothrow_constructible<_Tp, __is_construct::__nat, __is_construct::__nat>
#if __WI_HAS_FEATURE_HAS_NOTHROW_CONSTRUCTOR
	: integral_constant<bool, __has_nothrow_constructor(_Tp)>
#else
	: integral_constant<bool, is_scalar<_Tp>::value>
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_nothrow_constructible<_Tp, _Tp, __is_construct::__nat>
#if __WI_HAS_FEATURE_HAS_NOTHROW_COPY
	: integral_constant<bool, __has_nothrow_copy(_Tp)>
#else
	: integral_constant<bool, is_scalar<_Tp>::value>
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS
	is_nothrow_constructible<_Tp, const _Tp &, __is_construct::__nat>
#if __WI_HAS_FEATURE_HAS_NOTHROW_COPY
	: integral_constant<bool, __has_nothrow_copy(_Tp)>
#else
	: integral_constant<bool, is_scalar<_Tp>::value>
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_nothrow_default_constructible
...
template <class _Tp>
struct __WI_LIBCPP_TEMPLATE_VIS is_nothrow_destructible
	: public __libcpp_nothrow_destructor<
		  typename remove_all_extents<_Tp>::type> {};
...
struct __WI_LIBCPP_TEMPLATE_VIS is_pod
	: public integral_constant<
		  bool, is_trivially_default_constructible<_Tp>::value &&
					is_trivially_copy_constructible<_Tp>::value &&
...
#else
	: integral_constant<
		  bool, is_scalar<typename remove_all_extents<_Tp>::type>::value ||
...
struct __WI_LIBCPP_TEMPLATE_VIS is_trivially_copyable
	...
	: public integral_constant<bool, !is_volatile<_Tp>::value &&
...
inline __WI_LIBCPP_INLINE_VISIBILITY auto __invoke(_Fp &&__f, _A0 &&__a0,
												   _Args &&...__args)
	__WI_LIBCPP_INVOKE_RETURN((wistd::forward<_A0>(__a0).*
							   __f)(wistd::forward<_Args>(__args)...))
	...
	auto __invoke(_Fp &&__f, _A0 &&__a0, _Args &&...__args)
		__WI_LIBCPP_INVOKE_RETURN((__a0.get().*
								   __f)(wistd::forward<_Args>(__args)...))
	...
	inline __WI_LIBCPP_INLINE_VISIBILITY
	auto __invoke(_Fp &&__f, _A0 &&__a0) __WI_LIBCPP_INVOKE_RETURN(
		wistd::forward<_A0>(__a0).*__f)

		template <class _Fp, class _A0, class = __enable_if_bullet4<_Fp, _A0>>
		inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR
	auto __invoke_constexpr(_Fp &&__f, _A0 &&__a0) __WI_LIBCPP_INVOKE_RETURN(
		wistd::forward<_A0>(__a0).*__f)

		template <class _Fp, class _A0, class = __enable_if_bullet5<_Fp, _A0>>
		inline __WI_LIBCPP_INLINE_VISIBILITY
	auto __invoke(_Fp &&__f,
				  _A0 &&__a0) __WI_LIBCPP_INVOKE_RETURN(__a0.get().*__f)

		template <class _Fp, class _A0, class = __enable_if_bullet5<_Fp, _A0>>
		inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR
	auto __invoke_constexpr(_Fp &&__f,
							_A0 &&__a0) __WI_LIBCPP_INVOKE_RETURN(__a0.get().*
																  __f)
	...
	inline __WI_LIBCPP_INLINE_VISIBILITY
	auto __invoke(_Fp &&__f, _Args &&...__args) __WI_LIBCPP_INVOKE_RETURN(
		wistd::forward<_Fp>(__f)(wistd::forward<_Args>(__args)...))

		template <class _Fp, class... _Args>
		inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR
	auto __invoke_constexpr(_Fp &&__f, _Args &&...__args)
		__WI_LIBCPP_INVOKE_RETURN(
			wistd::forward<_Fp>(__f)(wistd::forward<_Args>(__args)...))
...
#undef __WI_LIBCPP_INVOKE_RETURN
	...
	// void, or incomplete array types as required by the standard.
...
inline __WI_LIBCPP_INLINE_VISIBILITY
	...
	swap_wil(_Tp &__x, _Tp &__y)
		__WI_NOEXCEPT_(is_nothrow_move_constructible<_Tp>::value
						   &&is_nothrow_move_assignable<_Tp>::value) {
...
inline __WI_LIBCPP_INLINE_VISIBILITY _ForwardIterator2
swap_ranges_wil(_ForwardIterator1 __first1, _ForwardIterator1 __last1,
...
inline __WI_LIBCPP_INLINE_VISIBILITY
	...
	swap_wil(_Tp (&__a)[_Np], _Tp (&__b)[_Np])
		__WI_NOEXCEPT_(__is_nothrow_swappable<_Tp>::value) {
	wistd::swap_ranges_wil(__a, __a + _Np, __b);
}

template <class _ForwardIterator1, class _ForwardIterator2>
inline __WI_LIBCPP_INLINE_VISIBILITY void iter_swap_wil(_ForwardIterator1 __a,
														_ForwardIterator2 __b)
	//                                  __WI_NOEXCEPT_(__WI_NOEXCEPT_(swap_wil(*__a,
	//                                  *__b)))
...
struct __nothrow_swappable_with {
	static const bool value =
		...
		noexcept(swap_wil(declval<_Tp>(), declval<_Up>())) && noexcept(
			swap_wil(declval<_Up>(), declval<_Tp>()));
#else
		false;
#endif
};
...
struct __WI_LIBCPP_TEMPLATE_VIS is_nothrow_swappable_with
	: public integral_constant<
		  bool, __detail::__nothrow_swappable_with<_Tp, _Up>::value> {};
...
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR int
__convert_to_integral(int __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR unsigned
__convert_to_integral(unsigned __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR long
__convert_to_integral(long __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR unsigned long
__convert_to_integral(unsigned long __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR long long
__convert_to_integral(long long __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR unsigned long long
...
#ifndef __WI_LIBCPP_HAS_NO_INT128
inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR __int128_t
__convert_to_integral(__int128_t __val) {
	return __val;
}

inline __WI_LIBCPP_INLINE_VISIBILITY __WI_LIBCPP_CONSTEXPR __uint128_t
...
template <class _Tp> struct __has_operator_addressof_member_imp {
	...
	static auto __test(int) ->
		typename __select_2nd<decltype(declval<_Up>().operator&()),
							  true_type>::type;
...
template <class _Tp> struct __has_operator_addressof_free_imp {
	template <class _Up>
	static auto __test(int) ->
		typename __select_2nd<decltype(operator&(declval<_Up>())),
							  true_type>::type;
	template <class> static auto __test(long) -> false_type;

...
template <class _Tp, class _Dp> struct __pointer_type {
	using type = typename __pointer_type_imp::__pointer_type<
		_Tp, typename remove_reference<_Dp>::type>::type;
};
...
struct __compressed_pair_elem {
	...
	__WI_LIBCPP_INLINE_VISIBILITY constexpr __compressed_pair_elem()
		: __value_() {}

	template <class _Up, class = typename enable_if<
							 !is_same<__compressed_pair_elem,
									  typename decay<_Up>::type>::value>::type>
	__WI_LIBCPP_INLINE_VISIBILITY constexpr explicit __compressed_pair_elem(
		_Up &&__u)
		: __value_(wistd::forward<_Up>(__u)) {}

	// NOTE: Since we have not added 'tuple' to 'wistd', the 'piecewise'
	// constructor has been removed
#else
	__WI_LIBCPP_INLINE_VISIBILITY __compressed_pair_elem() : __value_() {}
	__WI_LIBCPP_INLINE_VISIBILITY
	__compressed_pair_elem(_ParamT __p)
		: __value_(wistd::forward<_ParamT>(__p)) {}
#endif
...
class __declspec(empty_bases) __compressed_pair
  ...
  public:
	...
	template <class _Tp, typename enable_if<!is_same<typename decay<_Tp>::type,
											...
											bool>::type = true>
	__WI_LIBCPP_INLINE_VISIBILITY constexpr explicit __compressed_pair(
		_Tp &&__t)
		: _Base1(wistd::forward<_Tp>(__t)), _Base2() {}

	template <class _Tp>
	__WI_LIBCPP_INLINE_VISIBILITY constexpr __compressed_pair(__second_tag,
															  _Tp &&__t)
		: _Base1(), _Base2(wistd::forward<_Tp>(__t)) {}

	template <class _U1, class _U2>
	__WI_LIBCPP_INLINE_VISIBILITY constexpr __compressed_pair(_U1 &&__t1,
															  _U2 &&__t2)
	...
	__WI_LIBCPP_INLINE_VISIBILITY explicit __compressed_pair(_T1 __t1)
...
// Provide both 'swap_wil' and 'swap' since we now have two ADL scenarios that
// we need to work
template <class _T1, class _T2>
inline __WI_LIBCPP_INLINE_VISIBILITY void swap(__compressed_pair<_T1, _T2> &__x,
...
template <class _T1, class _T2>
inline __WI_LIBCPP_INLINE_VISIBILITY void
swap_wil(__compressed_pair<_T1, _T2> &__x, __compressed_pair<_T1, _T2> &__y)
	__WI_NOEXCEPT_(__is_nothrow_swappable<_T1>::value
					   &&__is_nothrow_swappable<_T2>::value) {
	__x.swap(__y);
}
...
template <class _Tp> struct __WI_LIBCPP_TEMPLATE_VIS default_delete {
...
#ifndef __WI_LIBCPP_CXX03_LANG
	__WI_LIBCPP_INLINE_VISIBILITY constexpr default_delete()
		WI_NOEXCEPT = default;
#else
	...
	__WI_LIBCPP_INLINE_VISIBILITY void
	operator()(_Tp *__ptr) const WI_NOEXCEPT {
...
template <class _Tp> struct __WI_LIBCPP_TEMPLATE_VIS default_delete<_Tp[]> {
  ...
  public:
	...
	template <class _Up>
	__WI_LIBCPP_INLINE_VISIBILITY typename _EnableIfConvertible<_Up>::type
...
class __WI_LIBCPP_TEMPLATE_VIS unique_ptr {
  ...
  private:
	...
	using _EnableIfDeleterConstructible = typename enable_if<
		is_constructible<deleter_type, _ArgType>::value>::type;
  ...
  public:
	...
	__WI_LIBCPP_INLINE_VISIBILITY constexpr unique_ptr() WI_NOEXCEPT
		: __ptr_(pointer()) {}

	template <bool _Dummy = true,
			  class = _EnableIfDeleterDefaultConstructible<_Dummy>>
	__WI_LIBCPP_INLINE_VISIBILITY constexpr unique_ptr(nullptr_t) WI_NOEXCEPT
		: __ptr_(pointer()) {}

	template <bool _Dummy = true,
			  class = _EnableIfDeleterDefaultConstructible<_Dummy>>
	__WI_LIBCPP_INLINE_VISIBILITY explicit unique_ptr(pointer __p) WI_NOEXCEPT
		: __ptr_(__p) {}
	...
	__WI_LIBCPP_INLINE_VISIBILITY unique_ptr &
  ...
  public:
	...
	__WI_LIBCPP_INLINE_VISIBILITY
	operator __rv<unique_ptr>() { return __rv<unique_ptr>(*this); }
	...
	__WI_LIBCPP_NODISCARD_ATTRIBUTE __WI_LIBCPP_INLINE_VISIBILITY pointer
	...
	__WI_LIBCPP_INLINE_VISIBILITY
	deleter_type &get_deleter() WI_NOEXCEPT { return __ptr_.second(); }
	__WI_LIBCPP_NODISCARD_ATTRIBUTE __WI_LIBCPP_INLINE_VISIBILITY const
		deleter_type &
		get_deleter() const WI_NOEXCEPT {
	...
	__WI_LIBCPP_INLINE_VISIBILITY
	void swap(unique_ptr &__u) WI_NOEXCEPT { __ptr_.swap(__u.__ptr_); }
};
...
class __WI_LIBCPP_TEMPLATE_VIS unique_ptr<_Tp[], _Dp> {
  ...
  public:
	...
	__WI_LIBCPP_INLINE_VISIBILITY explicit unique_ptr(_Pp __p) WI_NOEXCEPT
		: __ptr_(__p) {}
  ...
  private:
	...
	unique_ptr(
		...
		typename enable_if<is_convertible<_Up, pointer>::value, __nat>::type =
			__nat());
...
inline __WI_LIBCPP_INLINE_VISIBILITY
	...
	swap(unique_ptr<_Tp, _Dp> &__x, unique_ptr<_Tp, _Dp> &__y) WI_NOEXCEPT {
	__x.swap(__y);
}
...
inline __WI_LIBCPP_INLINE_VISIBILITY
	typename enable_if<__is_swappable<_Dp>::value, void>::type
	swap_wil(unique_ptr<_Tp, _Dp> &__x, unique_ptr<_Tp, _Dp> &__y) WI_NOEXCEPT {
	__x.swap(__y);
}

template <class _T1, class _D1, class _T2, class _D2>
inline __WI_LIBCPP_INLINE_VISIBILITY bool
...
template <class _Tp, class _Dp>
inline __WI_LIBCPP_INLINE_VISIBILITY unique_ptr<_Tp, _Dp>

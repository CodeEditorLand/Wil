...
// If we're using c++20, then <compare> must be included to use the string
// ordinal functions
...
struct weak_ordering {
	...
	[[nodiscard]] friend constexpr bool operator==(const weak_ordering left,
	...
	operator==(std::nullptr_t, const weak_ordering right) noexcept {
		return right == 0;
	}
...
namespace wil {
//! Strictly a function of the file system but this is the value for all known
//! file system, NTFS, FAT. CDFs has a limit of 254.
...
//! 32743 Character length not including the null. This is a system defined
//! limit. The 24 is for the expansion of the roots from "C:" to
//! "\Device\HarddiskVolume4" It will be 25 when there are more than 9 disks.
...
//
// Indentifiers require a locale-less (ordinal), and often case-insensitive,
// comparison (filenames, registry keys, XML node names, etc). DO NOT use
// locale-sensitive (lexical) comparisons for resource identifiers (e.g.wcs*()
...
CompareStringOrdinal(std::wstring_view left, std::wstring_view right,
					 bool caseInsensitive) WI_NOEXCEPT {
	// Casting from size_t (unsigned) to int (signed) should be safe from
	// overrun to a negative, merely truncating the string. CompareStringOrdinal
	// should be resilient to negatives.
...
[[nodiscard]] inline wistd::weak_ordering
compare_string_ordinal(std::wstring_view left, std::wstring_view right,
					   bool caseInsensitive) WI_NOEXCEPT {
...
long long const one_hour =
	10000000LL * 60 * 60; // 36000000000  or 36000000000LL
long long const one_day =
	10000000LL * 60 * 60 * 24; // 864000000000 or 864000000000LL
};							   // namespace filetime_duration
...
__WI_CONSTEXPR_BIT_CAST FILETIME from_int64(unsigned long long i64)
	WI_NOEXCEPT {
#if __cpp_lib_bit_cast >= 201806L
...
__WI_CONSTEXPR_BIT_CAST FILETIME add(_In_ FILETIME const &ft,
									 long long delta100ns) WI_NOEXCEPT {
...
/// Convert time as units of 100 nanoseconds to milliseconds. Fractional
/// milliseconds are truncated.
constexpr unsigned long long
convert_100ns_to_msec(unsigned long long time100ns) WI_NOEXCEPT {
	return time100ns / filetime_duration::one_millisecond;
}

/// Convert time as milliseconds to units of 100 nanoseconds.
constexpr unsigned long long
convert_msec_to_100ns(unsigned long long timeMsec) WI_NOEXCEPT {
	return timeMsec * filetime_duration::one_millisecond;
}

#if defined(_APISETREALTIME_) && (_WIN32_WINNT >= _WIN32_WINNT_WIN7)
/// Returns the current unbiased interrupt-time count, in units of 100
/// nanoseconds. The unbiased interrupt-time count does not include time the
/// system spends in sleep or hibernation.
///
/// This API avoids prematurely shortcircuiting timing loops due to system
/// sleep/hibernation.
///
/// This is equivalent to GetTickCount64() except it returns units of 100
/// nanoseconds instead of milliseconds, and it doesn't include time the system
/// spends in sleep or hibernation. For example
///
...
///
/// @note This is identical to QueryUnbiasedInterruptTime() but returns the
/// value as a return value (rather than an out parameter).
/// @see
...
/// Returns the current unbiased interrupt-time count, in units of milliseconds.
/// The unbiased interrupt-time count does not include time the system spends in
...
constexpr auto rect_contains_point(rect_type const &rect,
...
constexpr rect_type rect_from_size(length_type x, length_type y,
...
HRESULT AdaptFixedSizeToAllocatedResult(
	string_type &result,
	...
	size_t valueLengthNeededWithNull{}; // callback returns the number of
										// characters needed including the null
										// terminator.
	RETURN_IF_FAILED_EXPECTED(
		callback(value, ARRAYSIZE(value), &valueLengthNeededWithNull));
	...
	if (valueLengthNeededWithNull <= ARRAYSIZE(value)) {
		...
		// Did not fit in the stack allocated buffer, need to do 2 phase
		// construction. May need to loop more than once if external conditions
		// cause the value to change.
		size_t bufferLength;
		do {
			bufferLength = valueLengthNeededWithNull;
			// bufferLength includes the null so subtract that as make() will
			// add space for it.
			RETURN_IF_FAILED(maker.make(nullptr, bufferLength - 1));

			RETURN_IF_FAILED_EXPECTED(callback(maker.buffer(), bufferLength,
											   &valueLengthNeededWithNull));
			WI_ASSERT(valueLengthNeededWithNull > 0);

			// If the value shrunk, then adjust the string to trim off the
			// excess buffer.
			if (valueLengthNeededWithNull < bufferLength) {
				RETURN_IF_FAILED(
					maker.trim_at_existing_null(valueLengthNeededWithNull - 1));
			}
...
/** Expands the '%' quoted environment variables in 'input' using
 * ExpandEnvironmentStringsW(); */
template <typename string_type, size_t stackBufferLength = 256>
HRESULT ExpandEnvironmentStringsW(_In_ PCWSTR input,
	...
	return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		...
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			*valueLengthNeededWithNul = ::ExpandEnvironmentStringsW(
				input, value, static_cast<DWORD>(valueLength));
...
/** Searches for a specified file in a specified path using
 * ExpandEnvironmentStringsW(); */
template <typename string_type, size_t stackBufferLength = 256>
HRESULT SearchPathW(_In_opt_ PCWSTR path, _In_ PCWSTR fileName,
	...
	return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		...
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			*valueLengthNeededWithNul =
				::SearchPathW(path, fileName, extension,
							  static_cast<DWORD>(valueLength), value, nullptr);

			if (*valueLengthNeededWithNul == 0) {
				// ERROR_FILE_NOT_FOUND is an expected return value for
				// SearchPathW
				const HRESULT searchResult =
					HRESULT_FROM_WIN32(::GetLastError());
				RETURN_HR_IF_EXPECTED(
					searchResult,
					searchResult == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND));
				RETURN_IF_FAILED(searchResult);
			}

			// AdaptFixedSizeToAllocatedResult expects that the length will
			// always include the NUL. If the result is copied to the buffer,
			// SearchPathW returns the length of copied string, WITHOUT the NUL.
			// If the buffer is too small to hold the result, SearchPathW
			// returns the length of the required buffer WITH the nul.
...
template <typename string_type, size_t stackBufferLength = 256>
HRESULT QueryFullProcessImageNameW(HANDLE processHandle, _In_ DWORD flags,
								   string_type &result) WI_NOEXCEPT {
	return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		result,
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			...
			BOOL const success = ::QueryFullProcessImageNameW(
				processHandle, flags, value, &lengthToUse);
			...
			*valueLengthNeededWithNul =
				success ? (static_cast<size_t>(lengthToUse) + 1)
						: (static_cast<size_t>(lengthToUse) * 2);
			return S_OK;
		});
}
...
template <typename string_type, size_t stackBufferLength = 256>
HRESULT ExpandEnvAndSearchPath(_In_ PCWSTR input,
							   string_type &result) WI_NOEXCEPT {
	wil::unique_cotaskmem_string expandedName;
	RETURN_IF_FAILED(
		(wil::ExpandEnvironmentStringsW<string_type, stackBufferLength>(
			input, expandedName)));

	// ERROR_FILE_NOT_FOUND is an expected return value for SearchPathW
	const HRESULT searchResult =
		(wil::SearchPathW<string_type, stackBufferLength>(
			nullptr, expandedName.get(), nullptr, result));
	RETURN_HR_IF_EXPECTED(
		searchResult, searchResult == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND));
	RETURN_IF_FAILED(searchResult);
...
inline HRESULT GetEnvironmentVariableW(_In_ PCWSTR key,
									   string_type &result) WI_NOEXCEPT {
	return wil::AdaptFixedSizeToAllocatedResult<string_type,
												initialBufferLength>(
		result,
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			_Out_ size_t *valueLengthNeededWithNul) -> HRESULT {
			...
			// string and its terminating null character and the contents of
			...
			*valueLengthNeededWithNul = ::GetEnvironmentVariableW(
				key, value, static_cast<DWORD>(valueLength));
			RETURN_LAST_ERROR_IF_EXPECTED((*valueLengthNeededWithNul == 0) &&
 ...
 */
template <typename string_type, size_t initialBufferLength = 128>
HRESULT TryGetEnvironmentVariableW(_In_ PCWSTR key,
								   string_type &result) WI_NOEXCEPT {
	const auto hr =
		wil::GetEnvironmentVariableW<string_type, initialBufferLength>(key,
																	   result);
	RETURN_HR_IF(hr, FAILED(hr) &&
						 (hr != HRESULT_FROM_WIN32(ERROR_ENVVAR_NOT_FOUND)));
	return S_OK;
}

/** Retrieves the fully qualified path for the file containing the specified
module loaded by a given process. Note GetModuleFileNameExW is a macro.*/
template <typename string_type, size_t initialBufferLength = 128>
HRESULT GetModuleFileNameExW(_In_opt_ HANDLE process, _In_opt_ HMODULE module,
							 string_type &path) WI_NOEXCEPT {
	auto adapter = [&](_Out_writes_(valueLength) PWSTR value,
					   size_t valueLength,
					   _Out_ size_t *valueLengthNeededWithNul) -> HRESULT {
		DWORD copiedCount{};
		...
		if (process != nullptr) {
			// GetModuleFileNameExW truncates and provides no error or other
			// indication it has done so. The only way to be sure it didn't
			// truncate is if it didn't need the whole buffer. The count copied
			// to the buffer includes the nul-character as well.
			copiedCount = ::GetModuleFileNameExW(
				process, module, value, static_cast<DWORD>(valueLength));
			valueUsedWithNul = static_cast<size_t>(copiedCount) + 1;
			copyFailed = (0 == copiedCount);
			copySucceededWithNoTruncation =
				!copyFailed && (copiedCount < valueLength - 1);
		} else {
			// In cases of insufficient buffer, GetModuleFileNameW will return a
			// value equal to lengthWithNull and set the last error to
			// ERROR_INSUFFICIENT_BUFFER. The count returned does not include
			// the nul-character
			copiedCount = ::GetModuleFileNameW(module, value,
											   static_cast<DWORD>(valueLength));
			valueUsedWithNul = static_cast<size_t>(copiedCount) + 1;
			copyFailed = (0 == copiedCount);
			copySucceededWithNoTruncation =
				!copyFailed && (copiedCount < valueLength);
		}

		RETURN_LAST_ERROR_IF(copyFailed);

		// When the copy truncated, request another try with more space.
		*valueLengthNeededWithNul = copySucceededWithNoTruncation
										? valueUsedWithNul
										: (valueLength * 2);
	...
	return wil::AdaptFixedSizeToAllocatedResult<string_type,
												initialBufferLength>(
		path, wistd::move(adapter));
}

/** Retrieves the fully qualified path for the file that contains the specified
module. The module must have been loaded by the current process. The path
returned will use the same format that was specified when the module was loaded.
Therefore, the path can be a long or short file name, and can have the prefix
'\\?\'. */
template <typename string_type, size_t initialBufferLength = 128>
HRESULT GetModuleFileNameW(HMODULE module, string_type &path) WI_NOEXCEPT {
	return wil::GetModuleFileNameExW<string_type, initialBufferLength>(
		nullptr, module, path);
}
...
HRESULT GetSystemDirectoryW(string_type &result) WI_NOEXCEPT {
	return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
		...
		[&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
			_Out_ size_t *valueLengthNeededWithNul) -> HRESULT {
			*valueLengthNeededWithNul =
				::GetSystemDirectoryW(value, static_cast<DWORD>(valueLength));
			RETURN_LAST_ERROR_IF(*valueLengthNeededWithNul == 0);
			if (*valueLengthNeededWithNul < valueLength) {
				(*valueLengthNeededWithNul)++; // it fit, account for the null
			}
...
template <typename string_type = wil::unique_cotaskmem_string,
		  size_t stackBufferLength = 256>
string_type TrySearchPathW(_In_opt_ PCWSTR path, _In_ PCWSTR fileName,
						   PCWSTR _In_opt_ extension) {
	string_type result{};
	HRESULT searchHR = wil::SearchPathW<string_type, stackBufferLength>(
		path, fileName, extension, result);
	THROW_HR_IF(searchHR,
				FAILED(searchHR) &&
					(searchHR != HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND)));
	return result;
}
#endif
...
/** Looks up the environment variable 'key' and fails if it is not found. */
template <typename string_type = wil::unique_cotaskmem_string,
		  size_t initialBufferLength = 128>
string_type GetEnvironmentVariableW(_In_ PCWSTR key) {
	string_type result{};
	THROW_IF_FAILED(
		(wil::GetEnvironmentVariableW<string_type, initialBufferLength>(
			key, result)));
	return result;
}
...
string_type TryGetEnvironmentVariableW(_In_ PCWSTR key) {
	string_type result{};
	THROW_IF_FAILED(
		(wil::TryGetEnvironmentVariableW<string_type, initialBufferLength>(
			key, result)));
	return result;
}
...
template <typename string_type = wil::unique_cotaskmem_string,
		  size_t initialBufferLength = 128>
string_type
GetModuleFileNameW(HMODULE module = nullptr /* current process module */) {
	string_type result{};
	THROW_IF_FAILED((wil::GetModuleFileNameW<string_type, initialBufferLength>(
		module, result)));
	return result;
}
...
string_type GetModuleFileNameExW(HANDLE process, HMODULE module) {
	string_type result{};
	THROW_IF_FAILED(
		(wil::GetModuleFileNameExW<string_type, initialBufferLength>(
...
string_type GetWindowsDirectoryW() {
	string_type result;
	THROW_IF_FAILED(
		(wil::GetWindowsDirectoryW<string_type, stackBufferLength>(result)));
	return result;
}
#endif
...
string_type GetSystemDirectoryW() {
	string_type result;
	THROW_IF_FAILED(
		(wil::GetSystemDirectoryW<string_type, stackBufferLength>(result)));
	return result;
}
...
template <typename string_type = wil::unique_cotaskmem_string,
		  size_t stackBufferLength = 256>
string_type
QueryFullProcessImageNameW(HANDLE processHandle = GetCurrentProcess(),
						   DWORD flags = 0) {
	string_type result{};
	THROW_IF_FAILED(
		(wil::QueryFullProcessImageNameW<string_type, stackBufferLength>(
			processHandle, flags, result)));
	return result;
}

#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM)

// Lookup a DWORD value under HKLM\...\Image File Execution Options\<current
// process name>
inline DWORD GetCurrentProcessExecutionOption(PCWSTR valueName,
...
// Waits for a debugger to attach to the current process based on registry
// configuration.
//
// Example:
...
inline void WaitForDebuggerPresent(bool checkRegistryConfig = true) {
	for (;;) {
		auto configValue =
			checkRegistryConfig
				? GetCurrentProcessExecutionOption(L"WaitForDebuggerPresent")
				: 1;
		if (configValue == 0) {
		...
		if (IsDebuggerPresent()) {
			if (configValue == 2) {
				DebugBreak(); // debugger attached, SHIFT+F11 to return to the
							  // caller
			}
...
// GetModuleHandleExW was added to the app partition in version 22000 of the SDK
	...
	: WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP |                       \
							  WINAPI_PARTITION_SYSTEM |                        \
							  WINAPI_PARTITION_GAMES)
// Use this in threads that can outlive the object or API call that created
// them. Without this COM, or the API caller, can unload the DLL, resulting in a
// crash. It is very important that this be the first object created in the
// thread proc as when this runs down the thread exits and no destructors of
// objects created before it will run.
[[nodiscard]] inline auto get_module_reference_for_thread() noexcept {
	HMODULE thisModule{};
	FAIL_FAST_IF(!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
									 L"", &thisModule));
	return wil::scope_exit(
		[thisModule] { FreeLibraryAndExitThread(thisModule, 0); });
}
#endif
...
#pragma warning(disable : 4702) // https://github.com/Microsoft/wil/issues/2
	void success() WI_NOEXCEPT { m_flags = 0; }
#pragma warning(pop)
...
Simplifies using the Win32 INIT_ONCE structure to perform one-time
initialization. The provided `func` is invoked at most once.
~~~~
INIT_ONCE g_init{};
ComPtr<IFoo> g_foo;
HRESULT MyMethod()
...
@param func A function that will be invoked to perform initialization. If this
fails, the init call fails and the once-init is not marked as initialized. A
later caller could attempt to initialize it a second time.
@param callerCompleted Set to 'true' if this was the call that caused
initialization, false otherwise.
*/
template <typename T>
HRESULT
init_once_nothrow(_Inout_ INIT_ONCE &initOnce, T func,
	...
	__WIL_PRIVATE_RETURN_IF_WIN32_BOOL_FALSE(
		InitOnceBeginInitialize(&initOnce, 0, &pending, nullptr));
...
//! Similar to init_once_nothrow, but fails-fast if the initialization step
//! failed. The 'callerComplete' value is returned to the caller instead of
...
bool init_once_failfast(_Inout_ INIT_ONCE &initOnce, T &&func) WI_NOEXCEPT {
	...
	FAIL_FAST_IF_FAILED(
		init_once_nothrow(initOnce, wistd::forward<T>(func), &callerCompleted));

	return callerCompleted;
};

//! Returns 'true' if this `init_once` structure has finished initialization,
//! false otherwise.
inline bool init_once_initialized(_Inout_ INIT_ONCE &initOnce) WI_NOEXCEPT {
	BOOL pending = FALSE;
	return ::InitOnceBeginInitialize(&initOnce, INIT_ONCE_CHECK_ONLY, &pending,
									 nullptr) &&
		   !pending;
}
...
fails, the init call fails and the once-init is not marked as initialized. A
later caller could attempt to initialize it a second time.
@returns 'true' if this was the call that caused initialization, false
otherwise.
*/
...
// The return value is automatically cast to match the function prototype of the
// input function.
//
// Sample usage:
//
// auto sendMail = GetProcAddressByFunctionDeclaration(hinstMAPI,
// MAPISendMailW); if (sendMail)
// {
//    sendMail(0, 0, pmm, MAPI_USE_DEFAULT, 0);
// }
//  Declaration
#define GetProcAddressByFunctionDeclaration(hinst, fn)                         \
	reinterpret_cast<decltype(::fn) *>(GetProcAddress(hinst, #fn))
